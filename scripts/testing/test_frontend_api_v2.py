        ...
        try:
            ...
            for schedule in schedules:
                ...
                if api_response['next_run']:
                    ...
                    next_run_str = api_response['next_run'].replace('Z', '+00:00')
                    next_run_date = datetime.fromisoformat(next_run_str)
                    now = datetime.now(timezone.utc)
                    diff_s = (next_run_date - now).total_seconds()
                    diff_mins = int(diff_s / 60)
                    
                    print(f"\nFrontend Calculation:")
                    print(f"  Next run date: {next_run_date}")
                    print(f"  Current time: {now}")
                    print(f"  Difference (seconds): {diff_s:.0f}")
