                    ...
                    <div className="mt-3 space-y-2">
                      ...
                      {jobProgress && jobProgress.dispenser && singleJobId === job.jobId && (
                        <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-700 rounded p-2">
                          <div className="flex items-center justify-between text-xs">
                            <span className="font-medium text-blue-900 dark:text-blue-100">
                              Real-time: {jobProgress.dispenser.title}
                            </span>
                            <span className="text-blue-700 dark:text-blue-300">
                              {jobProgress.fuelType || 'Starting...'}
                            </span>
                          </div>
                          {jobProgress.percentage !== undefined && (
                      ...
                      {/* Fallback to stored dispenser progress */}
                      {job.dispenserProgress && job.dispenserProgress.dispensers && (
                        <div>
                          {(() => {
                            // Find the currently processing dispenser
                            const activeDispenser = job.dispenserProgress.dispensers.find(d => 
                              d.status === 'processing'
                            ) || job.dispenserProgress.dispensers.find(d => 
                              d.status === 'pending'  
                            ) || job.dispenserProgress.dispensers[0];
                            ...
                            if (activeDispenser) {
                              return (
                                <div className="bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded p-2">
                                  <div className="flex items-center justify-between text-xs">
                                    <span className="font-medium text-gray-900 dark:text-gray-100">
                                      {activeDispenser.dispenserTitle}
                                    </span>
                                    <span className="text-gray-600 dark:text-gray-400">
                                      {activeDispenser.status === 'processing' ? 'Processing' : 'Pending'}
                                    </span>
                                  </div>
                                  {activeDispenser.currentAction && (
                                    <div className="text-xs text-gray-600 dark:text-gray-400 mt-1">
                                      {activeDispenser.currentAction}
                                    </div>
                                  )}
                                  {activeDispenser.fuelGrades && (
                                    <div className="flex flex-wrap gap-1 mt-2">
                                      {activeDispenser.fuelGrades.map((grade, idx) => (
                                        <span key={idx} className="px-1.5 py-0.5 bg-gray-200 dark:bg-gray-700 text-xs rounded">
                                          {grade}
                                        </span>
                                      ))}
                                    </div>
                      ...
                      {!jobProgress?.dispenser && !job.dispenserProgress && (
                        <div className="text-xs text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded p-2">
