...
const JobMap = forwardRef<JobMapRef, JobMapProps>(({ jobs, selectedJob, onSelectJob, enableFlyTo }, ref) => {
    ...
    try {
      ...
      const mapInstance = new mapboxgl.Map({
        ...
        transformRequest: (url, resourceType) => {
          ...
          // Must return original URL in a RequestParameters format for non-tile requests
          return { url };
        }
      });
      ...
      // Set up custom error handler for map - use proper mapboxgl event type
      ...
      const tileErrorHandler = (e: { tile: any; error: any; status: number; }) => {
        if (e && e.tile) {
  ...
  useEffect(() => {
    ...
    jobs.forEach(job => {
      if (!job || !job.id) {
        console.warn('Skipping invalid job (missing job object or ID)');
        return;
      }
      ...
      try {
        ...
        // Debug the dispensers data - but protect against possible circular references
        try {
          console.log(`Job ${job.id} full data:`, JSON.parse(JSON.stringify(job)));
        } catch (jsonErr) {
          console.log(`Job ${job.id} (partial data - circular reference detected)`);
        }
        ...
        try {
          // Check if dispensers exists and is an array before accessing length
          if (job.dispensers && 
              typeof job.dispensers === 'object' && 
              Array.isArray(job.dispensers) && 
              job.dispensers.length > 0) {
            ...
            if (job.serviceTypes && Array.isArray(job.serviceTypes)) {
              ...
              const dispenserServices = job.serviceTypes.filter(service => 
                typeof service === 'string' && dispenserServiceRegex.test(service)
              );
            ...
            if (dispenserCount === 'N/A' && job.description && typeof job.description === 'string') {
            ...
            if (dispenserCount === 'N/A' && job.instructions && typeof job.instructions === 'string') {
          ...
          console.warn(`Error processing dispenser count for job ${job.id}:`, err);
          dispenserCount = '0'; // Safe fallback
        }
        ...
        if (dispenserCount === 'N/A') {
          ...
          dispenserCount = '0';
          console.warn(`Job ${job.id}: Dispenser count not found through any method. Defaulting to ${dispenserCount}.`);
        }
