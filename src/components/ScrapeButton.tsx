...
const ScrapeButton: React.FC = () => {
  ...
  useEffect(() => {
    const startPolling = () => {
      ...
      const interval = setInterval(async () => {
        try {
          const currentStatus = await getScrapeStatus();
          console.log('Work order status update:', {
    ...
    if (status.status === 'running' && !polling) {
      console.log('Starting polling for work order status');
      startPolling();
    }
  ...
  const getButtonIcon = () => {
    switch (status.status) {
      case 'idle':
        return <FiDownload className="w-4 h-4 mr-2 inline" />;
      case 'running':
  ...
  const getButtonText = () => {
    switch (status.status) {
      case 'idle':
        return 'Update Service Appointments';
      case 'running':
