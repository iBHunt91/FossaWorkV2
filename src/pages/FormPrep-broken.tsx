  ...
  const pollingManager = React.useRef({
    ...
    startPolling: function(
      ...
      const interval = setInterval(async () => {
        // Skip if polling has been stopped
        if (!this.activePolls[jobId]) {
        ...
        try {
          ...
          if (status.message !== this.activePolls[jobId].lastMessage) {
            // Create a temporary copy of the polling state to update safely
            const updatedPoll = {
              ...this.activePolls[jobId],
              messageUpdateTime: now,
              lastMessage: status.message,
              lastStatusUpdate: now
            };
            this.activePolls[jobId] = updatedPoll;
          } else {
            // Just update the lastStatusUpdate
            this.activePolls[jobId].lastStatusUpdate = now;
          }
          ...
          if (status.status === 'completed' || status.status === 'error') {
            ...
            if (this.activePolls[jobId]) {
              ...
              if (status.status === 'error') {
                onError(new Error(status.message || 'Unknown error'));
              } else {
                onComplete();
              }
            }
                                      ...
                                      {job.status === 'completed' 
                                        ...
                                        : job.message.toLowerCase().includes('processing fuel') || 
                                          ...
                                          ? (() => {
                                              ...
                                              const match = message.match(fuelTypeRegex);
                                              
                                              if (match && match[1] && match[2] && match[3]) {
                                                const fuelType = match[1].trim();
