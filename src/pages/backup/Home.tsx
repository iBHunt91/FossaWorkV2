...
import {   FiActivity,   FiAlertCircle,  FiAlertTriangle,  FiCalendar,  FiClock,   FiDatabase,  FiExternalLink,   FiFileText,   FiFilter,  FiHome,  FiInfo,  FiList,  FiMapPin,  FiPieChart,  FiRefreshCw,  FiSearch,  FiStar,  FiTrash2,  FiTrendingUp,  FiChevronDown,  FiCheckCircle,  FiX,  FiBox,  FiBarChart2,  FiChevronRight } from 'react-icons/fi';
...
// Custom hook for persisting scraper status - NO LONGER NEEDED WITH CONTEXT, BUT KEEPING FOR NOW
// Will be removed fully when sidebar is updated in another task
...
const HomeContent = () => {
  ...
  // ADDED: Force set the active user to "Bruce" if not already set
  // This ensures we're loading the correct user data
  useEffect(() => {
    const activeUserId = localStorage.getItem('activeUserId');
    if (!activeUserId || activeUserId !== "7bea3bdb7e8e303eacaba442bd824004") {
      console.log("[HomeContent] Setting active user to Bruce");
      localStorage.setItem('activeUserId', "7bea3bdb7e8e303eacaba442bd824004");
      // Force reload data after setting user
      setTimeout(() => loadData(true), 300);
    } else {
      console.log("[HomeContent] Active user already set to:", activeUserId);
    }
  ...
  type FilterNeedType = {
    ...
    stores: string[];
    orderId?: string;
    filterType?: string; // Added to support DEF filters
  };
  ...
  // No longer needed as we use the useMemo version below
  ...
  // Derivated states for UI - now using context
  ...
  // Effect to update work week dates when dependencies change
  useEffect(() => {
    const updatedDates = getWorkWeekDateRanges(workWeekStart, workWeekEnd, selectedDate);
    // Also update the selected date range based on work week dates
    setSelectedDateRange([updatedDates.currentWeekStart, updatedDates.currentWeekEnd]);
  }, [workWeekStart, workWeekEnd, selectedDate]);
  ...
  const dateFilteredWorkOrders = useMemo(() => {
    return filteredWorkOrders.filter(order => {
      ...
      // Check if visit falls within the selected date range
      return (
        visitDateTime >= selectedDateRange[0] && visitDateTime <= selectedDateRange[1]
      );
    });
  }, [filteredWorkOrders, selectedDateRange]);
  
  // Update category counts when date filtered orders change
  useEffect(() => {
    calculateCategoryCounts(dateFilteredWorkOrders);
  }, [dateFilteredWorkOrders]);

  // We're now using the useMemo approach for date filtering instead of this effect

  // Helper function to calculate category counts
  const calculateCategoryCounts = (workOrders: WorkOrder[]) => {
    ...
    workOrders.forEach(order => {
      ...
      if (storeType && counts[storeType] !== undefined) {
        counts[storeType]++;
      } else {
        counts['other']++;
      }
  ...
  // Process filter needs when date filtered work orders change
  useEffect(() => {
    if (isLoading || !dateFilteredWorkOrders || dateFilteredWorkOrders.length === 0) {
      setFilterNeeds([]);
      setFilterTypes([]);
      return;
    }
    
    const needs: FilterNeedType[] = [];
    const warnings = new Map<string, FilterWarning[]>();
    
    // Define filter types based on store chains
    ...
    // Function to determine meter type - similar to the one in Filters.tsx
    const getMeterType = (order: WorkOrder): string => {
      // If no dispenser data available, default to Electronic
      if (!order.dispensers || order.dispensers.length === 0) {
        return 'Electronic';
      }
      
      // Check for meter type in dispenser data
      for (const dispenser of order.dispensers) {
        // Check fields object
        if (dispenser.fields) {
          const fields = dispenser.fields as Record<string, string>;
          ...
          for (const [key, value] of Object.entries(fields)) {
            ...
            if (valueLower.includes('hd meter') || valueLower.includes('high definition')) {
              return 'HD Meter';
            }
            
            if (valueLower.includes('eco meter') || valueLower.includes('ecometer')) {
              return 'Ecometer';
            }
          }
        }
        ...
        // Check title
        if (dispenser.title) {
          const titleLower = dispenser.title.toLowerCase();
          
          if (titleLower.includes('hd meter') || titleLower.includes('high definition')) {
            return 'HD Meter';
          }
          
          if (titleLower.includes('eco meter') || titleLower.includes('ecometer')) {
            return 'Ecometer';
          }
        }
        
        // Check raw HTML content for clues about meter type
        if (dispenser.html) {
          const htmlLower = dispenser.html.toLowerCase();
          
          if (htmlLower.includes('hd meter') || htmlLower.includes('high definition')) {
            return 'HD Meter';
          }
          
          if (htmlLower.includes('eco meter') || htmlLower.includes('ecometer')) {
            return 'Ecometer';
          }
        }
      }
      
      return 'Electronic'; // Default if no specific meter type found
    };
    
    // Helper function to parse specific dispenser instructions (similar to Filters.tsx)
    const parseSpecificDispenserInstructions = (instructions: string): number[] => {
      ...
      // Common patterns for dispenser numbers
      const rangePattern = /dispenser(?:s)?\s*(#)?\s*(\d+)\s*(?:-|to|thru|through)\s*(\d+)/gi;
      const rangeMatches = [...instructionsLower.matchAll(rangePattern)];
      ...
      rangeMatches.forEach(match => {
        if (match[2] && match[3]) {
          ...
          for (let i = start; i <= end; i++) {
            dispenserNumbers.push(i);
          }
        }
      });
      
      // Match comma-separated numbers
      const commaPattern = /dispenser(?:s)?\s*(#)?\s*((?:\d+\s*,\s*)*\d+)/gi;
      const commaMatches = [...instructionsLower.matchAll(commaPattern)];
      
      commaMatches.forEach(match => {
        if (match[2]) {
          match[2].split(',').forEach(num => {
            const trimmed = num.trim();
            if (trimmed) {
              dispenserNumbers.push(parseInt(trimmed, 10));
            }
          });
        }
      });
      
      // Individual dispenser numbers
      ...
      return [...new Set(dispenserNumbers)]; // Remove duplicates
    };
    ...
    // Function to check for DEF or high flow diesel
    const checkForSpecialFuelTypes = (order: WorkOrder): { hasDEF: boolean; hasDieselHighFlow: boolean } => {
      ...
      // Check instructions and service descriptions
      ...
      if (order.dispensers && order.dispensers.length > 0) {
        order.dispensers.forEach(dispenser => {
          ...
          // Check fields for DEF or high flow
    ...
    dateFilteredWorkOrders.forEach(order => {
      try {
        // Check if this is a specific dispensers job (code 2862)
        ...
        // Parse dispenser numbers from instructions if needed
        ...
        // Calculate filters with special handling for specific dispenser jobs
        let gasFilters = 0;
        let dieselFilters = 0;
        ...
        if (isSpecificDispensersJob && specificDispenserNumbers.length > 0 && order.dispensers && order.dispensers.length > 0) {
          // For specific dispenser jobs, only count filters for specified dispensers
          ...
          // Create modified order with only specific dispensers
          const modifiedOrder = {
            ...order,
            dispensers: specificDispensers
          };
          
          // Calculate filters for the modified order
          const result = calculateFiltersSafely(modifiedOrder);
          ...
          const result = calculateFiltersSafely(order);
          gasFilters = result.gasFilters || 0;
          dieselFilters = result.dieselFilters || 0;
          filterWarnings = result.warnings || [];
        }
        
        // Store warnings for reference
        if (filterWarnings.length > 0) {
          warnings.set(order.id, filterWarnings);
        }
        
        // Determine store type (make it type-safe for HomeContent)
        let storeType = 'Circle K'; // Default
        
        if (order.customer?.name) {
          const storeName = order.customer.name.toLowerCase();
          if (storeName.includes('7-eleven') || storeName.includes('speedway') || storeName.includes('marathon')) {
            storeType = '7-Eleven';
        ...
        // Map to part numbers based on store type
        let gasFilterPart = '';
        let dieselFilterPart = '';
        
        if (storeType === '7-Eleven') {
          // Get meter type for correct filter selection
          const meterType = getMeterType(order);
          const meterTypeKey = meterType as keyof typeof STATION_FILTERS['7-Eleven']['GAS'];
          
          // Use the meter type to get the right filter
          ...
          // Check for DEF at 7-Eleven (same as Filters.tsx)
          ...
          if (hasDEF) {
            ...
            needs.push({
              ...
              quantity: 1, // Standard quantity for DEF
              stores: [storeName],
              orderId: order.id,
        ...
        if (gasFilters > 0) {
          needs.push({
            ...
            type: 'GAS',
            quantity: gasFilters,
            stores: [order.customer?.name || 'Unknown Store'],
            orderId: order.id
          });
        }
    ...
    const types = [...new Set(needs.map(need => need.partNumber))];
    
    // Store filter needs data
    setFilterNeeds(needs);
    setFilterTypes(types);
  }, [dateFilteredWorkOrders, isLoading]);
  ...
  const handleScrapeWorkOrders = async () => {
    try {
      // Prevent starting if already scraping
      if (isAnyScrapingInProgress) {
        addToast('warning', 'A scrape job is already in progress. Please wait for it to complete.');
        return;
      }
      
      addToast('info', 'Starting work order scrape job...');
      
      // Use API from scrapeService
      await startScrapeJob();
      
      // Trigger events to notify components
      window.dispatchEvent(new CustomEvent('scrape-started'));
      
      // Use the updateScrapeStatus from context to refresh status across all components
      updateScrapeStatus();
      
      // Poll until complete
      const interval = setInterval(async () => {
        await updateScrapeStatus();
        if (workOrderStatus.status === 'completed' || workOrderStatus.status === 'error' || workOrderStatus.progress === 100) {
          clearInterval(interval);
        }
      }, 1000);
      
      // Clear interval after 3 minutes as safety measure
  ...
  const handleScrapeDispenserData = async () => {
    try {
      ...
      addToast('info', 'Starting dispenser scrape job...');
      ...
      const interval = setInterval(async () => {
        await updateScrapeStatus();
        if (dispenserStatus.status === 'completed' || dispenserStatus.status === 'error' || dispenserStatus.progress === 100) {
          clearInterval(interval);
        }
      }, 1000);
      
      // Clear interval after 5 minutes as safety measure (dispenser scrapes take longer)
  ...
  const renderToolsPanel = () => {
    // Check window width to determine if the sidebar is visible
    // If the sidebar is visible, we don't need to show the timestamps here
    ...
    return (
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-4">
        {/* Data Timestamps - Only show on mobile when sidebar is hidden */}
        {!isLargeScreen && (
        ...
        {/* Work Order Scraping */}
        <div className={`flex flex-col space-y-3 ${!isLargeScreen ? 'md:col-span-2' : 'md:col-span-1'}`}>
  ...
  const renderFilterBreakdownPanel = () => {
    ...
    // Calculate total quantity per filter type
    const filterQuantities = filterTypes.map(type => ({
      partNumber: type,
      quantity: getTotalQuantity(type),
      isGas: type.includes('MB') || type.includes('510A') || type.includes('510D'),
      isDEF: type === '800HS-30',
      boxes: getBoxesNeeded(getTotalQuantity(type))
    }));
    
    // Sort by filter type (gas first, then diesel, then DEF)
    filterQuantities.sort((a, b) => {
      if (a.isGas && !b.isGas) return -1;
      if (!a.isGas && b.isGas) return 1;
      if (a.isDEF && !b.isDEF) return 1;
      if (!a.isDEF && b.isDEF) return -1;
      return b.quantity - a.quantity; // Then by quantity (highest first)
    });
    ...
    // Return empty state if no filters found
    if (filterTypes.length === 0) {
      return (
        <div className="text-center py-6 text-gray-500 dark:text-gray-400">
          <FiBox className="mx-auto h-8 w-8 mb-2 text-gray-400 dark:text-gray-500" />
          <p>No filter data available for selected dates</p>
          <p className="text-sm mt-1">Select a date range with work orders to view filters</p>
        </div>
      );
    }
    
    // Calculate total filters and boxes
    const totalFilters = filterQuantities.reduce((sum, item) => sum + item.quantity, 0);
    const totalBoxes = filterQuantities.reduce((sum, item) => sum + item.boxes, 0);
    
    return (
      <div className="pt-4">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-base font-medium text-gray-800 dark:text-white">Filter Requirements</h3>
          <div className="flex items-center space-x-3">
            <div className="px-2.5 py-1 rounded-full text-xs font-medium bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-300">
              Total: {totalFilters} Filters ({totalBoxes} Boxes)
            </div>
            <div className="px-2.5 py-1 rounded-full text-xs font-medium bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-300">
              {filterTypes.length} Filter Types
            </div>
          </div>
        </div>
        
        {/* Total Filter Breakdown Table */}
        <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 mb-4 overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 dark:bg-gray-700">
              <tr>
                <th className="px-4 py-2 text-left font-medium text-gray-500 dark:text-gray-400">Filter Part #</th>
                <th className="px-4 py-2 text-left font-medium text-gray-500 dark:text-gray-400">Type</th>
                <th className="px-4 py-2 text-center font-medium text-gray-500 dark:text-gray-400">Quantity</th>
                <th className="px-4 py-2 text-center font-medium text-gray-500 dark:text-gray-400">Boxes</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
              {filterQuantities.map(item => (
                <tr key={item.partNumber} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                  <td className="px-4 py-3 font-medium text-gray-900 dark:text-white">{item.partNumber}</td>
                  <td className="px-4 py-3 text-gray-700 dark:text-gray-300">
                    <span className="inline-flex items-center">
                      <span 
                        className={`w-3 h-3 rounded-full mr-2 ${item.isGas ? 'bg-blue-500' : item.isDEF ? 'bg-purple-500' : 'bg-amber-500'}`}
                      ></span>
                      {item.isGas ? 'Gas' : item.isDEF ? 'DEF' : 'Diesel'}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-center font-semibold text-gray-700 dark:text-gray-300">{item.quantity}</td>
                  <td className="px-4 py-3 text-center text-gray-700 dark:text-gray-300">{item.boxes}</td>
                </tr>
              ))}
            </tbody>
            <tfoot className="bg-gray-50 dark:bg-gray-700">
              <tr>
                <td className="px-4 py-2 font-medium text-gray-500 dark:text-gray-400" colSpan={2}>Total</td>
                <td className="px-4 py-2 text-center font-medium text-gray-700 dark:text-gray-300">{totalFilters}</td>
                <td className="px-4 py-2 text-center font-medium text-gray-700 dark:text-gray-300">{totalBoxes}</td>
              </tr>
            </tfoot>
          </table>
        </div>
        
        {/* Compact Grid for Filter Types */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {/* Paired filters cards */}
          {presentPairs.map((pair, idx) => {
            ...
            const quantities = pair.types.map(type => 
              filterTypes.includes(type) ? getTotalQuantity(type) : 0
            );
            ...
            const totalBoxes = pair.types.reduce((sum, type) => 
              sum + (filterTypes.includes(type) ? getBoxesNeeded(getTotalQuantity(type)) : 0), 0
            );
            ...
            return (
              <div key={idx} className="border dark:border-gray-700 rounded-lg overflow-hidden">
                ...
                <div className="p-3">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-xs font-medium text-gray-500 dark:text-gray-400">Total Filters:</span>
                    <span className="text-sm font-semibold text-gray-800 dark:text-gray-200">{totalQuantity}</span>
                  </div>
                  
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-medium text-gray-500 dark:text-gray-400">Boxes Needed:</span>
                    <span className="text-sm font-semibold text-gray-800 dark:text-gray-200">{totalBoxes}</span>
                  </div>
                      ...
                      return (
                        <div key={type} className="flex justify-between items-center">
                          <div className="flex items-center">
                            <span 
                              className={`w-2 h-2 rounded-full mr-1 ${isGas ? 'bg-blue-500' : 'bg-amber-500'}`}
                            ></span>
          ...
          {/* Individual filters cards */}
          {singleFilters.map(type => {
            const quantity = getTotalQuantity(type);
            const boxesNeeded = getBoxesNeeded(quantity);
            // Determine if gas or diesel based on part number pattern
            const isGas = type.includes('MB') || type.includes('510A') || type.includes('510D');
            const isDEF = type === '800HS-30';
            const filterCategory = isGas ? 'Gas' : isDEF ? 'DEF' : 'Diesel';
            const colorClass = isGas ? 'bg-blue-500 dark:bg-blue-600' : 
                             isDEF ? 'bg-purple-500 dark:bg-purple-600' : 'bg-amber-500 dark:bg-amber-600';
            ...
            return (
              <div key={type} className="border dark:border-gray-700 rounded-lg overflow-hidden">
                <div className="px-3 py-2 bg-gray-50 dark:bg-gray-700/50 border-b dark:border-gray-600">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center">
                      <span className={`inline-block w-3 h-3 rounded-full mr-1.5 ${isGas ? 'bg-blue-500' : isDEF ? 'bg-purple-500' : 'bg-amber-500'}`}></span>
                      <span className="font-medium text-gray-800 dark:text-gray-200 text-sm">
                        {type}
                      </span>
                    </div>
                    <span className="text-xs font-medium text-gray-500 dark:text-gray-400">{filterCategory}</span>
                  </div>
                </div>
                ...
                <div className="p-3">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-sm font-semibold text-gray-800 dark:text-gray-200">{quantity}</span>
                  </div>
                  
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-medium text-gray-500 dark:text-gray-400">Boxes:</span>
                    <span className="text-sm font-semibold text-gray-800 dark:text-gray-200">{boxesNeeded}</span>
                  </div>
        ...
        <div className="flex justify-end mt-4">
          ...
          >
            View Detailed Filter Breakdown <FiChevronRight className="w-4 h-4" />
          </a>
        </div>
      </div>
    );
  ...
  return (
    <div className="container mx-auto px-4 pt-5 pb-8">
      ...
      <div className="space-y-5">
        ...
        {renderPanel('filters', 'Filter Breakdown', <FiFilter className="w-5 h-5" />, renderFilterBreakdownPanel())}
