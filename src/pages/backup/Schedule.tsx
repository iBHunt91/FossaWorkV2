...
const Schedule: React.FC = () => {
  ...
  const renderJobRow = (order: WorkOrder, dateGroupClass?: string) => {
    ...
    return (
      ...
      >
        ...
        {/* Action Buttons Footer - Slide-out toolbar with menu indicator */}
        <div className="p-2 border-t border-gray-700/50 flex items-center justify-end relative group">
          {/* Button container with menu identifier always on the right */}
          <div className="flex items-center">
            {/* Slide-out button container - Hidden then slides in from right to left */}
            ...
            {/* Menu indicator - Always visible in the right corner */}
  ...
  const renderCompactJobItem = (order: WorkOrder, index: number, total: number) => {
    ...
    const filteredInstructions = processInstructions(order.instructions, order);
  ...
  const renderCompactView = () => {
    ...
    return (
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden mx-1 sm:mx-2 my-2">
        ...
        {groupedWorkOrders.other.length > 0 && (
          <div className="border-t border-gray-200 dark:border-gray-700">
            <div className="bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-900 p-3 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
              ...
              <button 
                onClick={() => {
                  // Toggle view of other jobs
                  setExpandedSections(prev => ({
                    ...prev,
                    'view-all-jobs': !prev['view-all-jobs']
                  }));
                }}
                  ...
                  return (
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 min-h-[180px] divide-x divide-gray-200 dark:divide-gray-700 gap-px">
                      {sortedDates.map(dateKey => {
                        ...
                        const sectionKey = `other-day-${dateKey}`;
                        const isExpanded = expandedSections[sectionKey] || false;
                        
                        // For other dates, initially show 3 jobs unless expanded
                        const initialDisplayLimit = 3;
                        const visibleJobs = isExpanded ? jobsForDay : jobsForDay.slice(0, initialDisplayLimit);
                        const hiddenJobCount = jobsForDay.length - visibleJobs.length;
                        
                        return (
                          <div key={dateKey} className="flex flex-col">
                            {/* Date header */}
                            <div className="p-3 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between bg-gray-500 dark:bg-gray-600">
                              <div className="flex flex-col">
                                <span className="text-xs text-white font-medium">
                                  {date.toLocaleDateString(undefined, { weekday: 'short' })}
                                </span>
        ...
        {(thisWeekJobs.length === 0 && nextWeekJobs.length === 0 && groupedWorkOrders.other.length === 0 && activeView === 'compact') && (
