...
const Settings: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'credentials' | 'email' | 'pushover' | 'schedule' | 'provers'>('credentials')

  // Schedule change test states
  const [scheduleChangeType, setScheduleChangeType] = useState<'add' | 'remove' | 'replace' | 'date'>('add')
  ...
  const handleScheduleChangeTest = async (e: React.FormEvent) => {
    ...
    try {
      console.log('==== SCHEDULE CHANGE TEST STARTED ====')
      console.log(`Testing schedule change with type: ${scheduleChangeType}, count: ${scheduleChangeCount}`)
      console.log('Using display preferences:', scheduleTestPreferences)
      ...
      if (window.electron?.testScheduleChange) {
        ...
        if (result.success) {
          setScheduleTestResult(`Successfully simulated ${scheduleChangeCount} ${scheduleChangeType} schedule change(s). Notifications sent.`)
        } else {
          throw new Error(result.message || 'Failed to simulate schedule change')
        }
      } else {
        throw new Error('Schedule change test functionality not available')
      }
      
      console.log('==== SCHEDULE CHANGE TEST COMPLETED ====')
    } catch (err: any) {
      const errorMsg = err.message || 'Failed to test schedule change'
      console.error(`ERROR: ${errorMsg}`)
          ...
          <div className="md:w-64 px-4 py-6 border-r border-gray-200 dark:border-gray-700">
            <nav className="space-y-2">
              <button
                ...
                className={getTabButtonClass('credentials')}
              >
                <FiKey className={getTabIconClass('credentials')} />
                <span>Fossa Credentials</span>
              </button>
            ...
            {activeTab === 'schedule' && (
              <div className="space-y-6">
                <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
                  ...
                  <div className="p-5">
                    <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
                      Test the schedule change notification system by simulating different types of changes.
                    </p>
                    ...
                    <form onSubmit={handleScheduleChangeTest} className="space-y-4">
                      ...
                      <div className="border-t border-gray-200 dark:border-gray-700 pt-4 mt-4">
                        <h4 className="font-medium text-gray-800 dark:text-gray-200 mb-3">Display Preferences</h4>
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="flex items-center">
                            ...
                            <label htmlFor="usePushoverPreferences" className="ml-2 block text-sm text-gray-900 dark:text-gray-300">
                              Use Pushover display preferences
                            </label>
                          </div>
                          ...
                          <div className="flex items-center">
                            ...
                            <label htmlFor="forceShowAllFields" className="ml-2 block text-sm text-gray-900 dark:text-gray-300">
                              Override and show all fields (for testing)
                            </label>
                          </div>
                        </div>
                      </div>
                        ...
                        >
                          {isTestingSchedule ? (
                            ...
                            <>
                              ...
                              Simulate Changes & Send Notifications
                            </>
                          )}
                        </button>
                      </div>
