...
import { getUserPushoverSettings, saveUserPushoverSettings, sendTestPushoverNotification, removePushoverSettings } from '../../scripts/pushover/pushoverService.js';
import { 
...
export function setupPushoverApi() {
  ...
  // Remove Pushover settings
  ipcMain.handle('settings:pushover:remove', async () => {
    try {
      // Stop the open client first
      await stopOpenClient();
      
      // Then remove the settings
      const result = removePushoverSettings();
      return {
        success: result,
        message: result ? 'Pushover settings removed successfully' : 'Failed to remove Pushover settings'
      };
    } catch (error) {
