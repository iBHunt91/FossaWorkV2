...
// Convert callback-based fs methods to promises
...
contextBridge.exposeInMainWorld(
  'electron',
  {
    ...
    // File system operations for Electron renderer
    fs: {
      ...
      writeFile: async (filePath, data, options = 'utf8') => {
        try {
          return await writeFileAsync(filePath, data, options);
        } catch (error) {
      ...
      mkdir: async (dirPath, options = { recursive: true }) => {
        try {
          return await mkdirAsync(dirPath, options);
        } catch (error) {
