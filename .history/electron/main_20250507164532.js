    ...
    try {
      ...
      if (fs.existsSync(settingsPath)) {
        ...
        if (currentUserId) {
          ...
          // Create base test user with pushover key from settings
          testUser = {
            id: currentUserId,
            email: '',
            ...
            notificationSettings: {
              ...
              pushover: {
                enabled: options.preferences?.usePushoverPreferences || true  // Set default to true
              }
            },
            preferences: {
              notifications: {
          ...
          try {
            ...
            // Also try to get user Pushover key from their settings
            const userPushoverSettingsPath = path.join(__dirname, '..', 'data', 'users', currentUserId, 'pushover_settings.json');
            if (fs.existsSync(userPushoverSettingsPath)) {
              const userPushoverSettings = JSON.parse(fs.readFileSync(userPushoverSettingsPath, 'utf8'));
              if (userPushoverSettings.userKey) {
                testUser.pushoverKey = userPushoverSettings.userKey;
                logger.info('Schedule Change Test', `Found Pushover key for user ${currentUserId}`);
              }
            }
          } catch (err) {
            logger.warn('Schedule Change Test', `Could not load user settings: ${err.message}`);
          }
        }
      }
      ...
      // Add pushover settings directly to the test user
      testUser.pushoverSettings = pushoverSettings;
