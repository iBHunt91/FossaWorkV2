...
import { getProverPreferences, updateProverPreference, getDataPath } from '../scripts/utils/prover_info.js';
...
app.whenReady().then(() => {
  ...
  // Add this in the IPC handlers section where other ipcMain.handle calls are defined
  ipcMain.handle('save-file', async (event, options) => {
    try {
      const { filename, content, path: dirPath } = options;
      ...
      const fullDirPath = path.resolve(dirPath || 'logs');
