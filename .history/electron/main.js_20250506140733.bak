...
function createWindow() {
  ...
  // Configure DevTools for better performance and fewer errors
  configureDevTools(mainWindow);
...
// Configure DevTools to reduce errors and improve performance
function configureDevTools(window) {
  // Only configure DevTools in development mode
  if (process.env.NODE_ENV !== 'development') return;

  // Set DevTools preferences when they're opened
  window.webContents.on('devtools-opened', () => {
    // Send commands to DevTools to disable problematic features
    window.webContents.executeJavaScript(`
      // Suppress specific DevTools errors
      if (window.DevToolsAPI) {
        const originalDispatchMessage = window.DevToolsAPI.dispatchMessage;
        window.DevToolsAPI.dispatchMessage = (message) => {
          // Filter out problematic messages
          if (message.includes('Autofill.enable') || 
              message.includes('Autofill.setAddresses')) {
            return;
          }
          return originalDispatchMessage(message);
        };
      }
      
      // Disable autofill in DevTools
      if (window.localStorage) {
        window.localStorage.setItem('devtools.preferences.autofillEnabled', 'false');
      }
    `).catch(err => {
      // Silently ignore errors from this script
    });
  });

  // Open DevTools in development with a delay for better performance
