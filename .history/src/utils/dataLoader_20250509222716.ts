...
export const loadWorkOrders = async (userId?: string): Promise<WorkOrder[]> => {
  try {
    ...
    let dispenserStoreData: { [key: string]: { dispensers?: any[] } } = {};
    if (dispenserStoreResponse.ok) {
      dispenserStoreData = await dispenserStoreResponse.json();
    } else {
    ...
    const mergedWorkOrders = workOrders.map(workOrder => {
      const dispenserEntry = dispenserStoreData[workOrder.id];
