...
const StoreFilterNeedsModal: React.FC<StoreFilterNeedsModalProps> = ({
  ...
  React.useEffect(() => {
    if (workOrder) {
      ...
      //   - When dispenser data is missing but needed for calculations
      
      // Check for specific dispenser jobs without identified dispensers
      ...
      // Check for missing dispenser data when needed
      const hasCalibrationService = workOrder.services?.some(service => 
        service.type === "Meter Calibration" || 
        (service.description && service.description.toLowerCase().includes("calibration"))
      );
      
      if (hasCalibrationService && !hasDispenserData) {
        newVerificationWarnings.push("Missing dispenser data - verify filter requirements on site.");
      }

      // Check for inconsistent dispenser data
      const calibrationQuantity = workOrder.services?.find(s => 
        s.type === "Meter Calibration" || 
        (s.description && s.description.toLowerCase().includes("calibration"))
      )?.quantity || 0;

      const dispenserCount = workOrder.dispensers?.length || 0;

      if (hasDispenserData && calibrationQuantity > 0 && dispenserCount !== calibrationQuantity) {
        newVerificationWarnings.push(`Inconsistent dispenser count: ${dispenserCount} dispensers found but calibration service indicates ${calibrationQuantity}.`);
      }
      
      // Check for diesel services without clear diesel dispenser identification
      const hasDieselService = workOrder.services?.some(s => 
        (s.description?.toLowerCase() || '').includes('diesel') && 
        !((s.description?.toLowerCase() || '').includes('def') || (s.description?.toLowerCase() || '').includes('exhaust'))
      );
      
      const hasDieselDispenser = workOrder.dispensers?.some(d => 
        (d.title?.toLowerCase() || '').includes('diesel') || 
        Object.values(d.fields || {}).some(v => (v as string)?.toLowerCase()?.includes('diesel'))
      );
      
      if (hasDieselService && hasDispenserData && !hasDieselDispenser) {
        newVerificationWarnings.push("Diesel service requested but no diesel dispenser clearly identified - verify on site.");
      }
    ...
    >
      ...
      >
        ...
        {verificationWarnings.length > 0 && (
          <>
            <div className={`p-3 mb-2 rounded-md border ${warningBgClass} ${isDarkMode ? 'border-yellow-600' : 'border-yellow-400'}`}>
              <h4 className={`font-semibold ${warningTextClass} mb-2 flex items-center`}>
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-1.5" viewBox="0 0 20 20" fill="currentColor">
                  <path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd" />
                </svg>
                Verification Required
              </h4>
              <ul className={`list-disc list-inside text-sm space-y-1 ${warningTextClass} ml-1`}>
                {verificationWarnings.map((warning, idx) => (
                  <li key={idx} className="ml-2">{warning}</li>
                ))}
              </ul>
            </div>

            {/* Action Steps Section */}
            <div className={`p-3 mb-4 rounded-md border ${isDarkMode ? 'bg-gray-700 border-gray-600' : 'bg-gray-50 border-gray-200'}`}>
              <h4 className={`font-medium text-sm mb-1.5 flex items-center ${isDarkMode ? 'text-blue-300' : 'text-blue-600'}`}>
                <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 mr-1.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" />
                </svg>
                Action Steps
              </h4>
              <ol className={`list-decimal list-inside text-xs space-y-1 ${isDarkMode ? 'text-gray-300' : 'text-gray-700'} ml-1`}>
                <li>Check all dispensers on-site and record actual quantities</li>
                <li>Confirm fuel types for each dispenser (Gas/Diesel/DEF)</li>
                <li>Verify specific dispensers mentioned in job instructions</li>
                <li>Count filters needed for actual dispensers being serviced</li>
                <li>Update job notes with accurate filter count information</li>
              </ol>
            </div>
          </>
        )}
        ...
        {/* Debug section - only visible when a URL parameter is present */}
        {(() => {
          const isDebugMode = new URLSearchParams(window.location.search).has('debug');
          if (isDebugMode && workOrder) {
            return (
              <div className="mt-5 pt-4 border-t border-gray-300 dark:border-gray-600">
                <h4 className="text-sm font-medium mb-2 text-gray-500 dark:text-gray-400">Debug Information:</h4>
                <div className="bg-gray-100 dark:bg-gray-700 p-2 rounded text-xs font-mono overflow-x-auto">
                  <div>Has Dispensers: {workOrder.dispensers && workOrder.dispensers.length > 0 ? 'Yes' : 'No'}</div>
                  <div>Dispenser Count: {workOrder.dispensers?.length || 0}</div>
                  <div>Calibration Service: {workOrder.services?.some(s => s.type === "Meter Calibration") ? 'Yes' : 'No'}</div>
                  <div>Calibration Quantity: {
                    workOrder.services?.find(s => s.type === "Meter Calibration")?.quantity || 'Not specified'
                  }</div>
                </div>
              </div>
            );
