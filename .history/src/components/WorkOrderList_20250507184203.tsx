...
const WorkOrderList: React.FC<WorkOrderListProps> = ({
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden transition-all duration-300 animate-fadeIn">
      ...
      <div className="p-5">
        {sortedWorkOrders.length === 0 ? (
          <div className="text-center py-10 bg-gray-50 dark:bg-gray-800/50 rounded-lg">
            <FiInfo className="h-10 w-10 mx-auto mb-3 opacity-40 text-gray-400" />
            <p className="text-gray-500 dark:text-gray-400 font-medium">No work orders match your search</p>
            {searchTerm && (
              <button 
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 2xl:grid-cols-3 gap-6">
            {sortedWorkOrders.map((workOrder, index) => (
              <div 
                ...
                style={{
                  opacity: 0,
                  animation: `fadeIn 0.3s ease-out ${Math.min(index * 50, 300)}ms forwards`
                }}
              >
                <WorkOrderCard
                  workOrder={workOrder}
      ...
      <div className="bg-gray-50 dark:bg-gray-700 px-5 py-3.5 border-t border-gray-200 dark:border-gray-600">
        <div className="flex items-center justify-between">
          <div className="flex items-center text-sm text-gray-700 dark:text-gray-300">
            <span className="bg-gray-200 dark:bg-gray-600 rounded-full px-2.5 py-1 text-xs font-medium mr-2">
          ...
          {sortedWorkOrders.length > 0 && (
            <div className="text-xs text-gray-500 dark:text-gray-400 flex items-center bg-white dark:bg-gray-600 px-2.5 py-1 rounded-full shadow-sm">
              {sortBy === 'date' ? (
                <>
                ...
                <>
                  ...
                  <span>Sorted by store name</span>
                </>
              )}
            </div>
          )}
