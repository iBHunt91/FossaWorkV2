...
import { WorkOrder, FilterNeed as BaseFilterNeed } from '../types'; // Renamed to avoid conflict
...
const StoreFilterNeedsModal: React.FC<StoreFilterNeedsModalProps> = ({
  ...
  const calculateNeedsForOrder = (order: WorkOrder): ExtendedFilterNeed[] => {
    ...
    if (!order || !order.customer || !order.customer.name) return needs; // Added null check for order.customer.name
