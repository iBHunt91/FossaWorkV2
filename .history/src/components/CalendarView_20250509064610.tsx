  ...
  const renderMonthView = () => {
    ...
    for (let i = 1; i <= daysInMonth; i++) {
      ...
      days.push(
        <div 
          key={`day-${i}`}
          className={`min-h-28 p-1 border rounded-md transition-colors relative ${
            isToday 
        ...
        >
          <div className="flex justify-between items-start">
            ...
            {eventsOnDay.length > 0 && (
              <span 
                className={`text-xs font-medium px-1.5 py-0.5 rounded-full cursor-pointer hover:bg-opacity-80 transition-all ${
                  isToday 
                    ? 'bg-blue-200 dark:bg-blue-800 text-blue-700 dark:text-blue-300' 
                    : 'bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300'
                }`}
                onClick={() => toggleDayExpansion(dayKey)}
                title={isExpanded ? "Collapse events" : "Expand events"}
              >
                {eventsOnDay.length}
              </span>
            )}
          ...
          {/* Event cards - using stacked approach instead of scrollbar */}
          <div className="mt-1 relative">
            {eventsOnDay.length > 0 && (
              <>
                {isExpanded ? (
                  // Expanded view - show all events in a clean list
                  ...
                  // Collapsed view - show stacked cards with visual overlap
                  <div className="relative mt-1 h-14 overflow-visible">
                    {eventsOnDay.slice(0, Math.min(3, eventsOnDay.length)).map((event, idx) => {
                      const storeStyle = getStoreStyle(event.storeType);
                      // Calculate slight overlap position and z-index
                      const topOffset = idx * 4; // 4px offset for each card
                      const zIndex = 10 - idx; // Higher z-index for first cards
                      ...
                      return (
                        <div 
                          ...
                          style={{ 
                            top: `${topOffset}px`,
                            zIndex: zIndex,
                            right: `${idx * 2}px`, // Slight horizontal offset
                            left: `${idx * 2}px`
                          }}
                          className={`absolute text-xs p-1.5 rounded-md cursor-pointer transform transition-all hover:translate-y-[-2px] ${storeStyle.badge} flex flex-col shadow-sm`}
                          onClick={() => onEventClick(event)}
                        >
                          <div className="font-medium truncate">{event.title}</div>
                        </div>
                    ...
                    {/* "More" indicator for days with more than 3 events */}
                    {eventsOnDay.length > 3 && (
                      <button
                        className="absolute bottom-[-6px] left-1/2 transform -translate-x-1/2 bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 text-[10px] px-2 py-0.5 rounded-full shadow-sm hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors z-20"
                        onClick={() => toggleDayExpansion(dayKey)}
                      >
