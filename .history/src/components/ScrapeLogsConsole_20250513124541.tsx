...
const ScrapeLogsConsole: React.FC<ScrapeLogsConsoleProps> = ({
  ...
  const getMessageType = (message: string) => {
    ...
    // Fall back to content-based detection if no emoji prefix
  ...
  // Don't add extra icons to messages, as we're now adding them server-side
  const getMessageIcon = (type: string) => {
    // If we want to keep icons here for legacy messages without emojis
    return '';
  };
            ...
            {[...logs].reverse().map((log, index) => {
              ...
              return (
                <div key={index} className={`mb-1.5 pl-5 relative border-l-2 ${
                  ...
                  messageType === 'info' || messageType === 'progress' ? 'border-blue-500' :
                  'border-gray-700'
                }`}>
