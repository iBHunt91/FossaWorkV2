...
const NextScrapeTime: React.FC<NextScrapeTimeProps> = ({ timestamp: initialTimestamp }) => {
  ...
  return (
    <div className="flex flex-col">
      <span className="text-xs font-medium text-gray-300 mb-0.5">Next update</span>
      {isLoading ? (
        ...
        <span 
          className="text-sm font-medium text-green-300 cursor-help"
          data-tooltip-id={tooltipId}
        >
          {relativeTime}
        </span>
      ) : (
        <span className="text-xs text-gray-400 italic">No schedule available</span>
      )}
