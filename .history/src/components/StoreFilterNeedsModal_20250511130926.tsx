  ...
  return (
    ...
    >
      ...
      >
        ...
        {(hasIdentifiedDispensers || calibrationInfo) && (
          <div className={`p-3 mb-4 rounded-md border ${infoBgClass} ${infoBorderClass}`}>
            <h4 className={`font-semibold ${infoTextClass} mb-1`}>Service Information:</h4>
            {hasIdentifiedDispensers && identifiedDispenserNumbers.length > 0 && (
              <p className={`text-xs ${infoTextClass} mb-1`}>
                This job is for specific dispensers: #{identifiedDispenserNumbers.join(', #')}
              </p>
            )}
            {calibrationInfo && (
              <p className={`text-xs ${infoTextClass}`}>
                Calibration service: {calibrationInfo.quantity} {calibrationInfo.quantity !== 1 ? 'dispensers' : 'dispenser'}
                {calibrationInfo.description !== "Meter Calibration" && (
                  <span className="block mt-0.5 italic">{calibrationInfo.description}</span>
                )}
              </p>
            )}
          </div>
