...
import { WorkOrder as OriginalWorkOrder, FilterNeed as BaseFilterNeed } from '../types'; // Renamed to avoid conflict
...
interface ModalWorkOrder extends Omit<OriginalWorkOrder, 'customer'> {
  customer: ModalCustomer;
}

// --- Interfaces ---
interface ExtendedFilterNeed extends Omit<BaseFilterNeed, 'stationType' | 'type'> { // Omit to redefine
  orderId: string;
  visitId: string;
  visitDate: string;
  storeName: string;
  filterType?: string; 
  stationType: string; // Broaden to string for now
  type: string; // Broaden to string for now
  partNumber: string; // Ensure partNumber is always a string
  quantity: number; // Ensure quantity is always a number
  stores: string[]; // Ensure stores is always string[]
}
...
const getMeterType = (order: OriginalWorkOrder): string => {
  ...
  return 'default'; // Changed from 'Unknown' to 'default' for safer key access
};

const checkForSpecialFuelTypes = (order: OriginalWorkOrder): { hasDEF: boolean; hasDieselHighFlow: boolean; isSpecificDispensersJob: boolean } => {
...
interface StoreFilterNeedsModalProps {
  isOpen: boolean;
  onClose: () => void;
  workOrder: ModalWorkOrder | null; // Use the modal-specific WorkOrder type
  isDarkMode?: boolean; 
}
...
const StoreFilterNeedsModal: React.FC<StoreFilterNeedsModalProps> = ({
  ...
  const calculateNeedsForOrder = (order: ModalWorkOrder): ExtendedFilterNeed[] => {
    const needs: ExtendedFilterNeed[] = [];
    if (!order || !order.customer || !order.customer.name) return needs;
    ...
    if (stationType === '7-Eleven') {
      ...
      // Ensure meterTypeKey is a valid key for STATION_FILTERS['7-Eleven'].GAS/DIESEL
      const gasMeterTypes = STATION_FILTERS['7-Eleven'].GAS;
      const dieselMeterTypes = STATION_FILTERS['7-Eleven'].DIESEL;
      const validMeterTypeKey = (gasMeterTypes.hasOwnProperty(meterType) ? meterType : 'default') as keyof typeof gasMeterTypes;
      
gasFilterPart = gasMeterTypes[validMeterTypeKey];
      dieselFilterPart = dieselMeterTypes[validMeterTypeKey as keyof typeof dieselMeterTypes]; // Assuming same key for diesel

      const { hasDEF } = checkForSpecialFuelTypes(order);
      if (hasDEF) {
        const defQuantity = getCalculatedQuantity(1);
        needs.push({
    ...
    if (needs.length === 0) {
        needs.push({
            partNumber: 'N/A',
            type: 'INFO', // Changed from 'UNKNOWN' to avoid type error, signifies informational
            quantity: 0,
            stores: [order.customer.name],
            stationType,
            orderId: order.id,
  ...
  return (
    <div className={`fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50 transition-opacity duration-300 ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
      <div className={`${modalBgClass} rounded-lg shadow-xl p-6 w-full max-w-lg transform transition-all duration-300 ${isOpen ? 'scale-100 opacity-100' : 'scale-95 opacity-0'}`}>
        ...
        <div className="mt-6 flex justify-end">
          <button
            onClick={onClose}
            className={`px-4 py-2 rounded-md text-sm font-medium ${closeButtonClass} transition-colors`}
          >
            Close
          </button>
        </div>
