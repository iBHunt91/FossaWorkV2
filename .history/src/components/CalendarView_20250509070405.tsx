  ...
  const renderMonthView = () => {
    ...
    for (let i = 1; i <= daysInMonth; i++) {
      ...
      days.push(
        <div 
          key={`day-${i}`}
          className={`min-h-28 p-1 border rounded-md transition-colors overflow-hidden ${
            isToday 
        ...
        >
          ...
          {/* Job visualization - direct preview of jobs */}
          <div className="flex flex-col space-y-1 overflow-y-auto" style={{ maxHeight: "94px" }}>
            {/* Always show a preview of jobs right in the calendar */}
            {eventsOnDay.length > 0 && !isExpanded && (
              <>
                {/* Show type indicators at top */}
                <div className="flex flex-wrap gap-1 mb-1">
                  {Object.entries(eventsByType).map(([type, events]) => {
                    ...
                    return (
                      <div 
                        key={type} 
                        className="flex items-center gap-1 rounded-full px-1 py-0.5 cursor-pointer" 
                        title={`${events.length} ${type === 'circle-k' ? 'Circle K' : type === '7-eleven' ? '7-Eleven' : type === 'wawa' ? 'Wawa' : 'Other'} jobs`}
                        onClick={() => toggleDayExpansion(dayKey)}
                      >
                        <div className={`w-2 h-2 rounded-full ${storeStyle.dot}`}></div>
                        <span className="text-[10px] text-gray-600 dark:text-gray-300">{events.length}</span>
                      </div>
                    );
                  })}
                </div>
                
                {/* Show job cards with titles - compact versions */}
                {eventsOnDay.slice(0, 3).map((event, idx) => {
                  ...
                  return (
                    <div 
                      ...
                      className={`p-1 rounded cursor-pointer hover:bg-opacity-90 transition-colors flex items-center gap-1 ${storeStyle.badge} border-l-2 ${storeStyle.dot.replace('bg-', 'border-')}`}
                    >
                      <div className="flex-1 truncate">
                        <span className="text-[10px] font-medium truncate">{event.title}</span>
                      </div>
                      
                      {/* Tiny service/equipment icons if needed */}
                      {(event.dispensers && event.dispensers.length > 0) && (
                        <FiTool className="h-2 w-2 flex-shrink-0 opacity-70" />
                      )}
                    </div>
                  );
                })}
                ...
                {eventsOnDay.length > 3 && (
                  <div className="text-center">
                    <button
                      onClick={() => toggleDayExpansion(dayKey)}
                      className="text-[9px] text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 transition-colors py-0.5"
                    >
            ...
            {isExpanded && (
              <div className="absolute z-20 left-0 right-0 bg-white dark:bg-gray-800 shadow-lg rounded-md border border-gray-200 dark:border-gray-600 p-2 mt-1 max-h-64 overflow-y-auto" style={{ width: '220px' }}>
                <div className="flex justify-between items-center mb-2 pb-1 border-b border-gray-200 dark:border-gray-700">
                  <span className="text-xs font-medium text-gray-700 dark:text-gray-300">
                    {eventsOnDay.length} Job{eventsOnDay.length !== 1 ? 's' : ''} on {currentDate.toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}
                  </span>
                  <button 
                    onClick={() => toggleDayExpansion(dayKey)}
                  ...
                  {eventsOnDay.map((event, idx) => {
                    ...
                    return (
                      <div 
                        key={idx}
                        onClick={() => onEventClick(event)}
                        className="p-1.5 rounded-md border border-gray-100 dark:border-gray-700 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors"
                      >
                        <div className="flex items-center gap-1.5">
                          <div className={`w-2 h-2 rounded-full ${storeStyle.dot}`}></div>
                          <span className="text-xs font-medium text-gray-800 dark:text-gray-200 truncate">{event.title}</span>
                        </div>
                        
                        {event.description && (
                          <div className="text-[10px] text-gray-500 dark:text-gray-400 mt-0.5 truncate pl-3.5">
                            {event.description}
                          </div>
                        )}
                        ...
                        {((event.dispensers && event.dispensers.length > 0) || (event.services && event.services.length > 0)) && (
                          <div className="mt-1 pt-1 border-t border-gray-100 dark:border-gray-700 flex items-center gap-2">
                            {event.dispensers && event.dispensers.length > 0 && (
