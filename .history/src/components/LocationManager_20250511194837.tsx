...
const LocationManager: React.FC<LocationManagerProps> = ({ 
  ...
  useEffect(() => {
    const loadSavedLocations = async () => {
      ...
      try {
        // Use a local variable first to avoid state updates if the operation fails
        let savedLocations: CustomLocation[] = [];
        
        try {
          // Wrap in an extra try/catch to prevent uncaught promises
          const loadPromise = locationStorageService.loadLocations();
          
          // Use Promise with timeout to prevent hanging if the operation takes too long
          const timeoutPromise = new Promise<CustomLocation[]>((_, reject) => {
            setTimeout(() => reject(new Error('Location loading timed out')), 5000);
          });
          
          // Race between the actual load and the timeout
          savedLocations = await Promise.race([loadPromise, timeoutPromise]);
          console.log('Successfully loaded locations:', savedLocations.length);
        } catch (storageErr) {
          console.error('Storage service error:', storageErr);
          
          // Try to get data from localStorage directly as a last resort
          try {
            const backupData = localStorage.getItem('mapview_custom_locations');
            if (backupData) {
              // Sanitize the backup data to prevent JSON parse errors
              const cleanData = backupData.replace(/[^\x20-\x7E]/g, '');
              
              try {
                const parsedData = JSON.parse(cleanData);
                if (Array.isArray(parsedData) && parsedData.length > 0) {
                  savedLocations = parsedData.map(loc => ({
                  ...
                  console.log('Recovered locations from localStorage backup:', savedLocations.length);
                }
              } catch (parseErr) {
                console.error('Failed to parse backup data:', parseErr);
              }
            }
          } catch (lsErr) {
            console.error('Failed to access localStorage fallback:', lsErr);
          }
        }
  ...
  const handleAddLocation = async () => {
    ...
    try {
      ...
      // Then try to save to persistent storage
      try {
        await locationStorageService.saveLocations(updatedLocations);
      } catch (storageErr) {
        console.error('Error saving to storage service:', storageErr);
        
        // Try direct localStorage save as fallback
        try {
          const sanitizedLocations = updatedLocations.map(loc => ({
            id: String(loc.id),
          ...
          localStorage.setItem('mapview_custom_locations', JSON.stringify(sanitizedLocations));
          ...
          // The state is already updated, so the user won't lose data in this session
        }
      }
  ...
  const handleDeleteLocation = async (locationId: string) => {
    try {
      ...
      // Then try to persist the change
      try {
        await locationStorageService.saveLocations(updatedLocations);
      } catch (storageErr) {
        console.error('Error saving to storage service after deletion:', storageErr);
        ...
        try {
          ...
          localStorage.setItem('mapview_custom_locations', JSON.stringify(sanitizedLocations));
          console.log('Saved updated locations after deletion directly to localStorage');
        } catch (localStorageErr) {
          console.error('Failed to save to localStorage after deletion:', localStorageErr);
        }
      }
