  ...
  React.useEffect(() => {
    if (workOrder) {
      ...
      if (isPotentiallySpecificJob) {
        ...
        // Don't add a warning when we've successfully identified dispensers, but let's provide the info elsewhere
      }

      let calculationOrder = workOrder;
      let hasDispenserData = workOrder.dispensers && workOrder.dispensers.length > 0;
      ...
      // Add any calculation warnings to our verification warnings list, being more selective
      warnings.forEach(warning => {
        const warningText = warning.warning.toLowerCase();
        
        // Be more selective about which warnings to show
        if ((warningText.includes('verify') && !warningText.includes('standard')) || 
            warningText.includes('unknown fuel') || 
            (warning.severity !== undefined && warning.severity >= 7)) {
      ...
      // Check for lack of dispenser data
      if (!hasDispenserData) {
        // Only warn about missing dispenser data if we need it for calculations
        // For example, when there's a specific dispenser job or when there's a service that requires dispenser data
        const needsDispenserData = isPotentiallySpecificJob || 
        ...
        if (needsDispenserData) {
          newVerificationWarnings.push("No dispenser data available - filter calculations are estimates only.");
        }
      }

      // Check for special fuel types
      const { hasDEF, hasDieselHighFlow } = checkForSpecialFuelTypes(calculationOrder);
      
      if (hasDEF) {
        // Only add DEF warning if it's clearly indicated and not just a passing mention
        const strongDefIndicator = 
          workOrder.services?.some(service => 
            (service.description?.toLowerCase() || '').includes("def filter") ||
            (service.description?.toLowerCase() || '').includes("diesel exhaust fluid filter")
          ) ||
          (workOrder.instructions?.toLowerCase() || '').includes("def filter") ||
          (workOrder.instructions?.toLowerCase() || '').includes("diesel exhaust fluid filter");
            
        if (strongDefIndicator) {
          newVerificationWarnings.push("DEF detected - check if special DEF filters are needed.");
        }
      }
      
      if (hasDieselHighFlow) {
        // Only add high-flow warning if it's clearly indicated and not just a passing mention
        const strongHighFlowIndicator = 
          workOrder.services?.some(service => 
            (service.description?.toLowerCase() || '').includes("high flow diesel filter") ||
            (service.description?.toLowerCase() || '').includes("diesel high flow filter")
          ) ||
          (workOrder.instructions?.toLowerCase() || '').includes("high flow diesel filter") ||
          (workOrder.instructions?.toLowerCase() || '').includes("diesel high flow filter");
            
        if (strongHighFlowIndicator) {
          newVerificationWarnings.push("High Flow Diesel detected - verify filter specifications.");
        }
      }
