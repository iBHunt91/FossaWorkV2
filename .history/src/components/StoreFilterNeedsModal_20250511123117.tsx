...
const StoreFilterNeedsModal: React.FC<StoreFilterNeedsModalProps> = ({
  ...
  React.useEffect(() => {
    if (workOrder) {
      ...
      if (isSpecificDispensersJob && specificDispenserNumbers.length > 0 && workOrder.dispensers && workOrder.dispensers.length > 0) {
      ...
      // Get full result from calculateFiltersForWorkOrder
      const { gasFilters, dieselFilters, warnings } = calculateFiltersForWorkOrder(calculationOrder);

      // Process warnings for verification messages
      warnings.forEach(warning => {
        const warningText = warning.warning.toLowerCase();
        if (warningText.includes('verify') || warningText.includes('manual verification') || warningText.includes('unknown fuel type')) {
    ...
    <div className={`fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50 transition-opacity duration-300 ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
      <div className={`${modalBgClass} rounded-lg shadow-xl p-6 w-full max-w-lg transform transition-all duration-300 ${isOpen ? 'scale-100 opacity-100' : 'scale-95 opacity-0'}`}>
        ...
        <div className="text-sm mb-2">
          ...
          <p><strong>Store #:</strong> {workOrder.customer?.storeNumber || 'N/A'}</p>
        </div>
