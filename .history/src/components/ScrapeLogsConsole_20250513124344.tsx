...
const ScrapeLogsConsole: React.FC<ScrapeLogsConsoleProps> = ({
  ...
  const formatTimestamp = (timestamp: string) => {
    ...
    // Determine message type for styling and icons  const getMessageType = (message: string) => {    // First check if the message already has an emoji prefix added by the server    const emojiMatch = message.match(/^([\p{Emoji}\u200d]+)\s+/u);    if (emojiMatch && emojiMatch[1]) {      const emoji = emojiMatch[1].trim();            // Map known emojis to their message types      switch(emoji) {        case '❌':          return 'error';        case '⚠️':          return 'warn';        case '✅':          return 'success';        case 'ℹ️':        case '🔧':        case '🌐':        case '⚙️':          return 'info';        case '⏳':        case '🔄':        case '🚀':          return 'progress';        default:          return 'default';      }    }        // Fall back to content-based detection if no emoji prefix    const lowerMessage = message.toLowerCase();    if (lowerMessage.includes('error')) return 'error';    if (lowerMessage.includes('warn')) return 'warn';    if (lowerMessage.includes('success') || lowerMessage.includes('completed')) return 'success';    if (lowerMessage.includes('start') || lowerMessage.includes('initialize')) return 'info';    if (lowerMessage.includes('progress') || lowerMessage.includes('processing')) return 'progress';    return 'default';  };  // Don't add extra icons to messages, as we're now adding them server-side  const getMessageIcon = (type: string) => {    // If we want to keep icons here for legacy messages without emojis    return '';  };
