            ...
            if (backupData) {
              ...
              try {
                ...
                if (Array.isArray(parsedData) && parsedData.length > 0) {
                  // Use our deserializeLocation helper for each item
                  savedLocations = parsedData.map(loc => {
                    // Try to deserialize the location object safely
                    const locJson = JSON.stringify(loc);
                    const deserialized = deserializeLocation(locJson);
                    
                    // Fall back to manual conversion if deserialization fails
                    return deserialized || {
  ...
  const handleAddLocation = async () => {
    ...
    try {
      ...
      try {
        ...
        saveLocationToLocalStorageFallback(updatedLocations);
      }
