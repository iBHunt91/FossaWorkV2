...
// Define the fuel grades directly to match the exact order in data/fuel_grades.js
const fuelGrades: string[] = [
  'Regular',
  'Anti-Freeze',
  'Regular/Premium',
...
const DispenserModal: React.FC<DispenserModalProps> = ({ 
  ...
  // Log the visitNumber prop to help debug
  useEffect(() => {
    console.log('DispenserModal received visitNumber:', visitNumber);
  }, [visitNumber]);
  ...
  const defaultSortFuelTypes = (gradeString: string): string[] => {
    ...
    return grades.sort((a: string, b: string) => {
      const indexA = fuelGrades.findIndex((g: string) => g.toLowerCase() === a.toLowerCase());
      const indexB = fuelGrades.findIndex((g: string) => g.toLowerCase() === b.toLowerCase());
  ...
  // Format the visit number for display
  const displayVisitNumber = () => {
    if (visitNumber === undefined || visitNumber === null || visitNumber === '') {
      return '—';
    }
    return visitNumber;
  };
          ...
          <div className="bg-gradient-to-r from-primary-600 to-primary-700 dark:from-primary-700 dark:to-primary-800 px-4 py-2 rounded-t-xl">
            <div className="flex justify-between items-center">
              ...
              >
                ...
                <span className="whitespace-nowrap">Visit #{displayVisitNumber()}</span> 
                {orderId && (
