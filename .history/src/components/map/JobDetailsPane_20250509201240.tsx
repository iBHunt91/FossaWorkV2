...
const JobDetailsPane: React.FC<JobDetailsPaneProps> = ({ selectedJob }) => {
  ...
  // Attempt to get dispenser count (similar logic to JobMap popup)
  let dispenserCount = 'N/A';
  if (selectedJob.serviceTypes && Array.isArray(selectedJob.serviceTypes)) {
    const dispenserServiceRegex = /dispenser|pump|fuel/i;
    const dispenserServices = selectedJob.serviceTypes.filter(service => 
  ...
  if (dispenserCount === 'N/A' && selectedJob.description) {
