...
const JobDetailsPane: React.FC<JobDetailsPaneProps> = ({ selectedJob }) => {
  ...
  if (selectedJob.dispensers && Array.isArray(selectedJob.dispensers) && selectedJob.dispensers.length > 0) {
    ...
    // Fallback to regex checks if dispensers array is not present or empty
    console.log("Dispensers array not found or empty, falling back to regex checks for description/instructions.");
