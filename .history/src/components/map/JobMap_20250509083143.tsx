...
const JobMap: React.FC<JobMapProps> = ({ jobs, selectedJob, onSelectJob }) => {
  ...
  useEffect(() => {
    ...
    console.log(`Filtered jobs from ${jobs.length} to ${filtered.length} for date: ${filterDate.toLocaleDateString()}`);
  }, [jobs, dateFilter]);
    ...
    <div className="absolute inset-0 w-full h-full">
      ...
      <div className="absolute top-3 left-3 z-10">
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-3 border border-gray-200 dark:border-gray-700">
          <button 
            className="flex items-center justify-center space-x-2 text-sm font-medium text-gray-700 dark:text-gray-200 hover:text-blue-600 dark:hover:text-blue-400"
          ...
          {showDateFilter && (
            <div className="mt-2">
              <DatePicker
                ...
                className="w-full p-2 text-sm border border-gray-300 dark:border-gray-600 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-white"
                placeholderText="Select a date"
                isClearable
                onClickOutside={() => setShowDateFilter(false)}
              />
              {dateFilter && (
                <button
                  onClick={() => setDateFilter(null)}
                  className="mt-2 w-full text-xs text-gray-500 dark:text-gray-400 hover:text-red-500 dark:hover:text-red-400 flex items-center justify-center"
                >
                  <FiX className="w-3 h-3 mr-1" /> Clear Filter
                </button>
              )}
            </div>
          )}
      ...
      {/* Filter indicators */}
      {dateFilter && filteredJobs.length !== jobs.length && (
        <div className="absolute bottom-3 left-3 right-3 bg-white/90 dark:bg-gray-800/90 p-2 rounded-lg shadow-md">
          <div className="flex items-center justify-between">
            <span className="text-sm text-gray-700 dark:text-gray-300">
              Showing {filteredJobs.length} of {jobs.length} jobs for {dateFilter.toLocaleDateString()}
            </span>
            <button 
