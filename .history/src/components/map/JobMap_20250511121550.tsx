  ...
  useEffect(() => {
    ...
    jobs.forEach(job => {
      ...
      try {
        ...
        markerEl.style.width = '30px'; // Slightly larger for icon
        markerEl.style.height = '30px';
        markerEl.style.backgroundColor = '#0891b2'; // Cyan-600
        ...
        markerEl.style.transition = 'transform 0.15s ease-out, boxShadow 0.15s ease-out'; // For hover
        // SVG for a map pin icon (white stroke, no fill)
        ...
        // Store default styles for hover reset
        const defaultStyle = {
          transform: '',
        ...
        markerEl.addEventListener('mouseenter', () => {
          if (selectedJob?.id !== job.id) { // Only apply hover if not selected
        ...
        markerEl.addEventListener('mouseleave', () => {
          if (selectedJob?.id !== job.id) { // Only reset if not selected
            markerEl.style.transform = defaultStyle.transform;
            markerEl.style.boxShadow = defaultStyle.boxShadow;
          }
        });
  ...
  useEffect(() => {
    ...
    if (prevSelectedJobIdRef.current && prevSelectedJobIdRef.current !== selectedJob?.id) {
      ...
      if (prevMarker) {
        ...
        // Reset to default style (using stored or re-declared default)
        prevMarkerEl.style.backgroundColor = defaultStyle.backgroundColor; // Cyan-600
        prevMarkerEl.style.border = defaultStyle.border;
        prevMarkerEl.style.width = defaultStyle.width;
        prevMarkerEl.style.height = defaultStyle.height;
        prevMarkerEl.style.boxShadow = defaultStyle.boxShadow;
        prevMarkerEl.innerHTML = defaultStyle.innerHTML;
        prevMarkerEl.style.zIndex = '';
        prevMarkerEl.style.transform = ''; // Ensure hover scale is reset
      }
    }
    ...
    if (selectedJob && selectedJob.coordinates) {
      ...
      if (currentMarker) {
        ...
        // Larger white map pin icon for selected
        ...
        currentMarkerEl.style.transform = 'scale(1.1)'; // Apply a slight scale to selected, or keep as is
