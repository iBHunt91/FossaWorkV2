  ...
  useEffect(() => {
    ...
    locations.forEach(location => {
      ...
      try {
        ...
        // If the title contains the word "Order" and coordinates are round numbers or along common grid lines, likely fallback
        const isProbablyFallbackLocation = 
          location.title.includes("Order W-") || 
          (Math.abs(location.lat - Math.round(location.lat)) < 0.05 && 
           Math.abs(location.lng - Math.round(location.lng)) < 0.05) ||
          // Detect approximated locations from our clustering algorithm
          (location.lat > 35 && location.lat < 42 && 
           location.lng > -98 && location.lng < -75 && 
           Math.abs((location.lat * 10) % 2) < 0.1);
          
        // Create marker element with visual distinction for fallback locations
        const markerEl = document.createElement('div');
        const markerSize = isProbablyFallbackLocation ? '24px' : '28px';
        ...
        Object.assign(markerEl.style, {
          ...
          zIndex: isProbablyFallbackLocation ? '5' : '10', // Real locations appear on top
          opacity: isProbablyFallbackLocation ? '0.8' : '1',
        });
        ...
        markerEl.innerHTML = isProbablyFallbackLocation
          ? '<svg viewBox="0 0 24 24" width="12" height="12" stroke="white" stroke-width="2.5" fill="none" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"></circle><line x1="12" y1="8" x2="12" y2="12"></line><line x1="12" y1="16" x2="12.01" y2="16"></line></svg>'
        ...
        markerEl.addEventListener('mouseleave', () => {
          ...
          markerEl.style.opacity = isProbablyFallbackLocation ? '0.8' : '1';
        });
        ...
        // Add popup with different content based on location type
        const popupContent = isProbablyFallbackLocation
          ? `<div class="p-2"><strong>${location.title}</strong><div class="text-xs text-gray-500 mt-1">Approximate Location</div></div>`
        ...
        const popup = new mapboxgl.Popup({
          ...
          closeOnClick: true,
          offset: 25,
          className: isProbablyFallbackLocation ? 'fallback-location-popup' : ''
        ...
        // For detailed view, we can add click handlers
        if (detailedView) {
          markerEl.addEventListener('click', () => {
            console.log(`Clicked on location: ${location.id} - ${location.title}`);
            // Here you could dispatch an event or call a callback for location selection
          });
        }
