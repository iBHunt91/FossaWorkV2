  ...
  useEffect(() => {
    ...
    if (selectedJob && selectedJob.coordinates) {
      ...
      if (currentMarker) {
        ...
        if (enableFlyTo) {
          // Apply full prominent style when flying to marker
          ...
          currentMarkerEl.style.boxShadow = '0 0 0 4px rgba(255, 0, 0, 0.6), 0 5px 15px rgba(0,0,0,0.5)'; 
          ...
          currentMarkerEl.style.zIndex = '10'; 
          currentMarkerEl.style.transform = 'scale(1.1)'; 
        } else {
          // Apply minimal style change (color only) when not flying (View All mode)
          currentMarkerEl.style.backgroundColor = '#FF0000'; // Bright Red
          currentMarkerEl.style.border = defaultMarkerStyle.border;
          currentMarkerEl.style.width = defaultMarkerStyle.width;
          currentMarkerEl.style.height = defaultMarkerStyle.height;
          currentMarkerEl.style.boxShadow = defaultMarkerStyle.boxShadow;
          currentMarkerEl.style.innerHTML = defaultMarkerStyle.innerHTML;
          currentMarkerEl.style.zIndex = '10';
          currentMarkerEl.style.transform = defaultMarkerStyle.transform; // Ensure no scaling
        }

        // Pan and zoom to the selected marker only if enabled (and coordinates are valid)
        if (enableFlyTo) {
