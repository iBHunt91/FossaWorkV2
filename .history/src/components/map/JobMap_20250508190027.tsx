...
const JobMap: React.FC<JobMapProps> = ({ jobs, selectedJob, onSelectJob }) => {
  ...
  useEffect(() => {
    ...
    if (!hasMapboxToken) {
      setMapError('Mapbox access token is missing. Please provide a valid token in your environment variables (VITE_MAPBOX_ACCESS_TOKEN).');
      return;
    }
  ...
  // Display a message if there's an error with the map
  if (mapError) {
    return (
      <div className="map-error">
        <h3>Map Error</h3>
        <p>{mapError}</p>
        <p>Your jobs are still available in the list view.</p>
      </div>
    );
  }
