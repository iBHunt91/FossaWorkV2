  ...
  useEffect(() => {
    ...
    if (prevSelectedJobIdRef.current && prevSelectedJobIdRef.current !== selectedJob?.id) {
      ...
      if (prevMarker) {
        ...
        prevMarkerEl.style.boxShadow = '0 2px 6px rgba(0,0,0,0.5)'; // Explicitly reset to default shadow
    ...
    if (selectedJob && selectedJob.coordinates) {
      ...
      if (currentMarker) {
        ...
        // Apply more distinct selected style
        currentMarkerEl.style.backgroundColor = '#ef4444'; // Vibrant Red (Tailwind red-500)
        currentMarkerEl.style.border = '3px solid white';    // Thicker white border
        currentMarkerEl.style.width = '32px';               // Even larger
        currentMarkerEl.style.height = '32px';
        currentMarkerEl.style.boxShadow = '0 0 0 2px #ef4444, 0 4px 12px rgba(0,0,0,0.6)'; // Outer glow, stronger shadow
        currentMarkerEl.innerHTML = '<span style="color: white; font-size: 16px; font-weight: bold;">★</span>'; // Larger star
