...
const JobDetailsPane: React.FC<JobDetailsPaneProps> = ({ selectedJob }) => {
  ...
  if (dispenserCount === 'N/A') {
    ...
    if (selectedJob.description) {
      const numericMatch = selectedJob.description.match(/(\d+)\s*dispensers?/i); // Added s? for singular/plural
      if (numericMatch && numericMatch[1]) {
        dispenserCount = numericMatch[1];
        console.log(`JobDetailsPane: Dispenser count from description (numeric regex): ${dispenserCount}`);
        foundByRegex = true;
      } else {
        const allMatch = selectedJob.description.match(/All\s*Dispensers?/i);
        if (allMatch) {
          dispenserCount = 'All';
          console.log(`JobDetailsPane: Dispenser count from description (found "All Dispensers"): ${dispenserCount}`);
          foundByRegex = true;
        }
      }
    }

    if (!foundByRegex && selectedJob.instructions) { // Only check instructions if not found in description
      const numericMatch = selectedJob.instructions.match(/(\d+)\s*dispensers?/i); // Added s? for singular/plural
      if (numericMatch && numericMatch[1]) {
        dispenserCount = numericMatch[1];
        console.log(`JobDetailsPane: Dispenser count from instructions (numeric regex): ${dispenserCount}`);
      } else {
        const allMatch = selectedJob.instructions.match(/All\s*Dispensers?/i);
        if (allMatch) {
          dispenserCount = 'All';
          console.log(`JobDetailsPane: Dispenser count from instructions (found "All Dispensers"): ${dispenserCount}`);
        }
      }
    }
  }
