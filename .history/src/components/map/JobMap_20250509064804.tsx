  ...
  useEffect(() => {
    ...
    jobs.forEach(job => {
      ...
      try {
        ...
        // Debug the dispensers data structure
        console.log(`Job ${job.id} dispensers:`, job.dispensers);
        
        // Improved dispenser count logic with better type handling
        let dispenserCount = 'N/A';
        if (job.dispensers !== undefined && job.dispensers !== null) {
          ...
          } else if (typeof job.dispensers === 'object' && 'length' in job.dispensers) {
            // Handle object with length property
            dispenserCount = String((job.dispensers as { length: number }).length);
          } else if (typeof job.dispensers === 'object') {
            // If it's an object, try to count keys or entries
            const keys = Object.keys(job.dispensers as object);
            if (keys.length > 0) {
              // Check if any of the keys contain an array of dispensers
              for (const key of keys) {
                const value = (job.dispensers as any)[key];
                if (Array.isArray(value)) {
                  dispenserCount = String(value.length);
                  break;
                }
              }
              
              // If no arrays found, just count the keys
              if (dispenserCount === 'N/A') {
                dispenserCount = String(keys.length);
              }
            }
          } else if (typeof job.dispensers === 'string' && !isNaN(parseInt(job.dispensers))) {
            // Handle string that can be parsed as number
            dispenserCount = job.dispensers;
          }
        }
        
        console.log(`Job ${job.id} dispenser count:`, dispenserCount);
