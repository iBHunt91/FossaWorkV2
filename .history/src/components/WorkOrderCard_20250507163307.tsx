...
const WorkOrderCard: React.FC<WorkOrderCardProps> = ({ 
  ...
  return (
    ...
    >
      ...
      <div className="relative p-4 bg-gradient-to-r from-gray-50 to-white dark:from-gray-700 dark:to-gray-750 border-b border-gray-200 dark:border-gray-600">
        <div className="flex justify-between items-start">
          ...
          <div className="flex flex-col items-end">
            <div className={`flex items-center ${statusColor} text-xs font-medium`}>
              {statusIcon}
              <span>{workOrder.visits.nextVisit.date}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-gray-50 dark:bg-gray-700/80 px-4 py-3 border-t border-gray-200 dark:border-gray-600 flex items-center justify-between relative z-10">
        <div className="flex items-center">
          {statusIcon}
          <span className={`text-xs font-medium ${statusColor}`}>
            {formattedDate <= today ? "Due now" : formattedDate <= oneWeek ? "Due soon" : "Scheduled"}
          </span>
        </div>
        
        <div className="flex items-center text-xs">
          <span className="flex items-center text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-gray-600 px-2 py-0.5 rounded-md">
            <FiTag className="mr-1" size={12} />{totalDispensers} Dispensers
          </span>
        </div>
      </div>
    </div>
