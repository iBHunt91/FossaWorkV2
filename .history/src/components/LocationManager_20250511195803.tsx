...
const LocationManager: React.FC<LocationManagerProps> = ({ 
  ...
  // Electron availability tracking
  const [useElectron, setUseElectron] = useState<boolean>(
    !BYPASS_ELECTRON_STORAGE && !!window.electron && !!window.electron.fs
  );
  ...
  useEffect(() => {
    const loadSavedLocations = async () => {
      ...
      try {
        ...
        // If we're bypassing Electron or Electron failed in previous attempts, go directly to localStorage
        if (!useElectron) {
          console.log('Bypassing Electron storage, using localStorage directly');
          ...
          try {
            ...
            // If service fails, disable Electron for future operations in this session
            setUseElectron(false);
