...
const ProverPreferences: React.FC = () => {
  ...
  const dropdownListRefs = useRef<{ [key: string]: HTMLDivElement | null }>({});
  ...
  useEffect(() => {
    const handleOutsideClick = (e: MouseEvent) => {
      if (openDropdownId && dropdownRefs.current[openDropdownId] && 
          ...
          !buttonRefs.current[openDropdownId]?.contains(e.target as Node)) {
  ...
  const calculateDropdownPosition = (proverId: string) => {
    if (!buttonRefs.current[proverId]) return 'bottom';
    
    const buttonRect = buttonRefs.current[proverId]?.getBoundingClientRect();
    if (!buttonRect) return 'bottom';
    
    // Get window height and check if there's enough space below
    const windowHeight = window.innerHeight;
    const spaceBelow = windowHeight - buttonRect.bottom;
    const requiredSpace = 280; // dropdown max height
    
    // Update position state
    if (spaceBelow < requiredSpace && buttonRect.top > requiredSpace) {
      return 'top';
    }
    return 'bottom';
  };
  ...
  const toggleDropdown = (proverId: string) => {
    if (openDropdownId === proverId) {
      setOpenDropdownId(null);
    } else {
      const position = calculateDropdownPosition(proverId);
      setDropdownPositions({ ...dropdownPositions, [proverId]: position as 'bottom' | 'top' });
      setOpenDropdownId(proverId);
    }
  };
  ...
  const handleDropdownToggle = (providerId: string, event: React.MouseEvent) => {
    event.stopPropagation();
    
    if (openDropdownId === providerId) {
      setOpenDropdownId(null);
    } else {
      setOpenDropdownId(providerId);
      
      // Position dropdown properly on next render cycle
      setTimeout(() => {
        if (dropdownListRefs.current[providerId]) {
          const dropdown = dropdownListRefs.current[providerId]!;
          ...
          const rightOverflow = parentRect.left + dropdownRect.width - viewportWidth;
          if (rightOverflow > 0) {
              ...
              <div className="p-5 border-t border-gray-700 dark:border-gray-700 bg-gray-800 dark:bg-gray-800">
                <div>
                  ...
                  <div className="flex flex-wrap gap-2 mb-4">
                    ...
                    <div className="relative inline-block" ref={(el) => { dropdownRefs.current[prover.prover_id] = el; }}>
                      <button
                        type="button"
                        ...
                        onClick={(e) => handleDropdownToggle(prover.prover_id, e)}
                      >
                      ...
                      {openDropdownId === prover.prover_id && (
                        <div 
                          ref={(el) => { dropdownListRefs.current[prover.prover_id] = el; }}
                          className="absolute z-[9999] mt-2 bg-gray-700 dark:bg-gray-700 border border-gray-600 dark:border-gray-600 rounded-lg shadow-lg"
                          style={{ 
                            width: '220px',
                            maxHeight: '280px',
                            overflow: 'auto',
