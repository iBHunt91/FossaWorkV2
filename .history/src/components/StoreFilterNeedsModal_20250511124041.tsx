...
const StoreFilterNeedsModal: React.FC<StoreFilterNeedsModalProps> = ({
  ...
  React.useEffect(() => {
    if (workOrder) {
      ...
      // Consistent verification check with Filters.tsx
      let jobNeedsVerification = false;
      if (workOrder.services?.some(s => s.code === "2862")) {
        jobNeedsVerification = true;
        newVerificationWarnings.push("Service code 2862 (Specific Dispenser) requires verification.");
      }

      warnings.forEach(warning => {
        const warningText = warning.warning.toLowerCase();
        // Match Filters.tsx logic: 'verify' or 'check'
        if (warningText.includes('verify') || warningText.includes('check')) {
          jobNeedsVerification = true;
          if (!newVerificationWarnings.includes(warning.warning)) { // Avoid duplicate messages
