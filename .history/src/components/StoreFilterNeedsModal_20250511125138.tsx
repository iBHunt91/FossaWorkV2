    ...
    if (workOrder) {
      ...
      if (isPotentiallySpecificJob) {
        if (specificDispenserNumbers.length === 0) {
          ...
          newVerificationWarnings.push(`Specific dispensers to service: ${specificDispenserNumbers.join(', ')}`);
        }
      }
      ...
      // Add any calculation warnings to our verification warnings list
      warnings.forEach(warning => {
        const warningText = warning.warning.toLowerCase();
        
        // Add meaningful warnings for verification
        if (warningText.includes('verify') || 
            warningText.includes('unknown') || 
            warningText.includes('standard') ||
            warningText.includes('estimation') ||
            warning.severity >= 6) {  // Add warning if severity is medium-high or higher
      ...
      if (storeName.includes('7-eleven') || storeName.includes('7 eleven') || storeName.includes('speedway')) {
        ...
        gasFilterPart = STATION_FILTERS['7-Eleven'].GAS[meterType];
        dieselFilterPart = STATION_FILTERS['7-Eleven'].DIESEL[meterType];
