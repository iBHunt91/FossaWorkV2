...
const StoreFilterNeedsModal: React.FC<StoreFilterNeedsModalProps> = ({
  ...
  React.useEffect(() => {
    if (workOrder) {
      ...
      if (isPotentiallySpecificJob && specificDispenserNumbers.length > 0 && workOrder.dispensers && workOrder.dispensers.length > 0) {
      ...
      // More precise verification check that matches Filters.tsx
      let shouldShowVerificationWarning = false;

      // Only treat specific dispenser jobs as needing verification if we couldn't determine specific dispensers
      if (isSpecificDispensersJobService && specificDispenserNumbers.length === 0) {
        shouldShowVerificationWarning = true;
        newVerificationWarnings.push("Specific dispenser job - requires verification of which dispensers to service.");
      }

      warnings.forEach(warning => {
        // Carefully filter warnings to match Filters.tsx display logic
        const warningText = warning.warning.toLowerCase();
        
        // Only warnings containing "verify" will trigger the verification warning display
      ...
      const stationType = workOrder.customer.name.includes('7-Eleven') ||
                         (workOrder.rawHtml && (workOrder.rawHtml.includes('Speedway') || workOrder.rawHtml.includes('Marathon'))) ?
                         '7-Eleven' :
                         workOrder.customer.name.includes('Wawa') ? 'Wawa' : 'Circle K';
      ...
      if (stationType === '7-Eleven') {
        ...
        dieselFilterPart = dieselMeterTypes[validMeterTypeKey as keyof typeof dieselMeterTypes];

        const { hasDEF } = checkForSpecialFuelTypes(workOrder);
        if (hasDEF) {
    ...
    >
      ...
      >
        ...
        {filterNeeds.length > 0 ? (
          ...
          // Only show this if there are no filter needs AND no verification warnings
          verificationWarnings.length === 0 && (
