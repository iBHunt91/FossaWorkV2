...
// Move JobCard component outside of the main component to prevent state resets
const JobCard: React.FC<{ 
  ...
  // Handle instructions toggle with explicit prevention
  const toggleInstructions = (e: React.MouseEvent) => {
    e.stopPropagation(); // Prevent event from bubbling up
    e.preventDefault(); // Prevent default behavior
    setShowInstructions(prev => !prev); // Use functional update for reliability
  };
  
  return (
    <div className={`p-3 rounded-md border border-gray-100 dark:border-gray-700 ${storeStyle.bg} hover:brightness-95 dark:hover:brightness-110 transition-colors`}>
      ...
      >
        ...
        {/* Only show dispenser count badge */}
        <div className="flex items-center">
          {event.dispensers && event.dispensers.length > 0 && (
            <div className="rounded-md bg-white/30 dark:bg-gray-700/40 px-2 py-1 flex items-center gap-1.5 text-xs text-gray-700 dark:text-gray-300">
              {/* Gas pump/dispenser icon */}
              <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M4 5C4 3.89543 4.89543 3 6 3H14C15.1046 3 16 3.89543 16 5V21H4V5Z" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/>
                <path d="M16 9H19C19.5523 9 20 9.44772 20 10V17C20 18.1046 19.1046 19 18 19V19" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/>
                <path d="M10 12V14" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/>
                <path d="M20 7L19 8" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/>
                <path d="M4 21H16" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/>
              </svg>
              <span className="font-semibold">{event.dispensers.length}</span>
            </div>
          )}
        </div>
      </div>
          ...
          <div className="flex flex-col space-y-1 overflow-y-auto" style={{ maxHeight: "94px" }}>
            ...
            {isExpanded && (
              <>
                ...
                <div 
                  className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-50 bg-white dark:bg-gray-800 shadow-xl rounded-lg border border-gray-200 dark:border-gray-600 p-4 overflow-auto origin-center animate-scale-in"
                  style={{ 
                    width: Math.min(650, Math.max(350, 280 + eventsOnDay.length * 30)), 
                ...
                >
                  ...
                  <div className="grid gap-3 grid-cols-1">
                    {eventsOnDay.map((event, idx) => (
                      <JobCard key={idx} event={event} onEventClick={onEventClick} />
                    ))}
                  </div>
                </div>
              </>
