...
const LocationManager: React.FC<LocationManagerProps> = ({ 
  ...
  useEffect(() => {
    ...
    // Fix the dependency - use a proper value instead of a function call
  }, []);

  // Add a separate effect to track electron storage status changes
  useEffect(() => {
    // Create a variable to track if Electron storage is disabled
    const isElectronDisabled = locationStorageService.isElectronStorageDisabled();
    
    // Re-load locations when storage method changes
    const loadSavedLocations = async () => {
      setIsLoading(true);
      try {
        ...
        setError(null);
        console.log(`Reloaded ${savedLocations.length} locations after storage method change`);
      } catch (err) {
        console.error('Failed to reload locations after storage method change:', err);
        setLocations([]);
