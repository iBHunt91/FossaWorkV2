                                      ...
                                      <div className="border-t border-gray-200 dark:border-gray-700 pt-3 space-y-3">
                                        ...
                                        {job.currentVisitPhase && (
                                          <div className="flex items-center text-xs">
                                            <span className="mr-2 text-gray-600 dark:text-gray-400">Phase:</span>
                                        ...
                                        {/* Current Status Text - Simple Version */}
                                        ...
                                        {/* Detailed Dispenser Progress - Extracted */}
                                        {(() => {
                                          const dispenserInfo = extractDispenserProgress(job.currentVisitStatus || '', job.formsTotal); // Use formsTotal if available as fallback total
                                          if (dispenserInfo.current > 0 && dispenserInfo.total > 0) {
                                            const progressPercent = Math.min(Math.round((dispenserInfo.current / dispenserInfo.total) * 100), 100);
                                            return (
                                              <div className="space-y-1">
                                                <div className="flex justify-between items-center text-xs">
                                                  <span className="font-medium text-blue-600 dark:text-blue-400">Dispenser</span>
                                                  <span className="font-mono text-blue-700 dark:text-blue-300">{dispenserInfo.current}/{dispenserInfo.total}</span>
                                                </div>
                                        ...
                                        {/* Detailed Fuel Progress - Extracted */}
                                        {(() => {
                                          const fuelInfo = extractFuelProgress(job.currentVisitStatus || '');
                                          if (fuelInfo.type && fuelInfo.current > 0 && fuelInfo.total > 0) {
                                            const progressPercent = Math.min(Math.round((fuelInfo.current / fuelInfo.total) * 100), 100);
                                            return (
                                              <div className="space-y-1">
                                                <div className="flex justify-between items-center text-xs">
                                                  <span className="font-medium text-green-600 dark:text-green-400">{fuelInfo.type || 'Fuel'}</span>
                                                  <span className="font-mono text-green-700 dark:text-green-300">{fuelInfo.current}/{fuelInfo.total}</span>
                                                </div>
                                                <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1.5">
                                                  <div 
                                        ...
                                        {!job.currentVisitStatus && !job.currentVisitPhase && (
                                          <div className="p-3 bg-blue-50 dark:bg-blue-900/20 rounded border border-blue-200 dark:border-blue-800 text-center">
                                            <FiRefreshCw className="text-blue-500 animate-spin mx-auto mb-2 h-5 w-5" />
                                            <span className="font-medium text-blue-700 dark:text-blue-400 text-sm">Processing Batch</span>
                                            <p className="text-xs text-blue-600 dark:text-blue-500 mt-1">Waiting for first visit update...</p>
                                          </div>
                                        )}
                                      </div>
                                    </div>
