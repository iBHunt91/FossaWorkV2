  ...
  const renderJobRow = (order: WorkOrder, dateGroupClass?: string) => {
    ...
    return (
      ...
      >
        ...
        {/* Action Buttons Footer - Store ID and slide-out toolbar */}
        <div className="p-2 border-t border-gray-700/50 flex items-center justify-between relative group">
          ...
          {/* Button container - Right aligned but properly contained within the card */}
          <div className="h-7 flex items-center">
            {/* Small indicator that toolbar exists - Only visible when not hovering */}
            <div className="h-6 w-6 bg-gray-700 hover:bg-gray-600 rounded-full flex items-center justify-center cursor-pointer transition-opacity group-hover:opacity-0 group-hover:hidden">
              <FiMoreHorizontal className="h-3.5 w-3.5 text-gray-300" />
            </div>
            
            {/* Slide-out button container - Initially invisible but slides in on hover */}
            <div className="inline-flex gap-1 opacity-0 -ml-6 group-hover:opacity-100 group-hover:ml-0 transition-all duration-300 ease-out">
              {/* WorkFossa Button */}
              <button
                onClick={(e) => {
                  e.stopPropagation();
