...
import { FiRefreshCw, FiMapPin, FiTool, FiFileText, FiCalendar, FiClock, FiAlertCircle, FiCheckCircle, FiInfo, FiBarChart2, FiTag, FiGlobe, FiPhone, FiLink } from 'react-icons/fi';
...
const WorkOrdersPage: React.FC = () => {
  ...
  const stats = useMemo(() => {
    ...
    const topServices = Object.entries(categorized.serviceTypes)
      .sort((a, b) => b[1] - a[1])
      .slice(0, 3)
      .map(([name, count]) => ({ name, count }));
    
    // Get top stores
    const topStores = Object.entries(categorized.stores)
      .sort((a, b) => b[1] - a[1])
      .slice(0, 3)
      .map(([name, count]) => ({ name, count }));
      ...
      <div className="bg-gradient-to-r from-gray-800 to-gray-900 dark:from-gray-900 dark:to-gray-950 text-white rounded-xl shadow-lg mb-6 overflow-hidden border border-gray-700 dark:border-gray-800">
        <div className="flex flex-wrap items-center justify-between p-4 gap-3">
          ...
          <div className="flex items-center gap-3">
            {stats && (
              <div className="hidden md:flex items-center gap-4 mr-2 pr-4 border-r border-gray-600">
                <div className="flex items-center">
                  <div className="h-8 w-8 rounded-md bg-accent-red-500/20 flex items-center justify-center mr-2">
                    <FiAlertCircle className="text-accent-red-400" />
                  </div>
                  <div>
                    <p className="text-xs text-gray-400">Due Now</p>
                    <p className="text-lg font-semibold text-white">{stats.dueNow}</p>
                  </div>
                </div>
                
                <div className="flex items-center">
                  <div className="h-8 w-8 rounded-md bg-accent-amber-500/20 flex items-center justify-center mr-2">
                    <FiClock className="text-accent-amber-400" />
                  </div>
                  <div>
                    <p className="text-xs text-gray-400">Due Soon</p>
                    <p className="text-lg font-semibold text-white">{stats.dueSoon}</p>
                  </div>
                </div>
                
                <div className="flex items-center">
                  <div className="h-8 w-8 rounded-md bg-accent-green-500/20 flex items-center justify-center mr-2">
                    <FiCheckCircle className="text-accent-green-400" />
                  </div>
                  <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6 animate-fadeIn">
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md border border-gray-200 dark:border-gray-700 p-4">
            <div className="flex items-start">
              ...
              <div>
                <h3 className="text-sm font-medium text-gray-500 dark:text-gray-400">Total Work Orders</h3>
                <p className="text-2xl font-bold text-gray-900 dark:text-white mt-1">{workOrders.length}</p>
                <div className="mt-2 flex flex-wrap gap-2">
                  <span className="inline-flex items-center px-2 py-0.5 text-xs rounded-md bg-accent-red-100 dark:bg-accent-red-900/30 text-accent-red-700 dark:text-accent-red-300">
                    <FiAlertCircle className="mr-1" size={10} /> {stats.dueNow} Due Now
                  </span>
                  <span className="inline-flex items-center px-2 py-0.5 text-xs rounded-md bg-accent-amber-100 dark:bg-accent-amber-900/30 text-accent-amber-700 dark:text-accent-amber-300">
                    <FiClock className="mr-1" size={10} /> {stats.dueSoon} Due Soon
                  </span>
                  <span className="inline-flex items-center px-2 py-0.5 text-xs rounded-md bg-accent-green-100 dark:bg-accent-green-900/30 text-accent-green-700 dark:text-accent-green-300">
                    <FiCheckCircle className="mr-1" size={10} /> {stats.scheduled} Scheduled
                  </span>
                </div>
              </div>
            </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md border border-gray-200 dark:border-gray-700 p-4">
            <div className="flex items-start">
              <div className="h-10 w-10 rounded-lg bg-purple-100 dark:bg-purple-900/30 flex items-center justify-center mr-3">
                <FiTool className="text-purple-600 dark:text-purple-400" />
              </div>
              <div>
                <h3 className="text-sm font-medium text-gray-500 dark:text-gray-400">Top Services</h3>
                <p className="text-2xl font-bold text-gray-900 dark:text-white mt-1">{stats.totalServices} Total</p>
                <div className="mt-2 space-y-1">
                  {stats.topServices.map((service, index) => (
                    <div key={index} className="flex items-center justify-between">
                      <span className="text-xs text-gray-600 dark:text-gray-400">{service.name}</span>
                      <span className="text-xs font-medium px-2 py-0.5 rounded-md bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300">
                        {service.count}
                      </span>
                    </div>
                  ))}
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md border border-gray-200 dark:border-gray-700 p-4">
            <div className="flex items-start">
              <div className="h-10 w-10 rounded-lg bg-teal-100 dark:bg-teal-900/30 flex items-center justify-center mr-3">
                <FiGlobe className="text-teal-600 dark:text-teal-400" />
              </div>
              <div>
                <h3 className="text-sm font-medium text-gray-500 dark:text-gray-400">Top Locations</h3>
                <p className="text-2xl font-bold text-gray-900 dark:text-white mt-1">{Object.keys(stats.stores).length} Stores</p>
                <div className="mt-2 space-y-1">
                  {stats.topStores.map((store, index) => (
                    <div key={index} className="flex items-center justify-between">
                      <span className="text-xs text-gray-600 dark:text-gray-400 truncate max-w-[140px]">{store.name}</span>
                      <span className="text-xs font-medium px-2 py-0.5 rounded-md bg-teal-100 dark:bg-teal-900/30 text-teal-700 dark:text-teal-300">
                        {store.count}
                      </span>
                    </div>
                  ))}
                ...
                <div className="p-5 animate-fadeIn">
                    ...
                    <div>
                      ...
                      <div className="mb-5">
                        ...
                        <div className="bg-gray-50 dark:bg-gray-700/50 rounded-lg p-4">
                          ...
                          <div className="flex items-start">
                            <div className="flex-shrink-0 w-7 h-7 rounded-full bg-gray-200 dark:bg-gray-600 flex items-center justify-center mr-2">
                              <FiLink className="text-gray-600 dark:text-gray-300" size={14} />
                            </div>
                      ...
                      {/* Appointment Section */}
                      <div className="mb-5">
                        <h4 className="text-sm font-medium text-gray-800 dark:text-gray-200 mb-2">Visit Information</h4>
                  ...
                  {viewTab === 'visit' && (
                    <div>
                      <div className="mb-5">
                        <div className="flex items-center justify-between mb-3">
                          <h4 className="text-sm font-medium text-gray-800 dark:text-gray-200">Visit Details</h4>
                          {getStatusBadge(selectedWorkOrder.visits.nextVisit.date)}
                        </div>
                      ...
                      <div>
                        <div className="flex items-center mb-3">
                          <FiLink className="text-gray-600 dark:text-gray-400 mr-2" />
                          <h4 className="text-sm font-medium text-gray-800 dark:text-gray-200">Resources</h4>
                        </div>
