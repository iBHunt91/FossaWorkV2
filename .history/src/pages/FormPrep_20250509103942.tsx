  ...
  const downloadLogs = () => {
    try {
      ...
      if (window.electron && typeof window.electron.saveFile === 'function') {
        ...
        addDebugLog('SYSTEM', 'Electron saveFile API not available');
        addToast('error', 'Unable to save logs - Electron API not available');
        
        // As a fallback, we'll create a file in the logs directory on the server
        // using the server-side API if available
        fetch('/api/save-logs', {
          method: 'POST',
          headers: {
        ...
        .then(data => {
          if (data.success) {
            addDebugLog('SYSTEM', 'Logs saved through server API', { path: data.path });
            addToast('success', `Debug logs saved through server: ${data.path}`);
          } else {
            addDebugLog('SYSTEM', 'Failed to save logs through server API', data);
            addToast('warning', 'Unable to save logs through server API');
          }
        })
        .catch(err => {
          addDebugLog('SYSTEM', 'Error saving logs through server API', err);
        });
      }
    } catch (error) {
