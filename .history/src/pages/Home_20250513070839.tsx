...
const HomeContent = () => {
  ...
  // Helper function to get valid coordinates from a WorkOrder
  const getValidCoordinates = (order: WorkOrder): { latitude: number; longitude: number } | null => {
    const parseAndValidate = (latInput: any, lonInput: any): { latitude: number; longitude: number } | null => {
      if (latInput == null || lonInput == null) return null; // Handles undefined and null
    ...
    const coordsToTry: Array<[any, any]> = [];

    // Gather all potential coordinate pairs, checking for existence of parent objects
    if (order.coordinates) coordsToTry.push([order.coordinates.latitude, order.coordinates.longitude]);
    if (order.location) coordsToTry.push([order.location.latitude, order.location.longitude]);
    if (order.clientLocation) coordsToTry.push([order.clientLocation.latitude, order.clientLocation.longitude]);
    if (order.address) {
      if (order.address.location) coordsToTry.push([order.address.location.latitude, order.address.location.longitude]);
      if (order.address.coordinates) coordsToTry.push([order.address.coordinates.latitude, order.address.coordinates.longitude]);
      if (order.address.geo) coordsToTry.push([order.address.geo.lat, order.address.geo.lng]);
    }
    if (order.site?.location) coordsToTry.push([order.site.location.latitude, order.site.location.longitude]);
    if (order.client?.location) coordsToTry.push([order.client.location.latitude, order.client.location.longitude]);
    if (order.store?.location) coordsToTry.push([order.store.location.latitude, order.store.location.longitude]);

    for (const [latVal, lonVal] of coordsToTry) {
      const parsed = parseAndValidate(latVal, lonVal);
  ...
  const renderMapPanel = () => {
    ...
    const locationsToShow = inRangeOrders
      .filter(order => {
          const extractedCoords = getValidCoordinates(order);
          const hasLocation = extractedCoords !== null;
          
          if (!hasLocation) {
              console.log(`[renderMapPanel Map] Order ${order.id}: Missing or invalid location data after parsing attempt.`);
      ...
      .map(order => {
        const extractedCoords = getValidCoordinates(order)!; // Safe to use ! due to the filter above
        return {
          id: order.id,
