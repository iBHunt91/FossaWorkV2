...
const FormPrep: React.FC<{}> = () => {
  ...
  const downloadLogs = async () => {
    try {
      ...
      try {
        // Try to write directly to logs directory with fetch
        const logsRequest = await fetch('/logs-save', {
          method: 'POST',
          headers: {
        ...
        if (logsResponse.success) {
          addToast('success', `Logs saved to project directory: ${logsResponse.path}`);
        } else {
          console.error('Failed to save logs to project directory:', logsResponse.error);
        }
      } catch (error) {
