...
const FormPrep: React.FC<{}> = () => {
  ...
  const handleBatchProcess = async () => {
    ...
    try {
      ...
      // Prepare the batch processing request
      const result: BatchProcessResult = await processBatchVisits(
        selectedVisits.join(','), // Convert array to comma-separated string
        isHeadless,
        resumeBatch ? lastFailedBatch?.completedVisits || 0 : 0
      );
      ...
      if (result.success) {
        ...
        const newBatchJob: BatchJob = {
          ...
          status: 'running',
          message: result.message || 'Processing batch visits...',
          totalVisits: result.totalVisits || selectedVisits.length,
          completedVisits: 0,
          headless: isHeadless
        };
        ...
        setIsProcessing(false);
        addToast('error', result.message || 'Failed to start batch processing');
      }
    } catch (error) {
      console.error('Error starting batch process:', error);
      setIsProcessing(false);
      addToast('error', 'Failed to start batch processing');
    }
  };
  ...
  const startBatchPolling = () => {
      ...
      try {
        ...
        if (status) {
          ...
          setBatchJobs(prev => {
            ...
            return updatedJobs; // Return the updated array to fix the type error
          });
        }
      } catch (error) {
    ...
    // Don't return anything to avoid the void return type error
  };
