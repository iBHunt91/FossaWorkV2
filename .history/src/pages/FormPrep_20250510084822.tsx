...
interface BatchAutomationStatus {
  ...
  currentVisitStatus?: string;
  currentVisitDetailedMessage?: string; // New field for detailed message of the current visit
  timestamp?: string;
  dispenserCount?: number;
  ...
  const startBatchPolling = () => {
    ...
    const poll = async () => {
        ...
        setBatchJobs(prevJobs => {
          ...
          jobToUpdate.currentVisitStatus = response.currentVisitStatus || jobToUpdate.currentVisitStatus; // General status for the current visit
          ...
          // --- START: New logic to parse detailed message for current visit progress ---
          if (response.currentVisitDetailedMessage) {
            const dispenserCountForCurrentVisit = response.dispenserCount; // This might be the total for *this* visit from API
            const dispenserProgress = extractDispenserProgressLocal(response.currentVisitDetailedMessage, dispenserCountForCurrentVisit);
            const fuelProgress = extractFuelProgressLocal(response.currentVisitDetailedMessage);

            jobToUpdate.formsCurrent = dispenserProgress.current;
            // If extractDispenserProgressLocal got a total, use it, otherwise keep existing or API's dispenserCount
            jobToUpdate.formsTotal = dispenserProgress.total > 0 ? dispenserProgress.total : dispenserCountForCurrentVisit !== undefined ? dispenserCountForCurrentVisit : jobToUpdate.formsTotal;

            jobToUpdate.currentVisitFuelType = fuelProgress.type || jobToUpdate.currentVisitFuelType;
            jobToUpdate.currentVisitFuelCurrent = fuelProgress.current;
            jobToUpdate.currentVisitFuelTotal = fuelProgress.total;
            
            // If the detailed message provided dispenser info, and the overall currentVisitStatus didn't, update currentVisitStatus.
            // This gives a more specific message if the general one is vague.
            if ((dispenserProgress.current > 0 || fuelProgress.current > 0) && 
                (jobToUpdate.currentVisitStatus === 'Processing...' || !jobToUpdate.currentVisitStatus)) {
                 jobToUpdate.currentVisitStatus = response.currentVisitDetailedMessage;
            }

          } else {
            // Fallback to existing logic if currentVisitDetailedMessage is not present
            // This ensures that if the new message isn't sent, it still tries to use older fields.
            ...
            if (dispenserProgressFromMessage.total > 0 && dispenserProgressFromMessage.total !== 100) {
              jobToUpdate.formsTotal = dispenserProgressFromMessage.total;
            }
            jobToUpdate.formsCurrent = dispenserProgressFromMessage.current;
            // Note: Fuel progress via old method was not explicitly done here, relying on direct API fields usually.
            // We can add it here if direct API fields for fuel are also becoming secondary.
            if (response.fuelType) jobToUpdate.currentVisitFuelType = response.fuelType;
            if (response.fuelCurrent) jobToUpdate.currentVisitFuelCurrent = response.fuelCurrent;
            if (response.fuelTotal) jobToUpdate.currentVisitFuelTotal = response.fuelTotal;
          }
          // --- END: New logic ---
