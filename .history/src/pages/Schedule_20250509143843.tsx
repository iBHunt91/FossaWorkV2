...
const Schedule: React.FC = () => {
  ...
  }, [addToast, loadDispenserDataContext]); // loadDispenserDataContext was missing
  ...
  const renderJobRow = (order: WorkOrder, dateGroupClass?: string) => {
    ...
    return (
      <div key={order.id} className={`bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden border ${storeStyle.cardBorder} hover:shadow-md transition-all duration-200 ${dateGroupClass ? 'date-grouped-card' : ''}`}>
        <div className={`flex justify-between items-center px-4 py-3 ${storeStyle.headerBg} border-b border-gray-200 dark:border-gray-700`}>
          <div className="flex items-center space-x-2">
            <button onClick={(e) => toggleFavorite(order.id.toString(), e)} className={`transition-colors focus:outline-none ${isFavorite ? 'text-amber-500 hover:text-amber-600' : 'text-gray-300 dark:text-gray-600 hover:text-amber-400 dark:hover:text-amber-400'}`} title={isFavorite ? "Remove from favorites" : "Add to favorites"}>
          ...
          <div className="flex items-center space-x-1">
            <div className="px-2.5 py-1 rounded-full text-xs font-medium bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300">{visitId}</div>
            <div className="px-2.5 py-1 rounded-full text-xs font-medium bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300">#{extractVisitNumber(order)}</div>
          </div>
        </div>
        <div className="p-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-3">
            <div>
              <div className="text-xs text-gray-500 dark:text-gray-400 mb-0.5 flex items-center"><FiCalendar className="h-3.5 w-3.5 mr-1.5" /> Visit Date</div>
              <p className="text-sm text-gray-800 dark:text-white">{order.visits?.nextVisit?.date ? new Date(order.visits.nextVisit.date).toLocaleDateString(undefined, { weekday: 'short', year: 'numeric', month: 'short', day: 'numeric' }) : 'Not Scheduled'}</p>
            </div>
            <div>
              <div className="text-xs text-gray-500 dark:text-gray-400 mb-0.5 flex items-center"><FiTool className="h-3.5 w-3.5 mr-1.5" /> Equipment</div>
              <p className="text-sm text-gray-800 dark:text-white">{order.dispensers ? `${order.dispensers.length} Dispenser(s)` : 'No dispenser data'}</p>
            </div>
          </div>
          {order.services && order.services.length > 0 && (
            <div className="mb-3">
              <div className="text-xs text-gray-500 dark:text-gray-400 mb-0.5 flex items-center"><FiFileText className="h-3.5 w-3.5 mr-1.5" /> Services</div>
              <ul className="list-disc list-inside pl-1 space-y-0.5">
                {order.services.slice(0,2).map((service, index) => (<li key={index} className="text-sm text-gray-700 dark:text-gray-300 truncate">{service.description} (Qty: {service.quantity})</li>))}
          ...
          <div className="border-t border-gray-200 dark:border-gray-700 pt-3 flex flex-wrap items-center justify-between gap-2">
            <div className="flex items-center space-x-2">
              <button onClick={(e) => handleViewInstructions(e, order)} className="flex items-center text-xs text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 transition-colors"><FiInfo className="h-3.5 w-3.5 mr-1" /> View Instructions</button>
              {order.dispensers && order.dispensers.length > 0 && (<button onClick={(e) => handleViewDispenserData(e, order)} className="flex items-center text-xs text-purple-600 hover:text-purple-700 dark:text-purple-400 dark:hover:text-purple-300 transition-colors"><GiGasPump className="h-3.5 w-3.5 mr-1" /> View Dispensers ({order.dispensers.length})</button>)}
            </div>
            <div className="flex items-center space-x-2">
              <button onClick={(e) => handleClearDispenserData(order.id, e)} className={`p-1.5 rounded-md hover:bg-red-100 dark:hover:bg-red-900/50 text-red-500 dark:text-red-400 transition-colors disabled:opacity-50 disabled:cursor-not-allowed`} title="Clear Dispenser Data" disabled={isClearing || isRescraping}>{isClearing ? <FiRefreshCw className="h-4 w-4 animate-spin" /> : <FiTrash2 className="h-4 w-4" />}</button>
              <button onClick={(e) => handleForceRescrapeDispenserData(order.id, e)} className={`p-1.5 rounded-md hover:bg-blue-100 dark:hover:bg-blue-900/50 text-blue-500 dark:text-blue-400 transition-colors disabled:opacity-50 disabled:cursor-not-allowed`} title="Force Rescrape Dispenser Data" disabled={isClearing || isRescraping}>{isRescraping ? <FiRefreshCw className="h-4 w-4 animate-spin" /> : <FiRefreshCw className="h-4 w-4" />}</button>
            </div>
          </div>
        </div>
      </div>
