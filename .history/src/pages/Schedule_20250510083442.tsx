...
const Schedule: React.FC = () => {
  ...
  // --- Dispenser Data Availability Logic (match Home page) ---
  // Check if dispenser data is available in the order or in the context
  ...
  // Updated handler for viewing dispenser data (merges context data if needed)
  const handleViewDispenserDataUnified = (e: React.MouseEvent, order: WorkOrder) => {
    e.stopPropagation();
    let dispensersToShow = order.dispensers && order.dispensers.length > 0
      ? order.dispensers
      : (contextDispensers && contextDispensers.length > 0 ? contextDispensers : []);
    setSelectedDispensers(dispensersToShow);
  ...
  const renderJobRow = (order: WorkOrder, dateGroupClass?: string) => {
    ...
    return (
      <div
        key={order.id}
        className={`job-card ${cardBgColor} shadow-lg rounded-md mb-4 border-l-4 ${styles.cardBorder} ${dateGroupClass || ''}`}
        data-store-type={storeType}
        data-work-order-id={order.workOrderId || 'N/A'}
        data-visit-id={visitNumber || 'N/A'}
      >
        {/* Header Section */}
        <div className={`${cardHeaderBgColor} p-3 flex justify-between items-center rounded-t-md`}>
          <div className="flex items-center">
            <button
              onClick={(e) => toggleFavorite(order.id, e)}
              className={`p-1 rounded-full hover:bg-slate-700 transition-colors duration-150 mr-2 ${isFav ? 'text-yellow-400' : subTextColor}`}
              aria-label={isFav ? 'Remove from favorites' : 'Add to favorites'}
              title={isFav ? 'Remove from favorites' : 'Add to favorites'}
            >
              <FiStar className={`h-5 w-5 ${isFav ? 'fill-current' : ''}`} />
            </button>
            <h3 className={`text-md font-semibold ${textColor}`}>
              {order.customer.name}
        ...
        <div className="p-3 space-y-3">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-x-4 gap-y-2 items-start">
            ...
            <div className="flex items-center col-span-1 md:col-span-1">
              <FiCalendar className={`h-4 w-4 mr-2 ${iconColor}`} />
              <div>
            ...
            <div className="flex items-center col-span-1 md:col-span-1">
              <GiGasPump className={`h-4 w-4 mr-2 ${iconColor}`} />
              <div className="flex items-baseline"> {/* Ensures number and text align nicely */}
                <p className={`text-sm font-medium ${textColor} mr-1`}>{dispenserCount}</p>
                <p className={`text-xs ${subTextColor}`}>Dispenser{dispenserCount !== 1 ? 's' : ''}</p>
              </div>
            </div>
            
            {/* Custom Instructions */}
            <div className="flex items-start col-span-1 md:col-span-1 md:justify-end">
                ...
                <div className='flex-grow'>
                    ...
                    {order.instructions ? (
                        ...
                        disabled={operationLoading[`instr_${order.id}`]}
                        className={`flex items-center px-2.5 py-1 text-xs rounded-md transition-colors duration-150 disabled:opacity-50 disabled:cursor-not-allowed ${viewInstructionsButtonBg} ${viewInstructionsButtonText} mt-0.5`}
                        >
                        <FiInfo className={`mr-1 h-3 w-3`} />
                        {operationLoading[`instr_${order.id}`] ? 'Loading...' : 'View instructions'}
                        </button>
                    ) : (
        ...
        {/* Action Icons Footer */}
        <div className="px-3 pb-3 pt-2 flex flex-wrap gap-1 items-center justify-end border-t border-slate-800">
          <a
            href={fossaUrl}
            target="_blank"
            rel="noopener noreferrer"
            onClick={(e) => e.stopPropagation()}
            className={`flex items-center p-1.5 rounded-md transition-colors duration-150 hover:bg-slate-700 ${buttonIconColor}`}
          ...
          <button
            onClick={(e) => hasDispenserData && handleViewDispenserDataUnified(e, order)}
            disabled={!hasDispenserData || operationLoading[`disp_${order.id}`]}
            className={`flex items-center p-1.5 rounded-md transition-colors duration-150 hover:bg-slate-700 disabled:opacity-50 disabled:cursor-not-allowed ${buttonIconColor}`}
          ...
          <button
            ...
            className={`flex items-center p-1.5 rounded-md transition-colors duration-150 hover:bg-slate-700 disabled:opacity-50 disabled:cursor-not-allowed ${buttonIconColor}`}
            title={operationLoading[`clear_${order.id}`] ? 'Clearing...' : 'Clear Dispenser Data'}
          >
          ...
          <button
            ...
            className={`flex items-center p-1.5 rounded-md transition-colors duration-150 hover:bg-slate-700 disabled:opacity-50 disabled:cursor-not-allowed ${buttonIconColor}`}
            title={operationLoading[`rescrape_${order.id}`] ? 'Rescraping...' : 'Force Rescrape'}
          >
              ...
              <CalendarView 
                ...
                onEventClick={(event: CalendarEvent) => {
                  ...
                  if (originalOrder) {
                    ...
                    handleViewDispenserDataUnified({ stopPropagation: () => {}, preventDefault: () => {} } as React.MouseEvent, originalOrder);
                  }
                }}
              />
            )}
