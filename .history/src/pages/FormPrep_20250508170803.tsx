...
const FormPrep = () => {
  ...
  // Helper functions for extracting progress information from status messages
  const extractDispenserProgress = (message: string) => {
    const result = { current: 0, total: 0 };
    
    // Look for patterns like "Dispenser 2/8" or "dispenser #3 of 6"
    const dispenserMatch = message.match(/[Dd]ispenser(?:s)?\s*(?:#?\s*)?(\d+)(?:\s*\/|\s*of\s*)(\d+)/);
    if (dispenserMatch) {
      result.current = parseInt(dispenserMatch[1]);
      result.total = parseInt(dispenserMatch[2]);
    }
  ...
  const extractFuelProgress = (message: string) => {
    const result = { current: 0, total: 0, type: '' };
    
    // Look for patterns like "Processing fuel type: Regular (2/4)"
    const fuelMatch = message.match(/[Pp]rocessing fuel type:?\s*([a-zA-Z0-9\s-]+)\s*\((\d+)\/(\d+)\)/);
    if (fuelMatch) {
      result.type = fuelMatch[1].trim();
      result.current = parseInt(fuelMatch[2]);
      result.total = parseInt(fuelMatch[3]);
    }
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      {/* Animation style tag */}
      <style>{progressBarAnimationStyle}</style>
          ...
          <div className="flex-1 overflow-auto">
            ...
            {activeTab === 'single' && (
              <div className="space-y-6">
                ...
                {/* Additional panels for form setup and results would go here */}
                {/* ... existing code for form setup panel ... */}
                {/* ... existing code for results panel ... */}
              </div>
            )}

            {activeTab === 'batch' && (
              <div className="space-y-6">
                {/* Batch mode content would go here */}
                {/* ... existing batch processing panels ... */}
              </div>
            )}
          </div>
        </div>
