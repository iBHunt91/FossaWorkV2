...
const FiltersRedesign: React.FC = () => {
  ...
  // Create a ref to store the latest dispenser data to avoid stale closures
  const dispenserDataRef = useRef<any>(null);
  ...
  useEffect(() => {
    // Function to handle data reloading with proper closure over the latest values
    const reloadData = async () => {
      // Prevent reloading if we've recently loaded data (last 5 seconds)
      const now = Date.now();
      if (now - lastLoadTimeRef.current < 5000) {
        console.log('Recent load detected, skipping reload');
        return;
      }
      ...
      try {
        ...
        // Use the function reference directly instead of captured reference
        await loadDispenserData(true, false);
        
        // Create enhanced work orders with dispenser data
        // Get a fresh copy of dispenserData from the ref to avoid stale closures
        const latestDispenserData = dispenserDataRef.current || dispenserData.dispenserData || {};
        
        const enhancedWorkOrders = apiData.workOrders.map((order: WorkOrder) => {
          if (latestDispenserData && latestDispenserData[order.id]) {
            const dispenserInfo = latestDispenserData[order.id] as { dispensers: Dispenser[] };
            const dispensers = dispenserInfo.dispensers || [];
            return { ...order, dispensers };
          }
    ...
    // Re-create the debounce function inside the effect to avoid stale closures
  ...
  }, []); // Empty dependency array - event handler setup only happens once
  
  // Keep our dispenserData ref updated
  useEffect(() => {
    dispenserDataRef.current = dispenserData.dispenserData || {};
  }, [dispenserData]);
  ...
  }, [workOrders, isLoading, selectedDateRange]); // Removed isLoaded dependency to prevent excessive recalculation
