    ...
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6 animate-fadeIn">
            ...
            <div className="flex flex-col space-y-3 z-10 relative">
              ...
              <div className="flex items-center justify-between mt-1">
                ...
                <div className="flex items-center gap-1">
                  <CSVLink
                    data={prepareCSVData()}
                    filename={`filter-inventory-${format(currentWeek, 'yyyy-MM-dd')}.csv`}
                  ...
                  >
                    <FiDownload className="h-3 w-3" /> Export CSV
                  </CSVLink>
                </div>
              </div>
            </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-5 border border-gray-200 dark:border-gray-700 hover:shadow-xl transition-all duration-300">
          <div className="relative">
            {/* Decorative element */}
            <div className="absolute top-0 right-0 w-32 h-32 -mt-8 -mr-8 bg-gradient-to-br from-amber-400/20 to-amber-600/10 rounded-full blur-xl"></div>
            
            <div className="flex flex-col space-y-3 z-10 relative">
              <div className="flex items-center gap-2">
                <div className="h-10 w-10 rounded-lg bg-amber-50 dark:bg-amber-900/30 flex items-center justify-center">
                  <FiMapPin className="text-amber-600 dark:text-amber-400" />
                </div>
                <div>
                  <div className="text-sm text-gray-500 dark:text-gray-400">Stores</div>
                  <div className="font-semibold text-gray-800 dark:text-white text-2xl">
                    {new Set(filterNeeds.map(need => need.storeName)).size}
                  </div>
                </div>
              </div>

              <div className="flex flex-col space-y-1.5 pt-1">
                {Array.from(new Set(filterNeeds.map(need => need.storeName))).slice(0, 3).map(store => {
                  const storeColor = getStoreColor(store);
                  return (
                    <div key={store} className="flex items-center justify-between">
                      <div className="flex items-center gap-1.5">
                        <div className={`h-2 w-2 rounded-full ${storeColor.bg}`}></div>
                        <span className="text-xs font-medium text-gray-600 dark:text-gray-300">
                          {store}
                        </span>
                      </div>
                      <span className="text-xs text-gray-500 dark:text-gray-400">
                        {filterNeeds.filter(need => need.storeName === store).reduce((sum, need) => sum + need.quantity, 0)} filters
                      </span>
                    </div>
                  );
                })}
                {new Set(filterNeeds.map(need => need.storeName)).size > 3 && (
                  <div className="text-xs text-gray-500 dark:text-gray-400 text-center">
                    +{new Set(filterNeeds.map(need => need.storeName)).size - 3} more stores
                  </div>
                )}
              </div>
            </div>
