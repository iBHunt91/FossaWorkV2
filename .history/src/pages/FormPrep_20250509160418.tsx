...
const FormPrep: React.FC<{}> = () => {
  ...
  const startBatchPolling = () => {
    ...
    const pollInterval = setInterval(async () => {
      try {
        // Verify we have an active batch job ID before polling
        const currentBatchJobId = getFromStorage(userStorageKeys.BATCH_JOB_ID, null);
        if (!currentBatchJobId) {
          addDebugLog('BATCH', 'No active batch job ID found, stopping poll');
        ...
        try {
          ...
          addDebugLog('BATCH', 'Received batch status update', status);
          
          // Get the latest state from both component state and localStorage
          const storedBatchJobs = getFromStorage(userStorageKeys.BATCH_JOBS, []);
          
          setBatchJobs(prev => {
            // Merge stored jobs with current state to ensure we don't lose any
            let mergedJobs = [...prev];
            if (storedBatchJobs.length > 0) {
              // Add any jobs from storage that don't exist in current state
              for (const storedJob of storedBatchJobs) {
                // Check if job with this ID already exists in mergedJobs
                const jobExists = mergedJobs.findIndex((j: any) => j.jobId === storedJob.jobId) !== -1;
                if (!jobExists) {
                  mergedJobs.push(storedJob);
                }
              }
            }
            
            // Find the job in our merged list
            const jobIndex = mergedJobs.findIndex((j: any) => j.jobId === currentBatchJobId);
            
            if (jobIndex !== -1) {
              const jobToUpdate = mergedJobs[jobIndex];
              const currentTime = Date.now();
              
              // IMPORTANT: Override server status in certain cases to provide better UI feedback
              // The API often returns 'idle' even when processing is ongoing
              let overrideStatus = status.status;
              
              // If we have a running job in our state, but server returns idle,
              // AND we have other indicators of activity, keep it as "running"
              if (status.status === 'idle' && jobToUpdate.status === 'running') {
                // Check for signs of activity
                const hasRecentUpdate = (status as any).lastUpdated && 
                  (new Date((status as any).lastUpdated).getTime() > currentTime - 60000); // Updated in last minute
                
                const hasProgress = 
                  ((status as any).currentItem !== undefined && (status as any).currentItem > 0) || 
                  (status.completedVisits !== undefined && status.completedVisits > 0) ||
                  ((status as any).progress !== undefined && (status as any).progress > 0);
                  
                // If any sign of activity, override status to stay running
                if (hasRecentUpdate || hasProgress || status.message) {
                  overrideStatus = 'running';
                }
              }
              
              // Determine if the job is truly finished based on server status or completedVisitIds
              const isJobFinishedByServer = 
                status.status === 'completed' || 
                status.status === 'error' || 
                ((status as any).endTime && status.status === 'idle');
              ...
              // Extract detailed progress information from status
              ...
              // Extract currentVisitStatus from status.message or try find work order ID from completedVisitIds
              let currentVisitStatus = (status as any).currentVisitStatus || status.message || jobToUpdate.currentVisitStatus || '';
              
              // If the API gives us a completedVisitIds array, use the latest ID for context
              if ((status as any).completedVisitIds && (status as any).completedVisitIds.length > 0) {
                const latestVisitId = (status as any).completedVisitIds[(status as any).completedVisitIds.length - 1];
                if (latestVisitId && !currentVisitId) {
                  currentVisitId = latestVisitId;
                  
                  // Try to find the work order details in our local cache
                  const matchingOrder = workOrders.find(order => order.id === latestVisitId);
                  if (matchingOrder && matchingOrder.customer && matchingOrder.customer.name) {
                    currentVisitName = matchingOrder.customer.name;
                    const dispenserCount = getDispenserCountDirect(latestVisitId);
                    if (dispenserCount > 0) {
                      formsTotal = dispenserCount;
                    }
                  }
                  
                  // Update status message to show what we're working on
                  if (currentVisitName && !status.message) {
                    currentVisitStatus = `Processing ${currentVisitName}...`;
                  }
                }
              }
              
              // If we have progress info from the API, use it
              if ((status as any).currentItem !== undefined && (status as any).totalItems) {
                formsCurrent = (status as any).currentItem;
                formsTotal = (status as any).totalItems;
              }
              
              // If we have current/total items, use that for form counts
              if ((status as any).progress !== undefined) {
                // progress is a number between 0-100
                // Calculate completed forms based on progress percent
                if (formsTotal > 0) {
                  formsCurrent = Math.floor(((status as any).progress / 100) * formsTotal);
                }
              }
              
              // IMPORTANT: Generate fake progress when the API doesn't provide useful updates
              // This gives feedback to the user that something is happening
              if (overrideStatus === 'running' && formsCurrent === 0 && formsTotal === 0) {
                // Calculate fake progress based on elapsed time
                const elapsedTime = currentTime - (jobToUpdate.startTime || currentTime);
                const fakeProgress = Math.min(Math.floor(elapsedTime / 1000) % 100, 95); // Max 95% until real completion
                
                // Set default values for progress display
                formsTotal = 100;
                formsCurrent = fakeProgress;
                
                if (!currentVisitStatus) {
                  currentVisitStatus = "Processing...";
                }
              }
              
              // Make sure we have valid completedVisits and totalVisits
              // If API gives us values use them, otherwise preserve our current count
              const completedVisits = status.completedVisits !== undefined && status.completedVisits > 0
                ? status.completedVisits
                : (status as any).completedVisitIds && (status as any).completedVisitIds.length > 0
                  ? (status as any).completedVisitIds.length
                  : jobToUpdate.completedVisits || 0;
                
              const totalVisits = status.totalVisits !== undefined && status.totalVisits > 0
                ? status.totalVisits
                : jobToUpdate.totalVisits || Math.max(1, completedVisits);
              ...
              mergedJobs[jobIndex] = updatedJob;
              
              // If job truly completed or errored according to server, stop polling
              if (isJobFinishedByServer) {
                if (status.status === 'error') {
                  setLastFailedBatch(updatedJob);
                }
                addDebugLog('BATCH', `Batch job ${currentBatchJobId} completed with status: ${status.status}`, {
                  jobToUpdate: updatedJob,
                  apiResponse: status
                });
                clearInterval(pollInterval);
                setIsProcessing(false);
              ...
              // Save the updated batch jobs to localStorage
              saveToStorage(userStorageKeys.BATCH_JOBS, mergedJobs);
              
              return mergedJobs;
            } else {
              // If the job is not found in our merged list, but we have a valid batch job ID,
              // create a new job entry to restore the polling state
              console.warn(`Batch job with ID ${currentBatchJobId} not found in state during polling. Creating recovery job.`);
              
              // Create a recovery job
              const recoveryJob: BatchJob = {
                filePath: 'Recovered job',
                timestamp: new Date().toLocaleString(),
                status: 'running',
                message: status.message || 'Recovering batch job status...',
                totalVisits: status.totalVisits || (status as any).completedVisitIds?.length || 1,
                completedVisits: status.completedVisits || 0,
                headless: true,
                jobId: currentBatchJobId,
                startTime: Date.now(),
                currentVisitStatus: (status as any).currentVisitStatus || status.message || 'Reconnecting to batch job...'
              };
              
              const updatedJobs = [recoveryJob, ...mergedJobs];
          ...
          addDebugLog('BATCH', 'Error getting batch status', apiError);
          console.error('Error getting batch status:', apiError);
          // For API errors, just log and continue polling, but don't stop it
          return; // Skip update for this cycle if API error occurs
        }
      } catch (error) {
        addDebugLog('BATCH', 'Error handling batch status update', error);
                                  ...
                                  <td className="px-4 py-3">
                                    {job.status === 'running' && (
                                      <div className="space-y-3 min-w-56 min-h-16"> {/* Consistent with single visit min-height */}
                                        ...
                                        {(() => {
                                          const currentVisitMessage = job.currentVisitStatus || job.message || '';
                                          // Assuming job.formsTotal is the dispenser count for the current active visit in the batch
                                          ...
                                          } else { // Skeleton for dispenser progress if no total or not active for current visit
                                            return (
                                              <div>
                                                <div className="flex justify-between items-center text-xs mb-1">
                                                  <div className="font-medium text-gray-400 dark:text-gray-500 flex items-center gap-1">
                                        ...
                                        {(() => {
                                          ...
                                          } else { // Skeleton for fuel progress for current visit
                                            return (
                                              <div>
                                                <div className="flex justify-between items-center text-xs mb-1">
                                                  <div className="font-medium text-gray-400 dark:text-gray-500 flex items-center gap-1">
