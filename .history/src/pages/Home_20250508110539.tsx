                ...
                <div className="p-4">
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-5 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50">
                        {['Mon', 'Tue', 'Wed', 'Thu', 'Fri'].map((day, i) => (
                          <div key={i} className="text-center py-2 text-xs font-medium text-gray-600 dark:text-gray-400">
                            {day}
                          </div>
                        ))}
                      </div>
                      ...
                      <div className="grid grid-cols-5 min-h-[180px]">
                        {Array.from({ length: 5 }).map((_, dayIndex) => {
                          ...
                          return (
                            <div 
                              key={dayIndex} 
                              className={`p-1 border-r last:border-r-0 border-gray-200 dark:border-gray-700 ${
                                isToday ? 'bg-blue-50/50 dark:bg-blue-900/10' : ''
                              }`}
                            >
                              {/* Day header */}
                              <div className="text-center mb-2">
                                <span className={`inline-flex items-center justify-center h-7 w-7 rounded-full font-medium text-sm ${
                                  isToday 
                                    ? 'bg-blue-500 text-white' 
                                    : 'text-gray-700 dark:text-gray-300'
                                }`}>
                                ...
                                {jobs.length > 0 && (
                                  <span className="ml-1 text-xs font-medium px-2 py-1 rounded-full bg-blue-500 text-white dark:bg-blue-600 dark:text-white shadow-sm">
                                    {jobs.length}
                                  </span>
                                )}
                              </div>
                              ...
                              {/* Jobs for this day */}
                              <div className={`max-h-[200px] overflow-y-auto space-y-2 pr-0.5 ${jobs.length > 1 ? 'has-multiple-jobs' : ''}`}>
                                {jobs.length === 0 ? (
                                  ...
                                  <>
                                    {jobs.map((job, idx) => {
                                      ...
                                      return (
                                        <div 
                                          key={job.id || idx} 
                                          className={`p-2 mb-1 rounded border ${storeStyle.cardBorder} bg-white dark:bg-gray-800 shadow-sm hover:shadow transition-shadow cursor-pointer flex flex-col ${jobs.length > 1 && idx === 0 ? 'relative -rotate-1' : ''} ${jobs.length > 1 && idx === jobs.length - 1 ? 'relative rotate-1' : ''}`}
                                    ...
                                    {jobs.length > 4 && (
                                      <div className="text-xs text-center font-medium py-1 text-blue-600 dark:text-blue-400">
                  ...
                  {groupedWorkOrders.nextWeek.length > 0 && (
                    <div className="bg-white dark:bg-gray-800 rounded-lg overflow-hidden shadow-sm border border-gray-200 dark:border-gray-700">
                      {/* Week header */}
                      <div className="bg-green-50 dark:bg-green-900/20 p-3 border-b border-gray-200 dark:border-gray-700 flex items-center">
                        <div className="h-9 w-9 rounded-full bg-green-100 dark:bg-green-900/40 flex items-center justify-center mr-3">
                          <FiArrowUp className="text-green-600 dark:text-green-400" />
                        </div>
                        <div>
                          <h2 className="font-semibold text-gray-800 dark:text-gray-200">Next Week</h2>
                          <p className="text-sm text-gray-600 dark:text-gray-400">
                            {formatDateRange(
                          ...
                          {Array.from({ length: 5 }).map((_, dayIndex) => {
                            ...
                            return (
                              ...
                              >
                                {/* Day header */}
                                <div className="text-center mb-2">
                                  <span className="inline-flex items-center justify-center h-7 w-7 rounded-full font-medium text-sm text-gray-700 dark:text-gray-300">
                                    {currentDate.getDate()}
                                  </span>
                                  {jobs.length > 0 && (
                                    <span className="ml-1 text-xs font-medium px-2 py-1 rounded-full bg-green-500 text-white dark:bg-green-600 dark:text-white shadow-sm">
                                      {jobs.length}
                                    </span>
                                  )}
                                </div>
