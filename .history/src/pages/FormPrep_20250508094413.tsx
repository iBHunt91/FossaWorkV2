...
const FormPrep: React.FC<{}> = () => {
  ...
  const openUrlWithLogin = async (url: string) => {
    try {
      // Use openUrlWithDebugMode if defined in the form service
      if (typeof openUrlWithDebugMode === 'function') {
        await openUrlWithDebugMode(url);
        addToast('info', 'Opening URL in debug mode');
        return;
      }
      
      // Fall back to regular window.open
      ...
      <div className="p-4 md:p-6 max-w-7xl mx-auto">
        <div className="space-y-6 animate-fadeIn">
          ...
          <div className="flex-1 overflow-auto">
                                        ...
                                        <div className="mt-2">
                                          ...
                                          {(() => {
                                            ...
                                            if (dispenserInfo.current > 0 && dispenserInfo.total > 0) {
                                              // Calculate progress as a percentage of the way through the dispensers
                                              // Make sure the first dispenser shows some progress (not 0%)
                                              // and the last dispenser can reach 100%
                                              ...
                                              // Each dispenser represents a portion of the progress:
                                              // - First dispenser (1/10) represents 0-10% progress
                                              // - Second dispenser (2/10) represents 10-20% progress
                                              // - Last dispenser (10/10) represents 90-100% progress
                                              const segmentSize = 100 / totalDispensers;
                                              
                                              // Calculate progress based on current dispenser
                                              // (currentDispenser - 1) represents completed dispensers
                                              // Adding 0.5 as an estimation of progress within current dispenser
                                              const progressPercent = Math.min(
                                                Math.round(((currentDispenser - 1 + 0.5) / totalDispensers) * 100), 
                                                100
                                              );
            ...
            {activeTab === 'batch' && (
              <div className="space-y-6">
                {/* Batch processing interface - style consistently with the single view */}
                <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
                  <div className="border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700/50 px-4 py-3">
                    <h2 className="text-sm font-medium text-gray-700 dark:text-gray-300 flex items-center gap-2">
                  ...
                  <div className="p-4">
                    {/* Keep your existing batch processing UI */}
