  ...
  const renderJobRow = (order: any, dateGroupClass?: string) => {
    ...
    const getDisplayName = (order: any) => {
      ...
      if (order.customer?.name) {
        ...
        if (order.customer.storeNumber) {
          const storeNumber = order.customer.storeNumber.toString().padStart(4, '0');
          displayName += ` #${storeNumber}`;
        }
      }
      return displayName || 'Unknown Customer';
    };
    ...
    if (order.services && Array.isArray(order.services)) {
      order.services.forEach((service: any) => {
        if (service.type) {
          // Filter out common type strings
          const typeStr = service.type
            .replace(/BASIC|PROGRAMMED|SERVICE|MAINTENANCE|FOR|AND|ON|WITH|TO/gi, '')
            .replace(/\s+/g, ' ')
            .trim();
          
          if (typeStr.length > 0 && !servicesList.includes(typeStr)) {
            servicesList.push(typeStr);
          }
          
          // Check for PM service
          if (service.type.toUpperCase().includes('PM') || 
              service.type.toUpperCase().includes('PROGRAMMED MAINTENANCE') ||
              service.type.toUpperCase().includes('PREVENTIVE MAINTENANCE')) {
            hasPM = true;
          }

          // Check for fuel type labels
          const fuelGradeMatch = service.description?.match(/grade\(s\):\s*([^)]+)/i);
          if (fuelGradeMatch && fuelGradeMatch[1]) {
            // Sort the fuel types according to standard order
            const sortedFuelTypes = sortFuelTypes(fuelGradeMatch[1]);
            fuelTypesList = [...new Set([...fuelTypesList, ...sortedFuelTypes])];
          }
        }
        
        // Add quantity
        if (service.quantity && !isNaN(service.quantity)) {
          quantity += parseInt(service.quantity, 10);
        }
      });
    }

    // Get store type to determine styles
    const storeType = getStoreTypeForFiltering(order);
    ...
    // Set default date classes (will be overridden if dateGroupClass is provided)
    let dateBorderClass = '';
    let dateBadgeClass = '';
    let dateTextClass = '';
            ...
            return (
              <div key={dateGroup.date} className="mb-8 relative">
                ...
                <div className="flex items-center mb-2">
                  ...
                  <div className={`ml-auto ${dateColor} text-xs font-medium px-2 py-0.5 rounded-full`}>
                ...
                {dateGroup.orders.length > 1 && (
                  <div className={`absolute left-1.5 top-8 bottom-4 w-0.5 ${dateColor.split(' ')[1]} opacity-50`}></div>
                )}
