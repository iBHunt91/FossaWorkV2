      ...
      <div className="mb-6 animate-fadeIn">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-5 border border-gray-200 dark:border-gray-700 hover:shadow-xl transition-all duration-300 relative overflow-hidden">
            ...
            {/* Dynamic grid based on store count - adjust columns and center content */}
            <div className={`grid gap-3 mt-3 relative z-10 mx-auto ${
              // Adjust grid columns based on store count
              storeDistributionArray.length === 1 ? 'grid-cols-1 max-w-xs' : 
              storeDistributionArray.length === 2 ? 'grid-cols-2 max-w-md' :
              storeDistributionArray.length === 3 ? 'grid-cols-3 max-w-lg' :
              ...
              {storeDistributionArray.map((store) => {
                ...
                return (
                  <div 
                    ...
                    className={`relative group rounded-xl cursor-pointer transition-all duration-300
                      ${storeFilter === store.type 
                        ? `bg-gradient-to-br ${storeStyle.gradient} shadow-md` 
                        : 'bg-gray-50 dark:bg-gray-700/70 hover:bg-gray-100 dark:hover:bg-gray-700'}
                    `}
                  >
                    {/* Color indicator bar at top of card */}
                    <div 
                      className={`h-1.5 w-full rounded-t-xl ${
                        storeFilter === store.type ? 'bg-white/30' : storeStyle.dot
                      }`}
                    ></div>
            ...
            {storeFilter !== 'all' && (
              <div className="text-center mt-4 relative z-10">
                <button 
                  ...
                  className="text-xs text-gray-500 dark:text-gray-400 hover:text-primary-600 dark:hover:text-primary-400 flex items-center gap-1 mx-auto"
                >
