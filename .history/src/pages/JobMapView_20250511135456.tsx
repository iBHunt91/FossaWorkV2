...
import { FiMapPin, FiFilter, FiRefreshCw, FiList, FiMap, FiX, FiCalendar, FiMaximize, FiNavigation, FiArrowRight, FiLoader, FiHome, FiPlus } from 'react-icons/fi';
...
const JobMapView: React.FC = () => {
  ...
  const [routeOrigin, setRouteOrigin] = useState<GeocodedJob | CustomLocation | null>(null);
  const [routeDestination, setRouteDestination] = useState<GeocodedJob | CustomLocation | null>(null);
  const [routeCalculating, setRouteCalculating] = useState<boolean>(false);
  const [routeInfo, setRouteInfo] = useState<{duration: number; distance: number} | null>(null);
  
  // Custom location state
  const [customLocations, setCustomLocations] = useState<CustomLocation[]>([]);
  const [showAddLocationForm, setShowAddLocationForm] = useState<{ type: 'origin' | 'destination' | null }>({ type: null });
  ...
  const handleAddCustomLocation = async () => {
    if (!newLocationName.trim() || !newLocationAddress.trim()) {
      setAddressError('Please enter both a name and address');
      return;
    }
    ...
    try {
      ...
      const newLocation = await routingService.createCustomLocation(
      ...
      // Update origin or destination based on which one we're adding for
      if (showAddLocationForm.type === 'origin') {
        setRouteOrigin(newLocation);
      } else if (showAddLocationForm.type === 'destination') {
        setRouteDestination(newLocation);
      }
      
      // Reset the form
      setNewLocationName('');
      setNewLocationAddress('');
      setShowAddLocationForm({ type: null });
  ...
  const toggleRoutingMode = () => {
    ...
    if (newRoutingMode) {
      // When entering routing mode, set the current selected job as origin if no custom locations
      if (selectedJob && !customLocations.length) {
            ...
            <div className="p-4">
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-start">
                ...
                <div className="col-span-1">
                  ...
                  {showAddLocationForm.type === 'origin' ? (
                ...
                <div className="col-span-1">
                  ...
                  {showAddLocationForm.type === 'destination' ? (
                    <div className="space-y-2">
                      <input
                        type="text"
                    ...
                    <div className="space-y-2">
                      <select
                        ...
                        onChange={(e) => {
                          const value = e.target.value;
                          if (value === "add-custom") {
                            setShowAddLocationForm({ type: 'destination' });
                          } else {
                            const selectedJob = filteredJobs.find(job => job.id === value);
                            const customLocation = customLocations.find(loc => loc.id === value);
                            setRouteDestination(selectedJob || customLocation || null);
                      ...
                      >
                        ...
                        {customLocations.length > 0 && (
                          <optgroup label="Custom Locations">
                            {customLocations
                              .filter(loc => !routeOrigin || loc.id !== routeOrigin.id)
                              .map(location => (
                                <option key={location.id} value={location.id}>
                                  {location.name}
                                </option>
                        ...
                        {filteredJobs.length > 0 && (
                          <optgroup label="Jobs">
                            {filteredJobs
                              .filter(job => !routeOrigin || job.id !== routeOrigin.id)
                              .map(job => (
                                <option key={job.id} value={job.id}>
                                  {job.clientName} {job.storeNumber ? `(${job.storeNumber})` : ''}
                                </option>
                      ...
                      >
                        ...
                        <button
                          onClick={() => {
                            ...
                            // If this location is used in a route, clear it
                            if (routeOrigin?.id === location.id) setRouteOrigin(null);
                            if (routeDestination?.id === location.id) setRouteDestination(null);
                          }}
