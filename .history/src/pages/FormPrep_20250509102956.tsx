...
interface BatchAutomationStatus {
  ...
  // Optional fields for dispenser progress
  dispenserCount?: number;
...
// Add Electron interface for file saving at the top of the file
declare global {
  interface Window {
    electron: {
      saveFile: (options: {
      ...
      openUrlWithActiveUser: (options: any) => Promise<{success: boolean; message?: string}>;
      openExplorer: (options: { path: string }) => Promise<void>;
      // ... other existing Electron API methods as needed
    };
  }
}

// Update any existing ElectronAPI interface to ensure compatibility with saveFile method
interface ElectronAPI {
  ...
  openUrlWithActiveUser: (options: any) => Promise<{success: boolean; message?: string}>;
  openExplorer: (options: { path: string }) => Promise<void>;
  // ... any other methods from the existing interface
}
...
const FormPrep: React.FC<{}> = () => {
  ...
  const downloadLogs = () => {
    try {
      ...
      // Define log directory path - this will be in the application's root directory
      const logDirectoryPath = './logs';
      
      // Save file to the specified directory using Electron IPC
      if (window.electron && window.electron.saveFile) {
        window.electron.saveFile({
          ...
          if (result.success) {
            ...
            // If on Windows, try to open explorer to the log file location
            if (navigator.platform.toLowerCase().includes('win') && window.electron.openExplorer) {
              window.electron.openExplorer({ path: result.path }).catch(() => {
                // Silently fail if we can't open explorer
              });
            }
          } else {
            addDebugLog('SYSTEM', 'Failed to save logs through Electron', result);
            // Try alternative save method with absolute path
            saveToAbsolutePath();
          }
        }).catch((error: any) => {
        ...
        // If Electron API is not available, try alternative save method
        saveToAbsolutePath();
      }
      
      // Alternative save method that attempts to use an absolute path
      function saveToAbsolutePath() {
        // Try to determine a good absolute path based on platform
        let absolutePath = '';
        
        if (navigator.platform.toLowerCase().includes('win')) {
          // For Windows, try to use the Desktop folder
          absolutePath = 'C:/Users/Bruce/Desktop/FM WORKING/logs';
        } else if (navigator.platform.toLowerCase().includes('mac')) {
          // For Mac, try to use the Documents folder
          absolutePath = '/Users/Shared/FossaMonitor/logs';
        } else {
          // For Linux or others, use a logs folder in home
          absolutePath = '/tmp/fossa_monitor_logs';
        }
        
        // Try saving to the absolute path if Electron API is available
        if (window.electron && window.electron.saveFile) {
          window.electron.saveFile({
            filename,
            content: jsonString,
            ...
            if (result.success) {
              addDebugLog('SYSTEM', 'Logs saved successfully to absolute path', { path: result.path });
              addToast('success', `Debug logs saved to ${result.path}`);
            } else {
              addDebugLog('SYSTEM', 'Failed to save logs to absolute path', result);
              // Last resort: fall back to browser download
              saveThroughBrowser();
            }
          }).catch(() => {
            // Last resort: fall back to browser download
          ...
          // If Electron API is completely unavailable, fall back to browser download
          saveThroughBrowser();
        }
      }
      ...
      function saveThroughBrowser() {
        ...
        addToast('info', 'Debug logs downloaded through browser (could not save to file system)');
      }
    } catch (error) {
      console.error('Error downloading logs:', error);
      addToast('error', 'Error downloading logs');
    }
  };
        ...
        try {
          ...
          setBatchJobs(prev => {
            ...
            if (jobIndex !== -1) {
              ...
              // Override status if needed - if server says idle but we have a job running,
              // and there's a completed visit ID, mark it as running
