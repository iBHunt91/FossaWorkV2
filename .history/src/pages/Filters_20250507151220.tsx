  ...
  const loadInitialData = useCallback(async () => { 
    ...
    if (currentDispenserData && Object.keys(currentDispenserData).length > 0) {
      scrapedData.workOrders.slice(0, 5).forEach(wo => {
        const calcResult = calculateFiltersForWorkOrder(wo);

        const filtersFromResult = calcResult.filters;
        ...
        if (warningsFromResult && warningsFromResult.length > 0) {
          sampleWarnings.set(wo.id, warningsFromResult.map(w => ({...w, orderId: wo.id, storeName: wo.customer?.name || 'N/A'})));
        }
      });
    }
