  ...
  const renderJobRow = (order: WorkOrder, dateGroupClass?: string) => {
    ...
    return (
      ...
      >
        ...
        {/* Action Icons Footer - With Slide Out Menu */}
        <div className="flex items-center justify-between p-1.5 border-t border-gray-700/50 relative group">
          {/* Store number badge - Left side */}
          <div className="text-xs text-gray-400">{order.customer.storeNumber ? `#${order.customer.storeNumber.replace(/^#+/, '')}` : ''}</div>
          
          {/* Primary Actions - Always visible - Right side */}
          <div className="flex gap-1.5">
            {/* Form Prep - Blue button */}
            <button
              onClick={() => navigate(`/app/form-prep?workOrderId=${order.id}&visitId=${visitNumber}`)}
              className="p-1.5 bg-blue-500 hover:bg-blue-600 text-white rounded-md transition-colors flex items-center justify-center"
              title="Go to Form Prep"
            >
              <FiEdit3 className="h-3.5 w-3.5" />
            </button>
            
            {/* Instructions - Orange/Amber button */}
            <button
              onClick={(e) => handleViewInstructions(e, order)}
              className="p-1.5 bg-amber-500 hover:bg-amber-600 text-white rounded-md transition-colors flex items-center justify-center"
              title="View Full Instructions"
            >
              <FiEye className="h-3.5 w-3.5" />
            </button>
            
            {/* View Dispenser Data - Purple button */}
            <button
              onClick={(e) => handleViewDispenserDataUnified(e, order)}
              className="p-1.5 bg-purple-500 hover:bg-purple-600 text-white rounded-md transition-colors flex items-center justify-center"
              title="View Dispenser Data"
            >
              <GiGasPump className="h-3.5 w-3.5" />
            </button>
            
            {/* More Options Button - Now with visual indicator */}
            <button
              className="p-1.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-md transition-colors relative z-20 flex items-center gap-1 overflow-hidden"
              title="More Actions Available"
            >
              <div className="flex items-center">
                <FiChevronDown className="h-3.5 w-3.5 group-hover:hidden" />
                <FiChevronUp className="h-3.5 w-3.5 hidden group-hover:block" />
              </div>
              {/* Visual indicator dots */}
              <div className="absolute right-0 top-0 h-full">
                <div className="flex flex-col h-full justify-evenly pr-0.5">
                  <div className="h-1 w-1 rounded-full bg-white opacity-75 group-hover:opacity-0 transition-opacity"></div>
                  <div className="h-1 w-1 rounded-full bg-white opacity-75 group-hover:opacity-0 transition-opacity"></div>
                  <div className="h-1 w-1 rounded-full bg-white opacity-75 group-hover:opacity-0 transition-opacity"></div>
                </div>
              </div>
              {/* Pulsing indicator */}
              <span className="absolute top-0 right-0 h-2 w-2 rounded-full bg-green-400 opacity-80 group-hover:opacity-0 animate-ping"></span>
            </button>
          </div>

          {/* Slide-out menu - Shows on hover with better organization */}
          <div className="absolute left-0 top-0 z-10 flex items-center bg-gray-800/95 backdrop-blur-md rounded-r-lg shadow-xl py-2 px-3
                        transition-all duration-300 ease-in-out origin-left border border-gray-700/50 border-l-0
                        scale-x-0 opacity-0 group-hover:scale-x-100 group-hover:opacity-100 overflow-x-auto max-w-[calc(100vw-2rem)]">
            {/* Better organized slide-out menu buttons with text labels and categories */}
            <div className="flex flex-wrap gap-2 items-center">
              ...
              <div className="flex items-center gap-2 pr-2 border-r border-gray-600">
                <button
                  ...
                  className="flex items-center gap-1 px-2 py-1 bg-blue-600 hover:bg-blue-500 text-white rounded-md transition-colors"
                  title="Open Visit URL in WorkFossa"
                >
                  <FiExternalLink className="h-3.5 w-3.5 flex-shrink-0" />
                  <span className="text-xs whitespace-nowrap hidden sm:inline">WorkFossa</span>
                </button>
              </div>
              
              {/* View Filter Needs */}
              <div className="flex flex-wrap items-center gap-2 pr-2 border-r border-gray-600">
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    const contextDispensers = dispenserData?.dispenserData?.[order.id]?.dispensers;
                  ...
                  className="flex items-center gap-1 px-2 py-1 bg-teal-600 hover:bg-teal-500 text-white rounded-md transition-colors"
                  title="View Filter Needs"
                >
                  <FiFilter className="h-3.5 w-3.5 flex-shrink-0" />
                  <span className="text-xs whitespace-nowrap hidden sm:inline">Filters</span>
                </button>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <button 
                  ...
                  className="flex items-center gap-1 px-2 py-1 bg-green-600 hover:bg-green-500 text-white rounded-md transition-colors"
                  title="Force Rescrape Dispenser Data"
                >
                  <FiRefreshCw className="h-3.5 w-3.5 flex-shrink-0" />
                  <span className="text-xs whitespace-nowrap hidden sm:inline">Rescrape</span>
                </button>
                
                {/* Clear Data */}
                <button 
                  onClick={(e) => handleClearDispenserData(order.id, e)}
                  className="flex items-center gap-1 px-2 py-1 bg-red-600 hover:bg-red-500 text-white rounded-md transition-colors"
                  title="Clear Dispenser Data (Local)"
                >
                  <FiTrash2 className="h-3.5 w-3.5 flex-shrink-0" />
                  <span className="text-xs whitespace-nowrap hidden sm:inline">Clear</span>
                </button>
              </div>
            </div>
            
            {/* Menu helper text - mobile only */}
            <div className="absolute bottom-0 right-0 px-1.5 py-0.5 bg-gray-900/60 rounded-tl text-[10px] text-gray-400 sm:hidden">
              More options
            </div>
          </div>
        </div>
