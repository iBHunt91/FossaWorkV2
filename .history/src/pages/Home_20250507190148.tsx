                  ...
                  <div 
                    ...
                    className={`relative group flex-grow flex items-center gap-2 rounded-xl py-2.5 px-5 cursor-pointer transition-all duration-200 ${
                      storeFilter === store.type || storeFilter === 'all' 
                        ? getStoreStyles(store.type).bg // Reverted to lighter active background
                        : 'bg-gray-100 dark:bg-gray-700'
                    } ${badgeSize} ${
                      storeFilter === store.type || storeFilter === 'all' 
                        ? `border-2 ${getStoreStyles(store.type).dot.replace('bg-', 'border-')}` // Add colored border if active
                        : 'border-2 border-transparent' // Transparent border if inactive
                    }`}
                  >
                    {/* Re-add colored dot */}
                    <span className={`
                      inline-block rounded-full h-3 w-3 mr-1 
                      ${getStoreStyles(store.type).dot}
                    `}></span>
                    <span className={`
                      font-medium
                      ${storeFilter === store.type || storeFilter === 'all' 
                        ? getStoreStyles(store.type).text // Reverted to store-specific text color for active
                        : 'text-gray-600 dark:text-gray-300'}
                    `}>
                      {store.name}
                    </span>
                    <span className={`
                      ml-2 px-3 py-1 rounded-full text-base font-semibold 
                      ${storeFilter === store.type || storeFilter === 'all' 
