  ...
  const loadInitialData = useCallback(async () => { 
    ...
    if (currentDispenserData && Object.keys(currentDispenserData).length > 0) {
      scrapedData.workOrders.slice(0, 5).forEach(wo => {
        // Reverting assumption: Try assigning calcResult directly to filtersFromResult
        // If FilterCalculationResult is actually FilterNeed[], this should work.
        // If not, this will still fail, but clarifies the type mismatch.
        const filtersFromResult: FilterNeed[] = calculateFiltersForWorkOrder(wo);

        // Check if filtersFromResult is actually an array before calling forEach
        if (Array.isArray(filtersFromResult)) {
          filtersFromResult.forEach((f: FilterNeed) => sampleNeeds.push({ 
            ...f, 
            orderId: wo.id, 
            visitId: wo.visits?.nextVisit?.visitId || 'N/A', 
            visitDate: wo.visits?.nextVisit?.date || 'N/A', 
            storeName: wo.customer?.name || 'N/A'  
          }));
        } else {
          // Log an error or handle the case where the expected array is not returned
          console.error('calculateFiltersForWorkOrder did not return an array for WO:', wo.id, 'Result:', filtersFromResult);
          // Potentially try to extract filters if it's an object with a known structure?
          // e.g., if (filtersFromResult && typeof filtersFromResult === 'object' && filtersFromResult.filters) { ... }
        }

        // Assuming warnings are handled separately or obtained differently for now
        // const warningsFromResult = ...;
        // if (warningsFromResult && warningsFromResult.length > 0) { ... }
      });
    }
    setFilterNeeds(sampleNeeds);
    setFilterWarnings(sampleWarnings); // Remains potentially empty
    setIsLoading(false);
  }, [dispenserDataContextValue, addToast]);

  const reloadData = useCallback(async () => { 
    setIsReloading(true);
    addToast({ type: 'info', message: 'Refreshing filter data...' }, 3000); // Added duration
    await loadInitialData(); 
    setIsReloading(false);
    addToast({ type: 'success', message: 'Filter data refreshed!' }, 3000); // Added duration
