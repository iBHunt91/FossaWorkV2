...
const HomeContent = () => {
  ...
  const renderVerificationPanel = () => {
    ...
    const generateRealWarnings = () => {
      ...
      // Process each order for potential warnings
      filteredWorkOrders.forEach(order => {
        ...
        if (order && order.id) {
          try {
            ...
            const { warnings: filterWarnings } = calculateFiltersForWorkOrder(order);
      ...
      // Check for orders missing dispenser data
      ...
      // Check for upcoming visits in the next 48 hours
      const now = new Date();
