...
const HomeContent = () => {
  ...
  const renderMapPanel = () => {
    ...
    const storeGeocodingFallback: Record<string, {lat: number, lng: number}> = {
      // Circle K stores (accurate coordinates)
      '1234': { lat: 40.7128, lng: -74.0060 }, // NYC
      '5678': { lat: 34.0522, lng: -118.2437 }, // LA
      
      // Work order IDs from logs - these would be replaced with actual store locations
      '126730': { lat: 39.0997, lng: -94.5786 }, // Kansas City, MO
      '126694': { lat: 38.6270, lng: -90.1994 }, // St. Louis, MO
      '126729': { lat: 39.7392, lng: -104.9903 }, // Denver, CO
      '126770': { lat: 41.8781, lng: -87.6298 }, // Chicago, IL
      '126723': { lat: 33.4484, lng: -112.0740 }, // Phoenix, AZ
      '126758': { lat: 32.7157, lng: -117.1611 }, // San Diego, CA
      '126750': { lat: 37.7749, lng: -122.4194 }, // San Francisco, CA
      '126666': { lat: 47.6062, lng: -122.3321 }, // Seattle, WA
      '126675': { lat: 45.5231, lng: -122.6765 }, // Portland, OR
      '126761': { lat: 36.1699, lng: -115.1398 }, // Las Vegas, NV
      '126736': { lat: 44.9778, lng: -93.2650 }, // Minneapolis, MN
      '126765': { lat: 33.7490, lng: -84.3880 }, // Atlanta, GA
      '126767': { lat: 25.7617, lng: -80.1918 }, // Miami, FL
      '132222': { lat: 30.2672, lng: -97.7431 }, // Austin, TX
      '126748': { lat: 29.7604, lng: -95.3698 }, // Houston, TX
      '126766': { lat: 32.7767, lng: -96.7970 }, // Dallas, TX
      '121101': { lat: 39.9526, lng: -75.1652 }, // Philadelphia, PA
      
      // With W- prefix for direct work order ID matching
      'W-126730': { lat: 39.0997, lng: -94.5786 }, // With W- prefix
      'W-126694': { lat: 38.6270, lng: -90.1994 },
      'W-126729': { lat: 39.7392, lng: -104.9903 },
      'W-126770': { lat: 41.8781, lng: -87.6298 },
      'W-126723': { lat: 33.4484, lng: -112.0740 },
      'W-126758': { lat: 32.7157, lng: -117.1611 },
      'W-126750': { lat: 37.7749, lng: -122.4194 },
      'W-126666': { lat: 47.6062, lng: -122.3321 },
      'W-126675': { lat: 45.5231, lng: -122.6765 },
      'W-126761': { lat: 36.1699, lng: -115.1398 },
      'W-126736': { lat: 44.9778, lng: -93.2650 },
      'W-126765': { lat: 33.7490, lng: -84.3880 },
      'W-126767': { lat: 25.7617, lng: -80.1918 },
      'W-132222': { lat: 30.2672, lng: -97.7431 },
      'W-126748': { lat: 29.7604, lng: -95.3698 },
      'W-126766': { lat: 32.7767, lng: -96.7970 },
      'W-121101': { lat: 39.9526, lng: -75.1652 },
      
      // Common store IDs for major chains
      'CK-1001': { lat: 33.4484, lng: -112.0740 }, // Circle K Phoenix
      'CK-1002': { lat: 32.7157, lng: -117.1611 }, // Circle K San Diego
      'CK-1003': { lat: 37.7749, lng: -122.4194 }, // Circle K San Francisco
      'SE-2001': { lat: 47.6062, lng: -122.3321 }, // 7-Eleven Seattle
      'SE-2002': { lat: 45.5231, lng: -122.6765 }, // 7-Eleven Portland
      'SE-2003': { lat: 36.1699, lng: -115.1398 }, // 7-Eleven Las Vegas
      'WW-3001': { lat: 39.9526, lng: -75.1652 }, // Wawa Philadelphia
      'WW-3002': { lat: 25.7617, lng: -80.1918 }, // Wawa Miami
      'WW-3003': { lat: 38.9072, lng: -77.0369 }, // Wawa DC
    };
    ...
    const locationsToShow: Location[] = inRangeOrders
      .map(order => {
          ...
          console.log(`[DEBUG] Order ${order.id} detailed location inspection:`, {
            ...
            // Raw properties that might be direct coordinates
            lat: order.lat,
            lng: order.lng,
            // Customer properties
            customerName: order.customer?.name,
            storeNumber: order.customer?.storeNumber || order.storeNumber
          });
          ...
          const workOrderId = order.id.replace(/^W-/i, ''); // Try without W- prefix
          const workOrderIdWithW = `W-${workOrderId}`; // Try with W- prefix

          // EXPAND search for coordinates in additional possible locations
          let extractedLat: number | undefined = undefined;
          let extractedLng: number | undefined = undefined;
          
          // Try all possible coordinate paths
          if (order.coordinates?.latitude !== undefined && order.coordinates?.longitude !== undefined) {
            extractedLat = order.coordinates.latitude;
            extractedLng = order.coordinates.longitude;
            console.log(`[renderMapPanel] Found coordinates in order.coordinates: ${extractedLat}, ${extractedLng}`);
          } 
          else if (order.location?.latitude !== undefined && order.location?.longitude !== undefined) {
            extractedLat = order.location.latitude;
            extractedLng = order.location.longitude;
            console.log(`[renderMapPanel] Found coordinates in order.location: ${extractedLat}, ${extractedLng}`);
          }
          else if (order.clientLocation?.latitude !== undefined && order.clientLocation?.longitude !== undefined) {
            extractedLat = order.clientLocation.latitude;
            extractedLng = order.clientLocation.longitude;
            console.log(`[renderMapPanel] Found coordinates in order.clientLocation: ${extractedLat}, ${extractedLng}`);
          }
          else if (order.address?.location?.latitude !== undefined && order.address?.location?.longitude !== undefined) {
            extractedLat = order.address.location.latitude;
            extractedLng = order.address.location.longitude;
            console.log(`[renderMapPanel] Found coordinates in order.address.location: ${extractedLat}, ${extractedLng}`);
          }
          else if (order.site?.location?.latitude !== undefined && order.site?.location?.longitude !== undefined) {
            extractedLat = order.site.location.latitude;
            extractedLng = order.site.location.longitude;
            console.log(`[renderMapPanel] Found coordinates in order.site.location: ${extractedLat}, ${extractedLng}`);
          }
          else if (order.client?.location?.latitude !== undefined && order.client?.location?.longitude !== undefined) {
            extractedLat = order.client.location.latitude;
            extractedLng = order.client.location.longitude;
            console.log(`[renderMapPanel] Found coordinates in order.client.location: ${extractedLat}, ${extractedLng}`);
          }
          else if (order.store?.location?.latitude !== undefined && order.store?.location?.longitude !== undefined) {
            extractedLat = order.store.location.latitude;
            extractedLng = order.store.location.longitude;
            console.log(`[renderMapPanel] Found coordinates in order.store.location: ${extractedLat}, ${extractedLng}`);
          }
          else if (order.address?.coordinates?.latitude !== undefined && order.address?.coordinates?.longitude !== undefined) {
            extractedLat = order.address.coordinates.latitude;
            extractedLng = order.address.coordinates.longitude;
            console.log(`[renderMapPanel] Found coordinates in order.address.coordinates: ${extractedLat}, ${extractedLng}`);
          }
          else if (order.address?.geo?.lat !== undefined && order.address?.geo?.lng !== undefined) {
            extractedLat = order.address.geo.lat;
            extractedLng = order.address.geo.lng;
            console.log(`[renderMapPanel] Found coordinates in order.address.geo: ${extractedLat}, ${extractedLng}`);
          }
          else if (order.lat !== undefined && order.lng !== undefined) {
            extractedLat = order.lat;
            extractedLng = order.lng;
            console.log(`[renderMapPanel] Found coordinates in order.lat/lng: ${extractedLat}, ${extractedLng}`);
          }
          // Try nested data under 'data' property which might exist in some responses
          else if ((order as any).data?.coordinates?.latitude !== undefined && (order as any).data?.coordinates?.longitude !== undefined) {
            extractedLat = (order as any).data.coordinates.latitude;
            extractedLng = (order as any).data.coordinates.longitude;
            console.log(`[renderMapPanel] Found coordinates in order.data.coordinates: ${extractedLat}, ${extractedLng}`);
          }
          else if ((order as any).data?.location?.latitude !== undefined && (order as any).data?.location?.longitude !== undefined) {
            extractedLat = (order as any).data.location.latitude;
            extractedLng = (order as any).data.location.longitude;
            console.log(`[renderMapPanel] Found coordinates in order.data.location: ${extractedLat}, ${extractedLng}`);
          }
          // Check for any field ending with *Location that has lat/lng or latitude/longitude
          else {
            for (const key in order) {
              if (key.endsWith('Location') && typeof order[key as keyof typeof order] === 'object' && order[key as keyof typeof order] !== null) {
                const loc = order[key as keyof typeof order] as any;
                if (loc.latitude !== undefined && loc.longitude !== undefined) {
                  extractedLat = loc.latitude;
                  extractedLng = loc.longitude;
                  console.log(`[renderMapPanel] Found coordinates in order.${key}: ${extractedLat}, ${extractedLng}`);
                  break;
                }
                else if (loc.lat !== undefined && loc.lng !== undefined) {
                  extractedLat = loc.lat;
                  extractedLng = loc.lng;
                  console.log(`[renderMapPanel] Found coordinates in order.${key}: ${extractedLat}, ${extractedLng}`);
                  break;
                }
              }
          ...
          // If we have valid coordinates at this point, use them
          if (typeof extractedLat === 'number' && typeof extractedLng === 'number' && 
            ...
            console.log(`[renderMapPanel] Using REAL coordinates for job ${order.id}: (${extractedLat}, ${extractedLng})`);
          ...
          // If we get here, we need to use fallback geocoding
          console.log(`[renderMapPanel] No valid coordinates found for job ${order.id}. Attempting fallback geocoding.`);
          ...
          if (fallbackLocation) {
              ...
              // Important: Mark this location as a fallback
              return {
                id: order.id,
                lat: extractedLat,
                lng: extractedLng,
                title: `${getDisplayName(order)} (Estimated Location)`,
                storeType: getStoreTypeForFiltering(order),
                isFallback: true
              } as Location;
          }
          
          // Only if all else fails, use approximate generated location
          console.log(`[renderMapPanel] No fallback found for job ${order.id}, skipping.`);
          return null; // Better to not show anything than to show fake random locations
      })
      .filter((location): location is Location => location !== null);

    console.log('[renderMapPanel] Locations to show on map:', locationsToShow.length, locationsToShow);

    // Around line 1695-1715 in renderMapPanel function, add additional debug logging
    // Debug the first few orders in their entirety to see the actual data structure
    if (inRangeOrders.length > 0) {
      console.log('[renderMapPanel] DETAILED ORDER #1:', JSON.stringify(inRangeOrders[0], null, 2));
      if (inRangeOrders.length > 1) {
        console.log('[renderMapPanel] DETAILED ORDER #2:', JSON.stringify(inRangeOrders[1], null, 2));
      }
      
      // Log property paths to help identify where location data might be
      console.log('[renderMapPanel] Property paths for Order 1:');
      const order = inRangeOrders[0];
      ...
      console.log('- order.site:', order.site);
      console.log('- order.clientLocation:', order.clientLocation);
    }
