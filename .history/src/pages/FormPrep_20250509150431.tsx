...
const FormPrep: React.FC<{}> = () => {
  ...
  const extractDispenserProgressLocal = (message: string, totalDispensers?: number): { current: number; total: number } => {
    ...
    if (match && match[1] && match[2]) {
      ...
      // Priority 2: "... dispenser: X/Y ..." or "... dispenser X/Y ..." (more general context)
      // This looks for "dispenser" followed by ":", optional spaces, then "X/Y" or "X of Y"
    ...
    if (totalDispensers && totalDispensers > 0) {
      ...
      // Optional: Log if parsedTotalFromMessage exists and differs from totalDispensers
      if (parsedTotalFromMessage !== undefined && parsedTotalFromMessage !== finalTotal) {
        console.warn(`Dispenser total mismatch: job.dispenserCount is ${finalTotal}, message parsed total as ${parsedTotalFromMessage}. Using job.dispenserCount.`);
      }
    } else if (parsedTotalFromMessage !== undefined) {
      // If job.dispenserCount not available, use total parsed from message
      finalTotal = parsedTotalFromMessage;
    }
    
    // If current is still 0 (no specific current number parsed) AND finalTotal is known,
    // default current to 1, unless the message explicitly mentions "dispenser 0" or "disp 0".
    if (finalTotal > 0 && current === 0 && !message.match(/(?:dispenser|disp)(?:\s*#)?\s*0/i) ) {
      current = 1;
    }

    // Ensure current does not exceed total if total is positive.
