  ...
  const renderJobRow = (order: WorkOrder, dateGroupClass?: string) => {
    ...
    return (
      ...
      >
        ...
        {/* Action Buttons Footer - Hidden slide-out toolbar */}
        <div className="p-2 border-t border-gray-700/50 flex items-center justify-between relative group">
          ...
          {/* Small indicator that toolbar exists */}
          <div className="h-5 w-5 bg-gray-700 hover:bg-gray-600 rounded-full flex items-center justify-center cursor-pointer group-hover:hidden transition-opacity">
            <FiMoreHorizontal className="h-3.5 w-3.5 text-gray-300" />
          </div>
          
          {/* Action buttons container - Hidden until hover */}
          <div className="absolute right-0 top-1/2 -translate-y-1/2 flex gap-1.5 transform translate-x-full opacity-0 group-hover:translate-x-2 group-hover:opacity-100 transition-all duration-300 ease-out">
            {/* WorkFossa Button */}
            <button
              onClick={(e) => {
                e.stopPropagation();
            ...
            <button 
              ...
              className="p-1.5 bg-cyan-600 hover:bg-cyan-700 text-white rounded shadow-sm transition-colors"
