 ...
 */
const extractDispenserProgress = (message: string, totalDispensers?: number) => {
  const dispenserRegex = /dispenser(?:[\s#]+)(\d+)(?:[^\d]+(\d+)|)/i;
  const dispenserMatch = message.match(dispenserRegex);
  
  const result = {
    current: 0,
    total: totalDispensers || 0
  };
  
  if (dispenserMatch && dispenserMatch[1]) {
    result.current = parseInt(dispenserMatch[1], 10);
    
    // If there's a second capture group, use it, otherwise use the provided total
    if (dispenserMatch[2]) {
      result.total = parseInt(dispenserMatch[2], 10);
    }
  }
 ...
 */
const extractFuelProgress = (message: string) => {
  const fuelTypeRegex = /processing fuel type:\s*([a-zA-Z0-9\s-]+)\s*\((\d+)\/(\d+)\)/i;
  const fuelTypeMatch = message.match(fuelTypeRegex);
  ...
  if (fuelTypeMatch && fuelTypeMatch[1] && fuelTypeMatch[2] && fuelTypeMatch[3]) {
    result.type = fuelTypeMatch[1].trim();
    result.current = parseInt(fuelTypeMatch[2], 10);
    result.total = parseInt(fuelTypeMatch[3], 10);
  }
