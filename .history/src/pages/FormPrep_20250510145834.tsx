...
const FormPrep: React.FC<{}> = () => {
  ...
  const startBatchPolling = () => {
    const pollInterval = setInterval(async () => {
      try {
        if (typeof batchJobId !== 'string') {
          console.error('batchJobId is not a string. Cannot poll status.');
          clearInterval(pollInterval); // Stop polling if batchJobId is invalid
          return;
        }
