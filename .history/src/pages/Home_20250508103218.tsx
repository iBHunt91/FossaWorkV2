...
// Import the date group styles component
import DateGroupStyles from '../components/DateGroupStyles'
...
const HomeContent = () => {
      ...
      <div className="mb-6 animate-fadeIn">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
            ...
            <div className="flex flex-col space-y-4 z-10 relative">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="font-semibold text-gray-800 dark:text-white">{filteredWorkOrders.length} Jobs</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-5 border border-gray-200 dark:border-gray-700 hover:shadow-xl transition-all duration-300">
            ...
            <div className="mb-4">
              <h3 className="text-base font-medium text-gray-700 dark:text-gray-300 mb-2">Current Week</h3>
              <div className="flex flex-wrap gap-2">
                {Object.entries(storeDistribution.currentWeek).map(([type, count]) => {
                  // Skip if count is 0
                  if (count === 0) return null;
                  
                  // Get name for the store type
                  const name = type === 'circle-k' ? 'Circle K' : 
                              type === '7-eleven' ? '7-Eleven' : 
                              type === 'wawa' ? 'Wawa' : 'Other';
                  
                  // Get color classes based on store type
                  const bgClass = `bg-${type === 'circle-k' ? 'amber' : 
                                    type === '7-eleven' ? 'red' :
                                    type === 'wawa' ? 'indigo' : 'blue'}-50`;
                  const darkBgClass = `dark:bg-${type === 'circle-k' ? 'amber' : 
                                        type === '7-eleven' ? 'red' :
                                        type === 'wawa' ? 'indigo' : 'blue'}-900/30`;
                  const textClass = `text-${type === 'circle-k' ? 'amber' : 
                                      type === '7-eleven' ? 'red' :
                                      type === 'wawa' ? 'indigo' : 'blue'}-700`;
                  const darkTextClass = `dark:text-${type === 'circle-k' ? 'amber' : 
                                          type === '7-eleven' ? 'red' :
                                          type === 'wawa' ? 'indigo' : 'blue'}-300`;
                  const borderClass = `border-${type === 'circle-k' ? 'amber' : 
                                        type === '7-eleven' ? 'red' :
                                        type === 'wawa' ? 'indigo' : 'blue'}-200`;
                  const darkBorderClass = `dark:border-${type === 'circle-k' ? 'amber' : 
                                            type === '7-eleven' ? 'red' :
                                            type === 'wawa' ? 'indigo' : 'blue'}-800/50`;
                  
                  return (
                    <div 
                      key={type} 
                      className={`flex items-center justify-between px-3 py-2 rounded-lg border ${bgClass} ${darkBgClass} ${textClass} ${darkTextClass} ${borderClass} ${darkBorderClass} min-w-[100px] flex-1`}
                    >
                ...
                {/* Show "None" if no jobs */}
                {Object.values(storeDistribution.currentWeek).reduce((a, b) => a + b, 0) === 0 && (
            ...
            <div>
              <h3 className="text-base font-medium text-gray-700 dark:text-gray-300 mb-2">Next Week</h3>
              <div className="flex flex-wrap gap-2">
                ...
                {/* Show "None" if no jobs */}
                {Object.values(storeDistribution.nextWeek).reduce((a, b) => a + b, 0) === 0 && (
                  <div className="text-sm text-gray-500 dark:text-gray-400 italic p-2">
                    No jobs scheduled
                  </div>
                )}
  ...
  // Update the renderJobRow function to use date grouping classes
  const renderJobRow = (order: any, dateGroupClass?: string) => {
    ...
    const getDispenserQuantity = (order: any) => {
      return order.dispensers?.length || 0;
    };
    ...
    const formattedVisitDate = nextVisitDate ? nextVisitDate.toLocaleDateString(undefined, { weekday: 'short', month: 'short', day: 'numeric' }) : 'No visit scheduled';
    ...
    // Parse date color classes if provided
    let dateBorderClass = '';
    let dateBadgeClass = '';
    let dateTextClass = '';
    ...
    if (dateGroupClass) {
      ...
      const colorClasses = dateGroupClass.split(' ');
      ...
      const positionClass = colorClasses.find(c => c.startsWith('date-group-'));
      
      if (borderClass) dateBorderClass = borderClass;
      if (bgClass) dateBadgeClass = bgClass;
      if (textClass) dateTextClass = textClass;
    }

    return (
      <div
        key={order.id}
        className={`bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden border ${dateGroupClass ? dateBorderClass || storeStyle.cardBorder : storeStyle.cardBorder} hover:shadow-md transition-all duration-200 ${dateGroupClass ? 'date-grouped-card' : ''}`}
      >
        <div className={`flex justify-between items-center px-4 py-3 ${storeStyle.headerBg} border-b border-gray-200 dark:border-gray-700`}>
          <div className="flex items-center space-x-2">
            ...
            {/* Date indicator dot */}
            {dateGroupClass && (
              <div className={`h-2.5 w-2.5 rounded-full ${dateBadgeClass} ml-1`} 
                   title="Jobs scheduled for the same day"></div>
            )}
          </div>
        ...
        <div className="p-4">
            ...
            {hasNextVisit && (
              <div className="flex items-start">
                <div className="flex-shrink-0 mt-0.5 mr-2">
                  <FiCalendar className={`h-4 w-4 ${dateGroupClass ? dateTextClass || 'text-gray-500' : 'text-gray-500'} dark:text-gray-400`} />
                </div>
                <div>
                  <div className="text-xs text-gray-500 dark:text-gray-400 font-medium mb-0.5">Visit Date</div>
                  <div className={`text-sm ${dateGroupClass ? dateTextClass || 'text-gray-700' : 'text-gray-700'} dark:text-gray-300`}>
                    {formattedVisitDate}
                  </div>
                </div>
              </div>
          ...
          {instructions && (
            <div className={`mt-2 rounded p-3 border ${storeStyle.cardBg} border-gray-200 dark:border-gray-700`}>
              <div className="flex items-start gap-2">
                <FiInfo className={`h-4 w-4 mt-0.5 ${storeStyle.text} flex-shrink-0`} />
          ...
          <div className="mt-4 flex justify-end gap-1"> 
            ...
            {order.visits?.nextVisit?.url && (
              <button
                className="p-2 text-green-500 hover:text-green-600 dark:text-green-400 dark:hover:text-green-300 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 focus:outline-none transition-colors"
                onClick={(e) => {
                  e.stopPropagation();
            ...
            <button
              className={`p-2 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 focus:outline-none transition-colors ${
                hasDispenserData
                  ? storeStyle.text + ' hover:' + storeStyle.textHover
                  : 'text-gray-400 dark:text-gray-500 cursor-not-allowed'
              }`}
              onClick={(e) => hasDispenserData && handleViewDispenserData(order, e)}
            ...
            <button
              className="p-2 text-gray-500 hover:text-red-500 dark:text-gray-400 dark:hover:text-red-400 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 focus:outline-none transition-colors"
            ...
            <button
              className="p-2 text-gray-500 hover:text-blue-500 dark:text-gray-400 dark:hover:text-blue-400 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 focus:outline-none transition-colors"
    ...
    switch (storeType) {
      case '7-eleven':
        return {
          ...
          count: 'bg-red-200 dark:bg-red-700 text-red-700 dark:text-red-200'
        };
      case 'circle-k':
        return {
          name: 'Circle K',
          ...
          count: 'bg-amber-200 dark:bg-amber-700 text-amber-700 dark:text-amber-200'
        };
      case 'wawa':
        return {
          name: 'Wawa',
          ...
          boxBg: 'bg-indigo-50 dark:bg-indigo-900/20',
          icon: '🦆',
          bg: 'bg-indigo-100 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300',
          dot: 'bg-indigo-500 dark:bg-indigo-400',
          count: 'bg-indigo-200 dark:bg-indigo-700 text-indigo-700 dark:text-indigo-200'
        };
      case 'other':
      default:
        return {
          ...
          count: 'bg-blue-200 dark:bg-blue-700 text-blue-700 dark:text-blue-200'
        };
    }
  };
  ...
  const renderWeeklySections = (grouped: GroupedWorkOrders) => {
    ...
    const getCurrentWeekDays = () => {
      const days = [];
      const startDate = new Date(workWeekDates.currentWeekStart);
      
      for (let i = 0; i < 7; i++) {
        const currentDate = new Date(startDate);
        currentDate.setDate(startDate.getDate() + i);
        
        // Only include days up to workWeekEnd
        const dayOfWeek = currentDate.getDay();
        if (i === 0 || dayOfWeek <= workWeekEnd || dayOfWeek >= workWeekStart) {
          days.push(currentDate);
        }
        
        // Break when we reach the end of the work week
        if (dayOfWeek === workWeekEnd) break;
      }
      
      return days;
    };
    
    // Get an array of dates for the next week
    const getNextWeekDays = () => {
      const days = [];
      const startDate = new Date(workWeekDates.nextWeekStart);
    ...
    // Sort jobs by date
    const sortJobsByDate = (jobs: WorkOrder[], targetDate: Date) => {
    ...
    // Generate a unique color for each date consistently
    const getDateColor = (dateStr: string) => {
      // Hash function to generate a consistent color for a date string
      ...
      // List of distinct colors for date groups - keeping them easy to distinguish
      const colors = [
        'bg-blue-100 border-blue-500 text-blue-800 dark:text-blue-300',
        'bg-purple-100 border-purple-500 text-purple-800 dark:text-purple-300',
        'bg-green-100 border-green-500 text-green-800 dark:text-green-300',
        'bg-yellow-100 border-yellow-500 text-yellow-800 dark:text-yellow-300',
        'bg-red-100 border-red-500 text-red-800 dark:text-red-300',
        'bg-pink-100 border-pink-500 text-pink-800 dark:text-pink-300',
        'bg-indigo-100 border-indigo-500 text-indigo-800 dark:text-indigo-300',
        'bg-cyan-100 border-cyan-500 text-cyan-800 dark:text-cyan-300',
      ];
      
      // Get a consistent index for a given date
      const index = Math.abs(hashCode(dateStr)) % colors.length;
      return colors[index];
    };
    
    // Helper function to render a group of work orders with date headers (for past/future sections)
      ...
      return (
        <div>
          {ordersByDate.map((dateGroup) => {
            // Get a unique color for this date group
            const dateColor = getDateColor(dateGroup.date);
            
            return (
              <div key={dateGroup.date} className="mb-8 relative">
                {/* Date header with job count badge */}
                <div className="flex items-center mb-2">
                  <h3 className="text-sm font-medium text-gray-500 dark:text-gray-400 flex items-center">
                    <span className={`inline-block w-3 h-3 rounded-full mr-2 border-2 ${dateColor.split(' ')[1]}`}></span>
                    <FiCalendar className="mr-1 h-3.5 w-3.5" /> 
                    {formatDateHeader(dateGroup.date)}
                  </h3>
                  <div className={`ml-auto ${dateColor} text-xs font-medium px-2 py-0.5 rounded-full shadow-sm`}>
                    {dateGroup.orders.length} job{dateGroup.orders.length !== 1 ? 's' : ''}
                  </div>
                </div>
                
                {/* Vertical line connector for cards (if more than one job) */}
                {dateGroup.orders.length > 1 && (
                  <div className={`absolute left-1.5 top-8 bottom-4 w-1 ${dateColor.split(' ')[1]} opacity-80`}></div>
                )}
                
                {/* Cards container with left spacing for connector line */}
                <div className="space-y-0 ml-4 relative">
                  {/* Wrap all cards from the same date in a container */}
                  <div className={`date-grouped-wrapper ${dateColor.split(' ')[2]}`}>
                    {dateGroup.orders.map((order, index) => {
                      // Pass date-specific class to the renderJobRow function
                      const isFirst = index === 0;
                      const isLast = index === dateGroup.orders.length - 1;
                      
                      // Class to indicate first/last/middle card in date group
                      ...
                      return (
                        <div key={order.id} className="relative mb-2">
                          {/* Horizontal connector line to card */}
                          <div className={`absolute -left-4 top-1/2 w-3 h-0.5 ${dateColor.split(' ')[1]} opacity-70`}></div>
                          
                          {/* Dot connector */}
                          <div className={`absolute -left-5 top-1/2 transform -translate-y-1/2 w-2 h-2 rounded-full ${dateColor.split(' ')[1]}`}></div>
                          
                          {/* Vertical connectors for cards (except for singles) */}
                          {!isFirst && (
                            <div className={`date-group-connector date-group-connector-top ${dateColor.split(' ')[1]}`}></div>
                          )}
                          {!isLast && (
                            <div className={`date-group-connector date-group-connector-bottom ${dateColor.split(' ')[1]}`}></div>
                          )}
                          
                          {/* The actual job card with date-specific border */}
                          <div className={`relative ${dateGroup.orders.length > 1 ? 'group' : ''}`}>
                            {dateGroup.orders.length > 1 && index < dateGroup.orders.length - 1 && (
                              <div 
                                className={`absolute -bottom-1 -right-1 w-full h-full rounded-lg border ${dateColor.split(' ')[1]} bg-white dark:bg-gray-800 opacity-40 z-0`}
                              ></div>
                            )}
                            {renderJobRow(order, `${dateColor} ${positionClass}`)}
                          </div>
                        </div>
                      );
                    })}
    ...
    // Render a job card (compact version for calendar view)
    const renderCompactJobCard = (job: WorkOrder) => {
      const storeType = getStoreTypeForFiltering(job);
      const storeStyle = getStoreStyles(storeType);
      const isFavorite = favorites.includes(job.id.toString());
      const hasDispenserData = job.dispensers && job.dispensers.length > 0;
      
      return (
        <div 
          key={job.id} 
          className={`p-2 mb-1 rounded border ${storeStyle.cardBorder} bg-white dark:bg-gray-800 shadow-sm hover:shadow transition-shadow cursor-pointer flex flex-col`}
          onClick={() => hasDispenserData && handleViewDispenserData(job, new MouseEvent('click') as any)}
        >
          <div className="flex items-center justify-between mb-1">
            <div className="flex items-center gap-1">
              <span className={`w-2 h-2 rounded-full ${storeStyle.dot}`}></span>
              <h4 className="text-xs font-medium text-gray-900 dark:text-white truncate max-w-[140px]">
                {job.customer?.name || 'Unknown'}
              </h4>
            </div>
            {isFavorite && (
              <FiStar className="h-3 w-3 text-amber-500 fill-current flex-shrink-0" />
            )}
          </div>
          {job.customer?.storeNumber && (
            <div className="text-[10px] text-gray-500 dark:text-gray-400">
              #{job.customer.storeNumber.replace(/#/g, '')}
            </div>
          )}
          {hasDispenserData && (
            <div className="mt-1 flex items-center text-[10px] text-gray-500 dark:text-gray-400">
              <GiGasPump className={`h-2.5 w-2.5 mr-0.5 ${storeStyle.text}`} />
              <span>{job.dispensers?.length || 0}</span>
            </div>
          )}
        </div>
      );
    ...
    // Render a day cell with its jobs
    const renderDayCell = (date: Date, jobs: WorkOrder[]) => {
      const dateStr = date.toLocaleDateString();
      const dateColor = getDateColor(dateStr);
        ...
        >
          ...
          <div className={`text-center p-2 border-b ${
            ...
            <div className={`text-lg font-bold ${
              isCurrentDay ? 'text-blue-700 dark:text-blue-400' : 'text-gray-700 dark:text-gray-300'
            }`}>
              {getDayNumber(date)}
            </div>
          </div>
          ...
          {/* Jobs container */}
          <div className="p-1 h-[180px] overflow-y-auto">
            {jobCount > 0 ? (
              <>
                {jobs.map(job => renderCompactJobCard(job))}
                {jobCount > 0 && (
                  <div className={`text-center mt-1 py-1 rounded-full text-xs font-medium ${dateColor} shadow-sm`}>
    ...
    // Render a week grid
    const renderWeekGrid = (title: string, subtitle: string, days: Date[], jobs: WorkOrder[], bgColor: string, textColor: string, iconElement: JSX.Element) => {
      return (
        <div className="mb-8">
          <div className="mb-4 border-b border-gray-200 dark:border-gray-700 p-4">
            <div className="flex items-center">
              <div className={`flex-shrink-0 h-10 w-10 ${bgColor} rounded-full flex items-center justify-center mr-3`}>
                {iconElement}
              </div>
              <div>
              ...
              <div className="ml-auto">
                <span className={`${bgColor} ${textColor} text-xs font-medium px-2.5 py-1 rounded-full`}>
          ...
          {jobs.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-3 md:grid-cols-5 gap-4 px-4">
              {days.map(day => renderDayCell(day, sortJobsByDate(jobs, day)))}
            </div>
          ) : (
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-6 text-center border border-gray-200 dark:border-gray-700">
              <div className={`inline-flex h-14 w-14 items-center justify-center rounded-full ${bgColor} mb-3`}>
                <FiCalendar className={`h-7 w-7 ${textColor}`} />
              </div>
              <h3 className="text-base font-medium text-gray-900 dark:text-white mb-1">No jobs scheduled</h3>
    ...
    // Render all week grids
    return (
      <div className="pb-8">
        {/* Current week calendar view */}
        {renderWeekGrid(
          "Current Week",
          formatDateRange(workWeekDates.currentWeekStart, workWeekDates.currentWeekEnd),
          getCurrentWeekDays(),
        ...
        {/* Other dates section - keep as list for now */}
        {grouped.other.length > 0 && (
          <div className="mb-8">
            <div className="mb-5 border-b border-gray-200 dark:border-gray-700 p-4">
              <div className="flex items-center">
                <div className="flex-shrink-0 h-10 w-10 bg-purple-50 dark:bg-purple-900/30 rounded-full flex items-center justify-center mr-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400">
                    Jobs scheduled beyond next week
                  </p>
                </div>
                <div className="ml-auto">
            ...
            {renderOrderGroup && renderOrderGroup(grouped.other)}
          </div>
        )}
        
        {/* Past jobs section - keep as list for now */}
        {!isActualCurrentWeek && grouped.past.length > 0 && (
          <div className="mb-8">
            <div className="mb-5 border-b border-gray-200 dark:border-gray-700 p-4">
              <div className="flex items-center">
                <div className="flex-shrink-0 h-10 w-10 bg-gray-50 dark:bg-gray-700 rounded-full flex items-center justify-center mr-3">
                ...
                <div className="ml-auto">
                  <span className="bg-gray-100 dark:bg-gray-800 text-gray-800 dark:text-gray-300 text-xs font-medium px-2.5 py-1 rounded-full">
                    {grouped.past.length} jobs
                  </span>
                </div>
              </div>
            </div>

            {renderOrderGroup && renderOrderGroup(grouped.past)}
          </div>
        )}
    
        {/* Show no jobs message if there are no jobs in any category */}
        {grouped.thisWeek.length === 0 && 
         ...
         grouped.past.length === 0 && (
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow p-8 text-center border border-gray-200 dark:border-gray-700">
            <div className="inline-flex h-20 w-20 items-center justify-center rounded-full bg-gray-100 dark:bg-gray-700 mb-5">
              <FiList className="h-10 w-10 text-gray-500 dark:text-gray-400" />
            </div>
            <h3 className="text-xl font-medium text-gray-900 dark:text-white mb-3">No jobs found</h3>
            <p className="text-gray-500 dark:text-gray-400 max-w-md mx-auto mb-6">
              There are no jobs matching your current filters. Try changing your filter settings or refreshing the data.
            </p>
            <div className="flex flex-wrap justify-center gap-3">
              <button
                className="px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg text-sm font-medium text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 dark:focus:ring-offset-gray-800"
                onClick={() => setActiveFilter('all')}
              >
                <FiFilter className="inline-block mr-1.5 -mt-0.5" /> 
                Clear Filters
              </button>
              <button
                className="px-4 py-2 bg-primary-600 border border-transparent rounded-lg text-sm font-medium text-white hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 dark:focus:ring-offset-gray-800"
                onClick={handleScrapeWorkOrders}
              >
                <FiRefreshCw className="inline-block mr-1.5 -mt-0.5" /> 
                Refresh Data
              </button>
            </div>
          </div>
  ...
  return (
    ...
    <div className="h-full max-w-full overflow-x-hidden">
      ...
      {/* Main toolbar */}
      <div className="bg-gradient-to-r from-gray-800 to-gray-900 dark:from-gray-900 dark:to-gray-950 text-white rounded-xl shadow-lg mb-6 flex flex-col overflow-hidden border border-gray-700 dark:border-gray-800 mx-4 sm:mx-6">
        {/* Top section - view toggles only, remove dark mode button */}
        <div className="flex items-center p-4 border-b border-gray-700/60 dark:border-gray-800/60">
          <div className="flex items-center gap-2">
            <button
              className={`px-3 py-2 rounded-md flex items-center gap-1.5 transition-all text-sm ${
                activeView === 'weekly'
        ...
        <div className="flex items-center p-4 relative gap-4">
          {/* Background effects */}
          ...
          <div className="relative flex-1 z-10 min-w-[200px]">
            <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
              <FiSearch className="h-5 w-5 text-gray-400" />
            </div>
            <input
              type="text"
              className="block w-full pl-10 pr-3 py-2.5 bg-gray-700/70 dark:bg-gray-800/70 border border-gray-600 dark:border-gray-700 rounded-lg text-gray-200 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm transition-colors"
          ...
          <div className="flex items-center gap-1 z-10">
            <button
              className="p-2 text-gray-300 hover:text-blue-400 bg-gray-700/70 dark:bg-gray-800/70 hover:bg-gray-600 dark:hover:bg-gray-700 rounded-lg focus:outline-none transition-colors"
              title="Refresh Data"
              onClick={() => {
            ...
            <button
              ...
              className={`p-2 rounded-lg focus:outline-none transition-colors ${showWorkWeekSettings ? 'bg-blue-600 text-white' : 'text-gray-300 hover:text-blue-400 bg-gray-700/70 dark:bg-gray-800/70 hover:bg-gray-600 dark:hover:bg-gray-700'}`}
      ...
      {showWorkWeekSettings && (
         <div
           ...
           className="bg-white dark:bg-gray-800 rounded-xl p-5 mb-6 border border-gray-200 dark:border-gray-700 shadow-md mx-4 sm:mx-6"
         >
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="bg-gray-50 dark:bg-gray-900/40 p-4 rounded-lg">
                ...
                <select
                  ...
                  className="w-full border border-gray-300 dark:border-gray-600 rounded-lg py-2 px-3 bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-200 focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                >
            ...
            <div className="mt-4 bg-blue-50 dark:bg-blue-900/20 p-3 rounded-lg border-l-4 border-blue-500">
              <h4 className="text-sm font-medium text-blue-800 dark:text-blue-300 mb-1">About Work Week Settings</h4>
              <p className="text-xs text-blue-700 dark:text-blue-400">
                After 5:00 PM on the selected End day, the dashboard will display the next work week's schedule.
              </p>
            </div>
          </div>
        )}
      ...
      <div className={`flex-grow px-4 sm:px-6 ${isFullscreenMode ? 'fixed inset-0 z-50 bg-white dark:bg-gray-900 p-4 overflow-auto' : ''}`}>
        {isFullscreenMode && (
        ...
        {isLoading ? (
          ...
          <>
                          ...
                          >
                            <div className="flex justify-between items-start">
                              ...
                              {eventsOnDay.length > 0 && (
                                <span className="text-xs font-medium px-2 py-1 rounded-full bg-blue-500 text-white dark:bg-blue-600 dark:text-white shadow-sm transform scale-110">
                                  {eventsOnDay.length}
                                </span>
                              )}
                            </div>
                            ...
                            <div className={`mt-1 space-y-1 overflow-hidden ${eventsOnDay.length > 1 ? 'relative' : ''}`} style={{ maxHeight: "77px" }}>
                              {eventsOnDay.slice(0, 2).map((order, idx) => {
                                ...
                                return (
                                  <div 
                                    key={idx} 
                                    className={`text-xs p-1 rounded truncate ${storeStyle.badge} ${eventsOnDay.length > 1 && idx === 0 ? 'shadow-sm' : ''}`}
                                    title={order.customer?.name || 'Unknown store'}
                                    style={eventsOnDay.length > 1 && idx === 0 ? { transform: 'rotate(-2deg)' } : {}}
                                  >
                              ...
                              {eventsOnDay.length > 2 && (
                                <div className="text-xs font-medium bg-gray-100 dark:bg-gray-700 p-1 rounded text-gray-700 dark:text-gray-300 text-center shadow-sm" style={{ transform: 'rotate(2deg)' }}>
            ...
            {activeView === 'compact' && (
              <div className="bg-gradient-to-b from-gray-50 to-white dark:from-gray-900 dark:to-gray-800 rounded-xl shadow-md overflow-hidden border border-gray-200 dark:border-gray-700">
                ...
                {/* Compact calendar grid layout for the week */}
                <div className="p-4">
                  ...
                  {(() => {
                    ...
                    const currentWeekDays = (() => {
                      const dateRanges = getWorkWeekDateRanges(workWeekStart, workWeekEnd, selectedDate);
                      const days = [];
                      const start = new Date(dateRanges.currentWeekStart);
                    ...
                    // Render the grid for current week
                    return (
                      <div className="bg-white dark:bg-gray-800 rounded-lg overflow-hidden shadow-sm border border-gray-200 dark:border-gray-700 mb-6">
                        {/* Week header */}
                        <div className="bg-blue-50 dark:bg-blue-900/20 p-3 border-b border-gray-200 dark:border-gray-700 flex items-center">
                          <div className="h-9 w-9 rounded-full bg-blue-100 dark:bg-blue-900/40 flex items-center justify-center mr-3">
                            <FiCalendar className="text-blue-600 dark:text-blue-400" />
                          </div>
                          <div>
                            <h2 className="font-semibold text-gray-800 dark:text-gray-200">Current Week</h2>
                            <p className="text-sm text-gray-600 dark:text-gray-400">
                              {formatDateRange(
                                getWorkWeekDateRanges(workWeekStart, workWeekEnd, selectedDate).currentWeekStart,
                                getWorkWeekDateRanges(workWeekStart, workWeekEnd, selectedDate).currentWeekEnd
                              )}
                            </p>
                          </div>
                          <div className="ml-auto">
