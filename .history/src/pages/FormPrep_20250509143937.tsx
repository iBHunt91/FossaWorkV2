...
const FormPrep: React.FC<{}> = () => {
  ...
  const extractDispenserProgressLocal = (message: string, totalDispensers?: number): { current: number; total: number } => {
    ...
    if (totalDispensers && totalDispensers > 0) {
      ...
      // If a message indicates a specific dispenser, current will be updated.
      // Otherwise, current remains 0, signifying that dispenser processing hasn't explicitly started.
    ...
    // Critical: If a specific dispenser number (e.g. 0) is parsed from the message
    // and total is > 0, we might still want to adjust it if "dispenser 0" is not a valid state to display.
    // For now, if parsing results in current = 0 and total > 0, it means no specific dispenser message was found for current.
    // The rendering logic will then show a skeleton.
                          ...
                          {/* Keep your existing form job rows implementation */}
                            {formJobs.map((job, index) => {
                              ...
                              return (
                                ...
                                <td className="px-4 py-3">
                                  {job.status === 'running' && (
                                    <div className="space-y-3 min-w-56 min-h-16"> {/* Added min-h-16 here */}
                                      {/* Dispenser progress */}
                                      {(() => {
                                        const dispenserInfo = extractDispenserProgressLocal(job.message || '', job.dispenserCount);
                                        ...
                                        <div>
                                          ...
                                          // Show skeleton loader for dispensers
                                          return (
                                            <div>
                                              <div className="flex justify-between items-center text-xs mb-1">
