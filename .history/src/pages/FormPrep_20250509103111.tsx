...
const FormPrep: React.FC<{}> = () => {
  ...
  const downloadLogs = () => {
    try {
      ...
      // Save file to a specific directory using Electron IPC if available
      if (window.electron && window.electron.saveFile) {
        const filename = `form-prep-logs-${new Date().toISOString().replace(/:/g, '-')}.json`;
        window.electron.saveFile({
          filename,
          content: jsonString,
          path: './logs', // Save in the logs directory
        }).then((result: any) => {
          if (result.success) {
        ...
        // Fall back to browser download if Electron is not available
        saveThroughBrowser();
      }
      ...
      function saveThroughBrowser() {
        ...
        a.href = url;
        a.download = `form-prep-logs-${new Date().toISOString().replace(/:/g, '-')}.json`;
        ...
        addToast('success', 'Debug logs downloaded successfully');
      }
    } catch (error) {
