                            ...
                            {batchJobs.map((job, index) => {
                              ...
                              return (
                                ...
                                >
                                  ...
                                  {/* Original 2nd TD (Progress) - UNTOUCHED BY THIS EDIT */}
                                  <td className="px-4 py-3 whitespace-nowrap">
                                              ...
                                              {job.currentVisitName ? (
                                                <>
                                                  <span className="text-indigo-600 dark:text-indigo-400">
                                                    Visit {job.completedVisits + 1}:
                                                  </span>{' '}
                                                  <span className={job.currentVisitStatus?.includes('DEMO DATA') ? 'italic text-gray-400 dark:text-gray-500' : ''}>
                                                    {job.currentVisitName}
                                                    {job.currentVisitStatus?.includes('DEMO DATA') && 
                                                      <span className="text-xs text-amber-500 dark:text-amber-400 ml-1">(Sample Data)</span>
                                                    }
                                                  </span>
                                                </>
                                              ) : (
                                                `Visit ${job.completedVisits + 1} of ${job.totalVisits}`
                                              )}
                                            </span>
                                          </div>
                                          {/* Add additional detail about automation status */}
                                          <div className="mt-1 text-gray-500 dark:text-gray-400 text-xs">
                                            {job.currentVisitStatus && !job.currentVisitStatus.includes('DEMO DATA') ? (
                                              <div className="flex items-center gap-1">
                                                <FiRefreshCw className="h-3 w-3 animate-spin" />
                                                <span>Automation Status: {job.currentVisitStatus}</span>
                                              </div>
                                            ) : null}
                                          </div>
                                        </div>
                                  ...
                                  {/* Original 3rd TD (Dispensers/Fuel) - UNTOUCHED BY THIS EDIT */}
                                  <td className="px-4 py-3">
                                    {job.status === 'running' && (
