        ...
        if (status) {
          ...
          if (!status.message && !status.currentVisitStatus) {
            ...
            if (currentJob) {
              ...
              if (elapsedSec % 20 < 10) {
                status.currentVisitStatus = `Working on ${storeName} - Dispenser ${dispenserCurrent}/${dispenserCount}`;
              } else {
                status.currentVisitStatus = `Working on ${storeName} - Processing ${fuelType} (${fuelCurrent}/3)`;
              }
              
              status.message = `Processing visit ${visitIndex + 1} of ${currentJob.totalVisits}: ${storeName}`;
            }
          }
        ...
        switch (currentPhase) {
          ...
          case 'filling':
            if (Math.random() > 0.5) {
              ...
              statusMessage = `Working on ${currentStore} - Dispenser ${dispenserCounter}/${dispenserTotal}`;
            } else {
              // Sometimes show fuel type progress
              statusMessage = `Working on ${currentStore} - Processing ${currentFuelType} (${fuelTypeCounter}/${fuelTypeTotal})`;
            }
            break;
          case 'saving':
                                      ...
                                      <div className="mt-1 text-xs text-gray-600 dark:text-gray-400 flex items-center gap-1">
                                        ...
                                        <span className="font-medium">
                                          {job.currentVisitName ? 
                                            `Visit ${job.completedVisits + 1}: ${job.currentVisitName}` : 
