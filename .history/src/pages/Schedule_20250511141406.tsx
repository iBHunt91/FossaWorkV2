    ...
    return (
      ...
      >
        ...
        <div className={`${cardHeaderBgColor} p-3 flex justify-between items-center rounded-t-md relative`}>
          {/* Service count indicator - new */}
          {order.services && order.services.length > 0 && (
            <div className="absolute -top-3 -right-3 bg-purple-600 text-white w-7 h-7 rounded-full flex items-center justify-center shadow-md" title={`${order.services.length} services`}>
              <span className="text-xs font-bold">{order.services.length}</span>
            </div>
          )}
        ...
        <div className="p-4 space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-x-6 gap-y-4 items-start">
            ...
            {/* Services - Add services information */}
            <div className="flex items-start col-span-1 md:col-span-1">
              <FiTool className={`h-4 w-4 mr-2 mt-1 ${iconColor} flex-shrink-0`} />
              <div className="flex flex-col">
                <p className={`text-xs ${subTextColor}`}>Services</p>
                {order.services && order.services.length > 0 ? (
                  <div className="flex flex-col">
                    <div className="flex items-baseline">
                      <p className={`text-sm font-medium ${textColor} mr-1`}>{order.services.length}</p>
                      <p className={`text-xs ${subTextColor}`}>{order.services.length !== 1 ? 'Services' : 'Service'}</p>
                    </div>
                    <div className="flex flex-wrap gap-1 mt-1">
                      {order.services.slice(0, 2).map((service, idx) => (
                        <span key={idx} className="text-xs px-1.5 py-0.5 rounded bg-gray-800 text-gray-300 whitespace-nowrap overflow-hidden text-ellipsis max-w-[120px]" title={service.description}>
                          {service.code || service.type.substring(0, 10)}
                        </span>
                      ))}
