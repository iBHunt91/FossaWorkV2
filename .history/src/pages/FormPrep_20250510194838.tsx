...
const FormPrep: React.FC<{}> = () => {
  ...
  const formatDisplayMessage = (message: string | undefined): string => {
    ...
    // These include any form filling, dispenser interactions, or fuel type processing.
    // We want to avoid showing detailed backend messages like "Processing dispenser 1/2..." if a progress bar shows this.
    if (
      ...
      // Exception: If the message is a clear final status (e.g., "All dispensers processed successfully"),
      // let it pass through instead of shortening to "Processing...".
      // This is a simple check; more sophisticated logic might be needed based on exact backend messages.
      if (lowerMessage.includes('successfully') || lowerMessage.includes('completed') || lowerMessage.includes('failed')) {
        // However, if it *still* looks like a detailed progress step, override to "Processing..."
        // e.g. "Dispenser 1 of 2 processing completed" -> still show "Processing..." because the bar shows 1/2.
        // This is tricky. For now, let's be more aggressive in showing "Processing..." to stop fluctuations.
        // If it contains typical progress keywords, and isn't one of the *very specific* full messages above, make it generic.
        return 'Processing...';
      }
      return 'Processing...'; // Default for progress-related messages
    }
