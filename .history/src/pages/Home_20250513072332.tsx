  ...
  const renderMapPanel = () => {
    ...
    const locationsToShow: Location[] = inRangeOrders
      .filter(order => {
          ...
          // (Keep all the existing checks)
          const hasStandardLocation = order.location && 
              typeof order.location.latitude === 'number' && 
              typeof order.location.longitude === 'number';
          
          // Check for coordinates property (used by JobMap.tsx)
          const hasCoordinates = order.coordinates && 
              typeof order.coordinates.latitude === 'number' && 
              typeof order.coordinates.longitude === 'number';
              
          // Check for clientLocation property (another common pattern)
          const hasClientLocation = order.clientLocation && 
              typeof order.clientLocation.latitude === 'number' && 
              typeof order.clientLocation.longitude === 'number';
          
          // Check for address nested location
          const hasAddressLocation = order.address?.location &&
              typeof order.address.location.latitude === 'number' &&
              typeof order.address.location.longitude === 'number';
              
          // Check for site nested location
          const hasSiteLocation = order.site?.location &&
              typeof order.site.location.latitude === 'number' &&
              typeof order.site.location.longitude === 'number';
          
          // Check for client nested location
          const hasClientNestedLocation = order.client?.location &&
              typeof order.client.location.latitude === 'number' &&
              typeof order.client.location.longitude === 'number';
              
          // Check for store nested location
          const hasStoreLocation = order.store?.location &&
              typeof order.store.location.latitude === 'number' &&
              typeof order.store.location.longitude === 'number';
              
          // Check for address nested coordinates
          const hasAddressCoords = order.address?.coordinates &&
              typeof order.address.coordinates.latitude === 'number' &&
              typeof order.address.coordinates.longitude === 'number';
              
          // Check for address geo
          const hasAddressGeo = order.address?.geo &&
              typeof order.address.geo.lat === 'number' &&
              typeof order.address.geo.lng === 'number';
              
          // New check: Simple lat/lng pattern at root level
          const hasLatLng = order.lat !== undefined && 
              order.lng !== undefined &&
              typeof order.lat === 'number' && 
              typeof order.lng === 'number';
              
          const hasLocation = hasStandardLocation || hasCoordinates || hasClientLocation || 
                             hasAddressLocation || hasSiteLocation || hasClientNestedLocation || 
                             hasStoreLocation || hasAddressCoords || hasAddressGeo || hasLatLng;
      ...
      .map(order => {
          ...
          const extractedLat = order.coordinates?.latitude || 
          ...
          const extractedLng = order.coordinates?.longitude || 
          ...
          // Skip any locations with undefined coordinates
          console.log(`[renderMapPanel] Filtered out order ${order.id} due to undefined coordinates after extraction.`);
          return null;
      })
