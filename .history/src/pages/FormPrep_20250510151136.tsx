...
const FormPrep: React.FC<{}> = () => {
  ...
  useEffect(() => {
    ...
    if (batchJobId) {
      ...
      getBatchAutomationStatus(batchJobId) // Pass batchJobId here
        .then(currentStatus => {
  ...
  const pollingManager = React.useRef({
    ...
    startPolling: function(
      ...
      if (this.activePolls[jobId]) {
        ...
        if (this.activePolls[jobId].isPaused) {
          console.log(`ðŸ” Resuming previously paused poll for job ID: ${jobId}`);
      ...
      const firstTimeout = setTimeout(() => {
        ...
        console.log('ðŸ” Job progress at 15 seconds:', poll.lastMessage);
      }, 15000);
      
      // Activity monitoring every 30 seconds
      const secondTimeout = setTimeout(() => {
        // This starts the activity monitoring system
        console.log('ðŸ” Starting activity monitoring for job');
        ...
        const activityInterval = setInterval(() => {
          ...
          console.log(`ðŸ” Activity check: ${lastMessage}`);
          console.log(`ðŸ” Time since last activity: ${Math.round(timeSinceLastChange/1000)}s`);
          ...
          if (isActive || messageIsRecent) {
            ...
            console.log('ðŸ” Automation is still active, continuing');
            return;
          }
          
          // Check for a long inactive period (2+ minutes with no change)
          if (timeSinceLastChange > 120000) {
            console.log('ðŸ” No activity for 2 minutes, assuming completion');
          ...
          console.log('ðŸ” Limited activity detected, but not forcing completion yet');
      ...
      const finalTimeout = setTimeout(() => {
        console.log('ðŸ” 5-minute maximum check');
        ...
        if (isClosingBrowser || messageRecentlyChanged) {
          console.log('ðŸ” Still active at 5 minutes, giving a bit more time');
          return;
        }
        
        console.log('ðŸ” Reached 5-minute maximum, forcing completion');
        this.activePolls[jobId].forceComplete = true;
        onUpdate({
          status: 'completed',
    ...
    pausePolling: function(jobId: string) {
      console.log('ðŸ” Pausing polling for job ID:', jobId);
      
      if (!this.activePolls[jobId]) {
        console.log(`ðŸ” No active polling found for job ID: ${jobId}`);
        return;
      }
    ...
    stopPolling: function(jobId: string) {
      console.log('ðŸ” Stopping polling for job ID:', jobId);
    ...
    pauseAll: function() {
      console.log('ðŸ” Pausing all active polls');
  ...
  const handleBatchProcess = async () => {
    ...
    try {
      ...
      startBatchPolling(batchResult.jobId);
    } catch (error) {
  ...
  const startBatchPolling = (currentBatchJobId: string | null) => {
    const pollInterval = setInterval(async () => {
      try {
        if (typeof currentBatchJobId !== 'string') {
          console.error('Batch Job ID is not a string in startBatchPolling. Stopping poll.');
          clearInterval(pollInterval);
          return;
        }
        const status = await getBatchAutomationStatus(currentBatchJobId);
                      ...
                      <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                          {formJobs.map((job, index) => {
                            ...
                            // Remove the âš¡ symbol from the store name if it exists
                            const cleanStoreName = visitInfo.storeName.replace(/âš¡/g, '').trim();
                                  ...
                                  <div className="flex items-center">
                                    ...
                                    <span className="text-xs text-gray-700 dark:text-gray-300">
                                      {job.completedVisits || 0}/{job.totalVisits || 0}
                                                                  ...
                                                                  <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{job.message}</td>                              </tr>                            ))}                          </tbody>                        </table>                      </div>                    </div>                  </div>                )}              </div>            </div>          </div>        </div>      </div>    </div>  );};export default FormPrep; 
