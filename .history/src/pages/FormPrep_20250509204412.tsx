...
const FormPrep: React.FC<{}> = () => {
  ...
  const startBatchPolling = () => {
    ...
    const poll = async () => {
        ...
        // --- START MOCK DATA INJECTION FOR UI DEMO ---
        // This section simulates a more detailed API response if the real one lacks progress details.
        // REMOVE THIS or make it conditional once your backend API sends this data.
        if (currentBatchJobId) {
          const mockProgress = mockBatchProgressRef.current;
          const isRunningOrIdleAsRunning = response.status === 'running' || 
                                         (response.status === 'idle' && 
                                          (response.completedVisits || 0) < (response.totalVisits || 0));

          if (isRunningOrIdleAsRunning) {
            if (mockProgress.jobId !== currentBatchJobId) {
              // New job or restarting mock for this job
              mockProgress.jobId = currentBatchJobId;
              mockProgress.dispenserCurrent = 0;
              mockProgress.fuelCurrent = 0;
              addDebugLog('BATCH_MOCK', 'Initializing mock progress for job', mockProgress);
            }

            // Inject mock data if real data is missing
            if (response.dispenserCount === undefined) {
              response.dispenserCount = mockProgress.dispenserTotal;
            }
            if (response.dispenserCurrent === undefined) {
              response.dispenserCurrent = mockProgress.dispenserCurrent;
            }
            if (response.fuelType === undefined) {
              response.fuelType = mockProgress.fuelType;
            }
            if (response.fuelTotal === undefined) {
              response.fuelTotal = mockProgress.fuelTotal;
            }
            if (response.fuelCurrent === undefined) {
              response.fuelCurrent = mockProgress.fuelCurrent;
            }
            
            // Cycle mock progress for next poll (simple increment and loop)
            mockProgress.dispenserCurrent++;
            if (mockProgress.dispenserCurrent > mockProgress.dispenserTotal) {
              mockProgress.dispenserCurrent = 0; // Reset or move to next phase
              mockProgress.fuelCurrent++;
              if (mockProgress.fuelCurrent > mockProgress.fuelTotal) {
                mockProgress.fuelCurrent = 0; // Reset fuel too
                // Optionally, could stop mocking here or simulate job completion after a few cycles
              }
            }
            addDebugLog('BATCH_MOCK', 'Updated mock progress for next poll', { ...mockProgress, nextDispenserCurrent: mockProgress.dispenserCurrent, nextFuelCurrent: mockProgress.fuelCurrent });
          } else if (mockProgress.jobId === currentBatchJobId) {
            // Job is no longer running, clear mock association
            mockProgress.jobId = null;
            addDebugLog('BATCH_MOCK', 'Real job not running, clearing mock association');
          }
        }
        ...
        setBatchJobs(prevJobs => {
          ...
          // Add specific log for currentVisitStatus if it exists
          if (jobToUpdate.currentVisitStatus) {
            addDebugLog('BATCH_VISIT_STATUS', `Received currentVisitStatus for Job ID ${currentBatchJobId}`, { visitId: jobToUpdate.currentVisitId, statusMessage: jobToUpdate.currentVisitStatus });
          }
          ...
          // Update dispenser progress for the current visit
          if (response.dispenserCount !== undefined) {
            jobToUpdate.formsTotal = response.dispenserCount;
          } // If not provided by API, formsTotal retains its previous value or default from job creation

          if (response.dispenserCurrent !== undefined) {
            jobToUpdate.formsCurrent = response.dispenserCurrent;
          } else if (jobToUpdate.currentVisitStatus) {
            // Fallback to extracting from message if API doesn't send current directly
            ...
            // If no direct current and no status message, ensure it's not undefined, defaulting to existing or 0
            jobToUpdate.formsCurrent = jobToUpdate.formsCurrent || 0;
          }

          // Update fuel progress for the current visit from API if available
          if (response.fuelType !== undefined) {
            jobToUpdate.currentVisitFuelType = response.fuelType;
          }
          if (response.fuelCurrent !== undefined) {
            jobToUpdate.currentVisitFuelCurrent = response.fuelCurrent;
          }
          if (response.fuelTotal !== undefined) {
            jobToUpdate.currentVisitFuelTotal = response.fuelTotal;
          }
          ...
          // This return statement is crucial for when the job is still running.
          return updatedJobs;
        });
                          ...
                          <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                            {batchJobs.map((job, index) => {
                              ...
                              >
                                ...
                                <td className="px-4 py-3">
                                  {job.status === 'running' && (
                                      <div className="space-y-3 min-w-56 min-h-16"> {/* Consistent with single visit min-height */}
                                      ...
                                      {(() => {
                                        ...
                                        if (dispenserInfo.total > 0) {
                                          return (
                                            <div>
                                              ...
                                              <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1.5 overflow-hidden">
                                                <div 
                                                  className={`h-1.5 rounded-full transition-all duration-500 bg-amber-500`}
                                                  style={{ width: `${Math.min(Math.round((dispenserInfo.current / Math.max(dispenserInfo.total, 1)) * 100), 100)}%` }}
                                                ></div>
                                              </div>
                                            </div>
                                          );
                                        } else { // Skeleton for dispenser progress
                                          return (
                                            <div>
                                              <div className="flex justify-between items-center text-xs mb-1">
                                                <div className="font-medium text-gray-400 dark:text-gray-500 flex items-center gap-1">
                                                ...
                                                <span className="font-mono bg-gray-200 dark:bg-gray-700/50 text-gray-400 dark:text-gray-500 px-1.5 py-0.5 rounded">--/--</span>
                                              </div>
                                        ...
                                        {/* Fuel type progress for current visit */}
                                        {(() => {
                                          const currentVisitMessage = job.currentVisitStatus || job.message || '';
                                          ...
                                          // Fallback to extraction if direct API data is not available
                                          if (fuelType === undefined || fuelCurrent === undefined || fuelTotal === undefined) {
                                              ...
                                              <div>
                                                ...
                                                <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1.5 overflow-hidden">
                                                  <div 
                                                    ...
                                                    style={{ width: `${Math.min(Math.round((fuelCurrent / Math.max(fuelTotal, 1)) * 100), 100)}%` }}
                                                  ></div>
                                                </div>
                                              </div>
                                            );
                                          } else { // Skeleton for fuel progress
                                            return (
                                              <div>
                                                <div className="flex justify-between items-center text-xs mb-1">
                                                  <div className="font-medium text-gray-400 dark:text-gray-500 flex items-center gap-1">
                                    ...
                                    {job.status === 'completed' && (
                                      <div className="flex items-center text-xs text-green-600 dark:text-green-500 h-full py-1 justify-center"> {/* Centered content */}
                                    ...
                                    {job.status === 'error' && job.message === 'Processing stopped by user' && (
                                      <div className="flex items-center text-xs text-red-600 dark:text-red-500 h-full py-1 justify-center"> {/* Centered content */}
                                        <FiAlertTriangle className="w-4 h-4 mr-1.5 flex-shrink-0" /> {/* Changed to alert triangle */}
                                        <span className="font-medium">
                                          {job.message}
                                        </span>
                                      </div>
