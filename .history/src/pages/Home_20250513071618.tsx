  ...
  const renderMapPanel = () => {
    ...
    const locationsToShow = inRangeOrders
      .filter(order => {
          ...
          if (!hasLocation) {
              console.log(`[renderMapPanel Map] Order ${order.id}: Missing or invalid location data after parsing attempt. Order data:`, JSON.stringify(order, null, 2)); // Enhanced log
      ...
      .map(order => {
        const extractedCoords = getValidCoordinates(order)!; // Safe due to filter
        return {
          id: order.id,
