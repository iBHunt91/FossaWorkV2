...
const FormPrep: React.FC<{}> = () => {
  ...
  return (
    <div className="h-full max-w-full overflow-x-hidden animate-fadeIn">
        ...
        <div className="p-5 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className="h-10 w-10 rounded-lg bg-blue-600/20 flex items-center justify-center">
              <FiFileText className="text-blue-400 h-5 w-5" />
            </div>
            <div>
              <h1 className="text-xl font-semibold">Form Automation</h1>
              <p className="text-gray-300 text-sm">Prepare and automate form submissions</p>
            </div>
          </div>
          
          <div className="flex items-center gap-3">
            {/* Tab Switcher */}
            <div className="bg-[#2d3c55] rounded-lg overflow-hidden flex">
              <button 
                onClick={() => setActiveTab('single')} 
                className={`px-4 py-2 text-sm flex items-center gap-2 transition-colors ${
                  activeTab === 'single' 
                    ? 'bg-blue-600 text-white' 
                    : 'text-gray-300 hover:bg-[#3a4a66]'
                }`}
              >
            ...
            <button 
              onClick={() => openUiControls()}
              className="px-3 py-1.5 rounded-lg bg-[#2d3c55] text-gray-300 text-sm hover:bg-[#3a4a66] transition-colors flex items-center gap-1.5"
            >
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-5 border border-gray-200 dark:border-gray-700 mb-6">
          <div className="flex flex-col md:flex-row items-start md:items-end gap-4 mb-4">
            <div className="flex-1">
              <label htmlFor="urlInput" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Visit URL</label>
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="block w-full rounded-lg border-gray-300 dark:border-gray-600 shadow-sm bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-200 pl-10 py-2 focus:ring-blue-500 focus:border-blue-500 text-sm"
                />
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              >
                {isProcessing ? <FiRefreshCw className="h-4 w-4 animate-spin" /> : <FiPlay className="h-4 w-4" />}
              ...
              {isProcessing && (
                <button
                  ...
                  className="px-4 py-2 rounded-lg bg-red-600 text-white text-sm hover:bg-red-700 flex items-center gap-2"
                >
          ...
          {/* Search and Select from Work Orders section */}
          <div className="mt-6">
            <h3 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Select from recent work orders</h3>
            <div className="relative">
              ...
              <input
                type="text"
                className="block w-full rounded-lg border-gray-300 dark:border-gray-600 shadow-sm bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-200 pl-10 py-2 focus:ring-blue-500 focus:border-blue-500 text-sm"
                placeholder="Search by store name or visit ID..."
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
              />
            </div>
            
            {/* Work Week Navigation Controls */}
              ...
              {isLoading ? (
                ...
                <ul className="divide-y divide-gray-200 dark:divide-gray-700">
                  {filteredWorkOrders.map((order) => {
                    ...
                    const fullVisitUrl = visitUrl ? (visitUrl.startsWith('http') ? visitUrl : `https://app.workfossa.com${visitUrl}`) : '';
                    ...
                    return (
                      ...
                      >
                        <div className="px-4 py-3 flex items-start justify-between">
                          <div className="flex-1">
                            <div className="flex items-center">
                              <h3 className="text-sm font-medium text-gray-900 dark:text-white">{order.customer.name}</h3>
                ...
                <div className="text-center py-8 px-4">
                  {searchTerm ? (
                    <>
                      ...
                      <p className="mt-2 text-gray-500 dark:text-gray-400">No work orders found matching "{searchTerm}"</p>
                    </>
                  ) : (
                    <>
                      <FiInfo className="mx-auto h-8 w-8 text-gray-400" />
                      <p className="mt-2 text-gray-500 dark:text-gray-400">No work orders available for this week</p>
                    </>
                  )}
                </div>
              )}
      ...
      {/* Content for Batch Form tab */}
      {activeTab === 'batch' && (
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-5 border border-gray-200 dark:border-gray-700 mb-6">
          {/* Keep existing batch tab content - will be migrated separately */}
          {/* This is intentionally left simple for now */}
          <div className="flex flex-col space-y-4">
            ...
            <div className="flex items-center gap-4">
              <div className="flex items-center space-x-2">
                ...
                <select
                  className="rounded-md border-gray-300 dark:border-gray-600 shadow-sm py-1.5 px-3 bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-200 text-sm focus:ring-blue-500 focus:border-blue-500"
                  value={groupBy}
      ...
      {/* Job status section at bottom - keep existing implementation */}
