        ...
        <div className="p-4">
          ...
          <div className="mt-3 flex flex-wrap items-center justify-between border-t border-gray-200 dark:border-gray-700 pt-3">
            <div className="flex items-center">
              ...
              <span className="text-xs text-gray-600 dark:text-gray-400 font-medium">Custom Instructions</span>
            </div>
          ...
          <>
            ...
            {activeView === 'calendar' && (
              <CalendarView
                ...
                setSelectedVisitNumber={setSelectedVisitNumber}
                setSelectedInstructions={setSelectedInstructions}
                setSelectedJobTitle={setSelectedJobTitle}
                setShowInstructionsModal={setShowInstructionsModal}
                setShowDispenserModal={setShowDispenserModal}
                setSelectedDispensers={setSelectedDispensers}
                setWorkWeekDates={setWorkWeekDates}
                setWorkWeekDateRanges={setWorkWeekDateRanges}
                setWorkWeekStart={setWorkWeekStart}
                setWorkWeekEnd={setWorkWeekEnd}
                setShowWorkWeekSettings={setShowWorkWeekSettings}
                setRecentScrapedOrders={setRecentScrapedOrders}
                ...
                setExpandedTechnicalDetails={setExpandedTechnicalDetails}
                setDistributionLogged={setDistributionLogged}
                setTransitionProcessed={setTransitionProcessed}
                setInitialWeekendModeShown={setInitialWeekendModeShown}
                setWorkOrderStatus={setWorkOrderStatus}
                setDispenserStatus={setDispenserStatus}
                setIsWorkOrderScraping={setIsWorkOrderScraping}
                setIsDispenserScraping={setIsDispenserScraping}
                setCountsByCategory={setCountsByCategory}
                setFilteredWorkOrders={setFilteredWorkOrders}
                ...
                setWorkWeekDates={setWorkWeekDateRanges}
                setSelectedDate={setSelectedDate}
                ...
                <div className="p-4 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 flex flex-wrap items-center justify-between gap-2">
                  ...
                  <div className="flex items-center gap-2 ml-auto md:ml-0">
                    <select
                      ...
                      value={activeFilter}
                      onChange={(e) => setActiveFilter(e.target.value as StoreFilter)}
                ...
                <div className="p-4">
                  ...
                  <div className="grid grid-cols-7 gap-1">
                    {(() => {
                      ...
                      for (let i = 0; i < firstDayOfMonth; i++) {
                        days.push(
                          <div key={`empty-${i}`} className="h-28 p-1 border border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50 rounded-md"></div>
                        );
                      }
                      ...
                      for (let i = 1; i <= daysInMonth; i++) {
                        ...
                        days.push(
                          ...
                          >
                            ...
                            {/* Show event cards with improved styling and interaction */}
                            <div className="mt-1 space-y-1 overflow-y-auto" style={{ maxHeight: "96px" }}>
                              {eventsOnDay.slice(0, 3).map((order, idx) => {
                                const storeType = getStoreTypeForFiltering(order);
                                const storeStyle = getStoreStyles(storeType);
                                  ...
                                  >
                                    ...
                                    {order.services && order.services.length > 0 && (
                                      <div className="text-xs opacity-80 truncate">
                                        {order.services[0]?.description || 'Service visit'}
                                      </div>
                                    )}
                                    {order.dispensers && order.dispensers.length > 0 && (
                                      <div className="mt-0.5 flex items-center gap-1 text-[10px] opacity-75">
                                        ...
                                        <span>{order.dispensers.length} Dispensers</span>
                                      </div>
                                    )}
                                  </div>
                                );
