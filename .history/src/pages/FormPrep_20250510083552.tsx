  ...
  const startBatchPolling = () => {
    ...
    const poll = async () => {
        ...
        setBatchJobs(prevJobs => {
          ...
          jobToUpdate.currentVisitStatus = response.currentVisitStatus ?? jobToUpdate.currentVisitStatus; // Store the raw message from API

          // Determine current visit name
          if (response.currentVisit) {
            const wo = workOrders.find(order => order.visits.nextVisit.visitId === response.currentVisit);
            jobToUpdate.currentVisitName = wo?.customer.name ?? response.storeInfo?.name ?? response.currentVisit ?? 'Unknown Visit';
          ...
          // --- Start: Refined Current Visit Progress Logic ---
          const currentVisitApiMessage = response.currentVisitStatus || ""; // Use API's currentVisitStatus for parsing

          // Dispenser Progress for current visit
          let newFormsTotal: number | undefined = response.dispenserCount;
          let newFormsCurrent: number | undefined = response.dispenserCurrent;

          if (newFormsTotal === undefined || newFormsTotal === 0 || newFormsCurrent === undefined) {
            const extractedDispenser = extractDispenserProgressLocal(currentVisitApiMessage, response.dispenserCount); // Pass API dispenserCount as potential total
            if (extractedDispenser.total > 0 && extractedDispenser.total !== 100) { // 100 might be a placeholder
              newFormsTotal = (newFormsTotal === undefined || newFormsTotal === 0) ? extractedDispenser.total : newFormsTotal; // Prioritize API if it was valid non-zero
              newFormsCurrent = extractedDispenser.current;
            } else if (newFormsTotal === undefined && extractedDispenser.total > 0) { // If API total was undefined, but message had one
                 newFormsTotal = extractedDispenser.total;
                 newFormsCurrent = extractedDispenser.current;
            }
          }
          jobToUpdate.formsTotal = newFormsTotal;
          jobToUpdate.formsCurrent = newFormsCurrent;
          
          // Fuel Progress for current visit
          let newFuelType: string | undefined = response.fuelType;
          let newFuelCurrent: number | undefined = response.fuelCurrent;
          let newFuelTotal: number | undefined = response.fuelTotal;

          if (!newFuelType || newFuelTotal === undefined || newFuelTotal === 0 || newFuelCurrent === undefined) {
            const extractedFuel = extractFuelProgressLocal(currentVisitApiMessage);
            if (extractedFuel.type && extractedFuel.total > 0) {
              newFuelType = newFuelType ?? extractedFuel.type;
              // Prioritize API if it was valid non-zero, else use extracted
              newFuelTotal = (newFuelTotal === undefined || newFuelTotal === 0) ? extractedFuel.total : newFuelTotal;
              newFuelCurrent = newFuelCurrent === undefined ? extractedFuel.current : newFuelCurrent; // If API current was undefined, use extracted
            } else if (!newFuelType && extractedFuel.type && extractedFuel.total > 0) { // If API had no type, but message did
                newFuelType = extractedFuel.type;
                newFuelTotal = extractedFuel.total;
                newFuelCurrent = extractedFuel.current;
            }
          }
          jobToUpdate.currentVisitFuelType = newFuelType;
          jobToUpdate.currentVisitFuelCurrent = newFuelCurrent;
          jobToUpdate.currentVisitFuelTotal = newFuelTotal;
          // --- End: Refined Current Visit Progress Logic ---
