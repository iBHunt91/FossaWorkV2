...
const HomeContent = () => {
  ...
  const renderMapPanel = () => {
    ...
    const storeGeocodingFallback: Record<string, {lat: number, lng: number}> = {
      ...
      // 7-Eleven stores
      '2345': { lat: 39.9526, lng: -75.1652 }, // Philadelphia
      '6789': { lat: 32.7157, lng: -117.1611 }, // San Diego
      '0123': { lat: 39.7392, lng: -104.9903 }, // Denver
      '4567': { lat: 38.9072, lng: -77.0369 }, // Washington DC
      '8901': { lat: 30.2672, lng: -97.7431 }, // Austin
    };
    ...
    const locationsToShow: Location[] = inRangeOrders
      .map(order => {
          ...
          // Try using fallback geocoding if no coordinates are found
          if (extractedLat === undefined || extractedLng === undefined) {
              const fallbackLocation = storeGeocodingFallback[storeNumberClean];
              if (fallbackLocation) {
                  console.log(`[renderMapPanel] Using fallback geocoding for store #${storeNumberClean}`);
                  extractedLat = fallbackLocation.lat;
                  extractedLng = fallbackLocation.lng;
              } else {
                  // If no store number match, use mock location based on order ID for demo purposes
                  // This ensures we always have some locations to show on the map
                  const mockLat = 35.0 + (parseInt(order.id.replace(/\D/g, '')) % 10) * 0.5;
                  const mockLng = -95.0 - (parseInt(order.id.replace(/\D/g, '')) % 10) * 0.5;
                  console.log(`[renderMapPanel] Using mock location for order ${order.id}`);
                  extractedLat = mockLat;
                  extractedLng = mockLng;
              }
          }
          ...
          // If we still don't have valid coordinates, log it and mark as missing
