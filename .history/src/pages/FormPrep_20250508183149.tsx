      ...
      try {
        ...
        try {
          ...
          if (apiError.message && apiError.message.includes('404')) {
            ...
            const currentBatch = batchJobs.find(job => job.status === 'running');
            if (!currentBatch) {
              console.warn('No running batch job found to simulate status for');
              return;
            }
            ...
            status = {
              status: 'running',
              message: currentBatch.message || 'Processing batch...',
              completedVisits: currentBatch.completedVisits || 0,
              totalVisits: currentBatch.totalVisits || 0,
              currentVisit: currentBatch.currentVisitName || 'Unknown store',
              currentVisitStatus: currentBatch.currentVisitStatus || 'Processing...',
              timestamp: new Date().toISOString()
            };
          } else {
            // For non-404 errors, just log and continue polling
            return;
          }
        }
        ...
        if (status) {
          ...
          setBatchJobs(prev => {
            ...
            if (jobIndex !== -1) {
              ...
              // Ensure proper completion status translation
              let jobStatus = status.status;
              
              // Always force status to running while we're still polling
              // The status will be set to completed or error only after the final check
              if (status.completedVisits !== undefined && 
                  status.totalVisits !== undefined &&
                  status.completedVisits < status.totalVisits) {
                jobStatus = 'running';
              }
                        ...
                        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                          <thead className="bg-gray-50 dark:bg-gray-700/50">
                            <tr>
                              ...
                              <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Overview</th>
                              <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Progress Detail</th>
                            </tr>
                          </thead>
                          <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                            {batchJobs.map((job, index) => (
                              ...
                              >
                                ...
                                <td className="px-4 py-3">
                                  {/* Enhanced Progress Visualization goes here - only display for running jobs */}
                                  {job.status === 'running' && (
                                    <div className="flex flex-col space-y-4">
                                      {/* Visit Progress Section */}
                                      <div className="mb-3">
                                        <div className="flex justify-between items-center text-xs mb-1">
                                          <div className="font-medium text-primary-600 dark:text-primary-400 flex items-center gap-1">
                                            <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                          ...
                                          <span className="font-mono bg-primary-100 dark:bg-primary-900/30 text-primary-800 dark:text-primary-300 px-1.5 py-0.5 rounded">
                                        ...
                                        <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1.5 overflow-hidden">
                                          <div 
                                            ...
                                            style={{ width: `${Math.round((job.completedVisits / Math.max(1, job.totalVisits)) * 100)}%` }}
                                          ></div>
                                        </div>
                                      </div>
                                      
                                      {/* Forms/Fields Progress Section */}
                                        ...
                                        <div className="mb-3">
                                          ...
                                          <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1.5 overflow-hidden">
                                            <div 
                                              ...
                                              style={{ width: `${Math.round(((job.formsCurrent || 0) / Math.max(1, job.formsTotal)) * 100)}%` }}
                                            ></div>
                                          </div>
                                        </div>
                                      )}
                                      ...
                                      {job.currentVisitStatus && (
                                        <div className="mb-3">
                                          ...
                                          <div className="p-2 bg-gray-100 dark:bg-gray-700/50 rounded text-xs text-gray-700 dark:text-gray-300">
                                            {job.currentVisitStatus}
                                          </div>
                                        </div>
                                      )}
                                      
                                      {/* Show message if no detailed status is available */}
                                      {!job.currentVisitStatus && (
                                        <div className="p-3 bg-blue-50 dark:bg-blue-900/20 rounded border border-blue-200 dark:border-blue-800">
                                          <div className="flex items-center mb-1">
                                            <FiRefreshCw className="text-blue-500 animate-spin mr-2 h-4 w-4" />
                                            <span className="font-medium text-blue-700 dark:text-blue-400 text-sm">Processing Batch</span>
                                          </div>
                                          <div className="text-xs text-blue-600 dark:text-blue-500">
                                            Initializing batch process, waiting for first update...
                                          </div>
                                        </div>
                                      )}
                                      ...
                                      {job.currentVisitPhase && (
                                        <div className="flex items-center text-xs">
                                          <span className="mr-2 text-gray-600 dark:text-gray-400">Current phase:</span>
                                          <span className={`font-medium capitalize px-2 py-0.5 rounded ${
                                            job.currentVisitPhase === 'setup' ? 'bg-purple-100 dark:bg-purple-900/30 text-purple-800 dark:text-purple-300' :
                                            job.currentVisitPhase === 'forms' ? 'bg-amber-100 dark:bg-amber-900/30 text-amber-800 dark:text-amber-300' :
                                            job.currentVisitPhase === 'filling' ? 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-800 dark:text-emerald-300' :
                                            job.currentVisitPhase === 'saving' ? 'bg-sky-100 dark:bg-sky-900/30 text-sky-800 dark:text-sky-300' :
                                            'bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-300'
                                          }`}>
                                            {job.currentVisitPhase}
                                          </span>
                                  ...
                                  {/* Show a completion message for non-running jobs */}
                                  {job.status === 'completed' && (
                                    <div className="p-3 bg-green-50 dark:bg-green-900/20 rounded border border-green-200 dark:border-green-800">
                                      <div className="flex items-center mb-2">
                                        <FiCheckCircle className="text-green-500 mr-2 h-5 w-5" />
                                        <span className="font-medium text-green-700 dark:text-green-400">Completed Successfully</span>
                                      </div>
                                      <div className="text-xs text-green-600 dark:text-green-500">
                                        All {job.totalVisits} visits processed in {formatDuration(job.startTime, job.endTime)}
                                      </div>
                                    </div>
                                  )}
                                  ...
                                  {job.status === 'error' && (
                                    <div className="p-3 bg-red-50 dark:bg-red-900/20 rounded border border-red-200 dark:border-red-800">
                                      <div className="flex items-center mb-2">
                                        ...
                                        <span className="font-medium text-red-700 dark:text-red-400">Processing Failed</span>
                                      </div>
                                      <div className="text-xs text-red-600 dark:text-red-500">
                                        Processed {job.completedVisits} of {job.totalVisits} visits before error occurred
                                      </div>
                                    </div>
                                  )}
                                </td>
