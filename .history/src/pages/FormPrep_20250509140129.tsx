...
const FormPrep: React.FC<{}> = () => {
  ...
  const downloadLogs = async () => {
    try {
      ...
      // Create a temp element and trigger download
      ...
      // Also save a copy to the project root for easier access
      try {
        // Use Electron API instead of fetch to save file to logs directory
        if (window.electron && window.electron.saveFile) {
          ...
          if (result.success) {
            ...
            console.error('Failed to save logs to project directory:', result.error);
          }
        } else {
          console.warn('Electron saveFile API not available');
        }
      } catch (error) {
        console.error('Error saving to project directory:', error);
      }
      
      addToast('success', 'Debug logs downloaded');
    } catch (error) {
