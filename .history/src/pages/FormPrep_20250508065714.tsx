...
import { 
  ...
  FiArrowRight, FiCalendar, FiAlertTriangle
} from 'react-icons/fi';
...
const FormPrep: React.FC<{}> = () => {
  ...
  useEffect(() => {
    ...
    if (singleJobId && pollingSingle) {
      ...
      getFormAutomationStatus().then((status: FormAutomationStatus) => {
        if (status) {
  ...
  useEffect(() => {
    ...
    if (storedSingleJobId && storedIsPolling) {
      ...
      // Get current status
      getFormAutomationStatus()
        .then((status: FormAutomationStatus) => {
          if (status) {
            updateJobStatus(status);
            
            // If job completed or errored, update state
            if (status.status === 'completed' || status.status === 'error') {
              setPollingSingle(false);
              setSingleJobId(null);
              setIsProcessing(false);
  ...
  useEffect(() => {
    ...
    if (singleJobId && !pollingSingle) {
      ...
      getFormAutomationStatus()
        .then((currentStatus: FormAutomationStatus) => {
    ...
    if (batchJobId) {
      ...
      getBatchAutomationStatus()
        .then((currentStatus: BatchAutomationStatus) => {
