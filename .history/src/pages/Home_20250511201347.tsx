...
const HomeContent = () => {
  ...
  const renderMapPanel = () => {
    ...
    const inRangeOrders = filteredWorkOrders.filter(order => {
      // Get visit date in various formats
      ...
      return (
        (visitDateTime >= dateRanges.currentWeekStart && visitDateTime <= dateRanges.currentWeekEnd) ||
        (visitDateTime >= dateRanges.nextWeekStart && visitDateTime <= dateRanges.nextWeekEnd)
      );
    });
    ...
    // State for showing detailed map view
    const [showDetailedMap, setShowDetailedMap] = useState(false);
      ...
      <div className="pt-4">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex gap-2">
            ...
            >
              <FiCalendar className="w-3 h-3" /> This Week
            </button>
            <button
        ...
        <div className="mt-2 text-center text-xs text-gray-500 dark:text-gray-400">
          {locationsToShow.length === 0 ? (
            ...
            `Showing ${locationsToShow.length} job locations for the selected date range`
          )}
        </div>
      </div>
    );
