...
const Schedule: React.FC = () => {
  ...
  const renderJobRow = (order: WorkOrder, dateGroupClass?: string) => {
    ...
    return (
      <div
        key={order.id}
        className={`job-card bg-white dark:bg-slate-800 shadow-lg rounded-lg p-4 mb-4 border-l-4 ${styles.borderColor} ${dateGroupClass || ''}`}
      ...
      >
        ...
        <div className="flex justify-between items-start mb-3">
          <div className="flex-grow">
            ...
            <p className="text-xs text-gray-400 dark:text-gray-500">
              Scheduled: {order.scheduledDate ? new Date(order.scheduledDate).toLocaleDateString() : 'Not Scheduled'}
            </p>
          </div>
        ...
        <div className="flex flex-wrap gap-2 items-center justify-start pt-2 border-t border-gray-200 dark:border-slate-700">
          <button
            ...
            className="flex items-center px-3 py-1.5 text-xs bg-blue-500 hover:bg-blue-600 text-white rounded-md transition-colors duration-150 disabled:opacity-50 disabled:cursor-not-allowed"
          >
          ...
          <button
            ...
            disabled={operationLoading[`disp_${order.id}`]}
            className="flex items-center px-3 py-1.5 text-xs bg-green-500 hover:bg-green-600 text-white rounded-md transition-colors duration-150 disabled:opacity-50 disabled:cursor-not-allowed"
          >
          ...
          <button
            ...
            disabled={operationLoading[`clear_${order.id}`]}
            className="flex items-center px-3 py-1.5 text-xs bg-yellow-500 hover:bg-yellow-600 text-white rounded-md transition-colors duration-150 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <FiTrash2 className="mr-1.5 h-4 w-4" />
            {operationLoading[`clear_${order.id}`] ? 'Clearing...' : 'Clear Cache'}
          </button>
          <button
            onClick={(e) => handleForceRescrapeDispenserData(order.id, e)}
            disabled={operationLoading[`rescrape_${order.id}`]}
            className="flex items-center px-3 py-1.5 text-xs bg-red-500 hover:bg-red-600 text-white rounded-md transition-colors duration-150 disabled:opacity-50 disabled:cursor-not-allowed"
          >
          ...
          <a
            ...
            className="flex items-center px-3 py-1.5 text-xs bg-purple-500 hover:bg-purple-600 text-white rounded-md transition-colors duration-150"
