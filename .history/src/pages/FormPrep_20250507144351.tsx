...
const FormPrep: React.FC<{}> = () => {
  ...
  const getStoreStyles = (storeName: string) => {
    // Default styles
    const defaultStyles = {
      bg: 'bg-gray-100 dark:bg-gray-700/60', // Increased opacity
    ...
    const styleMap: Record<string, typeof defaultStyles> = {
      // Gas stations
      'shell': {
        bg: 'bg-yellow-200 dark:bg-yellow-700/50', // More vibrant, adjusted dark opacity
      ...
      'exxon': {
        bg: 'bg-red-200 dark:bg-red-700/50',
        border: 'border-red-500 dark:border-red-600',
        icon: 'text-red-700 dark:text-red-400',
        text: 'text-red-900 dark:text-red-200'
      },
      'mobil': {
        bg: 'bg-red-200 dark:bg-red-700/50', // Same as Exxon
        border: 'border-red-500 dark:border-red-600',
        icon: 'text-red-700 dark:text-red-400',
        text: 'text-red-900 dark:text-red-200'
      },
      ...
      'caseys': {
        bg: 'bg-amber-200 dark:bg-amber-700/50', // Amber for Casey's
        border: 'border-amber-500 dark:border-amber-600',
        icon: 'text-amber-700 dark:text-amber-400',
        text: 'text-amber-900 dark:text-amber-200'
      },
      'wawa': {
        bg: 'bg-orange-200 dark:bg-orange-700/50', // Orange for Wawa (similar to Circle K)
        border: 'border-orange-500 dark:border-orange-600',
        icon: 'text-orange-700 dark:text-orange-400',
        text: 'text-orange-900 dark:text-orange-200'
      },
      'sheetz': {
        bg: 'bg-rose-200 dark:bg-rose-700/50', // Rose for Sheetz (similar to Costco)
        border: 'border-rose-500 dark:border-rose-600',
        icon: 'text-rose-700 dark:text-rose-400',
        text: 'text-rose-900 dark:text-rose-200'
      },
      'maverik': {
        bg: 'bg-sky-200 dark:bg-sky-700/50', // Sky for Maverik
        border: 'border-sky-500 dark:border-sky-600',
        icon: 'text-sky-700 dark:text-sky-400',
        text: 'text-sky-900 dark:text-sky-200'
      },
      ...
      'quick': { // QuickTrip/QuickChek
        bg: 'bg-fuchsia-200 dark:bg-fuchsia-700/50', // Fuchsia for Quick
      ...
      'love': { // Love's
        bg: 'bg-pink-200 dark:bg-pink-700/50', // Pink for Love's
        border: 'border-pink-500 dark:border-pink-600',
        icon: 'text-pink-700 dark:text-pink-400',
        text: 'text-pink-900 dark:text-pink-200'
      },
      'pilot': {
        bg: 'bg-indigo-200 dark:bg-indigo-700/50', // Indigo
        border: 'border-indigo-500 dark:border-indigo-600',
        icon: 'text-indigo-700 dark:text-indigo-400',
        text: 'text-indigo-900 dark:text-indigo-200'
      },
      ...
      'ta': { // TravelCenters of America
        bg: 'bg-emerald-200 dark:bg-emerald-700/50', // Emerald for TA
    ...
    <div className={`min-h-screen bg-gray-50 dark:bg-gray-900`}>
      <div className="p-6 max-w-7xl mx-auto">
        <div className="space-y-6">
          {/* Page header */}
          <div className="panel-header px-6 py-5 bg-gradient-to-r from-white to-blue-50 dark:from-gray-800 dark:to-gray-800/95 shadow-md border-b border-gray-200 dark:border-gray-700 rounded-t-lg">
            <div className="flex justify-between items-center">
              <h1 className="text-xl font-semibold text-gray-800 dark:text-white flex items-center gap-3">
                <FiFileText className="h-6 w-6 text-blue-500 dark:text-blue-400" />
                Form Preparation Automation
              </h1>
              <div className="flex items-center gap-3">
                <div className="text-xs text-gray-600 dark:text-gray-400 bg-gray-100 dark:bg-gray-700 px-3 py-1.5 rounded-md">
                  <span className="font-semibold">Status:</span> {statusMessage}
                </div>
                <div className="text-xs text-gray-600 dark:text-gray-400 bg-gray-100 dark:bg-gray-700 px-3 py-1.5 rounded-md">
                  <span className="font-semibold">Mode:</span> {isHeadless ? 'Headless' : 'Visible'}
                </div>
              </div>
            </div>
          </div>
                                  ...
                                  <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                    {groupedWorkOrders[currentWeekIndex].orders.map((order) => {
                                      ...
                                      return (
                                        <tr
                                          key={order.id}
                                          className={`hover:bg-gray-100 dark:hover:bg-gray-700/60 cursor-pointer transition-colors duration-150 ease-in-out ${
                                            selectedWorkOrder?.id === order.id ? 
                                            `${storeStyles.bg} ${storeStyles.border} border-l-4 shadow-md` : // Added shadow-md on selection
                                            'border-l-4 border-transparent' // Transparent border for consistent alignment
                                          }`}
                                          onClick={() => {
                                        ...
                                        >
                                          <td className="px-4 py-3 whitespace-nowrap">
                                            <div className="flex items-center">
                                              <div className={`flex-shrink-0 w-10 h-10 p-2 flex items-center justify-center rounded-lg ${storeStyles.bg} mr-3 shadow-sm`}> {/* Adjusted size, padding, and added shadow */}
                                                <FiMapPin className={`${storeStyles.icon} h-5 w-5`} /> {/* Ensured icon size */}
                                              </div>
                                              <div>
                                                <div className={`font-semibold ${storeStyles.text}`}>{order.customer.name}</div> {/* Applied text color */}
                                                <div className="text-xs text-gray-500 dark:text-gray-400">
                                                  ID: {order.id}
                                                </div>
                                              </div>
                      ...
                      <div className="overflow-hidden rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm">
                        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                          <thead className="bg-gray-100 dark:bg-gray-800/80 text-gray-600 dark:text-gray-300">
                            <tr>
                              <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider w-[50%] border-b border-gray-200 dark:border-gray-700">
                                Status
                              </th>
                              <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider w-[20%] border-b border-gray-200 dark:border-gray-700">
                                Store & Visit
                              </th>
                              <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider w-[15%] border-b border-gray-200 dark:border-gray-700">
                                Timestamp
                              </th>
                              <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider w-[15%] border-b border-gray-200 dark:border-gray-700">
                                Mode
                          ...
                          <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                            {formJobs.map((job, index) => {
                              ...
                              const statusColors = {
                                running: 'bg-primary-50 dark:bg-primary-900/30 border-l-4 border-primary-500 hover:bg-primary-100 dark:hover:bg-primary-900/40',
                                completed: 'bg-green-50 dark:bg-green-900/30 border-l-4 border-green-500 hover:bg-green-100 dark:hover:bg-green-900/40',
                                error: 'bg-red-50 dark:bg-red-900/30 border-l-4 border-red-500 hover:bg-red-100 dark:hover:bg-red-900/40',
                                idle: index % 2 === 1 ? 
                                  'bg-gray-50 dark:bg-gray-800/50 hover:bg-gray-100 dark:hover:bg-gray-700/50' : 
                                  'hover:bg-gray-50 dark:hover:bg-gray-800/80'
                              };
                              
                              const rowStyle = statusColors[job.status] || statusColors.idle;
                              
                              return (
                                <tr 
                                  key={index}
                                  className={`${rowStyle} transition-all duration-150 cursor-pointer group relative`}
                                  onClick={() => {
                                    // No action needed
                                  }}
                                >
                                ...
                                <td className="px-4 py-3 text-sm">
                                  ...
                                  <div className="flex items-center space-x-2 mt-1.5">
                                    <span className="font-mono bg-gray-100 dark:bg-gray-700 px-1.5 py-0.5 rounded-md text-gray-700 dark:text-gray-300 font-medium text-xs">
                                      #{visitInfo.visitNumber}
                                    </span>
                                    <span className="bg-gray-100 dark:bg-gray-700/70 text-gray-600 dark:text-gray-400 text-xs px-1.5 py-0.5 rounded-md font-mono">
                                      {job.url && job.url.match(/\/work\/(\w+)/) ? 
                                        `W-${job.url.match(/\/work\/(\w+)/)![1]}` : 
                                        ""}
                                    </span>
                                  </div>
                                  {visitInfo.dispenserCount > 0 && (
                                    <div className="text-xs mt-1.5 flex">
                                      <span className="bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-400 rounded-md px-2 py-0.5 flex items-center shadow-sm">
                                        <svg className="w-3 h-3 mr-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z" />
                                        </svg>
                                        <span className="font-medium">Dispensers</span>
                                        <span className="ml-1.5 bg-amber-200 dark:bg-amber-800/80 text-amber-800 dark:text-amber-300 rounded-md px-2 py-0.5 font-bold inline-flex items-center justify-center min-w-[24px]">{visitInfo.dispenserCount}</span>
                                      </span>
                                    </div>
                                  )}
                                </td>
