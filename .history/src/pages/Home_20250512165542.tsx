...
const HomeContent = () => {
  ...
  const renderMapPanel = () => {
    ...
    const locationsToShow = inRangeOrders
      .filter(order => {
          const hasLocation = order.location && typeof order.location.latitude === 'number' && typeof order.location.longitude === 'number';
          if (!hasLocation) {
              console.log(`[renderMapPanel Map] Order ${order.id}: Missing or invalid location data.`);
          }
          return hasLocation;
       })
      .map(order => ({
