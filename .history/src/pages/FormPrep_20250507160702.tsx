...
import { 
  ...
  FiClock, FiMapPin, FiCheckCircle, FiXCircle, FiStar
} from 'react-icons/fi';
                            ...
                            {formJobs.map((job, index) => {
                              ...
                              return (
                                ...
                                <td className="px-4 py-3 text-sm">
                                  <div className="flex flex-col space-y-1">
                                    <div className="flex items-center">
                                      ...
                                      <span className="font-medium text-gray-800 dark:text-gray-200">{cleanStoreName}</span>
                                    </div>
                                    <div>
                                      <span className="font-mono bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 px-2 py-0.5 rounded-md text-xs">#{visitInfo.visitNumber}</span>
                                    </div>
                                    {visitInfo.dispenserCount > 0 && (
                                      <div>
                                        <span className="bg-amber-100 dark:bg-amber-700/30 text-amber-700 dark:text-amber-400 rounded-md px-2 py-0.5 text-xs flex items-center shadow-sm">
                                          <FiStar className="w-3 h-3 mr-1 text-amber-500" />
                                          <span className="font-medium">Dispensers:</span>
                                          <span className="ml-1 bg-amber-200 dark:bg-amber-600 text-amber-800 dark:text-amber-200 rounded px-1.5 font-semibold text-xs">{visitInfo.dispenserCount}</span>
                                        </span>
                                      </div>
                                    )}
                                  </div>
                                ...
                                <td className="px-4 py-3 text-sm text-gray-500 dark:text-gray-400">
                                  <div className="flex flex-col space-y-1">
                                    <div className="flex items-center">
                                      <FiClock className="h-4 w-4 mr-2 text-indigo-500 flex-shrink-0" />
                                      <div className="font-medium text-gray-700 dark:text-gray-300">
                                        {job.timestamp}
                                      </div>
                                    </div>
                                    {job.startTime && (
                                      <div className="flex items-center ml-1"> {/* Indent duration slightly */}
                                        <FiClock className="h-3 w-3 mr-1.5 text-gray-400 dark:text-gray-500 flex-shrink-0" />
                                    ...
                                    {job.status === 'completed' && job.startTime && job.endTime && (
                                      <div className="mt-1.5 inline-flex items-center text-sm bg-green-600 dark:bg-green-700 text-white px-3 py-1.5 rounded-lg shadow-md self-start">
                                        <FiCheckCircle className="w-5 h-5 mr-2 flex-shrink-0" />
                                        <span className="font-medium">
                                          Completed {new Date(job.endTime).toLocaleTimeString([], {hour: '2-digit', minute: '2-digit', hour12: true})}
                                        </span>
                                      </div>
                                    )}
                                  
                                    {job.status === 'error' && job.startTime && job.endTime && (
                                      // Keeping previous error styling, can be adjusted if needed
                                      <div className="flex items-center text-xs text-red-600 dark:text-red-500 bg-red-50 dark:bg-red-900/30 px-2 py-1 rounded-md self-start mt-1.5">
