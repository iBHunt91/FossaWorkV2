              ...
              <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
                ...
                <div className="grid grid-cols-5 min-h-[180px] divide-x divide-gray-200 dark:divide-gray-700">
                  {Array.from({ length: 5 }).map((_, dayIndex) => {
                    ...
                    // Limit visible jobs to prevent scrollbars
                              ...
                              >
                                ...
                                {job.customer?.storeNumber && (
                                  <div className="text-[10px] text-gray-500 dark:text-gray-400 mb-1">
                                    Store #{job.customer.storeNumber.replace('#', '')}
                                  </div>
                                )}
                                
                                {job.services && job.services.some(s => s.type === "Meter Calibration") && (
                                  <div className="flex items-center mt-1">
                                    <GiGasPump className="h-3 w-3 text-gray-500 dark:text-gray-400 mr-1" />
                                    <span className="text-[10px] text-gray-500 dark:text-gray-400">
                                      {job.services.find(s => s.type === "Meter Calibration")?.quantity || 0} Dispensers
                                    </span>
                                  </div>
                                )}
                              </div>
                ...
                <div className="border-t border-gray-200 dark:border-gray-700">
                  ...
                  <div className="p-3 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                    {groupedWorkOrders.nextWeek.slice(0, 8).map(job => {
                      const storeType = getStoreTypeForFiltering(job);
                      const storeStyle = getStoreStyles(storeType);
                    ...
                    {groupedWorkOrders.nextWeek.length > 8 && (
                      <div className="col-span-full mt-1 py-2 px-3 bg-gray-50 hover:bg-gray-100 dark:bg-gray-700/50 dark:hover:bg-gray-700 text-sm font-medium text-gray-600 dark:text-gray-400 rounded-md text-center transition-colors">
                        +{groupedWorkOrders.nextWeek.length - 8} more jobs
                      </div>
                    )}
                    
                    {groupedWorkOrders.nextWeek.length === 0 && (
                      <div className="col-span-full p-4 text-center text-gray-500 dark:text-gray-400">
                        No jobs scheduled for next week
                      </div>
                    )}
                  </div>
                </div>
