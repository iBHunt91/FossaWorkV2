...
const getUserStorageKeys = (userId: string = 'default') => ({
  ...
  BATCH_JOBS: `form_prep_batch_jobs_${userId}`,
...
interface BatchJob {
  ...
  formsTotal?: number;
  formsCurrent?: number;
  currentVisitFuelType?: string;    // Added for detailed fuel progress
  currentVisitFuelCurrent?: number; // Added for detailed fuel progress
  currentVisitFuelTotal?: number;   // Added for detailed fuel progress
  startTime?: number;
  endTime?: number;
...
// Replace the existing BatchAutomationStatus interface with this one
...
// Add this constant near the top of the file where other constants are defined
const LOG_DIRECTORY_PATH = './logs'; // Relative to project root
...
const FormPrep: React.FC<{}> = () => {
  ...
  const [batchJobs, setBatchJobs] = useState<BatchJob[]>(() => 
    getFromStorage(userStorageKeys.BATCH_JOBS, []));
  ...
  // Ref to store mock progress for batch job demonstration
  const mockBatchProgressRef = React.useRef<{
    jobId: string | null;
    dispenserCurrent: number;
    dispenserTotal: number;
    fuelCurrent: number;
    fuelTotal: number;
    fuelType: string;
  }>({ 
    jobId: null, 
    dispenserCurrent: 0, 
    dispenserTotal: 5, // Example total
    fuelCurrent: 0,
    fuelTotal: 3, // Example total
    fuelType: 'Regular'
  });

  const isPollingBatchRef = useRef<boolean>(false);
  const pollingBatchTimeoutRef = useRef<NodeJS.Timeout | null>(null);
  
  // Add debug log state near the top of the component with other state variables
  ...
  const downloadLogs = async () => {
    try {
      ...
      // Collect current state and configuration
      const configInfo = {
        endpoints: ENDPOINTS,
        storageKeys: userStorageKeys
      };
      ...
      const logData = {
        ...
        batchJobs: batchJobs.map(job => ({
          ...
          totalVisits: job.totalVisits,
          currentVisitName: job.currentVisitName,
          currentVisitStatus: job.currentVisitStatus,
          formsCurrent: job.formsCurrent,
          formsTotal: job.formsTotal,
      ...
      const filename = `form-prep-logs-${timestamp}.json`;
      
      addDebugLog('SYSTEM', 'Downloading logs', { timestamp });

      let successfullySavedToProject = false;
      // Save a copy to the project root for easier access using Electron API
      if (window.electron && window.electron.saveFile) {
        try {
          const result = await window.electron.saveFile({
            filename,
            content: jsonString,
            path: 'logs' // Target the logs directory
          });
          
          if (result.success) {
            addToast('success', `Logs saved to project directory: ${result.path}`);
            successfullySavedToProject = true;
          } else {
            console.error('Failed to save logs to project directory via Electron:', result.error);
            addToast('error', 'Failed to save logs to project directory. Check console.');
          }
        } catch (error) {
          console.error('Error saving to project directory via Electron:', error);
          addToast('error', 'Error saving logs to project directory. Check console.');
        }
      } else {
        console.warn('Electron saveFile API not available. Falling back to browser download.');
      }
      
      // Fallback to browser download if Electron API is not available or failed
      if (!successfullySavedToProject) {
        // Create a temp element and trigger browser download
        ...
        addToast('info', 'Debug logs downloaded via browser.');
      }
      
    } catch (error) {
      console.error('Error generating logs:', error);
      addToast('error', 'Error generating logs');
    }
  };
  
  // Helper functions for progress extraction
  const extractDispenserProgressLocal = (message: string, totalDispensers?: number): { current: number; total: number } => {
    let current = 0;
    let parsedTotalFromMessage: number | undefined = undefined;
    let match;

    // Priority 1: "Dispenser #X of Y", "Dispenser X/Y", "Dispenser X of Y" (explicit keyword at start of phrase)
    const explicitDispenserXY = /Dispenser(?:\s*#)?\s*(\d+)\s*(?:of|\/)\s*(\d+)/i;
    match = message.match(explicitDispenserXY);
    if (match && match[1] && match[2]) {
      const valX = parseInt(match[1], 10);
      const valY = parseInt(match[2], 10);
      current = valX; // Use X directly for current
      parsedTotalFromMessage = valY; // Y is the total from this specific message pattern
      // Convert X/Y pair to physical dispenser number if it's a 1/2, 3/4 pattern representing physical dispensers
      if (valX % 2 === 1 && valY === valX + 1) {
        current = (valX + 1) / 2;
        // If totalDispensers is provided and reasonable, assume parsedTotalFromMessage (Y from X/Y) is # of sides for THIS dispenser, not overall total.
        // The actual totalPhysicalDispensers should come from totalDispensers if it's valid.
        // So, don't override parsedTotalFromMessage here unless totalDispensers is unreliable.
      } else {
        // For patterns like "Dispenser 1 of 6", current is 1, parsedTotalFromMessage is 6.
      }
    }

    // Priority 2: More general "dispenser: X/Y" or "dispenser X / Y" or "Dispenser: X"
    if (!parsedTotalFromMessage) { // Only if not already found by more specific pattern
      const generalDispenserXY = /(?:dispenser|form)\s*:?\s*(\d+)(?:\s*(?:of|\/)\s*(\d+))?/i;
      match = message.match(generalDispenserXY);
      if (match && match[1]) {
        current = parseInt(match[1], 10);
        if (match[2]) {
          parsedTotalFromMessage = parseInt(match[2], 10);
        }
      }
    }

    // Priority 3: "Working on dispenser X" or "Processing dispenser X"
    if (current === 0) { // Only if current not found yet
      const workingOnDispenser = /(?:Working on|Processing) dispenser (\d+)/i;
      match = message.match(workingOnDispenser);
      if (match && match[1]) {
        current = parseInt(match[1], 10);
        // No total in this pattern, rely on totalDispensers or previously parsedTotalFromMessage
      }
    }
    
    // Determine final total
    let finalTotal = 0;
    if (totalDispensers !== undefined && totalDispensers > 0 && totalDispensers !== 100) { // Prioritize totalDispensers if it's valid and not a placeholder
      finalTotal = totalDispensers;
      if (parsedTotalFromMessage && parsedTotalFromMessage !== finalTotal) {
        console.warn(`Dispenser total mismatch (using totalDispensers): job.dispenserCount/formsTotal is ${totalDispensers}, message parsed total as ${parsedTotalFromMessage}.`);
        // We are trusting totalDispensers, but good to know if message had something else
      }
    } else if (parsedTotalFromMessage !== undefined && parsedTotalFromMessage > 0) { // Fallback to message-parsed total if totalDispensers is invalid/placeholder
      finalTotal = parsedTotalFromMessage;
      if (totalDispensers === 100 || totalDispensers === 0) {
        console.info(`Used message-parsed total (${finalTotal}) because totalDispensers argument was ${totalDispensers}.`);
      }
    } else if (totalDispensers !== undefined && (totalDispensers === 0 || totalDispensers === 100) && current > 0) {
      // If totalDispensers is a known placeholder (0 or 100) and we have a current but NO message total,
      // this is ambiguous. For now, show 0 total, prompting a skeleton. Could try to infer if needed.
      finalTotal = 0; 
      console.warn(`Dispenser total is ambiguous: totalDispensers is ${totalDispensers}, no total in message, but current is ${current}.`);
    } else if (totalDispensers !== undefined) {
        finalTotal = totalDispensers; // Use totalDispensers if it's any other value (could be valid, or still a less common placeholder)
    }

    // If current is still 0 but we have a total, assume we are on the first one.
    if (current === 0 && finalTotal > 0) {
      current = 1;
    }
    // Ensure current does not exceed total if total is known and positive
    if (finalTotal > 0 && current > finalTotal) {
      // console.warn(`Current dispenser ${current} exceeds total ${finalTotal}. Clamping to total. Message: ${message}`);
      current = finalTotal; // Clamp current to total
    }
    
    return { current, total: finalTotal };
  };

  const extractFuelProgressLocal = (message: string): { current: number; total: number; type?: string } => {
  ...
  const updateJobStatus = (status: any) => {
    addDebugLog('SINGLE', 'Updating job status', status);
    
    if (!status) {
      addDebugLog('SINGLE', 'No status provided to updateJobStatus');
      return;
    }
    ...
    if (activeJobIndex !== -1) {
      const jobToUpdate = storedJobs[activeJobIndex];
      addDebugLog('SINGLE', 'Found running job to update', jobToUpdate);
      ...
      if (status.status === 'running') {
        ...
        // Extract dispenser information more accurately
        const dispenserInfo = extractDispenserProgressLocal(formattedMessage, dispenserCount);
        
        // Update log to see extracted information
        console.log('Extracted job progress:', {
          message: formattedMessage,
          dispenserInfo,
          fuelInfo: extractFuelProgressLocal(formattedMessage)
        });
      ...
      addDebugLog('SINGLE', 'No running job found to update');
    }
  };
  ...
  useEffect(() => {
    ...
    if (batchJobId) {
      // Pass the job ID to the function
  ...
  const handleSingleVisit = async () => {
    try {
      setIsProcessing(true);
      addDebugLog('SINGLE', 'Starting single visit processing', { url: visitUrl, isHeadless });
      ...
      addDebugLog('SINGLE', 'Calling processSingleVisit API', { 
        visitUrl, 
        isHeadless,
        visitNumber,
        workOrderId,
      ...
      addDebugLog('SINGLE', 'Received API response', result);
  ...
  const handleBatchProcess = async () => {
    ...
    addDebugLog('BATCH', 'Starting batch processing', {
      ...
      lastFailedBatch: lastFailedBatch ? {
        jobId: lastFailedBatch.jobId,
        completedVisits: lastFailedBatch.completedVisits,
        totalVisits: lastFailedBatch.totalVisits
      } : null
    });
    
    // Check if there's already a running batch job
    const runningJob = batchJobs.find(job => job.status === 'running');
    if (runningJob) {
      addToast('warning', 'There is already a batch job running. Please stop the current job before starting a new one.');
      return;
    }
    ...
    try {
      setIsProcessing(true);
        ...
        filePath: resumeBatch && lastFailedBatch ? `Resuming from failed batch` : 'Selected visits',
          timestamp: new Date().toLocaleString(),
          status: 'running',
          message: batchResult.message || 'Processing batch visits...',
          totalVisits: batchResult.totalVisits || selectedVisits.length,
        completedVisits: resumeBatch && lastFailedBatch ? lastFailedBatch.completedVisits || 0 : 0,
          headless: isHeadless,
      ...
      setBatchJobs(prev => {
        const updatedJobs = [newBatchJob, ...prev];
        // Also save to localStorage
        saveToStorage(userStorageKeys.BATCH_JOBS, updatedJobs);
        return updatedJobs;
      });
      
      // If resuming, show appropriate toast
      if (resumeBatch && lastFailedBatch) {
        addToast('success', `Resuming batch processing from last failed job with ${selectedVisits.length} visits`);
        } else {
  ...
  const startBatchPolling = () => {
    addDebugLog('BATCH', 'Starting batch polling');
    if (polling) {
      clearTimeout(polling);
    }
    
    const poll = async () => {
        const currentBatchJobId = getFromStorage(userStorageKeys.BATCH_JOB_ID, null);
        if (!currentBatchJobId) {
        addDebugLog('BATCH', 'Polling stopped: No active batch job ID');
        setPolling(null);
        localStorage.removeItem(userStorageKeys.BATCH_JOB_ID); // Clear stale job ID
          return;
        }
        
        try {
        const response: BatchAutomationStatus = await getBatchAutomationStatus(currentBatchJobId);
        addDebugLog('BATCH', 'Received batch status update', response);
        
        saveToStorage(userStorageKeys.LAST_STATUS_UPDATE, new Date().toISOString());

        setBatchJobs(prevJobs => {
          const jobIndex = prevJobs.findIndex(j => j.jobId === currentBatchJobId);
          if (jobIndex === -1) {
            addDebugLog('BATCH', `Polling anomaly: Job ID ${currentBatchJobId} not found in local state.`);
            return prevJobs; 
          }
          ...
          // Update progress details from the API response
          jobToUpdate.completedVisits = response.completedVisitIds?.length || response.completedVisits || jobToUpdate.completedVisits || 0;
          // totalVisits should be set at job creation and ideally not change, but API might send it
          jobToUpdate.totalVisits = response.totalVisits || jobToUpdate.totalVisits || 0; 
          jobToUpdate.currentVisitId = response.currentVisit || jobToUpdate.currentVisitId;
          jobToUpdate.currentVisitStatus = response.currentVisitStatus || jobToUpdate.currentVisitStatus;
          
          // If currentVisitId is available, try to find its name from workOrders
          if (response.currentVisit) {
            const wo = workOrders.find(order => order.visits.nextVisit.visitId === response.currentVisit);
            if (wo) {
              jobToUpdate.currentVisitName = wo.customer.name;
            } else {
              // Fallback if work order not found (e.g., from a different data set)
              jobToUpdate.currentVisitName = response.storeInfo?.name || response.currentVisit || 'Unknown Visit';
            }
          } else if (!jobToUpdate.currentVisitId && jobToUpdate.completedVisits < jobToUpdate.totalVisits && jobToUpdate.status === 'running') {
            // If no current visit ID but job is running and not all visits are done, it might be transitioning
            jobToUpdate.currentVisitName = 'Preparing next visit...';
          }

          jobToUpdate.formsTotal = response.dispenserCount !== undefined ? response.dispenserCount : jobToUpdate.formsTotal;
          // If currentVisitStatus contains dispenser info, it might be more up-to-date for the current visit
          const dispenserProgressFromMessage = extractDispenserProgressLocal(jobToUpdate.currentVisitStatus || '', jobToUpdate.formsTotal);
          if (dispenserProgressFromMessage.total > 0 && dispenserProgressFromMessage.total !== 100) { // 100 was a placeholder
            jobToUpdate.formsTotal = dispenserProgressFromMessage.total;
          }
          jobToUpdate.formsCurrent = dispenserProgressFromMessage.current;

          // Determine the definitive status of the batch job
          let determinedStatus = jobToUpdate.status;
          let determinedMessage = jobToUpdate.message;

          if (response.status === 'completed') {
            determinedStatus = 'completed';
            determinedMessage = response.message || 'Batch completed successfully.';
            jobToUpdate.completedVisits = jobToUpdate.totalVisits; // Ensure consistency
            jobToUpdate.endTime = response.endTime ? new Date(response.endTime).getTime() : Date.now();
            addDebugLog('BATCH', `Batch job ${currentBatchJobId} API status: completed.`);
          } else if (response.status === 'error') {
            determinedStatus = 'error';
            determinedMessage = response.message || 'Batch processing encountered an error.';
            jobToUpdate.endTime = response.endTime ? new Date(response.endTime).getTime() : Date.now();
            addDebugLog('BATCH', `Batch job ${currentBatchJobId} API status: error. Message: ${determinedMessage}`);
          } else if (response.status === 'running') {
            determinedStatus = 'running';
            determinedMessage = response.message || (jobToUpdate.currentVisitName ? `Processing ${jobToUpdate.currentVisitName}...` : 'Batch processing...');
          } else if (response.status === 'idle') {
            // Handle 'idle' from API more carefully
            if (jobToUpdate.completedVisits >= jobToUpdate.totalVisits && jobToUpdate.totalVisits > 0) {
              // If API says idle, but client-side accounting says all visits done, consider it completed.
              determinedStatus = 'completed';
              determinedMessage = jobToUpdate.message !== 'Batch processing started' && jobToUpdate.message !== '' && jobToUpdate.status === 'completed' 
                                ? jobToUpdate.message 
                                : 'Batch completed (inferred from visit count).';
              jobToUpdate.endTime = jobToUpdate.endTime || (response.endTime ? new Date(response.endTime).getTime() : Date.now());
              addDebugLog('BATCH', `Batch job ${currentBatchJobId} API status: idle. All visits completed (${jobToUpdate.completedVisits}/${jobToUpdate.totalVisits}). Marking as completed.`);
            } else if (jobToUpdate.status === 'running') {
              // If API says idle, but we were running and not all visits are done, remain running.
              // This could be a temporary state from the backend or a sign the job truly stalled.
              // We rely on currentVisitStatus to show activity.
              determinedStatus = 'running';
              determinedMessage = response.message || (jobToUpdate.currentVisitName ? `Processing ${jobToUpdate.currentVisitName}...` : 'Batch processing...');
              addDebugLog('BATCH', `Batch job ${currentBatchJobId} API status: idle, but not all visits completed (${jobToUpdate.completedVisits}/${jobToUpdate.totalVisits}). Keeping status as running.`);
            } else {
              // If API is idle and we weren't definitively running or completed, use API's idle.
              determinedStatus = 'idle';
              determinedMessage = response.message || 'Batch is idle.';
              addDebugLog('BATCH', `Batch job ${currentBatchJobId} API status: idle. Local state was ${jobToUpdate.status}. Setting to idle.`);
            }
          }

          if (jobToUpdate.status !== determinedStatus) {
            jobStatusChanged = true;
            jobToUpdate.status = determinedStatus;
            // Log toast-triggering flags upon actual status change to completed or error
            if (determinedStatus === 'completed') {
              jobToUpdate._completed = true; 
              jobToUpdate._error = false;
            }
            if (determinedStatus === 'error') {
              jobToUpdate._error = true;
              jobToUpdate._completed = false;
            }
          }
          jobToUpdate.message = determinedMessage;

          const updatedJobs = [...prevJobs];
          updatedJobs[jobIndex] = jobToUpdate;
          saveToStorage(userStorageKeys.BATCH_JOBS, updatedJobs);

          // Toast notifications for job completion or error
          if (jobStatusChanged) {
            if (jobToUpdate.status === 'completed') {
              addToast('success', `Batch job for "${jobToUpdate.filePath}" completed successfully.`);
            }
            if (jobToUpdate.status === 'error' && jobToUpdate.message !== 'Batch processing stopped by user') {
              addToast('error', `Batch job for "${jobToUpdate.filePath}" failed: ${jobToUpdate.message}`);
            }
          }

          // Stop polling if the job is definitively completed or errored
          if (jobToUpdate.status === 'completed' || jobToUpdate.status === 'error') {
            addDebugLog('BATCH', `Batch job ${currentBatchJobId} final status: ${jobToUpdate.status}. Stopping polling.`);
            setPolling(null);
            localStorage.removeItem(userStorageKeys.BATCH_JOB_ID); // Clear completed/errored job ID
            setBatchJobId(null); // Update component state
                setIsProcessing(false);
            
            // Log final state
            addDebugLog('BATCH', `Batch job ${currentBatchJobId} completed with status: ${jobToUpdate.status}`, { jobToUpdate, apiResponse: response });
            return updatedJobs;
            }
        ...
        // Continue polling if the job is still active
        setPolling(setTimeout(poll, 2000)); // Poll every 2 seconds

      } catch (error) {
        addDebugLog('BATCH', 'Error during batch polling', error);
        console.error('Error polling batch status:', error);
        // Potentially stop polling on repeated errors or specific error types
        // For now, continue polling to allow recovery if it's a temporary network issue.
        // Consider adding a counter for consecutive errors.
        setPolling(setTimeout(poll, 5000)); // Poll less frequently on error
      }
    };
  ...
  // Add effect to persist batchJobs to localStorage when they change
  useEffect(() => {
    saveToStorage(userStorageKeys.BATCH_JOBS, batchJobs);
  }, [batchJobs, userStorageKeys]);
  ...
  // Add this helper function somewhere in the component, e.g., before the return statement
  const formatDisplayMessage = (message: string | undefined): string => {
    if (!message) return '';

    if (message.startsWith('Processing visit:')) return 'Processing visit...';
    if (message.startsWith('Navigating to visit:')) return 'Navigating to visit...';
    
    // For "Filling form..." type messages, show a generic message
    if (message.includes('Filling form') && message.includes('dispenser')) return 'Updating forms & dispensers...';
    if (message.includes('Processing dispenser') || message.includes('Working on dispenser')) return 'Processing dispensers...';
    if (message.includes('Processing fuel type')) return 'Processing fuel types...';
    if (message.includes('Successfully retrieved visit details')) return 'Retrieved visit details...';
    if (message.includes('Saving visit details')) return 'Saving visit details...';
    if (message.includes('Attempting to log in')) return 'Logging in...';
    if (message.includes('Navigating to forms page')) return 'Navigating to forms...';
    if (message.includes('Checking for existing forms')) return 'Checking forms...';

    // No specific pattern matched, return original (will be truncated by CSS if too long)
    return message;
  };

  // Effect to start/stop batch polling
  useEffect(() => {
    addDebugLog('EFFECT_BATCH_POLL_TRIGGER', 'Batch poll effect triggered', { batchJobId, isPolling: isPollingBatchRef.current, batchJobsCount: batchJobs.length, workOrdersCount: workOrders.length });

    if (batchJobId && !isPollingBatchRef.current) {
      if (batchJobs.length > 0) {
        const jobToPoll = batchJobs.find(j => j.jobId === batchJobId);
        if (jobToPoll) {
          addDebugLog('EFFECT_BATCH_POLL_STARTING', 'Starting batch polling as job ID is active and found in batchJobs.', { batchJobId, status: jobToPoll.status });
          isPollingBatchRef.current = true;
          startBatchPolling(batchJobId); // Assumes startBatchPolling is defined elsewhere
        } else {
          addDebugLog('EFFECT_BATCH_POLL_DEFER_JOB_NOT_FOUND', 'Deferring batch polling: Active batchJobId not found in current batchJobs list.', { batchJobId });
        }
      } else {
        addDebugLog('EFFECT_BATCH_POLL_DEFER_NO_JOBS', 'Deferring batch polling: batchJobs array is currently empty.', { batchJobId });
      }
    } else if (!batchJobId && isPollingBatchRef.current) {
      addDebugLog('EFFECT_BATCH_POLL_STOPPING', 'Stopping batch polling as batchJobId is null or polling already active.', { batchJobId, isPolling: isPollingBatchRef.current });
      isPollingBatchRef.current = false;
      if (pollingBatchTimeoutRef.current) {
        clearTimeout(pollingBatchTimeoutRef.current);
        pollingBatchTimeoutRef.current = null;
        addDebugLog('EFFECT_BATCH_POLL_TIMEOUT_CLEARED', 'Cleared batch polling timeout.', { batchJobId });
      }
    }
  }, [batchJobId, batchJobs, workOrders, addDebugLog, startBatchPolling]); // Added all necessary dependencies

  // THIS IS THE EXISTING FUNCTION, DO NOT REDECLARE
  // const startBatchPolling = async (currentBatchJobIdFromEffect: string) => { ... }


  // Effect to load data from localStorage on initial mount and when user changes
  useEffect(() => {
    // Set up event listener for storage changes
    const handleStorageChange = (e: StorageEvent) => {
      if (e.key === userStorageKeys.FORM_JOBS || e.key === userStorageKeys.SINGLE_JOB_ID || e.key === userStorageKeys.BATCH_JOB_ID || e.key === userStorageKeys.IS_POLLING_SINGLE || e.key === userStorageKeys.VISIT_URL) {
        // Reload the user-specific data
        setFormJobs(getFromStorage(userStorageKeys.FORM_JOBS, []));
        setSingleJobId(getFromStorage(userStorageKeys.SINGLE_JOB_ID, null));
        setPollingSingle(getFromStorage(userStorageKeys.IS_POLLING_SINGLE, false));
        setVisitUrl(getFromStorage(userStorageKeys.VISIT_URL, ''));
        setBatchJobId(getFromStorage(userStorageKeys.BATCH_JOB_ID, null));
        setResumeBatch(getFromStorage(userStorageKeys.BATCH_JOBS, []).some(job => job.status === 'running'));
        setLastFailedBatch(getFromStorage(userStorageKeys.BATCH_JOBS, []).find(job => job.status === 'error') || null);
      }
    };
  ...
  // THIS IS THE NEW UNIFIED BATCH POLLING FUNCTION
  const startBatchPolling = async (currentBatchJobIdFromEffect: string) => {
    if (!isPollingBatchRef.current && batchJobId === currentBatchJobIdFromEffect) {
      // This guard ensures that if polling was stopped (isPollingBatchRef.current = false),
      // but a timeout fires, it doesn't restart if batchJobId still matches.
      // The useEffect is responsible for setting isPollingBatchRef.current = true when it intends to start.
      addDebugLog('BATCH_POLL_GUARD_STALE_TIMEOUT', 'Polling cycle for active job ID detected but polling is marked as stopped. Aborting.', { currentBatchJobIdFromEffect });
      return;
    }
    if (!isPollingBatchRef.current) { // General guard if polling was stopped for any reason
  return (
                    ...
                    <div className="flex flex-col md:flex-row md:items-center gap-4">
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        <button
                          ...
                          className="flex items-center px-3 py-2 rounded-lg text-sm font-medium bg-gray-200 hover:bg-gray-300 text-gray-700 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-gray-200 transition-all duration-200"
                        >
                        ...
                        <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                          {/* Keep your existing form job rows implementation */}
                            {formJobs.map((job, index) => {
                              ...
                              const dispenserInfo = extractDispenserProgressLocal(job.message || '', job.dispenserCount);
                              const fuelInfo = extractFuelProgressLocal(job.message || '');
                              ...
                              return (
                                ...
                                <td className="px-4 py-3">
                                  ...
                                  {/* Detailed status message for running jobs */}
                                  {job.status === 'running' && job.message && (
                                    <div 
                                      className="text-xs text-gray-500 dark:text-gray-400 mt-1 truncate max-w-xs" 
                                      title={job.message} // Full message on hover
                                    >
                                      {formatDisplayMessage(job.message)} {/* Use the formatter here */}
                                    </div>
                                  )}
                                
                                  {/* Status message for error state (unless it's 'Processing stopped by user') */}
                                  {job.status === 'error' && job.message !== 'Processing stopped by user' && (
                                ...
                                <td className="px-4 py-3">
                                  {job.status === 'running' && (
                                    <div className="space-y-3 min-w-56 min-h-16"> 
                                      {/* Dispenser progress - THIS IS FOR SINGLE VISIT JOB */}
                                      {(() => {
                                        const dispenserInfo = extractDispenserProgressLocal(job.message || '', job.dispenserCount);
                                        // Logic for single visit dispenser progress (skeleton or actual bar)
                                        if (dispenserInfo.total > 0) { // If total dispensers known, show actual progress
                                          return (
                                        <div>
                                        ...
                                        } else if (job.status === 'running') { // Else (total is 0 but job running), show skeleton
                                          return (
                                            <div>
                                              <div className="flex justify-between items-center text-xs mb-1">
                                                <div className="font-medium text-gray-400 dark:text-gray-500 flex items-center gap-1">
                                                  <svg className="w-3 h-3 text-gray-400 dark:text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z" />
                                                  </svg>
                                                  Dispensers
                                                </div>
                                                <span className="font-mono bg-gray-200 dark:bg-gray-700/50 text-gray-400 dark:text-gray-500 px-1.5 py-0.5 rounded">
                                                  --/--
                                                </span>
                                              </div>
                                              <div className="w-full bg-gray-200 dark:bg-gray-700/60 rounded-full h-1.5 overflow-hidden">
                                                <div className="h-1.5 bg-gray-300 dark:bg-gray-600/70 rounded-full animate-pulse" style={{ width: '100%' }}></div>
                                              </div>
                                            </div>
                                          );
                                        }
                                        return null; // Default: job not running or no dispenser info
                                      })()}
                                      
                                      {/* Fuel type progress - THIS IS FOR SINGLE VISIT JOB */}
                                      {(() => {
                                        const fuelInfo = extractFuelProgressLocal(job.message || '');
                                        // Logic for single visit fuel progress (skeleton or actual bar)
                                        ...
                                        <div>
                                          ...
                                          // Show refined skeleton loader for fuel
                                          return (
                                            <div>
                                              <div className="flex justify-between items-center text-xs mb-1">
                                                <div className="font-medium text-gray-400 dark:text-gray-500 flex items-center gap-1">
                                      ...
                                      {/* General progress message when no specific progress data FOR SINGLE VISIT*/}
                                      {job.status === 'running' && !dispenserInfo.total && !fuelInfo.total && job.message && (
                                        <div className="text-xs text-gray-600 dark:text-gray-400 py-1 italic">
                                          {job.message} {/* Show the raw message as a fallback if no bars */} 
                                        </div>
                                      )}
                                      
                                      {/* Indeterminate progress FOR SINGLE VISIT (only if no other bars and no message shown) */}
                                      {job.status === 'running' && !dispenserInfo.total && !fuelInfo.total && !job.message && (
                                  ...
                                  {/* Display "Completed in..." message in Progress column for completed jobs */}
                                  {job.status === 'completed' && (
                                    <div className="flex items-center text-xs text-green-600 dark:text-green-500 h-full py-1"> {/* Adjusted padding/height for consistency */}
                                  ...
                                  {/* Display "Processing stopped by user" message in Progress column for error jobs with that specific message */}
                                  {job.status === 'error' && job.message === 'Processing stopped by user' && (
                                    <div className="flex items-center text-xs text-red-600 dark:text-red-500 h-full py-1">
                                      <svg className="w-4 h-4 mr-1.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636" />{/* Stop icon */}
                                      </svg>
                                      <span className="font-medium">
                                        {job.message}
                                      </span>
                  ...
                  <div className="p-4 space-y-4">
                          ...
                          <span className="ml-2 text-gray-700 dark:text-gray-300">
                            ...
                            {lastFailedBatch && (
                              <span className="text-primary-500 dark:text-primary-400 text-xs ml-2">
                                (Failed at visit {lastFailedBatch.completedVisits} of {lastFailedBatch.totalVisits})
                              </span>
                            )}
                          </span>
                        </label>
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <div className="flex space-x-3">
                        ...
                        {batchJobs.some((job: BatchJob) => job.status === 'running') ? (
                          <button
                          ...
                          >
                            ...
                            <span>{resumeBatch && lastFailedBatch ? 'Resume Failed Batch' : `Process ${selectedVisits.length} Visit${selectedVisits.length !== 1 ? 's' : ''}`}</span>
                          </button>
                        )}
                      </div>
                    </div>
                        ...
                        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                          <thead className="bg-gray-50 dark:bg-gray-700/50">
                            <tr>
                              ...
                              <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Visit Progress</th>
                              <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Form & Dispenser Progress</th>
                              <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Actions</th>
                            </tr>
                          </thead>
                          <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                            {batchJobs.map((job, index) => {
                              const currentVisitMessage = job.currentVisitStatus || job.message || '';
                              // const dispenserInfo = extractDispenserProgressLocal(currentVisitMessage, job.formsTotal); // Will be used later
                              // const fuelInfo = extractFuelProgressLocal(currentVisitMessage); // Will be used later
                              ...
                              <tr
                                ...
                                `}
                              >
                                  {/* Status Column START (1st TD) - REPLACING CONTENT OF THIS TD */}
                                <td className="px-4 py-3 whitespace-nowrap">
                                  <div className="flex items-center">
                                    <div className={`flex items-center justify-center w-8 h-8 rounded-full 
                                      ${job.status === 'running' ? 'bg-primary-100 dark:bg-primary-800/50 text-primary-600 dark:text-primary-400 animate-pulse' :
                                    ...
                                    {/* Detailed status message for running jobs */}
                                    {job.status === 'running' && currentVisitMessage && (
                                      <div 
                                        className="text-xs text-gray-500 dark:text-gray-400 mt-1 truncate max-w-xs" 
                                        title={currentVisitMessage} // Full message on hover
                                      >
                                        {formatDisplayMessage(currentVisitMessage)}
                                      </div>
                                    )}
                                    {/* Error message (if not 'Processing stopped by user') */}
                                    {job.status === 'error' && job.message && job.message !== 'Processing stopped by user' && (
                                      <div className="flex items-center text-xs text-red-600 dark:text-red-500 mt-2">
                                        <FiXCircle className="w-3.5 h-3.5 mr-1 flex-shrink-0" />
                                        <span>{job.message}</span>
                                      </div>
                                    )}
                                </td>
                                  ...
                                  {/* Visit Progress Column START (2nd TD) - REPLACING CONTENT OF THIS TD */}
                                <td className="px-4 py-3 whitespace-nowrap">
                                    <div className="space-y-2" style={{ minWidth: '200px' }}> {/* Added minWidth for better layout */}
                                      <div>
                                    <div className="flex items-center justify-between text-xs mb-1">
                                        ...
                                        job.status === 'running' ? 'bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-300' : 
                                            job.status === 'error' ? 'bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-300' :
                                      ...
                                      <div 
                                        className={`h-full rounded-full transition-all duration-500 ${
                                          ...
                                          'bg-gray-500 dark:bg-gray-400'
                                            ...
                                            style={{ width: `${((job.completedVisits || 0) / Math.max(1, job.totalVisits || 1)) * 100}%` }}
                                      />
                                    </div>
                                      </div>
                                    {job.status === 'running' && (
                                        <div className="mt-1 text-xs text-gray-600 dark:text-gray-400 pt-1 border-t border-gray-200 dark:border-gray-700">
                                          <div className="flex items-center gap-1.5">
                                            <FiRefreshCw className="h-3.5 w-3.5 animate-spin flex-shrink-0 text-primary-500 dark:text-primary-400" /> 
                                            <span className="font-medium truncate">
                                              {job.currentVisitName ? 
                                                `Visit ${(job.completedVisits || 0) + 1}: ${job.currentVisitName}` : 
                                                `Processing Visit ${(job.completedVisits || 0) + 1} of ${(job.totalVisits || 0)}`}
                                          </span>
                                        </div>
                                      </div>
                                    )}
                                  ...
                                  {/* Visit Progress Column END */}
                                  
                                  {/* Form & Dispenser Progress Column START (3rd TD) - REPLACING CONTENT OF THIS TD */}
                                <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400 w-1/3">
                                  {/* Condition 1: Running but no detailed progress data yet */}
                                  {(job.status === 'running' || (job.status === 'idle' && job.completedVisits < job.totalVisits && job.totalVisits > 0 && batchJobId === job.jobId)) && 
                                   (!job.formsTotal && !job.formsCurrent && !job.currentVisitFuelType && 
                                    (!job.currentVisitStatus || (!extractDispenserProgressLocal(job.currentVisitStatus, job.formsTotal).total && !extractFuelProgressLocal(job.currentVisitStatus).total))
                                   ) ? (
                                    <div className="text-xs text-gray-500 dark:text-gray-400 italic py-2 min-h-16 flex items-center justify-center">
                                      Detailed progress will appear once current visit processing begins...
                                    </div>
                                  ) : 
                                  /* Condition 2: Running (or active idle) and has (or will have) progress data */
                                  (job.status === 'running' || (job.status === 'idle' && job.completedVisits < job.totalVisits && job.totalVisits > 0 && batchJobId === job.jobId)) && job.completedVisits < job.totalVisits ? (
                                    <div className="space-y-3 min-w-56 min-h-16">
                                      {/* Dispenser progress for current visit (prioritizing direct job fields) */}
                                      {(() => {
                                          const currentVisitMessage = job.currentVisitStatus || job.message || '';
                                          let formsTotal = job.formsTotal;
                                          let formsCurrent = job.formsCurrent;

                                          // Fallback to extraction if direct API data is not available or zero
                                          if (formsTotal === undefined || formsTotal === 0) {
                                            const extractedDispenserInfo = extractDispenserProgressLocal(currentVisitMessage);
                                            if (extractedDispenserInfo.total > 0) {
                                                formsTotal = extractedDispenserInfo.total;
                                                formsCurrent = extractedDispenserInfo.current;
                                            }
                                          }
                                          // Ensure formsCurrent is not undefined if formsTotal is defined
                                          if (formsTotal !== undefined && formsCurrent === undefined) formsCurrent = 0;


                                        if (formsTotal !== undefined && formsTotal > 0 && formsCurrent !== undefined) {
                                          return (
                                            <div>
                                              <div className="flex justify-between items-center text-xs mb-1">
                                                <div className="font-medium text-amber-600 dark:text-amber-400 flex items-center gap-1">
                                                  <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z" /></svg>
                                                  Dispensers (Current Visit)
                                                </div>
                                                <span className="font-mono bg-amber-100 dark:bg-amber-900/30 text-amber-800 dark:text-amber-300 px-1.5 py-0.5 rounded">
                                                  {formsCurrent}/{formsTotal}
                                                </span>
                                              </div>
                                              <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1.5 overflow-hidden">
                                                <div className="h-1.5 rounded-full transition-all duration-500 bg-amber-500" style={{ width: `${Math.min(Math.round((formsCurrent / Math.max(formsTotal, 1)) * 100), 100)}%` }}></div>
                                              </div>
                                            </div>
                                          );
                                        } else { // Skeleton for dispenser progress if no data
                                          return (
                                            <div>
                                              <div className="flex justify-between items-center text-xs mb-1">
                                                <div className="font-medium text-gray-400 dark:text-gray-500 flex items-center gap-1"><svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z" /></svg>Dispensers (Current Visit)</div><span className="font-mono bg-gray-200 dark:bg-gray-700/50 text-gray-400 dark:text-gray-500 px-1.5 py-0.5 rounded">--/--</span>
                                              </div><div className="w-full bg-gray-200 dark:bg-gray-700/60 rounded-full h-1.5"><div className="h-1.5 bg-gray-300 dark:bg-gray-600/70 rounded-full animate-pulse" style={{ width: '100%' }}></div></div>
                                            </div>
                                          );
                                        }
                                      })()}
                                        
                                      {/* Fuel type progress for current visit (prioritizing direct job fields) */}
                                      {(() => {
                                          const currentVisitMessage = job.currentVisitStatus || job.message || '';
                                          let fuelType = job.currentVisitFuelType;
                                          let fuelCurrent = job.currentVisitFuelCurrent;
                                          let fuelTotal = job.currentVisitFuelTotal;

                                          // Fallback to extraction if direct API data is not available or zero
                                          if ((fuelType === undefined || fuelTotal === undefined || fuelTotal === 0) && job.currentVisitStatus) {
                                              const extractedFuelInfo = extractFuelProgressLocal(currentVisitMessage);
                                              if (extractedFuelInfo.type && extractedFuelInfo.total > 0) {
                                                  fuelType = fuelType === undefined ? extractedFuelInfo.type : fuelType;
                                                  fuelCurrent = fuelCurrent === undefined ? extractedFuelInfo.current : fuelCurrent;
                                                  fuelTotal = fuelTotal === undefined ? extractedFuelInfo.total : fuelTotal;
                                              }
                                          }
                                          // Ensure fuelCurrent is not undefined if fuelType and fuelTotal are defined
                                          if (fuelType && fuelTotal !== undefined && fuelCurrent === undefined) fuelCurrent = 0;

                                        if (fuelType && fuelTotal !== undefined && fuelTotal > 0 && fuelCurrent !== undefined) {
                                          return (
                                            <div>
                                              <div className="flex justify-between items-center text-xs mb-1">
                                                <div className="font-medium text-blue-600 dark:text-blue-400 flex items-center gap-1"><svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>{fuelType} (Current Visit)</div>
                                                <span className="font-mono bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-300 px-1.5 py-0.5 rounded">{fuelCurrent}/{fuelTotal}</span>
                                              </div>
                                              <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1.5 overflow-hidden">
                                                <div className="h-1.5 rounded-full transition-all duration-500 bg-blue-500" style={{ width: `${Math.min(Math.round((fuelCurrent / Math.max(fuelTotal, 1)) * 100), 100)}%` }}></div>
                                              </div>
                                            </div>
                                          );
                                        } else { // Skeleton for fuel progress if no data
                                          return (
                                            <div>
                                              <div className="flex justify-between items-center text-xs mb-1">
                                                <div className="font-medium text-gray-400 dark:text-gray-500 flex items-center gap-1"><svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>Fuel Grade (Current Visit)</div><span className="font-mono bg-gray-200 dark:bg-gray-700/50 text-gray-400 dark:text-gray-500 px-1.5 py-0.5 rounded">--/--</span>
                                              </div><div className="w-full bg-gray-200 dark:bg-gray-700/60 rounded-full h-1.5"><div className="h-1.5 bg-gray-300 dark:bg-gray-600/70 rounded-full animate-pulse" style={{ width: '100%' }}></div></div>
                                            </div>
                                          );
                                        }
                                  ...
                                  /* Condition 3: Completed */
                                  job.status === 'completed' ? (
                                    <div className="flex items-center text-xs text-green-600 dark:text-green-500 h-full py-1 justify-center">
                                      <FiCheckCircle className="w-4 h-4 mr-1.5 flex-shrink-0" />
                                      <span className="font-medium">Completed in {formatDuration(job.startTime, job.endTime)}</span>
                                    </div>
                                  ) : 
                                  /* Condition 4: Error (specific user stopped message) */
                                  job.status === 'error' && job.message === 'Processing stopped by user' ? (
                                    <div className="flex items-center text-xs text-red-600 dark:text-red-500 h-full py-1 justify-center">
                                      <FiAlertTriangle className="w-4 h-4 mr-1.5 flex-shrink-0" />
                                      <span className="font-medium">{job.message}</span>
                                    </div>
                                  ) : 
                                  /* Condition 5: Other error states or truly idle and not active */
                                  job.status === 'error' ? (
                                    <div className="flex items-center text-xs text-red-600 dark:text-red-500 h-full py-1 justify-center">
                                      <FiAlertTriangle className="w-4 h-4 mr-1.5 flex-shrink-0" />
                                      <span className="font-medium truncate" title={job.message}>{job.message || 'An error occurred'}</span>
                                    </div>
                                  ) :
                                  /* Condition 6: Default/Idle and not fitting above active idle */
                                  (
                                    <div className="text-xs text-gray-500 dark:text-gray-400 italic py-2 min-h-16 flex items-center justify-center">
                                        {job.message && job.message !== 'Batch processing started' ? job.message : 'Awaiting batch start...'}
                                    </div>
                                  )}
                                </td>
                                  {/* Form & Dispenser Progress Column END */}
                                  
                                  {/* Actions Column START (4th TD) - REPLACING CONTENT OF THIS TD */}
                                  <td className="px-4 py-3 whitespace-nowrap">
                                    <div className="flex items-center space-x-2">
                                      {job.status === 'running' && job.currentVisitId && (
                                        (() => {
                                          let visitUrlToOpen = '#';
                                          if (job.currentVisitId) {
                                            if (job.currentVisitId.startsWith('http')) { // Should not happen based on type, but good check
                                              visitUrlToOpen = job.currentVisitId;
                                            } else {
                                              // Search through workOrders for the visitId
                                              for (const wo of workOrders) {
                                                if (wo.visits && Array.isArray(wo.visits)) { // Ensure wo.visits is an array
                                                  const foundVisit = wo.visits.find(v => v.id === job.currentVisitId);
                                                  if (foundVisit && foundVisit.url) {
                                                    visitUrlToOpen = foundVisit.url.startsWith('http') 
                                                      ? foundVisit.url 
                                                      : `https://app.workfossa.com${foundVisit.url}`;
                                                    break; // Found the URL
                                                  }
                                                }
                                              }
                                          ...
                                          return (
                                            <button
                                              onClick={() => {
                                                if (visitUrlToOpen && visitUrlToOpen !== '#') {
                                                  openUrlWithLogin(visitUrlToOpen);
                                                }
                                              }}
                                              disabled={visitUrlToOpen === '#'}
                                              className="text-primary-500 hover:text-primary-700 dark:text-primary-400 dark:hover:text-primary-300 disabled:opacity-50 disabled:cursor-not-allowed p-1" // Added padding
                                              title={visitUrlToOpen !== '#' ? "Open current visit in browser" : "Current visit URL not found"}
                                            >
                                              <FiEye className="w-5 h-5" />
                                            </button>
                                          );
                                      ...
                                      {job.status === 'running' && (
                                        <button
                                          onClick={handleStopBatchProcessing} 
                                          className="text-red-500 hover:text-red-700 dark:text-red-400 dark:hover:text-red-300 p-1" // Added padding
