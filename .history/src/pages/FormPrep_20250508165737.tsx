...
// Add the animation keyframes at the top level
  ...
  const handleBatchProcess = async () => {
    ...
    try {
      ...
      try {
        ...
        // Show a development message for missing endpoint
        if (error.message && error.message.includes('404')) {
          addToast('error', 'Batch processing API endpoint not available (404 Not Found). This feature is currently under development.');
        } else {
    ...
    const pollInterval = setInterval(async () => {
      try {
        ...
        try {
          ...
          // If the endpoint doesn't exist (404), show a message and clear polling
          if (apiError.message && apiError.message.includes('404')) {
            addToast('error', 'Batch status endpoint not available. This feature is still under development.');
            ...
            setBatchJobs(prev => {
              ...
              if (jobIndex !== -1) {
                updatedJobs[jobIndex] = {
                  ...updatedJobs[jobIndex],
                  status: 'error',
                  message: 'Batch status API endpoint not available'
                };
              }
          ...
          // For non-404 errors, just log and continue
          return;
        }
        ...
        if (status) {
          ...
          let currentVisitId = '';
          let currentVisitName = '';
          let formsTotal = 0;
          let formsCurrent = 0;
          let currentVisitPhase: 'setup' | 'forms' | 'filling' | 'saving' | undefined = undefined;
          
          // Extract current visit ID if available
          if (status.currentVisit) {
            ...
            // Try to get store name from message
          ...
          // Analyze the message for progress indicators
          if (status.message) {
            // Check for form preparation phase
            if (status.message.includes('Preparing') || status.message.includes('forms')) {
              currentVisitPhase = 'forms';
            } 
            // Check for form filling phase
            else if (status.message.includes('Filling')) {
              currentVisitPhase = 'filling';
            }
            // Check for form saving phase
            else if (status.message.includes('Saving')) {
              currentVisitPhase = 'saving';
            }
            // Default to setup phase
            else {
              currentVisitPhase = 'setup';
            }
            
            // Extract form counts if available
            ...
            // Try extracting dispenser progress
          ...
          // Update batch jobs with the current status and enhanced details
          setBatchJobs(prev => {
            const updatedJobs = [...prev];
            const jobIndex = updatedJobs.findIndex(job => job.status === 'running');
            ...
            if (jobIndex !== -1) {
              ...
              updatedJobs[jobIndex] = {
                ...
                currentVisitId,
                currentVisitName,
                currentVisitStatus: status.currentVisitStatus || status.message,
                currentVisitPhase,
                formsTotal: formsTotal || updatedJobs[jobIndex].formsTotal,
                formsCurrent: formsCurrent || updatedJobs[jobIndex].formsCurrent,
                startTime: updatedJobs[jobIndex].startTime || currentTime,
                endTime
              };
                                  ...
                                  <div className="mt-2 space-y-2.5">
                                      ...
                                      <>
                                        ...
                                        <div className="mt-2">
                                          <div className="flex justify-between items-center text-xs mb-1">
                                            ...
                                            {(() => {
                                              // Extract dispenser info without affecting the UI yet
                                              const dispenserInfo = extractDispenserProgress(job.message || '', job.dispenserCount);
                                              if (dispenserInfo.current > 0) {
                                                return (
                                                  <span className="font-mono bg-amber-100 dark:bg-amber-900/30 text-amber-800 dark:text-amber-300 px-1.5 py-0.5 rounded">
                                                    {dispenserInfo.current}/{dispenserInfo.total || job.dispenserCount || '?'}
                                                  </span>
                                                );
                                              }
                                              return (
                                                <span className="font-mono bg-amber-100 dark:bg-amber-900/30 text-amber-800 dark:text-amber-300 px-1.5 py-0.5 rounded animate-pulse">
                                                  Preparing...
                                                </span>
                                              );
                                            })()}
                                          ...
                                          {(() => {
                                            ...
                                            // If we have current dispenser info, show exact progress
                                            if (dispenserInfo.current > 0 && dispenserInfo.total > 0) {
                                              // Use the total dispensers count from the job object rather than
                                              // the value from the message, which might just be a subset
                                              const totalDispensers = job.dispenserCount || dispenserInfo.total;
                                              const currentDispenser = dispenserInfo.current;
                                              
                                              // Ensure progress never reaches 100% until we're completely done
                                              // Each dispenser gets an equal portion of the progress bar
                                              const completedPercentage = ((currentDispenser - 1) / totalDispensers) * 100;
                                              
                                              // Add a small fraction for the current dispenser's progress
                                              // This ensures we never hit 100% until the very end
                                              const currentProgress = (1 / totalDispensers) * 0.5 * 100;
                                              
                                              // Combine for total progress percentage
                                              const progressPercent = Math.min(
                                                Math.round(completedPercentage + currentProgress),
                                                currentDispenser === totalDispensers ? 100 : 99
                                              );
                                                ...
                                                <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1.5 overflow-hidden">
                                                  <div 
                                                    className={`h-1.5 rounded-full transition-all duration-500 ${
                                                      progressPercent < 30 ? 'bg-amber-400' :
                                            ...
                                            // If no dispenser info yet, show indeterminate progress
                                            return (
                                              <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1.5 overflow-hidden">
                                                <div className="h-1.5 bg-amber-500 rounded-full animate-pulse-progress" style={{ width: '30%' }}></div>
                                              </div>
                                            );
                                          })()}
                                        </div>
                                        
                                        {/* Fuel grade progress section - only show when we have actual fuel data */}
                                        {(() => {
                                          // Extract fuel grade information from message
                                          const fuelInfo = extractFuelProgress(job.message || '');
                                          
                                          // Only show if we have a specific fuel type and progress data
                                          if (fuelInfo.type && fuelInfo.current > 0 && fuelInfo.total > 0) {
                                            const progressPercent = Math.round((fuelInfo.current / Math.max(fuelInfo.total, 1)) * 100);
                                            ...
                                            return (
                                              <div className="mt-2">
                                                ...
                                                <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1.5 overflow-hidden">
                                                  <div 
                                                    className={`h-1.5 rounded-full transition-all duration-500 ${
                                                      progressPercent < 30 ? 'bg-blue-400' :
                                          ...
                                          // If we detect dispensers but no fuel grade yet, show a placeholder ready state
                                          const dispenserInfo = extractDispenserProgress(job.message || '', job.dispenserCount);
                                          if (dispenserInfo.current > 0) {
                                            return (
                                              <div className="mt-3">
                                                <div className="flex justify-between items-center text-xs mb-1 text-gray-500 dark:text-gray-400">
                                                  <div className="flex items-center gap-1 opacity-60">
                                                    ...
                                                    Waiting for fuel grade...
                                                  </div>
                                                </div>
                                                <div className="w-full bg-gray-200 dark:bg-gray-700/50 rounded-full h-1 overflow-hidden opacity-40">
                                    ...
                                    {job.status === 'completed' && (
                                      <div className="flex items-center text-xs text-green-600 dark:text-green-500">
                                        ...
                                        <span>
                                          Completed successfully in {formatDuration(job.startTime, job.endTime)}
                                        </span>
                                      </div>
                                    )}
              ...
              <div className="space-y-6">
                ...
                <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
                  ...
                  <div className="p-4 space-y-4">
                    ...
                    {/* Current Running Batch Status */}
                    {isProcessing && batchJobId && (
                      <div className="bg-primary-50 dark:bg-primary-900/20 rounded-lg p-4 border border-primary-200 dark:border-primary-700 animate-pulse-slow">
                        <div className="flex justify-between items-center mb-3">
                          <h3 className="text-sm font-medium text-primary-800 dark:text-primary-300 flex items-center">
                            <FiRefreshCw className="mr-2 h-4 w-4 animate-spin" />
                            Batch Processing Active
                          </h3>
                          <div className="text-xs font-semibold text-primary-700 dark:text-primary-400 bg-primary-100 dark:bg-primary-800/50 px-2 py-1 rounded">
                            Job ID: {batchJobId}
                          </div>
                        </div>
                        
                        {/* Find and display the active batch job */}
                        {batchJobs.length > 0 && batchJobs.find(job => job.status === 'running') && (
                          <div className="space-y-3">
                            {(() => {
                              const activeJob = batchJobs.find(job => job.status === 'running');
                                ...
                                <>
                                  <div className="flex justify-between items-center text-xs mb-1">
                                    <span className="text-primary-700 dark:text-primary-400">
                                      {activeJob.completedVisits} of {activeJob.totalVisits} visits completed
                                    </span>
                                    <span className="font-mono bg-primary-100 dark:bg-primary-800/50 text-primary-800 dark:text-primary-300 px-1.5 py-0.5 rounded">
                                      {Math.round((activeJob.completedVisits / Math.max(1, activeJob.totalVisits)) * 100)}%
                                    </span>
                                  </div>
                                  <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2.5 overflow-hidden">
                                    <div 
                                      className="h-full rounded-full transition-all duration-500 bg-primary-500 dark:bg-primary-400"
                                      style={{ width: `${(activeJob.completedVisits / Math.max(1, activeJob.totalVisits) * 100)}%` }}
                                    />
                                  </div>
                                  
                                  {activeJob.currentVisitName && (
                                    <div className="text-xs text-primary-700 dark:text-primary-400 mt-2">
                                      Currently processing: {activeJob.currentVisitName}
                                      {activeJob.currentVisitPhase && (
                                        <span className="ml-1 capitalize">
                                          ({activeJob.currentVisitPhase.replace('_', ' ')})
                                        </span>
                                      )}
                                    </div>
                                  )}
                                  
                                  {activeJob.currentVisitStatus && (
                        ...
                        <button
                          ...
                          className="w-full mt-3 px-4 py-2 bg-red-500 hover:bg-red-600 text-white rounded-md text-sm font-medium flex items-center justify-center space-x-2 transition-colors"
                        >
                          <FiX className="h-4 w-4" />
                          <span>Stop Batch Processing</span>
                        </button>
                      </div>
                    )}
                  ...
                  <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
                    <div className="border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700/50 px-4 py-3">
                      <h2 className="text-sm font-medium text-gray-700 dark:text-gray-300 flex items-center gap-2">
                        ...
                        <span className="ml-2 text-xs font-medium bg-gray-200 dark:bg-gray-600 text-gray-700 dark:text-gray-300 rounded-full px-2 py-0.5">
                          {batchJobs.length}
                        </span>
                      </h2>
                    </div>
                              ...
                              >
                                ...
                                <td className="px-4 py-3">
                                  <div className="space-y-2">
                                    ...
                                    {/* Enhanced visual feedback - added animation for running jobs */}
                                    {job.status === 'running' && (
                                      <div className="relative mt-1">
                                        <div className="absolute inset-0 bg-blue-300 dark:bg-blue-700 opacity-20 animate-pulse rounded-full"></div>
                                      </div>
                                    )}
                                    ...
                                    {job.status === 'running' && job.currentVisitId && (
                                      <div className="mt-3 pt-2 border-t border-gray-200 dark:border-gray-700">
                                        <div className="flex justify-between items-center text-xs mb-1">
                                          ...
                                          {job.formsTotal && job.formsTotal > 0 ? (
                                            ...
                                            <span className="font-mono bg-indigo-100 dark:bg-indigo-900/30 text-indigo-800 dark:text-indigo-300 px-1.5 py-0.5 rounded animate-pulse">
                                              In progress
                                            </span>
                                          )}
                                        </div>
                                        
                                        {/* Enhanced progress bar for individual visit - show progress when available */}
                                        {job.formsTotal && job.formsTotal > 0 ? (
                                          <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1.5 overflow-hidden">
                                            <div 
                                              className={`h-1.5 rounded-full transition-all duration-500 ${
                                          ...
                                          /* Indeterminate progress bar when no specific progress is available */
                                          <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1.5 overflow-hidden">
                                            <div className="h-1.5 bg-indigo-500 rounded-full animate-pulse-progress" 
                                              style={{ width: '30%' }}></div>
                                          </div>
                                        )}
                                        
                                        {/* Phase indicator with more visual feedback */}
                                        {job.currentVisitPhase && (
                                          <div className="flex items-center mt-1.5">
                                            <div className={`w-2 h-2 rounded-full mr-1.5 ${
                                              job.currentVisitPhase === 'setup' ? 'bg-purple-500 animate-pulse' :
                                              job.currentVisitPhase === 'forms' ? 'bg-amber-500 animate-pulse' :
                                              job.currentVisitPhase === 'filling' ? 'bg-emerald-500 animate-pulse' :
                                              job.currentVisitPhase === 'saving' ? 'bg-sky-500 animate-pulse' :
                                              'bg-gray-500'
                                            }`}></div>
                                            <div className="text-xs capitalize">
                                              <span className={`font-medium 
                                                ${job.currentVisitPhase === 'setup' ? 'text-purple-600 dark:text-purple-400' :
                                                job.currentVisitPhase === 'forms' ? 'text-amber-600 dark:text-amber-400' :
                                                job.currentVisitPhase === 'filling' ? 'text-emerald-600 dark:text-emerald-400' :
                                                job.currentVisitPhase === 'saving' ? 'text-sky-600 dark:text-sky-400' :
                                                'text-gray-600 dark:text-gray-400'}`
                                              }>
                                                {job.currentVisitPhase.replace('_', ' ')}
                                              </span>
                                            </div>
                                          </div>
                                        )}
                                ...
                                <td className="px-4 py-3">
                                  {job.status === 'running' && (
                                    <div className="flex flex-col space-y-1">
                                      <div className="flex items-center text-xs text-blue-600 dark:text-blue-500">
                                        <svg className="w-3.5 h-3.5 mr-1 animate-spin" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                                        </svg>
                                        <span>
                                          {job.currentVisitStatus || job.message || `Processing visit ${job.completedVisits + 1} of ${job.totalVisits}`}
                                        </span>
                                      </div>
                                      {job.currentVisitName && job.currentVisitPhase && (
                                        <div className="text-xs text-gray-600 dark:text-gray-400 pl-5">
                                          ...
                                          `}>
                                            ...
                                            <span className="capitalize">{job.currentVisitPhase.replace('_', ' ')}</span>
                                            {job.formsTotal && job.formsTotal > 0 ? ` (${job.formsCurrent || 0}/${job.formsTotal})` : ''}
                                          </span>
                                        </div>
                                      )}
                                    </div>
                                  ...
                                  {job.status === 'completed' && (
                                    <div className="flex items-center text-xs text-green-600 dark:text-green-500">
                                      ...
                                      <span>
                                        {job.message || "All visits processed successfully"}
                                      </span>
                                    </div>
                                  )}
                                  {job.status === 'error' && (
                                  ...
                                  <div className="flex space-x-2">
                                    {job.status === 'running' && (
                                      <>
                                        <button
                                          onClick={handleStopBatchProcessing}
                                          className="bg-red-100 hover:bg-red-200 text-red-600 dark:bg-red-900/30 dark:hover:bg-red-900/50 dark:text-red-400 p-1.5 rounded transition-colors flex items-center space-x-1 px-2"
                                        ...
                                        <button
                                          onClick={() => {
                                            ...
                                            addToast('info', 'Pause functionality is in development');
                                          }}
                                          className="bg-amber-100 hover:bg-amber-200 text-amber-600 dark:bg-amber-900/30 dark:hover:bg-amber-900/50 dark:text-amber-400 p-1.5 rounded transition-colors flex items-center space-x-1 px-2"
                                    ...
                                    {job.status === 'completed' && (
                                      <button
                                        className="bg-green-100 hover:bg-green-200 text-green-600 dark:bg-green-900/30 dark:hover:bg-green-900/50 dark:text-green-400 p-1.5 rounded transition-colors flex items-center space-x-1 px-2"
                                        title="View details"
                                      >
                                    ...
                                    {job.status === 'error' && (
                                      <button
                                        className="bg-blue-100 hover:bg-blue-200 text-blue-600 dark:bg-blue-900/30 dark:hover:bg-blue-900/50 dark:text-blue-400 p-1.5 rounded transition-colors flex items-center space-x-1 px-2"
                                        title="Retry batch"
                                        onClick={() => setResumeBatch(true)}
                                      >
