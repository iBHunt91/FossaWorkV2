                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-5 min-h-[180px] divide-x divide-gray-200 dark:divide-gray-700">
                        {Array.from({ length: 5 }).map((_, dayIndex) => {
                          ...
                          // Limit visible jobs to max 3
                          const visibleJobs = jobs.slice(0, 3);
                          const hiddenJobCount = Math.max(0, jobs.length - 3);
                          ...
                          return (
                            ...
                            >
                              ...
                              {/* Jobs for this day - no scrolling */}
                                ...
                                {jobs.length === 0 ? (
                                  ...
                                  <div className="space-y-2">
                                    ...
                                    {visibleJobs.map((job, idx) => {
                                      ...
                                      const visitNumber = extractVisitNumber(job);
                                      
                                      return (
                                        <div 
                                          key={job.id || idx} 
                                          className={`relative group p-2 rounded-lg border ${storeStyle.cardBorder} bg-white dark:bg-gray-800 shadow-sm hover:shadow-md transition-all duration-300 cursor-pointer ${
                                        ...
                                        >
                                          <div className="flex items-center">
                                            ...
                                            <div className="flex-shrink-0 mr-2">
                                              <span className={`block w-2.5 h-2.5 rounded-full ${storeStyle.dot}`}></span>
                                            </div>
                                            
                                            {/* Main content */}
                                            <div className="flex-1 min-w-0">
                                              <div className="flex items-center justify-between">
                                                <h4 className="text-xs font-medium text-gray-900 dark:text-white truncate pr-5">
                                                  {job.customer?.name || 'Unknown'}
                                                </h4>
                                                
                                                {/* Visit badge - now with # prefix */}
                                                <div className={`text-[10px] px-1.5 py-0.5 rounded ${storeStyle.badge} whitespace-nowrap`}>
                                                  #{visitNumber}
                                                </div>
                                              </div>
                                              
                                              {/* Store number and dispenser count */}
                                              <div className="flex items-center justify-between mt-1 text-[10px]">
                                                <div className="text-gray-500 dark:text-gray-400 truncate">
                                                  {job.customer?.storeNumber && (
                                                    <span>ID: {job.customer.storeNumber.replace(/#/g, '')}</span>
                                                  )}
                                                </div>
                                                
                                                {hasDispenserData && (
                                                  <div className="flex items-center bg-gray-50 dark:bg-gray-700/50 rounded px-1.5 py-0.5 ml-1">
                                                    <GiGasPump className={`h-2.5 w-2.5 mr-1 ${storeStyle.text}`} />
                                    ...
                                    {/* Show hidden job count label if necessary */}
                                    {hiddenJobCount > 0 && (
                                      <div className="mt-1 py-1 px-2 bg-gray-50 hover:bg-gray-100 dark:bg-gray-700/50 dark:hover:bg-gray-700 text-xs font-medium text-gray-600 dark:text-gray-400 rounded-md text-center transition-colors">
                                ...
                                <div className="space-y-2">
                                  {jobs.length === 0 ? (
                                    ...
                                    <div className="space-y-2">
                                      ...
                                      {/* Show "X more" button if there are more items to display */}
                                      {hasMoreToShow && (
                                        <button
                                          onClick={() => setExpandedDays({...expandedDays, [dayKey]: true})}
                                          className="w-full py-1 px-2 bg-gray-50 hover:bg-gray-100 dark:bg-gray-700/50 dark:hover:bg-gray-700 text-xs font-medium text-gray-600 dark:text-gray-400 rounded-md transition-colors flex items-center justify-center"
                                        >
                                      ...
                                      {isExpanded && jobs.length > 3 && (
                                        <button
                                          onClick={() => setExpandedDays({...expandedDays, [dayKey]: false})}
                                          className="w-full py-1 px-2 bg-gray-50 hover:bg-gray-100 dark:bg-gray-700/50 dark:hover:bg-gray-700 text-xs font-medium text-gray-600 dark:text-gray-400 rounded-md transition-colors flex items-center justify-center"
                                        >
                                          <FiChevronUp className="mr-1 h-3 w-3" />
                                          Show less
                                        </button>
                                      )}
                                    </div>
