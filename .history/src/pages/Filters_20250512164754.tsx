...
const FiltersRedesign: React.FC = () => {
            ...
            <div className="flex flex-col space-y-3 z-10 relative">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <div className="font-semibold text-gray-800 dark:text-white text-2xl">
                    {(() => {
                      // Get total warnings across all orders
                      let warningCount = 0;
                      filterWarnings.forEach(warnings => {
              ...
              <div className="flex flex-col space-y-1 pt-1">
                {filterWarnings.size > 0 ? (
                  Array.from(filterWarnings.entries()).slice(0, 3).map(([orderId, warnings]) => {
                ...
                {filterWarnings.size > 3 && (
                  <div className="text-xs text-gray-500 dark:text-gray-400 text-center">
                    +{filterWarnings.size - 3} more stores with warnings
                  </div>
                )}
              </div>
            </div>
  ...
  const renderFilterWarnings = () => {
    ...
    interface WarningGroup {
      ...
      count: number; // Number of occurrences (visits)
      instances: ExtendedFilterWarning[]; // Individual warnings in this group
      severity: number; // Highest severity in the group
      stores: string[]; // Unique stores affected
    ...
    const groupedWarnings = flattenedWarnings.reduce((acc: Record<string, WarningGroup>, warning: ExtendedFilterWarning) => {
      ...
      // Use the raw message as the key, perhaps normalize slightly if needed later
      const key = message; 
      ...
      if (!acc[key]) {
        acc[key] = {
          ...
          count: 0,
          instances: [],
          severity: 0, // Will be updated below
          stores: [],
    ...
    // State to manage expanded warnings
      ...
      <div className="panel mb-6 animate-fadeIn">
        ...
        <div className="space-y-3 mt-4">
          {sortedWarningGroups.map((group, index) => {
            const { message, count, instances, severity, needsVerification, missingDispenserData } = group;
            const groupKey = message; // Use the message as the key
            const isExpanded = expandedWarnings[groupKey] || false;
            
            // Determine icon and color based on severity/verification
            ...
            const cardClass = needsVerification
              ...
              : severity >= 8 
                ...
                : severity >= 5 
                  ...
                  : 'card-subtle'; // Default subtle card
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  >
                    {isExpanded ? '▲' : '▼'} <span className="ml-1 text-xs">View {count} related warnings</span>
                  </button>
                </div>
                ...
                {isExpanded && (
                  <ul className="mt-3 pl-8 space-y-2 border-l border-gray-300 dark:border-gray-600 ml-2">
                    {instances.map((warning, i) => {
                      ...
                      return (
                        <li key={`${warning.orderId}-${i}`} className="text-xs text-gray-600 dark:text-gray-400">
                          <div className="flex items-center space-x-2">
                            ...
                            <span className="inline-flex items-center px-1.5 py-0.5 rounded bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300">
                              ...
                              Visit: {warning.visitId || 'Unknown'}
                            </span>
                          </div>
                          {warning.missingDispenserData && (
                            <p className="mt-1 text-red-600 dark:text-red-400 font-medium">
                              (No dispenser data available. Using standard filter estimates.)
                            </p>
                          )}
                        </li>
                      );
