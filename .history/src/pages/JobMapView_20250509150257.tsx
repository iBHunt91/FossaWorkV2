...
const JobMapView: React.FC = () => {
  ...
  const handlePreviousWeek = () => {
    ...
    setDateFilter('week'); // Ensure week filter is active
  };
  ...
  const applyFilters = useCallback(() => {
    ...
    // Use displayedDate for week calculations, otherwise use new Date() for day, month, year filters
    const referenceDate = dateFilter === 'week' ? displayedDate : new Date();
    let filteredByDate = [...jobs];
    
    switch (dateFilter) {
      case 'day':
        const dayStart = startOfDay(referenceDate); // Use referenceDate (which is 'now' for day filter)
      ...
      case 'week':
        const weekStart = startOfWeek(referenceDate, { weekStartsOn: 0 }); // Use referenceDate (displayedDate for week)
      ...
      case 'month':
        const monthStart = startOfMonth(referenceDate); // Use referenceDate (now for month filter)
      ...
      case 'year':
        const yearStart = startOfYear(referenceDate); // Use referenceDate (now for year filter)
