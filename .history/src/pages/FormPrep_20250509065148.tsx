...
const FormPrep: React.FC<{}> = () => {
      ...
      try {
        ...
        if (error.message && error.message.includes('404')) {
          ...
          const mockBatchJob: BatchJob = {
            filePath: resumeBatch && lastFailedBatch ? 'Resuming batch (DEMO)' : 'Selected visits (DEMO)',
            timestamp: new Date().toLocaleString(),
            status: 'running',
            message: resumeBatch && lastFailedBatch ? 
              'Resuming batch processing (DEMO MODE)...' : 
              'Processing batch visits (DEMO MODE)...',
            totalVisits: selectedVisits.length,
            completedVisits: resumeBatch && lastFailedBatch ? 
              (lastFailedBatch.completedVisits || Math.floor(selectedVisits.length / 3)) : 
              0,
            headless: isHeadless,
            jobId: mockJobId,
            startTime: Date.now(),
          ...
          setBatchJobs(prev => {
            const updatedJobs = [mockBatchJob, ...prev];
            // Also save to localStorage
            saveToStorage(userStorageKeys.BATCH_JOBS, updatedJobs);
            return updatedJobs;
          });
          ...
          // If resuming, show appropriate toast
          if (resumeBatch && lastFailedBatch) {
            addToast('info', 'Resuming batch in DEMO mode. API endpoint not available.');
          } else {
            addToast('info', 'Batch processing API endpoint not available. Running in DEMO mode.');
          }
          
          // Start a demo polling simulation
          startDemoPolling(mockJobId, selectedVisits.length, resumeBatch && lastFailedBatch ? 
            (lastFailedBatch.completedVisits || Math.floor(selectedVisits.length / 3)) : 0);
    ...
    const pollInterval = setInterval(async () => {
      try {
        ...
        if (!currentBatchJobId) {
          console.log('No active batch job ID found, stopping poll');
        ...
        try {
          ...
          if (apiError.message && apiError.message.includes('404')) {
            ...
            // Get current batch jobs from localStorage to avoid state issues
            const storedBatchJobs = getFromStorage(userStorageKeys.BATCH_JOBS, []);
            // Find the job with the matching ID
            const currentBatch = storedBatchJobs.find(job => job.jobId === currentBatchJobId);
            
            if (!currentBatch) {
              console.warn(`No running batch job found in storage for job ID: ${currentBatchJobId}`);
              // Instead of stopping, create a dummy job for resilience
              setBatchJobs(prev => {
                if (prev.some(job => job.jobId === currentBatchJobId)) {
                  return prev; // Already exists in state
                }
                ...
                const recoveryJob: BatchJob = {
                  ...
                  timestamp: new Date().toLocaleString(),
                  status: 'running',
                  message: 'Recovering batch job status...',
                  totalVisits: 5, // Default
                  completedVisits: 0,
                  headless: true,
                  jobId: currentBatchJobId,
                  startTime: Date.now(),
                  currentVisitStatus: 'Reconnecting to batch job...'
                };
                
                const updatedJobs = [recoveryJob, ...prev];
                saveToStorage(userStorageKeys.BATCH_JOBS, updatedJobs);
                return updatedJobs;
              });
              
              // Give it another cycle to recover
              return;
            }
        ...
        if (status) {
          // Generate demo data when the API returns empty status
          if (!status.message && !status.currentVisitStatus) {
            console.log('API returned empty status data, adding demo content');
            
            // Get the current job to use for context
            const storedBatchJobs = getFromStorage(userStorageKeys.BATCH_JOBS, []);
            const currentJob = storedBatchJobs.find(j => j.jobId === currentBatchJobId) || 
                              batchJobs.find(j => j.jobId === currentBatchJobId);
            
            if (currentJob) {
              // Generate progress data based on elapsed time
              const elapsedMs = Date.now() - (currentJob.startTime || Date.now());
              const elapsedSec = Math.floor(elapsedMs / 1000);
              
              // Calculate simulated progress based on time
              const visitIndex = Math.min(Math.floor(elapsedSec / 20), (currentJob.totalVisits || 5) - 1);
              const dispenserCount = 3 + (visitIndex % 3); // 3-5 dispensers
              const dispenserCurrent = 1 + (Math.floor(elapsedSec / 5) % dispenserCount);
              
              const fuelTypes = ['Regular', 'Plus', 'Premium', 'Diesel'];
              const fuelTypeIndex = Math.floor(elapsedSec / 10) % fuelTypes.length;
              const fuelType = fuelTypes[fuelTypeIndex];
              const fuelCurrent = 1 + (Math.floor(elapsedSec / 8) % 3);
              
              // Create demo store name
              const demoStores = [
                'Circle K #12345', 
                'Shell Gas Station', 
                '7-Eleven Store', 
              ...
              const storeName = demoStores[visitIndex % demoStores.length];
              
              // Tag demo data
              status.completedVisits = visitIndex;
              status.totalVisits = currentJob.totalVisits || 5;
              status.currentVisit = `Visit ${visitIndex + 1}`;
              
              // Alternate between dispenser and fuel progress messages
              if (elapsedSec % 20 < 10) {
                status.currentVisitStatus = `Working on ${storeName} - Dispenser ${dispenserCurrent}/${dispenserCount} (DEMO DATA)`;
              } else {
                status.currentVisitStatus = `Working on ${storeName} - Processing ${fuelType} (${fuelCurrent}/3) (DEMO DATA)`;
              }
              
              status.message = `Processing visit ${visitIndex + 1} of ${currentJob.totalVisits}: ${storeName} (DEMO DATA)`;
            }
          }
          
          // Debug the current status message to help troubleshoot
          console.log('Current batch status:', {
            batchJobId: currentBatchJobId,
            message: status.message,
            currentVisitStatus: status.currentVisitStatus || null,
            isDemo: (!status.message || status.message.includes('DEMO DATA')),
            extractedDispenser: extractDispenserProgressLocal(status.currentVisitStatus || status.message || '', undefined),
            extractedFuel: extractFuelProgressLocal(status.currentVisitStatus || status.message || '')
          });
          ...
          setBatchJobs(prev => {
            ...
            if (storedBatchJobs.length > 0) {
              ...
              for (const storedJob of storedBatchJobs) {
                ...
                const jobExists = mergedJobs.findIndex(j => j.jobId === storedJob.jobId) !== -1;
                if (!jobExists) {
            ...
            const jobIndex = mergedJobs.findIndex(j => j.jobId === currentBatchJobId);
            ...
            if (jobIndex !== -1) {
              ...
              if (status.message || status.currentVisitStatus) {
                ...
                // Only update if we found a valid store name and it's not a demo indicator
                if (extractedStoreName && !extractedStoreName.includes('DEMO DATA')) {
                  currentVisitName = extractedStoreName;
                }
                
                if (message.includes('Preparing') || message.includes('forms')) currentVisitPhase = 'forms';
                else if (message.includes('Filling')) currentVisitPhase = 'filling';
                else if (message.includes('Saving')) currentVisitPhase = 'saving';
              ...
              // Make sure we have a valid completedVisits and totalVisits
  ...
  const startDemoPolling = (jobId: string, totalVisits: number, startingVisitIndex: number = 0) => {
    console.log('Starting demo polling simulation for batch job:', jobId);
    let visitCounter = startingVisitIndex;
    let formCounter = 0;
    let dispenserCounter = 0;
    let dispenserTotal = Math.floor(Math.random() * 5) + 2; // Random number between 2-6 dispensers
    let fuelTypeCounter = 0;
    let fuelTypeTotal = 3; // Typically 3 fuel types (Regular, Plus, Premium)
    let currentFuelType = "Regular"; // Simulated fuel type
    ...
    const demoStoreNames = [
      'Circle K #12345 (Sample)', 
      'Shell Gas Station (Sample)', 
      '7-Eleven Store (Sample)', 
      'BP Station (Sample)', 
      'Chevron (Sample)', 
      'Exxon (Sample)', 
      'Mobil (Sample)', 
      ...
      setBatchJobs(prev => {
        ...
        if (formCounter > maxForms) {
          ...
          if (phaseIndex === 0) {
            visitCounter++;
            dispenserCounter = 0;
            dispenserTotal = Math.floor(Math.random() * 5) + 2; // Reset dispensers for next visit
            ...
            if (visitCounter >= totalVisits) {
              ...
              if (completedJobIndex !== -1) {
                completedJobs[completedJobIndex] = {
                  ...completedJobs[completedJobIndex],
                  status: 'completed',
                  message: 'Batch processing completed successfully (DEMO DATA)',
        ...
        // Alternate between updating dispenser and fuel type every other cycle
        const cycle = Math.random() > 0.5;
        
        if (cycle && dispenserCounter < dispenserTotal) {
          dispenserCounter++;
        } else if (!cycle && fuelTypeCounter < fuelTypeTotal) {
          fuelTypeCounter++;
          // Cycle through fuel types
          if (fuelTypeCounter % 3 === 0) currentFuelType = "Premium";
          else if (fuelTypeCounter % 3 === 1) currentFuelType = "Regular";
          else currentFuelType = "Plus";
        }
        ...
        switch (currentPhase) {
          case 'setup':
            statusMessage = `Setting up for ${currentStore} - Preparing forms (DEMO DATA)`;
            break;
          case 'forms':
            statusMessage = `Working on ${currentStore} - Loading form ${formCounter} of ${maxForms} (DEMO DATA)`;
            break;
          case 'filling':
            if (Math.random() > 0.5) {
              // Sometimes show dispenser progress
              statusMessage = `Working on ${currentStore} - Dispenser ${dispenserCounter}/${dispenserTotal} (DEMO DATA)`;
            } else {
              // Sometimes show fuel type progress
              statusMessage = `Working on ${currentStore} - Processing ${currentFuelType} (${fuelTypeCounter}/${fuelTypeTotal}) (DEMO DATA)`;
            }
            break;
          case 'saving':
            statusMessage = `Working on ${currentStore} - Saving completed form ${formCounter}/${maxForms} (DEMO DATA)`;
            break;
        }
        ...
        updatedJobs[jobIndex] = {
          ...
          formsTotal: maxForms,
          message: `Processing visit ${visitCounter + 1} of ${totalVisits}: ${currentStore} (DEMO DATA)`
        };
