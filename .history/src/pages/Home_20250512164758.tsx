...
const HomeContent = () => {
  ...
  const renderVerificationPanel = () => {
    ...
    const generateRealWarnings = () => {
      ...
      // **MODIFIED:** Check for visits within the selected date range (already filtered)
      const upcomingVisits = dateFilteredWorkOrders.filter(order => {
        const visitDate = order.visits?.nextVisit?.date || order.nextVisitDate || order.visitDate;
        if (!visitDate) return false;
        const visitDateTime = new Date(visitDate);
        // Additional check: is it *actually* upcoming from today? (Optional, depending on desired logic)
        // return visitDateTime >= now; 
        // For now, just count all visits within the selected range
        return true; 
      });
      
      if (upcomingVisits.length > 0) {
        warnings.push(`${upcomingVisits.length} visit${upcomingVisits.length !== 1 ? 's' : ''} scheduled in the selected date range`);
      }
      
      // Check for potential overlapping schedules - **Use dateFilteredWorkOrders**
