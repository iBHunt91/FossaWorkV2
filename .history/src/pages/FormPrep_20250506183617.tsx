                                                    ...
                                                    <div className="flex items-center justify-between mb-1">
                                                      ...
                                                      <div className="flex items-center space-x-2">
                                                        <span className="text-xs font-mono bg-gray-100 dark:bg-gray-700 px-1.5 py-0.5 rounded text-gray-700 dark:text-gray-300">
                                                          {(() => {
                                                            // Extract directly from message for display
                                                            if (job.message) {
                                                              const dispenserPattern = /(?:dispenser|pump|meter)(?:[\s#:]+)(\d+)(?:[^\d]+(\d+)|)/i;
                                                              const match = job.message.match(dispenserPattern);
                                                              if (match && match[1]) {
                                                                return match[2] ? `#${match[1]}/${match[2]}` : `#${match[1]}`;
                                                              }
                                                            }
                                                            // Fallback to current value without "/total"
                                                            return `#${dispenserCurrent}`;
                                                          })()}
                                                        </span>
