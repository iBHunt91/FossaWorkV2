...
const JobMapView: React.FC = () => {
      ...
      try {
        ...
        if (savedLocationsJson) {
          ...
          if (Array.isArray(savedLocations) && 
            ...
            console.log(`Loaded ${savedLocations.length} saved locations from local storage`);
          } else {
            console.warn('Invalid custom locations data in local storage, ignoring');
          }
        }
      } catch (error) {
        console.error('Error loading custom locations from local storage:', error);
      }
    };
  ...
  useEffect(() => {
    ...
    if (customLocations.length > 0 || document.readyState === 'complete') {
      try {
        ...
        console.log(`Saved ${customLocations.length} locations to local storage`);
      } catch (error) {
        console.error('Error saving custom locations to local storage:', error);
      }
    }
  }, [customLocations]);
  ...
  const handleLocationSelected = (location: CustomLocation) => {
    // Can be used if you want to immediately select a location when picked in the manager
