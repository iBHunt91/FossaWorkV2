...
interface BatchJob {
  ...
  headless: boolean;
  jobId?: string; // Add jobId field to match usage in the code
                              ...
                              >
                                ...
                                <td className="px-4 py-3 whitespace-nowrap">
                                  <div className="space-y-2">
                                    ...
                                    {/* New detail progress for current visit when running */}
                                    {job.status === 'running' && job.currentVisitId && job.formsCurrent !== undefined && job.formsTotal !== undefined && job.formsTotal > 0 && (
                                      <div className="mt-2 pt-2 border-t border-gray-100 dark:border-gray-700">
                                        <div className="flex items-center justify-between text-xs mb-1">
                                          <span className="text-gray-500 dark:text-gray-400">
                                            Current Visit: {job.formsCurrent} of {job.formsTotal} {job.currentVisitPhase === 'filling' ? 'forms' : 'steps'}
                                          </span>
                                          <span className={`font-mono bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-300 px-1.5 py-0.5 rounded`}>
                                            {Math.round((job.formsCurrent / Math.max(1, job.formsTotal)) * 100)}%
                                          </span>
                                        </div>
                                        <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1.5 overflow-hidden">
                                          <div 
                                            className="h-full rounded-full transition-all duration-500 bg-blue-500/80 dark:bg-blue-400/80"
                                            style={{ width: `${(job.formsCurrent / Math.max(1, job.formsTotal) * 100)}%` }}
                                          />
                                        </div>
                                      </div>
                                    )}
                                    
                                    {/* Show time duration for completed, error or running jobs */}
                                ...
                                <td className="px-4 py-3 whitespace-nowrap">
                                  {job.status === 'running' && (
                                    <div className="flex flex-col space-y-1">
                                      <div className="flex items-center text-xs text-blue-600 dark:text-blue-500">
                                        ...
                                        <span>
                                          {job.message || `Processing visit ${job.completedVisits + 1} of ${job.totalVisits}`}
                                        </span>
                                      </div>
                                      
                                      {/* Show current visit name and status if available */}
                                      {job.currentVisitName && (
                                        <div className="text-xs text-gray-600 dark:text-gray-400 pl-5">
                                          ...
                                          `}>
                                            {job.currentVisitName}:&nbsp;
                                            {job.currentVisitPhase === 'setup' ? 'Setting up' :
                                             job.currentVisitPhase === 'forms' ? 'Creating forms' :
                                             job.currentVisitPhase === 'filling' ? 'Filling forms' :
                                             job.currentVisitPhase === 'saving' ? 'Saving forms' :
                                             'Processing'}
                                          </span>
                                        </div>
                                      )}
                                      
                                      {job.currentVisitStatus && job.currentVisitStatus !== job.message && (
                                        <div className="text-xs text-gray-500 dark:text-gray-400 pl-5 truncate max-w-xs">
                                  ...
                                  {/* Keep existing completed state UI */}
                                  {job.status === 'completed' && (
                                    <div className="flex items-center text-xs text-green-600 dark:text-green-500">
                                      <svg className="w-3.5 h-3.5 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
