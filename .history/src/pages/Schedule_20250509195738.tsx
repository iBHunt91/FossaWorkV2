...
const Schedule: React.FC = () => {
  ...
  const renderCompactJobItem = (order: WorkOrder) => {
    const storeType = getStoreTypeForFiltering(order);
    const styles = getStoreStyles(storeType); // Use existing styles for consistency (e.g., border color)
    const visitNumber = extractVisitNumber(order);
    const jobDate = order.visits?.nextVisit?.date || order.nextVisitDate || order.visitDate || order.scheduledDate || order.date;
    ...
    return (
      <div 
        key={order.id}
        className={`compact-job-item flex items-center justify-between p-3 mb-2 rounded-lg shadow-sm border-l-4 ${styles.cardBorder} bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors duration-150`}
      >
        <div className="flex-grow overflow-hidden">
          <div className="flex items-baseline">
            <h4 className={`text-sm font-semibold truncate ${styles.text}`}>
              {order.customer.name}
              {order.customer.storeNumber && (
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Date: {formattedJobDate}</p>
          {filteredInstructionsOnCard && (
             <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5 truncate">Notes: {filteredInstructionsOnCard}</p>
          )}
        </div>
        <div className="flex-shrink-0 flex items-center gap-1 ml-2">
          <button
            onClick={(e) => handleViewInstructions(e, order)}
            disabled={!order.instructions}
      ...
      <div className="p-4">
        {filteredWorkOrders.length > 0 ? (
          <div className="space-y-0">
            {filteredWorkOrders.map(order => renderCompactJobItem(order))}
          </div>
        ) : (
          <div className="text-center py-10 text-gray-500 dark:text-gray-400">
            <FiInfo className="mx-auto h-10 w-10 mb-2 opacity-60" />
            No jobs to display in compact view for the current filter.
          </div>
        )}
      </div>
    );
