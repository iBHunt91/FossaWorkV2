...
// Main component
const Home: React.FC = () => {
  const { innerHeight } = usePersistentViewContext();
  
  return (
    <div style={{ height: innerHeight }} className="flex flex-col overflow-auto bg-gradient-to-br from-gray-200 to-gray-50 dark:from-gray-800 dark:to-gray-900">
      <HomeContent />
    </div>
  );
}
...
const HomeContent = () => {
  ...
  const handleScrapeWorkOrders = async () => {
    try {
        ...
        try {
          ...
          if (status.status === 'completed' || status.status === 'error') {
            ...
            if (status.status === 'completed') {
              showToast('Work orders scraped successfully!', 'success');
              loadData(); // Refresh data
            } else {
              showToast(`Scrape error: ${status.message || 'Unknown error'}`, 'error');
            }
          }
        } catch (error) {
          ...
          showToast('Failed to check scrape status', 'error');
        }
      }, 2000);
    } catch (error) {
      ...
      showToast('Failed to start work order scraper', 'error');
    }
  };
  ...
  const handleScrapeDispenserData = async () => {
    try {
      ...
      showToast('Failed to start dispenser scraper', 'error');
    }
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        ...
        <div className="lg:col-span-2">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md border border-gray-100 dark:border-gray-700 overflow-hidden">
            ...
            <div className="p-5">
              <ScrapeLogsConsole type="dashboard" />
            </div>
          </div>
        </div>
      </div>
