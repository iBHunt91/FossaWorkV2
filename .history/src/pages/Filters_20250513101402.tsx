...
const FiltersRedesign: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const loadInitialData = async () => {
      ...
      try {
        // Skip API call for now and use the imported data
        // Use the imported scrapedData directly
        console.log('Using imported scrapedData with', scrapedData.workOrders?.length || 0, 'work orders');
        
        // Get the dispenser data and merge
        const enhancedWorkOrders = scrapedData.workOrders.map((order: WorkOrder) => {
          return order; // Use orders as-is for now
        });
