...
const FormPrep: React.FC<{}> = () => {
  ...
  const extractDispenserProgressLocal = (message: string, totalDispensers?: number): { current: number; total: number } => {
    ...
    // Prioritize totalDispensers if available and valid
    if (totalDispensers && totalDispensers > 0) {
      total = totalDispensers;
      
      // Try to parse current dispenser from "Dispenser X of Y" or "Working on dispenser X"
      const matchDispenser = message.match(/Dispenser (\d+) of (\d+)/);
      if (matchDispenser) {
        current = parseInt(matchDispenser[1], 10);
        // Optionally, if message contains a different total, it could be used, but job.dispenserCount is preferred.
        // if (totalDispensers !== parseInt(matchDispenser[2], 10)) { /* log discrepancy or decide priority */ }
      } else {
        const matchWorkingOn = message.match(/Working on dispenser (\d+)/i);
        if (matchWorkingOn) {
          current = parseInt(matchWorkingOn[1], 10);
        }
      }
    } else {
      // Fallback: if totalDispensers is not provided or invalid, try to parse both current and total from message
      const matchExplicitTotal = message.match(/Dispenser (\d+) of (\d+)/);
      if (matchExplicitTotal) {
        current = parseInt(matchExplicitTotal[1], 10);
        total = parseInt(matchExplicitTotal[2], 10);
      }
    }
    
    // If total dispensers are being processed (total > 0) and current is 0 
    // (either parsed as "0" or defaulted because no specific number was found in the message),
    // adjust current to 1. This avoids displaying "dispenser 0".
    ...
    // If, after all parsing, total is 0, then current must also be 0.
    if (total === 0) {
        current = 0;
    }
