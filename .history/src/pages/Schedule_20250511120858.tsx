...
const Schedule: React.FC = () => {
  ...
  const renderJobRow = (order: WorkOrder, dateGroupClass?: string) => {
    ...
    return (
      ...
      >
        ...
        {/* Action Icons Footer */}
        <div className="flex items-center justify-end space-x-2 p-2 border-t border-gray-200 dark:border-gray-700">
          <button
          ...
          {/* Open Visit URL Button - Copied and adapted from Home.tsx */}
          {order.visits?.nextVisit?.url && (
            <button
              className="p-1.5 bg-purple-500 hover:bg-purple-600 text-white rounded-md transition-colors" // Adjusted styling
              onClick={(e) => {
                e.stopPropagation();
  ...
  const calculateScheduleStats = () => {
    ...
    return { currentWeekJobCount, nextWeekJobCount, storeDistributionForCurrentWeek };
  };
  ...
  const renderSchedulePageHeader = () => {
    ...
    const { currentWeekJobCount, nextWeekJobCount, storeDistributionForCurrentWeek } = calculateScheduleStats();
    const storeDistributionArray = Object.entries(storeDistributionForCurrentWeek)
      .map(([type, count]) => ({
        type,
        name: type.charAt(0).toUpperCase() + type.slice(1).replace('-k', ' K'), // Basic formatting
        count
      }))
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          <div className="p-2 rounded-md bg-gray-50 dark:bg-gray-700/50">
            <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1.5 flex items-center">
              ...
              Store Distribution (Current Week)
            </h3>
            {storeDistributionArray.length > 0 ? (
              <div className="space-y-1">
                {storeDistributionArray.slice(0, 3).map(store => { // Show top 3 or less
                  const styles = getStoreStyles(store.type);
                  return (
                    <div key={store.type} className="flex items-center justify-between text-xs">
                ...
                {storeDistributionArray.length > 3 && (
                  <p className="text-xs text-center text-gray-500 dark:text-gray-400 pt-1">+ {storeDistributionArray.length - 3} more</p>
                )}
              </div>
            ) : (
              <p className="text-xs text-center text-gray-500 dark:text-gray-400 pt-1">No jobs this week to show distribution.</p>
            )}
          </div>
        </div>
      </div>
