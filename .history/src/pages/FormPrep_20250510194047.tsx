...
const FormPrep: React.FC<{}> = () => {
  ...
  const formatDisplayMessage = (message: string | undefined): string => {
    ...
    // Specific messages first (ordered by presumed process flow or specificity)
    ...
    if (message.includes('Checking for existing forms')) return 'Checking for forms...'; // Simplified
    
    // New message for adding/creating forms
    if (message.toLowerCase().includes('adding forms') || message.toLowerCase().includes('creating forms')) {
      return 'Preparing forms for visit...';
    }

    // Message for general dispenser processing (distinct from filling a form for a specific dispenser)
    if (message.toLowerCase().includes('processing dispenser') || message.toLowerCase().includes('working on dispenser')) {
      // This handles messages like "Processing dispenser X" or "Working on dispenser Y"
      // If a message about "filling form" also includes these terms, this might be caught first depending on message structure.
      // However, the more specific "filling form" + "dispenser" check is below.
      return 'Processing dispensers...';
    }

    // For messages related to actually filling form details for a dispenser or processing specific fuel types,
    // the progress bars provide detailed visual feedback.
    // We'll show a generic "Processing..." to indicate activity without redundant text.
    const lowerMessage = message.toLowerCase();
    if (
      (lowerMessage.includes('filling form') && lowerMessage.includes('dispenser')) || // Previously led to "Updating forms & dispensers..."
      lowerMessage.includes('processing fuel type')  // Previously led to "Processing fuel types..."
    ) {
      return 'Processing...'; 
    }
    ...
    // If no specific pattern matched above, return the original message.
    // This is useful for debugging and seeing any new/unhandled backend messages.
    // It will be truncated by CSS if too long.
    return message;
  };
