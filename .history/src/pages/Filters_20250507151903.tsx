...
const FiltersRedesign: React.FC = () => {
  ...
  const loadInitialData = useCallback(async () => { 
    ...
    scrapedData.workOrders.forEach(wo => {
      // Pass only wo, as indicated by previous linter errors
      ...
      // Define stationType with a constrained type if possible, otherwise use string
      let stationType: '7-Eleven' | 'Wawa' | 'Circle K' | 'Unknown' = 'Unknown';
      ...
      // Check if stationType is known before indexing
      if (stationType !== 'Unknown') { 
        const stationConfig = STATION_FILTERS[stationType];
        if (stationConfig) {
          if (stationType === '7-Eleven') {
            ...
            // Handle complex GAS/DIESEL types for 7-Eleven
            if (typeof stationConfig.GAS === 'object' && stationConfig.GAS !== null) { // Type check
              gasFilterPart = stationConfig.GAS[meterKey as keyof typeof stationConfig.GAS] || stationConfig.GAS.default;
            }
            if (typeof stationConfig.DIESEL === 'object' && stationConfig.DIESEL !== null) { // Type check
              dieselFilterPart = stationConfig.DIESEL[meterKey as keyof typeof stationConfig.DIESEL] || stationConfig.DIESEL.default;
            }
            // Check for DEF property safely
            if ('DEF' in stationConfig && checkForSpecialFuelTypes(wo).hasDEF) {
               defFilterPart = stationConfig.DEF;
            }
          } else { // For Wawa, Circle K, etc. where GAS/DIESEL are likely strings
            if (typeof stationConfig.GAS === 'string') { // Type check
              gasFilterPart = stationConfig.GAS;
            }
            if (typeof stationConfig.DIESEL === 'string') { // Type check
      ...
      if (gasFilters > 0 && gasFilterPart) {
        processedNeeds.push({
          ...
          quantity: gasFilters,
          stores: [storeName],
          stationType, // Use the constrained type
          orderId: wo.id,
          visitId,
          visitDate,
          storeName,
  ...
  const reloadData = useCallback(async () => { 
    setIsReloading(true);
    // Assuming addToast takes type, message, duration as separate args
    addToast('info', 'Refreshing filter data...', 3000);
    await loadInitialData(); 
    setIsReloading(false);
    addToast('success', 'Filter data refreshed!', 3000);
