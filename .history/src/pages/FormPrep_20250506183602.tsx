                              ...
                              return (
                                        ...
                                        : job.status === 'idle' || job.status === 'running'
                                          ? (() => {
                                              ...
                                              if (job.message) {
                                                ...
                                                const fuelTypes = ['premium', 'regular', 'diesel', 'supreme', 'unleaded', 'fuel type', 'fuel grade'];
                                                ...
                                                if (premiumMatch && premiumMatch[1] && premiumMatch[2] && premiumMatch[3]) {
                                                  premium = premiumMatch[1].charAt(0).toUpperCase() + premiumMatch[1].slice(1).toLowerCase();
                                                  if (premium.toLowerCase() === 'fuel type' || premium.toLowerCase() === 'fuel grade') {
                                                    premium = 'Fuel Grade';
                                                  }
                                                ...
                                                if (dispenserMatch && dispenserMatch[1]) {
                                                  ...
                                                  // Extract the exact total from the message if available
                                                  if (dispenserMatch[2]) {
                                                    dispenserTotal = parseInt(dispenserMatch[2]);
                                                  }
                                                  // Otherwise use job.dispenserCount or fallback
                                                  else if (job.dispenserCount) {
                                                  ...
                                                  // Store the raw total string for display
                                                  // @ts-ignore
                                                  dispenser._rawTotal = dispenserMatch[2] || '';
                                                }
                                              }
                                              ...
                                              const calculatedDispenserPercent = dispenserCurrent > 0 && authoritativeTotalForPercent > 0
                                                ? Math.round((dispenserCurrent / authoritativeTotalForPercent) * 100)
                                                : 0;
                                              const dispenserPercent = Math.min(calculatedDispenserPercent, 100);
                                              ...
                                              return (
                                                <div className="flex flex-col h-24 justify-center">
                                                  {/* Always render the dispenser block with conditional visibility */}
                                                  <div className={`relative ${dispenser ? 'opacity-100' : 'opacity-0'} h-8 mb-3`}>
                                                    <div className="flex items-center justify-between mb-1">
                                                      ...
                                                      <div className="flex items-center space-x-2">
                                                        <span className="text-xs font-mono bg-gray-100 dark:bg-gray-700 px-1.5 py-0.5 rounded text-gray-700 dark:text-gray-300">
                                                          {job.message && job.message.match(/(?:dispenser|pump|meter)(?:[\s#:]+)(\d+)(?:[^\d]+(\d+)|)/i) ?
                                                            `#${job.message.match(/(?:dispenser|pump|meter)(?:[\s#:]+)(\d+)(?:[^\d]+(\d+)|)/i)[1]}${job.message.match(/(?:dispenser|pump|meter)(?:[\s#:]+)(\d+)(?:[^\d]+(\d+)|)/i)[2] ? 
                                                              `/${job.message.match(/(?:dispenser|pump|meter)(?:[\s#:]+)(\d+)(?:[^\d]+(\d+)|)/i)[2]}` : ''}`
                                                            : `#${dispenserCurrent}`}
                                                        </span>
                                ...
                                <td className="px-4 py-3 text-sm text-gray-500 dark:text-gray-400">
                                  ...
                                  {job.status === 'completed' && job.startTime && job.endTime && (
                                    <div className="mt-2 ml-10 flex items-center text-xs text-green-500 dark:text-green-400 bg-green-50 dark:bg-green-900/20 px-2 py-1 rounded-md">
                                  ...
                                  {job.status === 'error' && job.startTime && job.endTime && (
                                    <div className="mt-2 ml-10 flex items-center text-xs text-red-500 dark:text-red-400 bg-red-50 dark:bg-red-900/20 px-2 py-1 rounded-md">
                                      <svg className="w-3.5 h-3.5 mr-1.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
