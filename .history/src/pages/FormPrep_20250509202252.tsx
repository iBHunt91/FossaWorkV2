    ...
    const poll = async () => {
        ...
        setBatchJobs(prevJobs => {
          ...
          if (response.dispenserCurrent !== undefined) {
            ...
            // If the message provides a total and it seems more accurate (e.g. not a placeholder and different from initial API count)
            if (dispenserProgressFromMessage.total > 0 && 
                dispenserProgressFromMessage.total !== 100 && 
                jobToUpdate.formsTotal !== dispenserProgressFromMessage.total) {
                // This logic should be reviewed: potentially prioritize API's dispenserCount for total.
                // For now, keeping similar to existing logic but favoring API direct values first.
                // jobToUpdate.formsTotal = dispenserProgressFromMessage.total; 
            }
          } else {
            // If no direct current and no status message, ensure it's not undefined
