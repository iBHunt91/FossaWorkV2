...
const HomeContent = () => {
  ...
  const renderVerificationPanel = () => {
    ...
    const generateRealWarnings = () => {
      ...
      filteredWorkOrders.forEach(order => {
        ...
        if (visitDate) {
          const dateStr = new Date(visitDate).toDateString();
          const key = `${storeName}-${dateStr}`;
          
          if (storeVisitDates.has(key)) {
            warnings.push(`Potential schedule overlap for ${storeName} on ${new Date(visitDate).toLocaleDateString()}`);
          } else {
            storeVisitDates.set(key, true);
          }
        }
      });
      ...
      if (ordersWithoutDispensers.length > 0) {
        warnings.push(`Missing dispenser data for ${ordersWithoutDispensers.length} work orders`);
      }
  ...
  const formatChangeItem = (change: ChangeItem, timestamp: Date) => {
    ...
    const formattedChange = {
      id: change.jobId || change.removedJobId || change.addedJobId || 'Unknown',
      store: change.storeName || change.store || change.removedStoreName || change.addedStoreName || 'Unknown Store',
      type: change.type || 'updated',
      date: timestamp.toISOString(),
      reason: `Work order ${change.type}`
    };
    
    // Add type-specific details
    if (change.type === 'removed') {
      formattedChange.reason = 'Work order removed';
    } else if (change.type === 'added') {
      formattedChange.reason = 'Work order added';
      if (change.date) {
        formattedChange.reason += ` on ${new Date(change.date).toLocaleDateString()}`;
      }
    } else if (change.type === 'date_changed' || change.type === 'modified') {
      formattedChange.reason = 'Work order updated';
      if (change.oldDate && change.newDate) {
        formattedChange.reason = `Work order rescheduled from ${new Date(change.oldDate).toLocaleDateString()} to ${new Date(change.newDate).toLocaleDateString()}`;
      }
    }
