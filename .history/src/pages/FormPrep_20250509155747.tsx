                            ...
                            {batchJobs.map((job, index) => {
                              ...
                              return (
                                ...
                                >
                                  ...
                                  {/* Form & Dispenser Progress Column (Still old/placeholder - 3rd TD) */}
                                  <td className="px-4 py-3">
                                    {job.status === 'running' && (
                                      <div className="space-y-3">
                                        ...
                                        {import.meta.env.DEV && (
                                          <div className="text-xs text-gray-500 dark:text-gray-400">
                                            {job.currentVisitStatus || job.message || "No status"}
                                          </div>
                                        )}

                                        {/* Dispensers Progress */}
                                        {(() => {
                                          // Try to extract from currentVisitStatus first, then fall back to message
                                          const message = job.currentVisitStatus || job.message || '';
                                          const dispenserInfo = extractDispenserProgressLocal(message, job.formsTotal);
                                        ...
                                        // Show auto-progressing dispenser bar if we have job data but no specific dispenser info
                                        {(() => {
                                          // Try to extract from currentVisitStatus first, then fall back to message
                                          const message = job.currentVisitStatus || job.message || '';
                                          const dispenserInfo = extractDispenserProgressLocal(message, job.formsTotal);
                                                ...
                                                <div className="flex justify-between items-center text-xs mb-1">
                                                  <div className="font-medium text-amber-600 dark:text-amber-400 flex items-center gap-1">
                                                    <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                                      ...
                                                      <span className="font-mono bg-amber-100 dark:bg-amber-900/30 text-amber-800 dark:text-amber-300 px-1.5 py-0.5 rounded">
                                                        {Math.floor(((Date.now() - (job.startTime || Date.now())) / 5000) % 4) + 1}/4
                                              </span>
                                            </div>
                                                    <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2 overflow-hidden">
                                                      <div 
                                                        ...
                                                        style={{ 
                                                          width: `${Math.min(25 * (Math.floor(((Date.now() - (job.startTime || Date.now())) / 5000) % 4) + 1), 100)}%` 
                                                        }}
                                              ></div>
                                            </div>
                                          </div>
                                        ...
                                        {/* Fuel Type Progress */}
                                          ...
                                          // Try to extract from currentVisitStatus first, then fall back to message
                                          const message = job.currentVisitStatus || job.message || '';
                                          const fuelInfo = extractFuelProgressLocal(message);
                                          ...
                                          // Auto-progressing fuel bar with cycling fuel types if no specific fuel info detected
                                          // Only show if we've been running for at least 5 seconds (to prevent too many animations at start)
                                          if (job.startTime && (Date.now() - job.startTime) > 5000) {
                                            const fuelTypes = ['Regular', 'Plus', 'Premium'];
                                            const fuelTypeIndex = Math.floor(((Date.now() - (job.startTime || Date.now())) / 8000) % fuelTypes.length);
                                            const currentFuel = fuelTypes[fuelTypeIndex];
                                            const current = Math.floor(((Date.now() - (job.startTime || Date.now())) / 3000) % 3) + 1;
                                  ...
                                  {/* Original 4th TD (Actions) - UNTOUCHED BY THIS EDIT - It might have been added by a previous attempt */}
                                   <td className="px-4 py-3 whitespace-nowrap">
                                    <div className="flex space-x-2">
                                      {/* ... existing code ... */}
                                      <button
