                    ...
                    {Object.entries(instancesByVisit).map(([visitId, visitInstances], i) => {
                      ...
                      const unknownGradeDetails = visitInstances
                        ...
                        .map(instance => {
                          ...
                          const gradeMatch = warningMsg.match(/Unknown grade: ["']?([\w\s\d-]+)["']?/i);
                          const dispenserMatch = warningMsg.match(/(?:dispenser|disp|pump)[\s:#]*(\d+(?:\/\d+)?)/i);
                      ...
                      return (
                        <li key={`${groupKey}-${visitId}-${i}`} className="text-xs text-gray-600 dark:text-gray-400">
                          ...
                          {unknownGradeDetails.length > 0 && (
                            <div className="mt-2 space-y-1 bg-yellow-50 dark:bg-yellow-900/20 p-2 rounded-md border-yellow-200 dark:border-yellow-800/50 border">
                              <div className="font-medium text-yellow-700 dark:text-yellow-400">Unknown Grade Details:</div>
