...
const Schedule: React.FC = () => {
  ...
  const renderJobRow = (order: WorkOrder, dateGroupClass?: string) => {
    ...
    const rawProcessedInstructions = processInstructions(order.instructions);
    
    // Custom instruction preview logic (attempt to match Home.tsx)
    let instructionPreview = '';
    if (rawProcessedInstructions && rawProcessedInstructions !== 'No specific instructions provided.' && rawProcessedInstructions !== 'No instructions provided.') {
      const lines = rawProcessedInstructions.split('\n');
      const filteredLines = lines.filter(line => {
        const trimmedLine = line.trim();
        // Filter out empty lines, lines with only symbols like '---' or '***', or very short lines
        return trimmedLine.length > 2 && !trimmedLine.match(/^[-*=\s]+$/);
      });
      instructionPreview = filteredLines.slice(0, 3).join('\n'); // Take first 3 meaningful lines
      ...
      >
        ...
        <div className="flex flex-wrap gap-2 items-center justify-start pt-2 border-t border-gray-200 dark:border-gray-700">
          <button
            ...
            disabled={(!order.instructions && rawProcessedInstructions.includes('No specific instructions provided.')) || operationLoading[`instr_${order.id}`]}
