...
const HomeContent = () => {
  ...
  const renderMapPanel = () => {
    ...
    // SUPER DETAILED LOGGING - dump the full structure of the first 3 orders
    console.log('========== FULL ORDER STRUCTURE DEBUGGING ==========');
    inRangeOrders.slice(0, 3).forEach((order, index) => {
      console.log(`----------- FULL ORDER ${index + 1} DUMP -----------`);
      console.log(JSON.stringify(order, null, 2));
      
      // Walk through the object tree to find ANY property that might contain lat/lng or latitude/longitude
      console.log(`----------- LOCATION PROPERTY SEARCH FOR ORDER ${order.id} -----------`);
      const findLocationProperties = (obj: any, path = '') => {
        if (!obj || typeof obj !== 'object') return;
        
        // Check if this object has lat/lng or latitude/longitude directly
        const hasLat = obj.lat !== undefined || obj.latitude !== undefined;
        const hasLng = obj.lng !== undefined || obj.longitude !== undefined;
        
        if (hasLat && hasLng) {
          const lat = obj.lat !== undefined ? obj.lat : obj.latitude;
          const lng = obj.lng !== undefined ? obj.lng : obj.longitude;
          console.log(`FOUND POTENTIAL LOCATION at ${path}: lat=${lat}, lng=${lng}`);
        }
        
        // Check if this object has location, coordinates, or geo in its name
        if (path.toLowerCase().includes('location') || 
            path.toLowerCase().includes('coordinates') || 
            path.toLowerCase().includes('geo') ||
            path.toLowerCase().includes('position')) {
          console.log(`POTENTIAL LOCATION OBJECT at ${path}:`, obj);
        }
        
        // Recursively check all properties
        Object.keys(obj).forEach(key => {
          if (obj[key] !== null && typeof obj[key] === 'object') {
            findLocationProperties(obj[key], path ? `${path}.${key}` : key);
          }
        });
      };
      
      findLocationProperties(order);
    });
    console.log('================================================');
    ...
    // Update the mapping function to be even more aggressive in looking for coordinates
    const locationsToShow: Location[] = inRangeOrders
      .map(order => {
          // Create a deep search function to find lat/lng or latitude/longitude in any depth
          function deepFindCoordinates(obj: any): {lat?: number, lng?: number} | null {
            ...
            // Case 2: Direct latitude/longitude properties
            if (typeof obj.latitude === 'number' && typeof obj.longitude === 'number') {
              return { lat: obj.latitude, lng: obj.longitude };
            }
            
            // Case 3: For Mapbox format (reversed order)
            if (Array.isArray(obj.coordinates) && obj.coordinates.length === 2 && 
                typeof obj.coordinates[0] === 'number' && typeof obj.coordinates[1] === 'number') {
              return { lng: obj.coordinates[0], lat: obj.coordinates[1] };
            }
            
            // Case 4: For GeoJSON format
            ...
            for (const key in obj) {
              if (obj[key] !== null && typeof obj[key] === 'object') {
                const result = deepFindCoordinates(obj[key]);
          ...
          // Try every possible approach to get coordinates
          
          // Step 1: Deep search the entire object
          const deepSearchResult = deepFindCoordinates(order);
          if (deepSearchResult && typeof deepSearchResult.lat === 'number' && typeof deepSearchResult.lng === 'number' &&
              !isNaN(deepSearchResult.lat) && !isNaN(deepSearchResult.lng) &&
              deepSearchResult.lat >= -90 && deepSearchResult.lat <= 90 && 
              deepSearchResult.lng >= -180 && deepSearchResult.lng <= 180) {
            console.log(`[renderMapPanel] DEEP SEARCH found coordinates for job ${order.id}: (${deepSearchResult.lat}, ${deepSearchResult.lng})`);
            
            return {
              id: order.id,
              lat: deepSearchResult.lat,
              lng: deepSearchResult.lng,
          ...
          // Step 2: Try the systematic approach to check common patterns as before
          ...
          if (extractedLat !== undefined && extractedLng !== undefined &&
              typeof extractedLat === 'number' && typeof extractedLng === 'number' && 
              !isNaN(extractedLat) && !isNaN(extractedLng) &&
              extractedLat >= -90 && extractedLat <= 90 && 
              extractedLng >= -180 && extractedLng <= 180) {
              
            console.log(`[renderMapPanel] SYSTEMATIC SEARCH found coordinates for job ${order.id}: (${extractedLat}, ${extractedLng})`);
          ...
          // Step 3: Use geocoding fallback database
          const storeNumber = order.customer?.storeNumber || order.storeNumber || '';
          const storeNumberClean = storeNumber.replace(/#/g, '').trim();
          const workOrderId = order.id.replace(/^W-/i, '');
          const workOrderIdWithW = `W-${workOrderId}`;
          
          console.log(`[renderMapPanel] Trying fallback geocoding for ${order.id}, Store #${storeNumberClean}`);
          console.log(`   > Checking keys: '${workOrderId}', '${workOrderIdWithW}', '${order.id}', '${storeNumberClean}'`);
          ...
          let fallbackLocation = 
                ...
                storeGeocodingFallback[storeNumberClean];
          
          if (fallbackLocation) {
              console.log(`[renderMapPanel] FALLBACK found coordinates for job ${order.id}: (${fallbackLocation.lat}, ${fallbackLocation.lng})`);
              
              return {
                id: order.id,
                lat: fallbackLocation.lat,
                lng: fallbackLocation.lng,
                title: `${getDisplayName(order)} (Geocoded Location)`,
                storeType: getStoreTypeForFiltering(order),
                isFallback: true
              } as Location;
          }
          ...
          console.log(`[renderMapPanel] NO LOCATION found for job ${order.id}`);
