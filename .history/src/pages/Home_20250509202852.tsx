...
const HomeContent = () => {
  ...
  const [calendarViewType, setCalendarViewType] = createState<'month' | 'week'>('calendarViewType', 'month');
      ...
      >
        <div className={`flex justify-between items-center px-4 py-3 ${storeStyle.headerBg} border-b border-gray-200 dark:border-gray-700`}>
          <div className="flex items-center space-x-2">
            ...
            <h3 className={`text-lg font-semibold truncate max-w-[200px] sm:max-w-xs ${storeStyle.text}`}> {/* Applied storeStyle.text */}
        ...
        <div className="p-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-3">
            {hasNextVisit && (
              <div className="flex items-start">
                <div className="flex-shrink-0 mt-0.5 mr-2">
                  <div className={`p-1 rounded-full ${storeStyle.badge}`}> {/* Added badge background */}
                    <FiCalendar className={`h-3 w-3 ${storeStyle.text}`} /> {/* Adjusted icon size, kept storeStyle.text */}
                  </div>
                </div>
                <div>
                  <div className="text-xs text-gray-500 dark:text-gray-400 font-medium mb-0.5">Visit Date</div>
                  <div className={`text-sm ${storeStyle.text}`}>{formattedVisitDate}</div> {/* Applied storeStyle.text */}
                </div>
              </div>
            )}
            ...
            {dispenserQty > 0 && (
              <div className="flex items-start"> 
                <div className={`flex-shrink-0 mt-0.5 mr-2 p-1 rounded-full ${storeStyle.badge}`}> {/* Added badge background to existing wrapper */}
                  <GiGasPump className={`h-4 w-4 ${storeStyle.text}`} /> {/* Icon size was h-5, adjusted to h-4 to fit p-1 padding better */}
                </div>
                <div>
                  {/* Removed the explicit "Equipment" label */}
                  <div className={`text-sm ${storeStyle.text}`}> {/* Applied storeStyle.text */}
          ...
          <div className="mt-3 flex flex-wrap items-center justify-between border-t border-gray-200 dark:border-gray-700 pt-3">
            <div className="flex items-center">
              <div className={`p-1 rounded-full mr-1.5 ${storeStyle.badge}`}> {/* Added badge background */}
                 <FiFileText className={`h-3 w-3 ${storeStyle.text}`} /> {/* Adjusted icon size, kept storeStyle.text */}
              </div>
              <span className={`text-xs font-medium ${storeStyle.text}`}>Custom Instructions</span>
            </div>
          ...
          {instructions && (
            <div className="mt-2 text-center"> {/* Centered the preview container */}
              <p className={`text-sm italic whitespace-normal break-words ${storeStyle.text} ${!isDarkMode ? 'opacity-80' : 'opacity-90'}`}> {/* Applied storeStyle.text, adjusted opacity */}
                {instructions} {/* Removed quotes around instructions variable */}
              </p>
            </div>
          )}
          ...
          <div className="mt-4 flex items-center justify-end"> 
            ...
            <div className="flex items-center space-x-2"> 
              ...
              {order.visits?.nextVisit?.url && (
                ...
                >
                  <FiExternalLink className={`h-5 w-5 ${storeStyle.text}`} /> {/* Applied storeStyle.text */}
                </button>
              )}
            
              {/* View Dispensers Icon Button */}
              <button
                className={`p-2 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 dark:focus:ring-offset-gray-800 focus:ring-blue-500 transition-colors ${hasDispenserData ? 'hover:' + storeStyle.text : 'text-gray-400 dark:text-gray-500 cursor-not-allowed' // Indicate no data
                }`}
              ...
              >
                <GiGasPump className={`h-5 w-5 ${hasDispenserData ? storeStyle.text : ''}`} /> {/* Applied storeStyle.text conditionally */}
              </button>
              ...
              >
                {clearingDispenserId === order.id ? (
                  <FiRefreshCw className={`h-5 w-5 animate-spin ${storeStyle.text}`} /> /* Applied storeStyle.text */
                ) : (
                  <FiTrash2 className={`h-5 w-5 ${storeStyle.text}`} /> /* Applied storeStyle.text */
                )}
              </button>
              ...
              >
                {reScrapeDispenserId === order.id ? (
                  ...
                  <FiRefreshCw className={`h-5 w-5 ${storeStyle.text}`} /> /* Applied storeStyle.text */
                )}
              </button>
            </div>
          </div>
            ...
            {activeView === 'calendar' && (
              <CalendarView 
                ...
                onEventClick={(event) => {
                  ...
                  if (order) {
                    // Only set the visit number, but don't show any modal
                    setSelectedVisitNumber(extractVisitNumber(order));
                    
                    // Display a toast notification instead of showing the modal
                    addToast('info', `Selected job: ${order.customer?.name || 'Unknown Store'}`, 2000);
                    
                    // No longer showing instructions modal
