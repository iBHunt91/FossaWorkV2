...
const FormPrep: React.FC<{}> = () => {
  ...
  const downloadLogs = () => {
    try {
      ...
      // Hard-code logs path to project directory 
      const logsPath = 'logs';  // This will save to the logs directory in the project root
      
      // Save file to the specified directory using Electron IPC if available
      if (window.electron && typeof window.electron.saveFile === 'function') {
        addDebugLog('SYSTEM', 'Saving logs via Electron', { path: logsPath, filename });
        
        window.electron.saveFile({
          filename,
          content: jsonString,
          path: logsPath,  // Use logs directory in project root
        }).then((result) => {
          if (result.success) {
            addDebugLog('SYSTEM', 'Logs saved successfully', { path: result.path });
            addToast('success', `Debug logs saved to ${result.path}`);
          } else {
            addDebugLog('SYSTEM', 'Failed to save logs through Electron', result);
            addToast('error', `Failed to save logs: ${result.error || 'Unknown error'}`);
            // Fall back to browser download
            saveThroughBrowser();
          }
        }).catch((error) => {
          addDebugLog('SYSTEM', 'Error saving logs through Electron', error);
          addToast('error', 'Error saving logs: ' + (error.message || 'Unknown error'));
          // Fall back to browser download
          saveThroughBrowser();
        });
      } else {
        // If Electron API is not available, fall back to browser download
        addDebugLog('SYSTEM', 'Electron saveFile API not available, falling back to browser download');
        saveThroughBrowser();
      }

      // Browser download fallback function
      function saveThroughBrowser() {
        try {
          ...
          addDebugLog('SYSTEM', 'Logs downloaded through browser');
          addToast('success', 'Debug logs downloaded through browser (Electron API unavailable)');

          // Also save filename in localStorage for reference
          try {
            const previousLogs = JSON.parse(localStorage.getItem('debug_log_history') || '[]');
            previousLogs.unshift({
              filename,
              timestamp: new Date().toISOString(),
              size: jsonString.length
            });
            // Keep only the last 10 logs in history
            if (previousLogs.length > 10) {
              previousLogs.length = 10;
            }
            localStorage.setItem('debug_log_history', JSON.stringify(previousLogs));
          } catch (e) {
            console.error('Error saving log history to localStorage:', e);
          }
        } catch (browserError) {
          console.error('Error creating browser download:', browserError);
          addToast('error', 'Failed to download logs through browser');
        }
      }
    } catch (error) {
  ...
  const extractDispenserProgressLocal = (message: string, totalDispensers?: number): { current: number; total: number } => {
    ...
    try {
      ...
      // Improved regex to capture more dispenser patterns
      const dispenserRegex = /(?:dispenser|pump|meter)(?:[\s#:]+)(\d+)(?:[^\d]+(\d+)|)/i;
      const directNumberRegex = /(\d+)\s*\/\s*(\d+)\s*dispensers?/i;
      ...
      if (directMatch && directMatch[1] && directMatch[2]) {
        // Direct format: "X/Y dispensers"
        const current = parseInt(directMatch[1]);
        const total = parseInt(directMatch[2]);
        return { current, total: Math.max(total, 1) };
      } else if (dispenserMatch && dispenserMatch[1]) {
          ...
          setBatchJobs(prev => {
            ...
            if (jobIndex !== -1) {
              ...
              if (status.status === 'idle' && jobToUpdate.status === 'running') {
                ...
                const hasRecentUpdate = status.lastUpdated && 
                  (new Date(status.lastUpdated).getTime() > currentTime - 60000); // Updated in last minute
                
                const hasProgress = 
                  (status.currentItem !== undefined && status.currentItem > 0) || 
              ...
              const isJobFinishedByServer = 
                ...
                status.status === 'error' || 
                (status.endTime && status.status === 'idle');
              ...
              let currentVisitStatus = status.currentVisitStatus || status.message || jobToUpdate.currentVisitStatus || '';
              
              // If the API gives us a completedVisitIds array, use the latest ID for context
              if (status.completedVisitIds && status.completedVisitIds.length > 0) {
                const latestVisitId = status.completedVisitIds[status.completedVisitIds.length - 1];
              ...
              if (status.currentItem !== undefined && status.totalItems) {
                formsCurrent = status.currentItem;
                formsTotal = status.totalItems;
              }
              ...
              if (status.progress !== undefined) {
                ...
                if (formsTotal > 0) {
                  formsCurrent = Math.floor((status.progress / 100) * formsTotal);
                }
              }
              ...
              const completedVisits = status.completedVisits !== undefined && status.completedVisits > 0
                ...
                : status.completedVisitIds && status.completedVisitIds.length > 0
                  ? status.completedVisitIds.length
              ...
              const recoveryJob: BatchJob = {
                ...
                totalVisits: status.totalVisits || status.completedVisitIds?.length || 1,
                completedVisits: status.completedVisits || 0,
                headless: true,
                jobId: currentBatchJobId,
                startTime: Date.now(),
                currentVisitStatus: status.currentVisitStatus || status.message || 'Reconnecting to batch job...'
              };
