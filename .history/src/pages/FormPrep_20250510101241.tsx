...
import type { UnifiedAutomationStatus } from '../services/formService';
...
const FormPrep: React.FC<{}> = () => {
  ...
  // Unified polling configuration for both single and batch jobs
  ...
  // Unified polling function that works for both single visits and batch jobs
  const startPollingJob = (jobId: string, isBatch: boolean) => {
    addDebugLog(isBatch ? 'BATCH' : 'SINGLE', `Starting unified polling for ${isBatch ? 'batch' : 'single visit'} job ${jobId}`);
    ...
    setPollingConfig(prev => ({
      ...
      jobId,
      isBatch,
      // We'll set the interval ID after creating it
    }));
    
    // Set up the local storage state based on job type
    if (isBatch) {
      // Batch job
      setBatchJobId(jobId);
      saveToStorage(userStorageKeys.BATCH_JOB_ID, jobId);
    } else {
      // Single job
      setSingleJobId(jobId);
      setPollingSingle(true);
      saveToStorage(userStorageKeys.SINGLE_JOB_ID, jobId);
      saveToStorage(userStorageKeys.IS_POLLING_SINGLE, true);
    }
    
    // Create and save the polling interval
    const newInterval = setInterval(async () => {
      try {
        // Use our unified status API
        const status = await getUnifiedAutomationStatus(jobId, isBatch);
        
        addDebugLog(isBatch ? 'BATCH' : 'SINGLE', `Received job status update`, status);
        ...
        if (isBatch) {
          ...
          setBatchJobs(prevJobs => {
            ...
            if (jobIndex === -1) {
              addDebugLog('BATCH', `Polling anomaly: Job ID ${jobId} not found in local state.`);
              return prevJobs;
            }
            
            const jobToUpdate = { ...prevJobs[jobIndex] };
            const prevStatus = jobToUpdate.status;
            
            // Update job status from the unified response
            jobToUpdate.status = status.status;
            jobToUpdate.message = status.message || jobToUpdate.message;
            
            // Update progress details
            jobToUpdate.completedVisits = status.completedVisits ?? jobToUpdate.completedVisits;
            jobToUpdate.totalVisits = status.totalVisits ?? jobToUpdate.totalVisits;
            jobToUpdate.currentVisitId = status.currentVisit || jobToUpdate.currentVisitId;
            ...
            // Update dispenser and fuel progress
            ...
            // Set end time if the job has completed or errored
            if (status.status === 'completed' || status.status === 'error') {
              jobToUpdate.endTime = Date.now();
              
              // Set notification flags
              jobToUpdate._completed = status.status === 'completed' && prevStatus !== 'completed';
              jobToUpdate._error = status.status === 'error' && prevStatus !== 'error';
            }
          ...
          // Update formJobs for single visit
          setFormJobs(prevJobs => {
            const jobIndex = prevJobs.findIndex(j => j.jobId === jobId);
            if (jobIndex === -1) {
              addDebugLog('SINGLE', `Polling anomaly: Job ID ${jobId} not found in local state.`);
              return prevJobs;
            }
            
            const prevStatus = prevJobs[jobIndex].status;
            ...
            const updatedJob = {
              ...
              message: status.message || prevJobs[jobIndex].message,
              endTime: endTime || prevJobs[jobIndex].endTime,
              // Set notification flags
        ...
        // If the job has completed or errored, stop polling
        if (status.status === 'completed' || status.status === 'error') {
          stopPollingJob(jobId, isBatch);
        }
        
      } catch (error) {
        console.error(`Error polling for ${isBatch ? 'batch' : 'single'} job status:`, error);
        addDebugLog(isBatch ? 'BATCH' : 'SINGLE', `Polling error: ${error instanceof Error ? error.message : 'Unknown error'}`);
      }
    }, 2000); // Poll every 2 seconds
    
    // Save the interval ID to the state
    setPollingConfig(prev => ({
      ...prev,
  ...
  const stopPollingJob = (jobId: string, isBatch: boolean) => {
    addDebugLog(isBatch ? 'BATCH' : 'SINGLE', `Stopping polling for job ${jobId}`);
    ...
    if (isBatch) {
      // Reset batch polling state
      setBatchJobId(null);
      saveToStorage(userStorageKeys.BATCH_JOB_ID, null);
    } else {
      // Reset single job polling state
      setSingleJobId(null);
  ...
  const extractFuelProgressLocal = (message: string): { current: number; total: number; type?: string } => {
    ...
    try {
      // Enhanced fuel type list with ethanol-free gasoline variants
      const fuelTypes = [
        'premium', 'regular', 'plus', 'diesel', 'supreme', 'unleaded', 
        'mid-grade', 'midgrade', 'super', 'ethanol-free', 'ethanol free', 
        'ethanol-free gasoline', 'ethanol free gasoline',
        'ethanol-free gasoline plus', 'ethanol free gasoline plus',
        'fuel type', 'fuel grade'
      ];
      
      // Pattern for handling Ethanol-Free specific patterns
      const ethanolFreePattern = /(?:processing\s+)?(?:ethanol-free\s+gasoline\s+plus)\s*\((\d+)\/(\d+)\)/i;
      const ethanolMatch = message.match(ethanolFreePattern);
      if (ethanolMatch && ethanolMatch[1] && ethanolMatch[2]) {
        return {
          current: parseInt(ethanolMatch[1]),
          total: parseInt(ethanolMatch[2]),
          type: "Ethanol-Free Gasoline Plus" // Fixed name to avoid fluctuation
        };
      }
      
      // First check for the most specific pattern: "Fuel Grade: Type (X/Y)"
      ...
      if (specificMatch && specificMatch[1] && specificMatch[2] && specificMatch[3]) {
        const type = specificMatch[1].trim();
        const current = parseInt(specificMatch[2]);
        const total = parseInt(specificMatch[3]);
        
        // Normalize fuel type names to prevent fluctuation
        let normalizedType = type.charAt(0).toUpperCase() + type.slice(1); // Capitalize first letter
        
        // Map common variations to standard names
        if (/regular/i.test(normalizedType)) normalizedType = "Regular";
        else if (/premium/i.test(normalizedType)) normalizedType = "Premium";
        else if (/plus/i.test(normalizedType)) normalizedType = "Plus";
        else if (/diesel/i.test(normalizedType)) normalizedType = "Diesel";
        else if (/supreme/i.test(normalizedType)) normalizedType = "Supreme";
        else if (/unleaded/i.test(normalizedType)) normalizedType = "Unleaded";
        else if (/mid-?grade/i.test(normalizedType)) normalizedType = "Mid-Grade";
        else if (/super/i.test(normalizedType)) normalizedType = "Super";
        else if (/ethanol[-\s]free/i.test(normalizedType)) normalizedType = "Ethanol-Free";
        
        return { current, total, type: normalizedType };
      }
      
      // Check for patterns like "Processing Premium (3/3)" or "Fuel Grade: Regular (2/4)"
      ...
      if (fuelMatch && fuelMatch[1] && fuelMatch[2] && fuelMatch[3]) {
        ...
        else if (/fuel\s+(?:type|grade)/i.test(normalizedType)) normalizedType = "Fuel Grade";
        
        return { current, total, type: normalizedType };
      }
      
      // Check for pattern with "Processing fuel types..." prefix
      const processingFuelPattern = /processing\s+fuel\s+types...\s*-\s*([a-z\-\s]+?)(?:\s*-\s*fuel\s+grade)?\s*(?:\((\d+)\/(\d+)\))?/i;
      const processingMatch = message.match(processingFuelPattern);
      if (processingMatch && processingMatch[1]) {
        const type = processingMatch[1].trim();
        let current = 0;
        let total = 0;
        
        // Try to find numbers in the message
        if (processingMatch[2] && processingMatch[3]) {
          current = parseInt(processingMatch[2]);
          total = parseInt(processingMatch[3]);
        } else {
          // Look for numbers elsewhere in the message
          const numberMatch = message.match(/\((\d+)\/(\d+)\)/);
          if (numberMatch && numberMatch[1] && numberMatch[2]) {
            current = parseInt(numberMatch[1]);
            total = parseInt(numberMatch[2]);
          }
        }
      ...
      // Check for more general "Fuel Type" or "Fuel Grade" patterns
      const generalFuelPattern = /(?:fuel\s+(?:type|grade))(?:[^(]*)\((\d+)\/(\d+)\)/i;
      ...
      if (generalMatch && generalMatch[1] && generalMatch[2]) {
        return {
          ...
          type: "Fuel Grade" // Generic type when specific fuel not mentioned
        };
      }
  ...
  useEffect(() => {
    ...
    // Clean up function
    return () => {
      // Clean up any polling intervals if component unmounts
      if (pollingConfig.interval) {
        clearInterval(pollingConfig.interval);
      }
    };
  }, [singleJobId, pollingSingle, batchJobId, userStorageKeys]);
  ...
  const updateJobStatus = (status: any) => {
    ...
    if (activeJobIndex !== -1) {
      ...
      if (status.status === 'running') {
        ...
        // Extract dispenser and fuel information for logging
        const dispInfoForLogs = extractDispenserProgressLocal(formattedMessage, dispenserCount);
        const fuelInfoForLogs = extractFuelProgressLocal(formattedMessage);
        
        // Update log to see extracted information
        // console.warn('Dispenser total mismatch (using totalDispensers): job.dispenserCount/formsTotal is ${totalDispensers}, message parsed total as ${parsedTotalFromMessage}.');
        
        // Extract fuel and dispenser info from the message for UI updating
        const fuelInfo = extractFuelProgressLocal(formattedMessage);
        const dispenserInfo = extractDispenserProgressLocal(formattedMessage, dispenserCount);
        
        // Only update the message if we have real progress to show
        if (fuelInfo.type && fuelInfo.total > 0) {
          ...
          if (dispenserMatch && dispenserMatch[1]) {
            ...
            const dispenserTotal = dispenserMatch[2] || jobToUpdate.dispenserCount || dispenserInfo.total || '?';
            
            // Create a stable status message to prevent UI jumping back and forth
            formattedMessage = `Processing ${fuelInfo.type} (${fuelInfo.current}/${fuelInfo.total}) - Dispenser #${dispenserNum}/${dispenserTotal}`;
          } else if (dispenserInfo.current > 0 && dispenserInfo.total > 0) {
            // If we have dispenser info from the extractDispenserProgressLocal function
            formattedMessage = `Processing ${fuelInfo.type} (${fuelInfo.current}/${fuelInfo.total}) - Dispenser #${dispenserInfo.current}/${dispenserInfo.total}`;
          } else {
            // Just show fuel info
            formattedMessage = `Processing ${fuelInfo.type} (${fuelInfo.current}/${fuelInfo.total})`;
          }
        }
      }
  ...
  const handleSingleVisit = async () => {
    try {
      ...
      // Start polling for status updates using our unified polling approach
      startPollingJob(result.jobId, false);
  ...
  const handleStopProcessing = async () => {
    try {
      ...
      if (jobIdToStop) {
        stopPollingJob(jobIdToStop, false);
      } else {
        // Fallback to the old way if needed
        if (polling) {
          clearInterval(polling);
          setPolling(null);
        }
  ...
  const handleStopBatchProcessing = async () => {
    try {
      ...
      // Stop polling using our unified approach
      if (jobIdToCancel) {
        stopPollingJob(jobIdToCancel, true);
      } else {
  ...
  const getStatusIcon = (status: 'idle' | 'running' | 'completed' | 'error', job?: BatchJob) => {
    // If job is provided and status is 'idle' but completed visits > 0, treat as 'running'
    if (job && status === 'idle' && job.completedVisits > 0) {
      return <FiRefreshCw className="h-4 w-4 text-primary-500 animate-spin" />;
    }
  ...
  // Clear form job history
  const handleClearFormJobs = () => {
    // Ask for confirmation
    if (window.confirm('Are you sure you want to clear all completed and errored form jobs?')) {
      // Filter out running jobs
      const runningJobs = formJobs.filter(job => job.status === 'running');
      setFormJobs(runningJobs);
      saveToStorage(userStorageKeys.FORM_JOBS, runningJobs);
      addToast('success', 'Form job history cleared');
    }
  };
  
  // Clear batch job history
  const handleClearBatchJobs = () => {
    // Ask for confirmation
    if (window.confirm('Are you sure you want to clear all completed and errored batch jobs?')) {
      // Filter out running jobs
      const runningJobs = batchJobs.filter(job => job.status === 'running');
      setBatchJobs(runningJobs);
      saveToStorage(userStorageKeys.BATCH_JOBS, runningJobs);
  ...
  const handleBatchProcess = async () => {
    ...
    try {
      ...
      // Start polling for batch status using our unified approach
      startPollingJob(batchResult.jobId, true);
    } catch (error) {
      ...
      // Batch polling is now handled by the unified startPollingJob function
                    ...
                    <div className="overflow-x-auto bg-white dark:bg-gray-800">
                        ...
                        <div className="text-center py-8 text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-800/50 rounded-lg">
                          ...
                          <thead className="bg-gray-50 dark:bg-gray-900/30">
                            <tr>
                              <th scope="col" className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider w-[140px]">Customer</th>
                              <th scope="col" className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider w-[140px]">Location</th>
                              <th scope="col" className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider w-[100px]">Date</th>
                              <th scope="col" className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider w-[100px]">Dispensers</th>
                              <th scope="col" className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider w-[100px]">Actions</th>
                                    </tr>
                                  </thead>
                  ...
                  <div className="border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700/50 px-4 py-3">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {formJobs.length > 0 && (
                          <button 
                            onClick={handleClearFormJobs}
                            className="flex items-center px-2.5 py-1 text-xs font-medium rounded text-red-700 bg-red-50 hover:bg-red-100 dark:bg-red-900/30 dark:text-red-400 dark:hover:bg-red-800/40"
                          >
                  ...
                  <div className="overflow-x-auto">
                    {formJobs.length === 0 ? (
                      <div className="text-center py-8 text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-800/50 rounded-lg">
                        ...
                        <thead className="bg-gray-50 dark:bg-gray-900/30">
                          <tr>
                            <th scope="col" className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider w-[140px]">Status</th>
                            <th scope="col" className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider w-[150px]">Store</th>
                            <th scope="col" className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider w-[100px]">Visit</th>
                            <th scope="col" className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider w-[300px]">Progress</th>
                            <th scope="col" className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider w-[120px]">Actions</th>
                            </tr>
                          </thead>
                        <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                      ...
                      <div className="overflow-hidden rounded-lg border border-gray-200 dark:border-gray-700">
                        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700 table-fixed">
                          <thead className="bg-gray-50 dark:bg-gray-700/50">
                            <tr>
                              <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider w-[140px]">Status</th>
                              <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider w-[200px]">Visit Progress</th>
                              <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider w-[300px]">Form & Dispenser Progress</th>
                              <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider w-[120px]">Actions</th>
                            </tr>
                          </thead>
                          <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                            {batchJobs.map((job, index) => {
                              const currentVisitMessage = job.currentVisitStatus || job.message || '';
                              // Uncomment and use these for progress tracking
                              const dispenserInfo = extractDispenserProgressLocal(currentVisitMessage, job.formsTotal);
                              const fuelInfo = extractFuelProgressLocal(currentVisitMessage);
                              ...
                              >
                                ...
                                <td className="px-4 py-3 whitespace-nowrap">
                                  <div className="flex items-center">
                                    ...
                                    {/* Detailed status message for running jobs */}
                                    {(job.status === 'running' || (job.status === 'idle' && job.completedVisits > 0)) && currentVisitMessage && (
                                      <div 
                                        className="text-xs text-gray-500 dark:text-gray-400 mt-1 truncate max-w-xs" 
                                ...
                                <td className="px-4 py-3 whitespace-nowrap">
                                    ...
                                    {(job.status === 'running' || (job.status === 'idle' && job.completedVisits > 0)) && (
                                ...
                                <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400 w-1/3">
                                  ...
                                  {(job.status === 'running' || (job.status === 'idle' && job.completedVisits < job.totalVisits && job.totalVisits > 0 && batchJobId === job.jobId)) && 
                                   (!job.formsTotal && !job.formsCurrent && !job.currentVisitFuelType && 
                                    ...
                                    <div className="space-y-3 min-w-56 min-h-16">
                                      {/* Generic progress bar when no specific data is available */}
                                      <div>
                                        <div className="flex justify-between items-center text-xs mb-1">
                                          <div className="font-medium text-amber-600 dark:text-amber-400 flex items-center gap-1">
                                            <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                        ...
                                        <div className="w-full bg-gray-200 dark:bg-gray-700/60 rounded-full h-1.5 overflow-hidden">
                                          <div className="h-1.5 bg-amber-500 dark:bg-amber-600/70 rounded-full animate-pulse-progress" style={{ width: '60%' }}></div>
                                        </div>
                                      </div>
