...
const FormPrep: React.FC<{}> = () => {
  ...
  // Add openUiControls function near other UI-related functions
  const openUiControls = () => {
    // Show a dialog or modal with additional UI controls
    // You could repurpose an existing modal or create a new one
    if (visitUrl) {
      openUrlWithLogin(visitUrl);
    } else {
      addToast('info', 'Please enter a visit URL first');
    }
  };

  return (
    <div className="form-prep-container">
      <div className="header border-b border-gray-200 dark:border-gray-700 pb-4 mb-6">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100 flex items-center">
          <FiFileText className="mr-2 text-primary-500" />
          Form Automation
        </h1>
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
          Prepare and automate forms for site visits
        </p>
      </div>

      <div className="tabs flex border-b border-gray-200 dark:border-gray-700 mb-4">
        <button
        ...
        >
          <FiPlay className="inline-block mr-1" />
          Single Form
        </button>
        <button
      ...
      {activeTab === 'single' && (
        <div className="form-section">
            ...
            <div className="flex space-x-2">
              <div className="relative flex-grow">
                ...
                <input
                  ...
                  placeholder="Enter URL of the visit form page"
                  className="block w-full pl-10 pr-3 py-2 rounded-md border-gray-300 dark:border-gray-600 shadow-sm focus:ring-primary-500 focus:border-primary-500 sm:text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100"
                />
              </div>
              <button
                onClick={() => {
          ...
          <div className="controls-container mb-4 flex flex-wrap space-x-2 items-center">
            <div className="headless-option flex items-center">
              <input
                id="headless"
                type="checkbox"
                checked={isHeadless}
              ...
              />
              <label htmlFor="headless" className="ml-2 block text-sm text-gray-700 dark:text-gray-300">
          ...
          <div className="work-order-section mt-8">
            <h3 className="text-lg font-medium text-gray-900 dark:text-gray-100 mb-2">Recent Work Orders</h3>
            ...
            <div className="week-navigation flex items-center justify-between mb-2">
              <div className="flex items-center">
                <button
                ...
                <span className="mx-2 text-sm font-medium text-gray-700 dark:text-gray-300">
                  {groupedWorkOrders.length > 0 && currentWeekIndex < groupedWorkOrders.length
                    ? groupedWorkOrders[currentWeekIndex].week
                    : 'No work orders'}
                </span>
                <button
                  ...
                  className={`p-1 rounded-full ${
                    currentWeekIndex === groupedWorkOrders.length - 1
                      ? 'text-gray-400 cursor-not-allowed'
                      : 'text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-800'
                  }`}
                >
            ...
            <div className="work-order-list max-h-96 overflow-y-auto border border-gray-200 dark:border-gray-700 rounded-md">
              {isLoading ? (
                ...
                <ul className="divide-y divide-gray-200 dark:divide-gray-700">
                  {filteredWorkOrders.map((order) => {
                    // Parse URL and determine if it's selected
                    const visitUrl = order.visits?.nextVisit?.url;
                    const fullVisitUrl = visitUrl
                    ...
                    const isSelected = selectedWorkOrder && selectedWorkOrder.id === order.id;
                      ...
                      >
                        <div className="px-4 py-4 flex items-start sm:px-6">
                          <div className="flex-grow min-w-0">
                            ...
                            <p className="mt-1 text-xs text-gray-500 dark:text-gray-400 truncate flex flex-wrap gap-2">
                              {order.visits?.nextVisit?.date && (
                                <span className="flex items-center">
                                  <FiClock className="mr-1" />
                                  {formatDate(order.visits.nextVisit.date)}
                                  {order.visits.nextVisit.time && (
                                    <span className="ml-1">{order.visits.nextVisit.time}</span>
                                  )}
                                </span>
                              )}
                              ...
                              {order.dispensers && order.dispensers.length > 0 && (
                                <span className="flex items-center">
                                  ...
                                  {order.dispensers.length} dispenser{order.dispensers.length !== 1 ? 's' : ''}
                                </span>
                              )}
                            </p>
                          </div>
                          <div className="ml-5 flex-shrink-0 flex">
                            {visitUrl && (
                              <button
                                onClick={(e) => {
                                  e.stopPropagation(); // Don't select the work order
                                  openUrlWithLogin(fullVisitUrl);
                                }}
                  ...
                  {searchTerm ? (
                    <>
                      ...
                      <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                        We couldn't find any work orders matching your search.
                      </p>
                    </>
                  ) : (
                    <>
                      <FiInfo className="mx-auto h-12 w-12 text-gray-400" />
                      <h3 className="mt-2 text-sm font-medium text-gray-900 dark:text-gray-100">No work orders</h3>
                      <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                        There are no work orders available for this week.
                      </p>
                    </>
                  )}
                </div>
      ...
      {activeTab === 'batch' && (
        <div className="batch-section">
          <div className="bg-white dark:bg-gray-800 shadow-sm rounded-lg border border-gray-200 dark:border-gray-700 p-4 mb-6">
            <h3 className="text-lg font-medium text-gray-900 dark:text-gray-100 mb-2">Batch Processing</h3>
            <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
              Select and process multiple visits at once.
            </p>
            ...
            <div className="flex flex-col space-y-4">
              ...
              <div className="flex items-center gap-4">
                <div className="flex items-center space-x-2">
                  ...
                  <select
                    className="rounded-md border-gray-300 dark:border-gray-600 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100"
                    value={groupBy}
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    className="inline-flex items-center px-3 py-1.5 border border-gray-300 dark:border-gray-600 text-sm font-medium rounded-md shadow-sm text-gray-700 dark:text-gray-200 bg-white dark:bg-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500"
      ...
      {formJobs.length > 0 && (
        <div className="status-section mt-8">
          <h3 className="text-lg font-medium text-gray-900 dark:text-gray-100 mb-4">Recent Jobs</h3>
          ...
          <div className="overflow-hidden rounded-lg border border-gray-200 dark:border-gray-700 divide-y divide-gray-200 dark:divide-gray-700">
            {formJobs.slice(0, 5).map((job, index) => {
              // Get the visit number from the URL
              const visitNumber = extractVisitNumber(job.url) || job.visitNumber || 'Unknown';
              ...
              return (
                <div
                  ...
                  className={`bg-white dark:bg-gray-800 p-4 ${
                    job.status === 'running' ? 'animate-pulse-slow' : ''
                  }`}
                >
                  <div className="flex items-center">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-gray-900 dark:text-gray-100 flex items-center">
                        {job.storeName || 'Visit'} <span className="px-2 text-gray-400 dark:text-gray-500">•</span>
                        <span className="font-mono text-xs text-gray-500 dark:text-gray-400">{visitNumber}</span>
                        {job.dispenserCount > 0 && (
                          <span className="ml-2 text-xs bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400 px-2 py-0.5 rounded-full">
                            {job.dispenserCount} dispensers
                          </span>
                        )}
                      </p>
                      <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                        {job.message || (
                          job.status === 'completed' ? 'Processing completed' :
                          job.status === 'running' ? 'Processing...' :
                          job.status === 'error' ? 'Error during processing' :
                          'Waiting to start'
                        )}
                      </p>
                    </div>
          ...
          {formJobs.length > 5 && (
            <div className="text-center mt-2">
              <button
                onClick={() => {}} // Would expand to show more
                className="text-sm text-primary-600 dark:text-primary-400 hover:text-primary-700 dark:hover:text-primary-500"
              >
                Show {formJobs.length - 5} more jobs
              </button>
            </div>
          )}
        </div>
