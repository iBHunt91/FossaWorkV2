...
const JobMapView: React.FC = () => {
  ...
  const [routeWaypoints, setRouteWaypoints] = useState<Array<GeocodedJob | CustomLocation>>([]);
  ...
  const calculateRoute = useCallback(async () => {
    if (routeWaypoints.length < 2 || !jobMapRef.current) return;
    ...
    try {
      ...
      if (routeWaypoints.length > 2) {
        ...
        const route = await routingService.calculateMultiStopRoute(routeWaypoints);
        
        // Show the route on map
        await jobMapRef.current.showMultiPointRoute(routeWaypoints, route);
        ...
        const [origin, destination] = routeWaypoints;
  ...
  useEffect(() => {
    if (isRoutingMode && routeWaypoints.length >= 2) {
      // Check if we have at least two distinct locations
      const locationIds = new Set(routeWaypoints.map(wp => wp.id));
  ...
  const addWaypointSlot = () => {
    setRouteWaypoints([...routeWaypoints, undefined as any]);
  };
                        ...
                        <select
                          ...
                          onChange={(e) => {
                            ...
                            if (value === "add-custom") {
                              ...
                              const newWaypoints = [...routeWaypoints];
                              newWaypoints[index] = selectedJob || customLocation || undefined;
                              setRouteWaypoints(newWaypoints);
