...
const FormPrep: React.FC<{}> = () => {
  ...
  const formatDisplayMessage = (message: string | undefined): string => {
    ...
    // 1. Specific, non-progress states (should be shown fully and take precedence)
    // These are distinct actions or states not primarily represented by a progress bar.
    if (lowerMessage.startsWith('processing visit:')) return 'Processing visit...';
    if (lowerMessage.startsWith('navigating to visit:')) return 'Navigating to visit...';
    ...
    if (lowerMessage.includes('adding forms') || lowerMessage.includes('creating forms')) return 'Preparing forms for visit...';
    if (lowerMessage.includes('saving visit details')) return 'Saving visit details...';

    // 2. Actions that are typically covered by progress bars should show "Processing..."
    // This is to prevent fluctuation with detailed backend messages when bars are active.
    if (
      lowerMessage.includes('filling form') ||
      lowerMessage.includes('dispenser') || // Catches "processing dispenser", "working on dispenser"
      lowerMessage.includes('fuel type') || // Catches "processing fuel type"
      lowerMessage.includes('entering data') ||
      lowerMessage.includes('updating meter')
    ) {
      // If the message indicates a step that's part of a progressing operation
      // (and isn't one of the specific full messages above), simplify it to "Processing...".
      // We avoid showing backend's "Dispenser 1/8..." if the progress bar already shows that.
      
      // Exception: If it's a very clear, final success/failure message not tied to intermediate progress,
      // let it pass through IF it doesn't seem like an intermediate step.
      // This logic is tricky. Example: "All dispensers processed successfully" vs "Dispenser 1 of 8 processing completed".
      // The latter should be "Processing..." because "1 of 8" is intermediate.
      // The former is a final summary.
      if ( (lowerMessage.includes('successfully') || lowerMessage.includes('completed') || lowerMessage.includes('failed')) &&
           !(lowerMessage.match(/\d+\s*of\s*\d+/) || lowerMessage.match(/\d+\/\d+/)) // Does not look like "X of Y"
         ) {
        return message; // Let specific final success/error messages pass
      }
      return 'Processing...'; // Otherwise, it's an intermediate progress step
    }

    // 3. Fallback: Return the original message for unhandled cases or specific final statuses not caught above.
    return message;
  };
