...
// SimpleJobMap removed due to issues with location data
...
const HomeContent = () => {
  ...
  const renderMapPanel = () => {
    ...
    const inRangeOrders = filteredWorkOrders.filter(order => {
      ...
      // Skip if somehow we don't have a valid date
      if (!visitDateTime) {
        return false;
      }

      // Check if order is in current or next week
    ...
    // Create locations array for map from the filtered orders - FIXED VERSION
    const locationsToShow: Location[] = inRangeOrders
      .map(order => {
        // If order has location object with latitude/longitude
        if (order.location && typeof order.location.latitude === 'number' && typeof order.location.longitude === 'number') {
          return {
            id: order.id,
        ...
        // If order has direct coordinates like clientLocation
        if (order.clientLocation && typeof order.clientLocation.latitude === 'number' && typeof order.clientLocation.longitude === 'number') {
          return {
            id: order.id,
            lat: order.clientLocation.latitude,
            lng: order.clientLocation.longitude,
            title: getDisplayName(order),
            storeType: getStoreTypeForFiltering(order),
            isFallback: false
          };
        ...
        // If order has direct latitude/longitude properties
        ...
        // If order has nested locations in other properties
        ...
        // Check if we can extract geographic data from HTML or serialized data
        // (This is an extra step in case your job data has location info embedded in HTML)
        let extractedLocation = null;
        const htmlSources = [
          order.customer?.rawHtml,
          order.description,
          order.instructions,
          order.clientNotes
        ];
        ...
        for (const html of htmlSources) {
          if (html && typeof html === 'string') {
            // Try to extract GPS coordinates like "lat: 40.123, lng: -74.456" or similar patterns
            const latMatch = html.match(/lat(?:itude)?[:\s=]+(-?\d+\.\d+)/i);
            const lngMatch = html.match(/l(?:o|n)(?:g|n)(?:itude)?[:\s=]+(-?\d+\.\d+)/i);
        ...
        // No valid location data found for this order
        ordersWithMissingLocations.push({
          id: order.id,
          name: getDisplayName(order)
        });
        
        // Return null for orders without valid location data
        return null;
      })
      // Filter out null values
      .filter((location): location is Location => location !== null);

    console.log('[renderMapPanel] Locations to show on map:', locationsToShow.length);
    
    // Log first few locations for debugging
    if (locationsToShow.length > 0) {
      console.log('[renderMapPanel] Sample locations:', locationsToShow.slice(0, 3));
    }
    ...
    return (
      <div className="pt-4">
        ...
        <div className="rounded-lg border border-gray-200 dark:border-gray-700 flex items-center justify-center" style={{ height: '350px', position: 'relative' }}>
          <div className="text-center p-8">
            <FiMapPin className="mx-auto h-12 w-12 text-gray-400 mb-3" />
            <h3 className="text-lg font-medium text-gray-500 dark:text-gray-400">Map View Unavailable</h3>
            <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">
              Please use the detailed job view to see location information.
            </p>
          </div>
        </div>
