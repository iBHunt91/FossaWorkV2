...
import { 
  ...
  FiClock, FiMapPin, FiCheckCircle, FiXCircle, FiLoader
} from 'react-icons/fi';
                ...
                <div className="panel-header">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <button
                      ...
                      className="btn btn-sm btn-outline-danger flex items-center space-x-1 px-3 py-1 text-xs border border-red-300 dark:border-red-700 text-red-700 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded"
                              ...
                              <td className="px-4 py-3 whitespace-nowrap">
                                <div className="flex items-center">
                                  ...
                                  <div>
                                    {job.status === 'running' ? (
                                      (() => {
                                        // Logic for extracting progress information from messages
                                        let premium = '';
                                        let premiumTotal = 0;
                                        ...
                                        // Extract premium/fuel type information from message
                                        if (job.message) {
                                          // Extract Premium/Fuel Grade information
                                          const premiumMatch = job.message.match(/Processing\s+(.+?)\s+\((\d+)\/(\d+)\)/i);
                                          if (premiumMatch) {
                                            premium = premiumMatch[1]; // Fuel type (e.g., "Premium")
                                            premiumCurrent = parseInt(premiumMatch[2], 10);
                                            premiumTotal = parseInt(premiumMatch[3], 10);
                                          }
                                          
                                          // Extract Dispenser information
                                          const dispenserMatch = job.message.match(/Dispenser\s+(?:#)?(\d+)(?:\s+of\s+(\d+)|\/(\d+))?/i);
                                          if (dispenserMatch) {
                                            dispenserCurrent = parseInt(dispenserMatch[1], 10);
                                            dispenserTotal = parseInt(dispenserMatch[2] || dispenserMatch[3] || "0", 10);
                                            
                                            // If we have dispenser info, but no total, use the job's dispenser count
                                            if (dispenserTotal === 0 && job.dispenserCount) {
                                              dispenserTotal = job.dispenserCount;
                                            }
                                            
                                            dispenser = `Dispenser ${dispenserCurrent}`;
                                          }
                                        }
                                        ...
                                        if (dispenserCurrent && authoritativeTotalForPercent) {
                                          if (authoritativeTotalForPercent === 1) {
                                            ...
                                            // Ensure progress starts from a non-zero value for the first dispenser
                                            // and reaches 100% for the last dispenser.
                                            dispenserPercent = Math.max(5, Math.min(100, Math.round(((dispenserCurrent -1 + 0.5) / authoritativeTotalForPercent) * 100) ));
                                            if(dispenserCurrent === authoritativeTotalForPercent) dispenserPercent = 100;
                                          }
                                        }
                                        
                                        // Helper functions for determining progress bar colors
                                          ...
                                          <div className="flex flex-col h-24 justify-center">
                                            ...
                                            <div className="relative h-8 mb-3">
                                              ...
                                              <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2 overflow-hidden">
                                                {dispenserCurrent > 0 ? (
                                                  <div 
                                                    className={`${getDispenserColor()} h-2 rounded-full transition-all duration-500 ease-in-out ${pulseAnimation} relative overflow-hidden`}
                                            ...
                                            <div className={`relative ${premium ? 'opacity-100' : 'opacity-0'} h-8`}>
                                              ...
                                              <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2 overflow-hidden">
                                                <div 
                                                  className={`${getPremiumColor()} h-2 rounded-full transition-all duration-500 ease-in-out ${pulseAnimation} relative overflow-hidden`}
                                    ...
                                    : job.status === 'error'
                                      ? <span className="text-red-600 dark:text-red-400">{job.message}</span>
                                      : <span className="block truncate max-w-md">{job.message}</span>
                                    }
                                  </div>
                                </div>
                              </td>
                              ...
                              <td className="px-4 py-3 whitespace-nowrap">
                                <div className="flex flex-col space-y-1">
                                      ...
                                      {job.startTime && (
                                        <div className="flex items-center text-xs mt-0.5">
                                          <span className="inline-flex items-center bg-indigo-50 dark:bg-indigo-900/20 text-indigo-700 dark:text-indigo-400 px-2 py-0.5 rounded-md">
                                            ...
                                            {(() => {
                                              const durationMs = job.endTime ? (job.endTime - job.startTime) : (Date.now() - job.startTime);
                                  ...
                                  {job.status === 'completed' && job.startTime && job.endTime && (
                                    <div className="flex items-center text-xs text-green-600 dark:text-green-500 bg-green-50 dark:bg-green-900/30 px-2 py-1 rounded-md self-start">
                                  ...
                                  {job.status === 'error' && job.startTime && job.endTime && (
                                    <div className="flex items-center text-xs text-red-600 dark:text-red-500 bg-red-50 dark:bg-red-900/30 px-2 py-1 rounded-md self-start">
                                      <FiXCircle className="w-4 h-4 mr-1.5 flex-shrink-0" />
                                      <span>
