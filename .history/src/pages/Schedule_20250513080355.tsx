...
const Schedule: React.FC = () => {
  ...
  // Add global styles for the job toolbar via a style element
  useEffect(() => {
    const styleEl = document.createElement('style');
    styleEl.id = 'job-toolbar-styles';
    styleEl.innerHTML = `
      .job-toolbar {
        position: relative;
      }
      .job-toolbar.active .toolbar-icon-closed {
        display: none;
      }
      .job-toolbar.active .toolbar-icon-open {
        display: block;
      }
      .job-toolbar.active .toolbar-menu {
        opacity: 1;
        visibility: visible;
    ...
    if (!document.getElementById('job-toolbar-styles')) {
      document.head.appendChild(styleEl);
    }
    
    // Handle click outside to close all active toolbars
    const handleClickOutside = (e: MouseEvent) => {
      const toolbars = document.querySelectorAll('.job-toolbar.active');
      if (!toolbars.length) return;
      
      toolbars.forEach(toolbar => {
        // Check if click is outside the toolbar
        if (toolbar instanceof HTMLElement && !toolbar.contains(e.target as Node)) {
    ...
    return () => {
      const existingStyle = document.getElementById('job-toolbar-styles');
      if (existingStyle) {
          ...
          <div className="grid grid-cols-12 gap-x-3 gap-y-2 items-start">
            ...
            <div className="flex items-start col-span-5">
              ...
              <div className='flex-grow'>
                ...
                {/* Action Icons Footer - Interactive Toolbar */}
        <div className="job-toolbar border-t border-gray-700/50 p-1.5 relative">
          {/* Primary Actions - Always visible */}
          <div className="flex justify-end gap-1.5">
            {/* Form Prep Button */}
            <button
              onClick={() => navigate(`/app/form-prep?workOrderId=${order.id}&visitId=${visitNumber}`)}
              className="p-1.5 bg-blue-500 hover:bg-blue-600 text-white rounded-md shadow-sm transition-all duration-150"
            ...
            <button
              ...
              className="p-1.5 bg-amber-500 hover:bg-amber-600 text-white rounded-md shadow-sm transition-all duration-150"
            ...
            >
              ...
              <span className="sr-only">View Instructions</span>
                          ...
                          {/* More Options Button */}
              <button
                className="toolbar-toggle p-1.5 bg-gray-600 hover:bg-gray-700 text-white rounded-md shadow-sm transition-all duration-150 relative z-20"
                title="Toggle More Options"
                onClick={(e) => {
                  e.stopPropagation();
                  
                  // Close any other open toolbars first 
                  document.querySelectorAll('.job-toolbar.active').forEach(toolbar => {
                    if (toolbar !== e.currentTarget.closest('.job-toolbar')) {
                      toolbar.classList.remove('active');
                    }
                  });
                  
                  // Toggle this toolbar
                  e.currentTarget.closest('.job-toolbar')?.classList.toggle('active');
                }}
                aria-label="Show more actions"
              >
                <FiChevronDown className="h-3.5 w-3.5 toolbar-icon-closed" />
                <FiChevronUp className="h-3.5 w-3.5 toolbar-icon-open hidden" />
              </button>
          </div>

          {/* Slide-out menu */}
          <div className="toolbar-menu absolute right-0 top-0 z-10 bg-slate-800 border border-gray-700/30 rounded-md shadow-lg py-2 px-2
            ...
            <div className="flex items-center space-x-1.5 mb-2">
              <button
                onClick={() => navigate(`/app/form-prep?workOrderId=${order.id}&visitId=${visitNumber}`)}
                className="action-btn p-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded transition-all duration-150"
                title="Form Prep"
              >
                <FiEdit3 className="h-3.5 w-3.5" />
              </button>
              ...
              <button
                onClick={(e) => handleViewInstructions(e, order)}
                className="action-btn p-1.5 bg-amber-600 hover:bg-amber-700 text-white rounded transition-all duration-150"
                title="Instructions"
              >
                <FiEye className="h-3.5 w-3.5" />
              </button>
              ...
              <button
                ...
                className="action-btn p-1.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded transition-all duration-150"
            ...
            {/* Divider */}
            <div className="h-px w-full bg-gray-600/70 mb-2"></div>
            
            {/* Dispenser Actions Row */}
            <div className="flex items-center space-x-1.5 mb-2">
              <button
                onClick={(e) => handleViewDispenserDataUnified(e, order)}
                className="action-btn p-1.5 bg-teal-600 hover:bg-teal-700 text-white rounded transition-all duration-150"
              ...
              <button
                ...
                className="action-btn p-1.5 bg-cyan-600 hover:bg-cyan-700 text-white rounded transition-all duration-150"
                title="Filter Needs"
              >
            ...
            <div className="flex items-center space-x-1.5">
              <button 
                ...
                className="action-btn p-1.5 bg-green-600 hover:bg-green-700 text-white rounded transition-all duration-150"
              ...
              <button 
                ...
                className="action-btn p-1.5 bg-red-600 hover:bg-red-700 text-white rounded transition-all duration-150"
          ...
          {/* The styles for this toolbar are added in the component's useEffect */}
        </div>
      </div>
    );
  };
