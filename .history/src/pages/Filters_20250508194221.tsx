...
const FiltersRedesign: React.FC = () => {
  ...
  const renderFilterWarnings = () => {
    ...
    Array.from(filterWarnings.entries()).forEach(([orderId, warnings]) => {
      // Check if this order has dispenser data
      const order = workOrders.find(order => order.id === orderId);
      ...
      warnings.forEach(warning => {
        ...
        const extendedWarning: ExtendedFilterWarning = {
          ...
          // Add a flag for missing dispenser data
    ...
    const groupedWarnings = flattenedWarnings.reduce((acc: Record<string, WarningGroup>, warning: ExtendedFilterWarning) => {
      ...
      // If any instance has missing dispenser data, the group has missing dispenser data
      ...
      <div className="panel mb-6 animate-fadeIn">
        ...
        <div className="space-y-3 mt-4">
          {sortedWarningGroups.map((group, index) => {
            const { type, filterType, count, instances, severity, stores, needsVerification, missingDispenserData } = group;
                    ...
                    <div className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                      {count > 1 ? (
                        <details className="cursor-pointer">
                          ...
                          <ul className="mt-2 ml-2 space-y-1 text-xs">
                            {instances.map((warning, i) => {
                              const hasNoDispenserData = warning.missingDispenserData;
                              return (
                                <li key={i} className="list-disc ml-4">
                                  {warning.message || warning.warning}
                                  {hasNoDispenserData && (
                                    <div className="mt-0.5 text-red-600 dark:text-red-400 font-medium">
                        ...
                        <div>
                          ...
                          {instances[0].missingDispenserData && (
