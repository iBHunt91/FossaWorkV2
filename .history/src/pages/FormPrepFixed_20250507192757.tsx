                            ...
                            return (
                                    ...
                                    {job.status === 'completed' 
                                      ...
                                      : job.status === 'idle' || job.status === 'running'
                                        ? (() => {
                                              ...
                                              if (dispenserMatch && dispenserMatch[1]) {
                                                ...
                                                if (workOrderId && dispenserDetails[`W-${workOrderId}`]) {
                                                  ...
                                                  if (dispensersData && dispensersData.length > 0) {
                                                    let dispenserIndex = dispenserCurrent - 1; 
                                            ...
                                            const progressColorClass = dispenserCurrent > 0 ? 'bg-purple-500' : 'bg-primary-500';
                                            const progressTextColorClass = dispenserCurrent > 0 ? 'text-purple-600 dark:text-purple-400' : 'text-primary-600 dark:text-primary-400';
                                            const progressBgColorClass = dispenserCurrent > 0 ? 'bg-purple-100 dark:bg-purple-900/30' : 'bg-primary-100 dark:bg-primary-900/30';
                                            
                                            // Only show detailed progress if we have a label (premium or dispenser)
                                            if (progressLabel) {
                                              return (
                                                <div className="flex flex-col space-y-1.5">
                                                  <div className="flex items-center justify-between">
                                                    <span className={`px-1.5 py-0.5 ${progressBgColorClass} ${progressTextColorClass} rounded text-xs inline-flex items-center`}>
                                                      <span className={`w-1 h-1 ${dispenserCurrent > 0 ? 'bg-purple-500' : 'bg-primary-500'} rounded-full mr-1.5`}></span>
                                                      {progressLabel}
                                                    </span>
                                                    {totalProgressCount > 0 && (
                                                      <span className={`flex items-center text-xs ${progressTextColorClass}`}>
                                                        <span className="font-medium">{currentProgressCount}</span>
                                                        <span className="mx-0.5 text-gray-400 dark:text-gray-500">/</span>
                                                        <span>{totalProgressCount}</span>
                                                      </span>
                                                    )}
                                                  </div>
                                                  {overallProgress > 0 && totalProgressCount > 0 && (
                                                    <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1.5 overflow-hidden">
                                                      <div 
                                                        className={`h-1.5 rounded-full ${progressColorClass} relative overflow-hidden`}
                                            ...
                                            // Fallback to raw message if no specific progress info can be parsed
                                            return <span className="block truncate max-w-md">{job.message}</span>;
                                          })()
                                        : job.status === 'error'
                                          ? <span className="text-red-600 dark:text-red-400">{job.message}</span>
                                          : <span className="block truncate max-w-md">{job.message}</span> /* Added truncate for long idle messages */
                                    }
                                  </div>
                                )}
                              </td>
                              <td className="px-4 py-3 whitespace-nowrap">
                                <div className="flex flex-col">
                                  <span className="text-sm font-medium text-gray-900 dark:text-gray-100">{cleanStoreName}</span>
                                  <span className="text-xs text-gray-500 dark:text-gray-400">Visit #{visitInfo.visitNumber}</span>
                                  {visitInfo.dispenserCount > 0 && (
                                    <span className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                                      {visitInfo.dispenserCount} Dispenser{visitInfo.dispenserCount !== 1 ? 's' : ''}
                                    </span>
                                  )}
                                </div>
                              </td>
