...
const HomeContent = () => {
  ...
  const renderMapPanel = () => {
    ...
    if (!dateRanges || !dateRanges.currentWeekStart || !dateRanges.currentWeekEnd || !dateRanges.nextWeekStart || !dateRanges.nextWeekEnd) {
      console.error("Invalid date ranges calculated.");
    ...
    const inRangeOrders = filteredWorkOrders.filter(order => {
      ...
      if (!visitDateStr) {
        // console.log(`Order ${order.id}: No visit date found.`);
        return false;
      }
      ...
      try {
        ...
        const parsedDate = parseISO(visitDateStr);
         if (isValid(parsedDate)) {
           ...
           // console.warn(`Order ${order.id}: Could not parse date string "${visitDateStr}" as ISO. Trying fallback.`);
           ...
           if (isValid(fallbackDate)) {
             ...
             // console.log(`Order ${order.id}: Fallback parsing successful for "${visitDateStr}"`);
           } else {
             // console.error(`Order ${order.id}: Invalid date after fallback parsing: "${visitDateStr}"`);
             return false; // Skip if date is invalid after parsing attempts
           }
         }
      } catch (e) {
        // console.error(`Order ${order.id}: Error parsing date string "${visitDateStr}":`, e);
        return false; // Skip if parsing throws an error
      }

       // Defensive check in case parsing somehow resulted in null despite logic
      if (!visitDateTime) {
          // console.error(`Order ${order.id}: visitDateTime is unexpectedly null after parsing.`);
          return false;
      }
      ...
      // console.log(`Order ${order.id}: Date ${visitDateTime.toISOString()} - In Current? ${isInCurrentWeek} (Range: ${dateRanges.currentWeekStart.toISOString()} - ${dateRanges.currentWeekEnd.toISOString()}) - In Next? ${isInNextWeek} (Range: ${dateRanges.nextWeekStart.toISOString()} - ${dateRanges.nextWeekEnd.toISOString()})`);

      return isInCurrentWeek || isInNextWeek;
    });

    // Prepare locations for map from the filtered orders
    const locationsToShow = inRangeOrders
      .filter(order => order.location && typeof order.location.latitude === 'number' && typeof order.location.longitude === 'number') // Added type check for lat/lng
      .map(order => ({
        id: order.id,
        // Use non-null assertion ! after filtering ensures location and lat/lng exist and are numbers
    ...
    // State for showing detailed map view is managed in HomeContent
    // const [showDetailedMap, setShowDetailedMap] = useState(false); // Do not redefine here
    ...
    return (
      <div className="pt-4">
        ...
        <div className={`rounded-lg overflow-hidden border border-gray-200 dark:border-gray-700 ${showDetailedMap ? 'h-[400px]' : 'h-[250px]'}`}>
          <SimpleJobMap
            locations={locationsToShow}
            detailedView={showDetailedMap} // Use showDetailedMap state from HomeContent
          />
        </div>

        <div className="mt-2 text-center text-xs text-gray-500 dark:text-gray-400">
          {locationsToShow.length === 0 ? (
            'No locations to display in the selected date range'
          ) : (
  ...
  const renderVerificationPanel = () => {
    ...
    const generateRealWarnings = () => {
      ...
      // Get the date ranges from the current view
      // We use the same date ranges that are filtering the work orders
      const ranges = getWorkWeekDateRanges(1, 5, selectedDate);
      const currentWeekStart = ranges.currentWeekStart;
      const currentWeekEnd = ranges.currentWeekEnd;
      
      // Process each order for potential warnings - only include orders within the selected date range
      filteredWorkOrders.forEach(order => {
        ...
        if (order && order.id) {
          try {
            ...
            const { warnings: filterWarnings } = calculateFiltersSafely(order);
            
            if (filterWarnings && filterWarnings.length > 0) {
              // Add order data to the warnings
              ...
              // Add each warning to our warnings array with relevant context
              extendedWarnings.forEach(warning => {
                const severity = warning.severity || 5;
                const needsVerification = (warning.message || warning.warning || '').toLowerCase().includes('verify');
                
                if (needsVerification || severity >= 7) {
                  const storeName = order.customer?.name || 'Unknown Store';
                  const dispenserId = warning.dispenserId ? `Dispenser ${warning.dispenserId}` : 'Dispensers';
                  warnings.push(`${storeName}: ${warning.message || warning.warning} (${dispenserId})`);
                }
              });
            }
          } catch (error) {
      ...
      // Check for orders missing dispenser data - only for those in the selected date range
      const ordersWithoutDispensers = filteredWorkOrders.filter(
        order => !order.dispensers || order.dispensers.length === 0
      );
      
      if (ordersWithoutDispensers.length > 0) {
        const storesList = ordersWithoutDispensers
          .slice(0, 3)
          .map(order => order.customer?.name || 'Unknown')
          .join(', ');
          
        const additionalStores = ordersWithoutDispensers.length > 3 ? 
          ` and ${ordersWithoutDispensers.length - 3} more` : '';
          
        warnings.push(`Missing dispenser data for ${storesList}${additionalStores}`);
      }
      
      // Check for upcoming visits in the next 48 hours - ensure they're in the selected date range
      const now = new Date();
      const next48Hours = new Date(now);
      next48Hours.setHours(now.getHours() + 48);
      
      // Only include upcoming visits that fall within both next 48 hours AND the selected date range
      const upcomingVisits = filteredWorkOrders.filter(order => {
        const visitDate = order.visits?.nextVisit?.date || order.nextVisitDate || order.visitDate;
        if (!visitDate) return false;
        
        const visitDateTime = new Date(visitDate);
        return visitDateTime >= now && visitDateTime <= next48Hours;
      });
      
      if (upcomingVisits.length > 0) {
        warnings.push(`${upcomingVisits.length} visit${upcomingVisits.length !== 1 ? 's' : ''} scheduled in the next 48 hours`);
      }
      
      // Check for potential overlapping schedules - same day visits at the same store
      // Only consider visits within the selected date range
      const visitsByStoreAndDate = new Map<string, Date[]>();
      
      filteredWorkOrders.forEach(order => {
        const storeName = order.customer?.name || 'Unknown';
        const visitDate = order.visits?.nextVisit?.date || order.nextVisitDate || order.visitDate;
        
        if (visitDate) {
          const visitDateTime = new Date(visitDate);
          const dateStr = visitDateTime.toISOString().split('T')[0]; // YYYY-MM-DD
          const key = `${storeName}-${dateStr}`;
          
          if (!visitsByStoreAndDate.has(key)) {
            visitsByStoreAndDate.set(key, []);
          }
          
          visitsByStoreAndDate.get(key)?.push(visitDateTime);
        }
      });
      
      // Check each store+date combination for overlaps
      visitsByStoreAndDate.forEach((dates, key) => {
        if (dates.length > 1) {
          const [storeName, dateStr] = key.split('-');
          const formattedDate = new Date(dateStr).toLocaleDateString();
          warnings.push(`Potential schedule overlap for ${storeName} on ${formattedDate} (${dates.length} visits)`);
        }
      });
      
      return warnings.length > 0 ? warnings : ['No verification warnings found'];
    };
            ...
            {verificationWarnings.map((warning, idx) => (
              ...
              >
                {warning === 'No verification warnings found' ? (
                  <FiCheckCircle className="h-5 w-5 text-green-500 dark:text-green-400 mt-0.5" />
                ) : (
            ...
            {showVerificationDetails && (
              <div className="mt-3 p-4 rounded-lg bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700">
                ...
                <ul className="list-disc list-inside space-y-1.5 text-sm text-gray-600 dark:text-gray-400">
                  <li>Check for overlapping schedules at the same store</li>
                  <li>Verify that dispenser information is complete for all work orders</li>
                  <li>Review upcoming visits in the next 48 hours</li>
                  <li>Inspect filter warnings that require verification</li>
                </ul>
              </div>
            )}
          </div>
