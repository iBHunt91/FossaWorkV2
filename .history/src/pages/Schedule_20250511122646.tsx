    ...
    return (
      ...
      >
        ...
        <div className="flex items-center justify-end space-x-2 p-2 border-t border-gray-200 dark:border-gray-700">
          ...
          {/* Open Visit URL Button - Adapted to use existing URL structure if actions are not present */}
          <button
            onClick={(e) => {
              e.stopPropagation();
              // Prefer actions if available, otherwise fallback to nextVisit.url
              const actionUrl = order.actions?.find((a: { label: string; url: string }) => a.label === 'Visit URL')?.url;
              const visitUrl = actionUrl || order.visits?.nextVisit?.url || order.visitUrl; // Added another fallback to order.visitUrl
              
              if (visitUrl) {
                // Ensure URL is absolute
                const fullUrl = visitUrl.startsWith('http') ? visitUrl : `https://app.workfossa.com${visitUrl.startsWith('/') ? visitUrl : '/' + visitUrl}`;
