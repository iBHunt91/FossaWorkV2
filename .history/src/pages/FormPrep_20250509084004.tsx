...
// Debugging utilities for form automation
const FormLogger = {
  logToConsole: true,
  logToStorage: true,
  storageKey: 'form_automation_logs',
  maxLogEntries: 500,

  // Get the current logs from localStorage
  getLogs() {
    try {
  ...
  saveLogs(logs: any[]) {
    try {
      // Keep only the most recent logs to avoid localStorage overflow
  ...
  log(type: string, message: string, data?: any) {
    ...
    if (this.logToConsole) {
      console.log(`[FormLogger ${timestamp}] ${type}: ${message}`, data || '');
    }
  ...
  clearLogs() {
    ...
    return this.log('system', 'Logs cleared');
  },

  // Get logs as a downloadable format
  getLogsAsJson() {
  ...
  progress(message: string, data?: any) {
    return this.log('progress', message, data);
  }
};

// Download logs function - to be called from a button
const downloadFormLogs = () => {
  const logs = FormLogger.getLogsAsJson();
...
const FormPrep: React.FC<{}> = () => {
  ...
  const extractDispenserProgressLocal = (message: string, totalDispensers?: number): { current: number; total: number } => {
    ...
    try {
      FormLogger.debug('Extracting dispenser progress from message', { message, totalDispensers });
      ...
      if (directMatch && directMatch[1] && directMatch[2]) {
        ...
        FormLogger.debug('Extracted dispenser progress from direct number format', { current, total });
        ...
        FormLogger.debug('Extracted dispenser progress from dispenser regex', { current, total });
        return { current, total: Math.max(total, 1) };
      }
      
      FormLogger.debug('No dispenser progress found in message, returning default', defaultResult);
      return defaultResult;
    } catch (error) {
      FormLogger.error('Error extracting dispenser progress:', error);
      return defaultResult;
    }
  };
  ...
  const startBatchPolling = () => {
    // Redefine the interface here to ensure it's used correctly in this scope
    interface LocalBatchStatus {
      status: 'idle' | 'running' | 'completed' | 'error';
      message?: string;
      completedVisits?: number;
      totalVisits?: number;
    ...
    const pollInterval = setInterval(async () => {
      try {
        ...
        if (!currentBatchJobId) {
          FormLogger.info('No active batch job ID found, stopping poll');
        ...
        try {
          // Log the API call
          FormLogger.api('Calling getBatchAutomationStatus API');
          
          // Get the status from the API
          const rawStatus = await getBatchAutomationStatus();
          
          // Log the raw API response
          FormLogger.api('Received batch status response', rawStatus);
          
          // Create a properly typed status object
          const status: LocalBatchStatus = {
            status: rawStatus.status,
            message: rawStatus.message,
            completedVisits: rawStatus.completedVisits,
            totalVisits: rawStatus.totalVisits,
            currentVisit: (rawStatus as any).currentVisit,
            currentVisitStatus: (rawStatus as any).currentVisitStatus,
            timestamp: (rawStatus as any).timestamp,
            dispenserCount: (rawStatus as any).dispenserCount,
            dispenserCurrent: (rawStatus as any).dispenserCurrent,
            fuelType: (rawStatus as any).fuelType,
            fuelCurrent: (rawStatus as any).fuelCurrent,
            fuelTotal: (rawStatus as any).fuelTotal,
            storeInfo: (rawStatus as any).storeInfo
          };
          ...
          // Log the stored batch jobs
          FormLogger.debug('Current batch jobs in storage', storedBatchJobs);
          ...
          setBatchJobs(prev => {
            ...
            if (jobIndex !== -1) {
              ...
              // Log the job we're updating
              FormLogger.debug('Updating batch job', jobToUpdate);
              ...
              if (status.status === 'idle' && currentBatchJobId && !isJobFinishedByServer) {
                newStatus = 'running'; 
              }
              ...
              // Extract currentVisitStatus from status.message if it's not explicitly provided 
              let currentVisitStatus = status.currentVisitStatus || status.message || jobToUpdate.currentVisitStatus || '';

              // Extract store information from the status message
              if (status.storeInfo && status.storeInfo.name) {
                currentVisitName = status.storeInfo.name;
                FormLogger.info('Using store name from storeInfo', status.storeInfo);
              } else if (status.currentVisit) {
                const visitMatch = status.currentVisit.match(/W-(\d+)/);
                if (visitMatch) {
                  currentVisitId = visitMatch[0];
                  FormLogger.info(`Extracted visit ID: ${currentVisitId} from currentVisit: ${status.currentVisit}`);
                }
                
                const storeMatch = status.message?.match(/Processing visit for (.+?)(?:$|\s-\s)/);
                if (storeMatch) {
                  currentVisitName = storeMatch[1];
                  FormLogger.info(`Extracted store name: ${currentVisitName} from message`);
                }
              }

              if (status.message || status.currentVisitStatus) {
                const message = status.message || status.currentVisitStatus || '';
                
                // Log the message to help debugging
                FormLogger.progress('Processing message for progress extraction', message);
                
                // Try to extract store name from the message
                let extractedStoreName = '';
                
                // Look for patterns like "Processing visit X for [Store Name]" or "Working on [Store Name]"
                const storePattern1 = /Processing visit (?:\d+) (?:of|for) (.+?)(?:$|:|\.|\s-\s|\()/i;
                const storePattern2 = /Working on (.+?)(?:$|:|\.|\s-\s|\()/i;
                const storePattern3 = /(?:processing|filling forms for|completing) (.+?)(?:$|:|\.|\s-\s|\()/i;
                
                const storeMatch1 = message.match(storePattern1);
                const storeMatch2 = message.match(storePattern2);
                const storeMatch3 = message.match(storePattern3);
                
                if (storeMatch1 && storeMatch1[1]) {
                  extractedStoreName = storeMatch1[1].trim();
                  FormLogger.info(`Extracted store name using pattern 1: ${extractedStoreName}`);
                } else if (storeMatch2 && storeMatch2[1]) {
                  extractedStoreName = storeMatch2[1].trim();
                  FormLogger.info(`Extracted store name using pattern 2: ${extractedStoreName}`);
                } else if (storeMatch3 && storeMatch3[1]) {
                  extractedStoreName = storeMatch3[1].trim();
                  FormLogger.info(`Extracted store name using pattern 3: ${extractedStoreName}`);
                }
                
                // Only update if we found a valid store name
                if (extractedStoreName) {
                  currentVisitName = extractedStoreName;
                  FormLogger.info(`Set current visit name to: ${currentVisitName}`);
                }
                
                if (message.includes('Preparing') || message.includes('forms')) {
                  currentVisitPhase = 'forms';
                  FormLogger.info('Set phase to: forms');
                }
                else if (message.includes('Filling')) {
                  currentVisitPhase = 'filling';
                  FormLogger.info('Set phase to: filling');
                }
                else if (message.includes('Saving')) {
                  currentVisitPhase = 'saving';
                  FormLogger.info('Set phase to: saving');
                }
                else if (newStatus === 'running') {
                  currentVisitPhase = 'setup'; // Default to setup if running and no other phase detected
                  FormLogger.info('Set phase to: setup (default)');
                }

                const formCountMatch = message.match(/(\d+)\s*of\s*(\d+)\s*forms/i);
                if (formCountMatch) {
                  formsCurrent = parseInt(formCountMatch[1]);
                  formsTotal = parseInt(formCountMatch[2]);
                  FormLogger.info(`Extracted form count: ${formsCurrent}/${formsTotal} from message`);
                }
                
                // Try to extract from both message and currentVisitStatus
                const dispenserProgress = extractDispenserProgressLocal(message, 0);
                if (dispenserProgress.total > 0) {
                  formsCurrent = dispenserProgress.current;
                  formsTotal = dispenserProgress.total;
                  FormLogger.info(`Extracted dispenser progress: ${formsCurrent}/${formsTotal}`, dispenserProgress);
                }
                
                const fuelProgress = extractFuelProgressLocal(message);
                if (fuelProgress.total > 0) {
                  formsCurrent = fuelProgress.current;
                  formsTotal = fuelProgress.total;
                  FormLogger.info(`Extracted fuel progress: ${formsCurrent}/${formsTotal}`, fuelProgress);
                }
                
                // Log a summary of all extracted information
                FormLogger.progress('Progress extraction summary', {
                  message,
                  extractedStoreName,
                  currentVisitName,
                  currentVisitPhase,
              ...
              // Use direct dispenser information from status if available
              if (status.dispenserCurrent && status.dispenserCount) {
                FormLogger.info(`Using direct dispenser info from API: ${status.dispenserCurrent}/${status.dispenserCount}`);
                formsCurrent = status.dispenserCurrent;
                formsTotal = status.dispenserCount;
              }
              
              // Ensure we always have some values for formsCurrent/formsTotal when running
              if (newStatus === 'running' && formsTotal === 0) {
                formsTotal = 4; // Default to 4 forms if no other information is available
                FormLogger.info(`No form count detected, using default forms total: ${formsTotal}`);
              }
              
              // Make sure we have valid completedVisits and totalVisits
              const completedVisits = status.completedVisits !== undefined ? status.completedVisits : jobToUpdate.completedVisits || 0;
              const totalVisits = status.totalVisits !== undefined ? status.totalVisits : jobToUpdate.totalVisits || 1;
              ...
              // Log the updated job
              FormLogger.progress('Updated batch job', updatedJob);
              ...
              if (isJobFinishedByServer) {
                FormLogger.info(`Job ${currentBatchJobId} finished with status: ${status.status}`);
                if (status.status === 'error') {
                  setLastFailedBatch(updatedJob);
                  FormLogger.error(`Job ${currentBatchJobId} failed`, status);
                }
                clearInterval(pollInterval);
                setIsProcessing(false);
              ...
              FormLogger.warn(`Batch job with ID ${currentBatchJobId} not found in state during polling. Creating recovery job.`);
              ...
              const recoveryJob: BatchJob = {
                ...
                totalVisits: status.totalVisits || 5,
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
        <div className="border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700/50 px-4 py-3">
          <h2 className="text-sm font-medium text-gray-700 dark:text-gray-300 flex items-center gap-2">
            <FiFileText className="text-primary-500 dark:text-primary-400" />
        ...
        <div className="p-4 space-y-4">
          <div className="flex flex-col space-y-2">
            <p className="text-sm text-gray-600 dark:text-gray-400">
              If you're experiencing issues with progress tracking, you can download the logs to help troubleshoot.
            </p>
            
            <div className="flex space-x-3">
              <button
                onClick={downloadFormLogs}
                className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-md text-sm font-medium flex items-center space-x-2 transition-colors"
              >
              ...
              <button
                onClick={() => {
                  ...
                  addToast('info', 'Logs cleared successfully');
                }}
                className="px-4 py-2 bg-gray-200 hover:bg-gray-300 text-gray-700 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-gray-300 rounded-md text-sm font-medium flex items-center space-x-2 transition-colors"
              >
