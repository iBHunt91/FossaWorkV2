...
const History: React.FC = () => {
  ...
  const getChangeIcon = (type: string) => {
    switch (type) {
      case 'removed':
        return <FiPlusCircle className="w-5 h-5 text-red-500 dark:text-red-400" style={{ transform: 'rotate(45deg)' }} />;
      case 'added':
        return <FiPlusCircle className="w-5 h-5 text-green-500 dark:text-green-400" />;
      case 'date_changed':
        return <FiCalendar className="w-5 h-5 text-blue-500 dark:text-blue-400" />;
      case 'replacement':
        return <FiArrowRight className="w-5 h-5 text-purple-500 dark:text-purple-400" />;
      case 'swap':
        return <FiRefreshCw className="w-5 h-5 text-indigo-500 dark:text-indigo-400" />;
      default:
