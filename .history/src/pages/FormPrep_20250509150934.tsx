...
const FormPrep: React.FC<{}> = () => {
  ...
  const extractDispenserProgressLocal = (message: string, totalDispensers?: number): { current: number; total: number } => {
    ...
    if (match && match[1] && match[2]) {
      ...
      // If it looks like a sub-unit pair (e.g., X is odd, Y is X+1, like 1/2, 3/4, 5/6)
      if (valX % 2 === 1 && valY === valX + 1) {
        current = (valX + 1) / 2; // Convert to physical dispenser number
      } else {
        current = valX; // Fallback: use X directly if not the standard pair
      }
      parsedTotalFromMessage = valY; // Store the Y from "X/Y" as context for message's total
    } else {
      // Priority 2: "... dispenser: X/Y ..." or "... dispenser X/Y ..." (general context)
      const generalDispenserXY = /dispenser:?\s*(?:#)?\s*(\d+)\s*(?:of|\/)\s*(\d+)/i;
      ...
      if (match && match[1] && match[2]) {
        ...
        parsedTotalFromMessage = valY;
      } else {
        // Priority 3: "Dispenser #X", "Dispenser X", or "Processing dispenser X", "Working on dispenser X"
        // These patterns likely refer to the physical dispenser number directly.
        const explicitDispenserX = /Dispenser(?:\s*#)?\s*(\d+)|(?:Processing|Working on)\s+(?:dispenser|disp)\s+(\d+)/i;
        match = message.match(explicitDispenserX);
        if (match) {
          if (match[1]) { // Matched "Dispenser #X" or "Dispenser X"
            current = parseInt(match[1], 10);
          } else if (match[2]) { // Matched "Processing/Working on dispenser X"
            current = parseInt(match[2], 10);
          }
        } else {
          // Priority 4: "... dispenser: X ..." or "... dispenser X ..." (general context for single number)
          const generalDispenserX = /dispenser:?\s*(?:#)?\s*(\d+)/i;
          match = message.match(generalDispenserX);
          if (match && match[1]) {
            current = parseInt(match[1], 10);
          }
        }
    ...
    let finalTotal = 0;
    // Prioritize totalDispensers from job.dispenserCount if available and positive
    if (totalDispensers && totalDispensers > 0) {
      finalTotal = totalDispensers;
      // Optional: Log if parsedTotalFromMessage (e.g., Y from X/Y) exists and differs from totalDispensers
      if (parsedTotalFromMessage !== undefined && parsedTotalFromMessage !== finalTotal && parsedTotalFromMessage < finalTotal && finalTotal % parsedTotalFromMessage !== 0) {
        // More specific warning: only if parsedTotalFromMessage seems like a sub-count that doesn't align well
         console.warn(`Dispenser progress: Using overall total ${finalTotal} (from job.dispenserCount). Message mentioned a local context of ${parsedTotalFromMessage} dispensers.`);
      }
    } else if (parsedTotalFromMessage !== undefined) {
      // If job.dispenserCount not available, use total parsed from message context (less ideal for overall progress)
      finalTotal = parsedTotalFromMessage;
    }
    
    // If current is still 0 (no specific current number parsed) AND finalTotal is known (meaning we know total physical dispensers),
    // default current to 1, unless the message explicitly mentions "dispenser 0".
    if (finalTotal > 0 && current === 0 && !message.match(/(?:dispenser|disp)(?:\s*#)?\s*0/i) ) {
      current = 1; // Default to 1st physical dispenser
    }

    // Ensure current does not exceed total number of physical dispensers.
    if (finalTotal > 0 && current > finalTotal) {
      current = finalTotal;
    }
    
    // If total ended up as 0, current must also be 0.
    if (finalTotal === 0) {
        current = 0;
    }
                                  ...
                                  {job.status === 'running' && (
                                    <div className="space-y-3 min-w-56 min-h-16"> 
                                      ...
                                      {(() => {
                                        ...
                                        return null; // Default: job not running or no dispenser info (should be covered by above)
                                      })()}
