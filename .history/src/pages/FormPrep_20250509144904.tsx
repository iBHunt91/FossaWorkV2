...
const FormPrep: React.FC<{}> = () => {
  ...
  const extractDispenserProgressLocal = (message: string, totalDispensers?: number): { current: number; total: number } => {
    ...
    // Regex to capture "Dispenser #X of Y", "Dispenser X/Y", "Dispenser X", "Processing dispenser X"
    const dispenserRegex = /Dispenser(?:\s*#)?\s*(\d+)(?:\s*(?:of|\/)\s*(\d+))?/i;
    const processingDispenserRegex = /(?:Processing|Working on) dispenser (\d+)/i;
    
    let match = message.match(dispenserRegex);
    if (match && match[1]) {
      current = parseInt(match[1], 10);
      if (match[2]) { // Total captured from "Dispenser X of Y" or "Dispenser X/Y"
        parsedTotalFromMessage = parseInt(match[2], 10);
      }
    } else {
      match = message.match(processingDispenserRegex);
      if (match && match[1]) {
        current = parseInt(match[1], 10);
        // No total is typically in "Processing dispenser X" format, so parsedTotalFromMessage remains undefined
      }
    }
    ...
    if (totalDispensers && totalDispensers > 0) {
      ...
      // If parsedTotalFromMessage exists and differs, job.dispenserCount (totalDispensers) takes precedence.
      // Could add a log here if parsedTotalFromMessage && parsedTotalFromMessage !== finalTotal
    } else if (parsedTotalFromMessage !== undefined) {
      finalTotal = parsedTotalFromMessage;
    }
    
    // If total is known (finalTotal > 0) and current is still 0 (e.g. generic message before dispenser numbers appear)
    // default current to 1. This assumes processing starts at dispenser 1.
    if (finalTotal > 0 && current === 0) {
      current = 1;
    }
                                ...
                                <td className="px-4 py-3">
                                  {job.status === 'running' && (
                                    <div className="space-y-3 min-w-56 min-h-16"> 
                                      ...
                                      {(() => {
                                        const dispenserInfo = extractDispenserProgressLocal(job.message || '', job.dispenserCount);
                                        const isDispensersActive = job.message && 
                                                                   (job.message.match(/Dispenser /i) || 
                                                                    job.message.match(/Working on dispenser/i) ||
                                                                    job.message.match(/Processing dispenser/i));

                                        if (dispenserInfo.total > 0 && isDispensersActive) {
                                          // ACTUAL DISPENSER PROGRESS BAR JSX
                                          return (
                                            <div>
                                          ...
                                          // SKELETON DISPENSER PROGRESS BAR JSX
                                          return (
                                            <div>
                                              <div className="flex justify-between items-center text-xs mb-1">
                                        ...
                                        return null; // Return null if not running and no active dispenser info
                                      })()}
