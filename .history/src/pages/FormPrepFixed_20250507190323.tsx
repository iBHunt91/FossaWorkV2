  ...
  const startBatchPolling = () => {
    const batchPollingInterval = setInterval(async () => {
      try {
        ...
        // Check if the status has a jobId and it matches our stored batchJobId
        if (status && 'jobId' in status && status.jobId === batchJobId) {
          ...
          setBatchJobs(prev => {
            ...
            if (jobIndex !== -1) {
              updatedJobs[jobIndex] = {
                ...
                status: status.status,
                message: status.message || updatedJobs[jobIndex].message,
                completedVisits: 'completedCount' in status ? 
                  (typeof status.completedCount === 'number' ? status.completedCount : updatedJobs[jobIndex].completedVisits) : 
                  updatedJobs[jobIndex].completedVisits
              };
        ...
        <div className="space-y-6 animate-fadeIn">
                        ...
                        <tr>
                          <td colSpan={5} className="px-4 py-8 text-center text-sm text-gray-500 dark:text-gray-400">
                            <div className="flex flex-col items-center justify-center">
                              <FiFileText className="w-8 h-8 text-gray-300 dark:text-gray-600 mb-3" />
                              <p>No automation jobs yet</p>
                              <p className="text-xs mt-1">Jobs will appear here after you process forms</p>
                            </div>
                          </td>
                        </tr>
                      ) : (
                        formJobs.map((job, index) => {
                          // Calculate progress based on message patterns
                          ...
                          if (job.status === 'completed') {
                            ...
                            const fuelTypeMatch = job.message?.match(/\((\d+)\/(\d+)\)/);
                            if (fuelTypeMatch && fuelTypeMatch[1] && fuelTypeMatch[2]) {
                              const current = parseInt(fuelTypeMatch[1], 10);
                              const total = parseInt(fuelTypeMatch[2], 10);
                              if (!isNaN(current) && !isNaN(total) && total > 0) {
                                progressPercentage = Math.min(Math.round((current / total) * 100), 99);
                              }
                            } else {
                              // Default progress for running jobs with no progress info
                          ...
                          let duration = '';
                          if (job.startTime) {
                            const endTimeToUse = job.endTime || Date.now();
                            const durationMs = endTimeToUse - job.startTime;
                          ...
                          return (
                            <tr 
                              ...
                              className={`hover:bg-gray-50 dark:hover:bg-gray-700/50 ${
                                job.status === 'running' ? 'bg-primary-50/30 dark:bg-primary-900/10' : 
                                job.status === 'completed' ? 'bg-green-50/30 dark:bg-green-900/10' : 
                                job.status === 'error' ? 'bg-red-50/30 dark:bg-red-900/10' : ''
                              }`}
                            >
                              <td className="px-4 py-3 whitespace-nowrap text-sm">
                                ...
                                {job.storeName && (
                                  <div className="text-xs text-gray-500 dark:text-gray-400 pl-6 mt-1">
                                    {job.storeName}
                                  </div>
                                )}
                              </td>
                              <td className="px-4 py-3 whitespace-nowrap">
                                ...
                                `}>
                                  {job.status === 'running' && <FiRefreshCw className="h-3 w-3 mr-1 animate-spin" />}
                                  {job.status === 'completed' && <FiCheckCircle className="h-3 w-3 mr-1" />}
                                  {job.status === 'error' && <FiXCircle className="h-3 w-3 mr-1" />}
                                  {job.status}
                                </span>
                              </td>
                              <td className="px-4 py-3 whitespace-nowrap">
                                <div className="flex items-center">
                                  <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2.5 mr-2 max-w-[100px] relative overflow-hidden">
                                    {/* Base layer - always shows full width background */}
                                    <div 
                                    ...
                                    {/* Add stage markers for multi-stage jobs */}
                                    {job.status === 'running' && progressPercentage > 5 && (
                                      <div className="absolute inset-0 flex items-center justify-between px-1">
                                        {[25, 50, 75].map(marker => (
                                          <div 
                                            key={marker}
                                            className={`w-0.5 h-1.5 ${progressPercentage >= marker ? 'bg-white/50' : 'bg-gray-400/30'}`}
                                            style={{ left: `${marker}%` }}
                                          ></div>
                                        ))}
                                      </div>
                                  ...
                                  {/* Richer progress information */}
                                  <div className="text-xs flex items-center">
                                    <span className={`font-medium ${
                                      job.status === 'completed' ? 'text-green-600 dark:text-green-400' :
                                      job.status === 'running' ? 'text-primary-600 dark:text-primary-400' :
                                      job.status === 'error' ? 'text-red-600 dark:text-red-400' : 'text-gray-600 dark:text-gray-400'
                                    }`}>
                                      {progressPercentage}%
                                    </span>
                                    
                                    {/* Extract and display fuel type info when available */}
                                    {job.status === 'running' && job.message && (
                                      <>
                                        {/* Use regular JSX instead of IIFE for better type safety */}
                                        {(() => {
                                          // Extract detailed progress from the message if possible
                                          const fuelTypeMatch = job.message ? job.message.match(/processing fuel type:\s*([a-zA-Z0-9\s-]+)\s*\((\d+)\/(\d+)\)/i) : null;
                                          const dispenserMatch = job.message ? job.message.match(/dispenser(?:[\s#]+)(\d+)(?:[^\d]+(\d+)|)/i) : null;
                                          
                                          if (fuelTypeMatch && fuelTypeMatch[1] && fuelTypeMatch[2] && fuelTypeMatch[3]) {
                                            const fuelType = fuelTypeMatch[1].trim();
                                            const current = parseInt(fuelTypeMatch[2], 10);
                                            const total = parseInt(fuelTypeMatch[3], 10);
                                            ...
                                            return (
                                              <div className="ml-2 flex items-center">
                                                ...
                                                <span className="ml-2 inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400">
                                                  {fuelType} {current}/{total}
                                                </span>
                                                
                                                {dispenserMatch && dispenserMatch[1] && (
                                                  <span className="ml-1 inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-400">
                                                    Disp. {dispenserMatch[1]}{dispenserMatch[2] ? `/${dispenserMatch[2]}` : ''}
                                                  </span>
                                                )}
                                              </div>
                                            );
                                    ...
                                    {/* Stages indicator dot */}
                                    {job.status === 'running' && (
                                      <span className="ml-1.5 h-1.5 w-1.5 rounded-full bg-primary-500 animate-pulse"></span>
                                    )}
                                  </div>
                                </div>
                                
                                {/* Show a hint about what stage we're in based on the percentage */}
                                {job.status === 'running' && (
                                  <div className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                                    {progressPercentage < 25 ? 'Loading form...' : 
                                     progressPercentage < 50 ? 'Processing fuel types...' : 
                                     progressPercentage < 80 ? 'Filling dispensers...' : 
                                     'Finalizing form...'}
                                  </div>
                                )}
                              </td>
          ...
          {activeTab === 'batch' && (
            <div className="space-y-6">
              ...
              {/* Add Stop Processing button for batch mode */}
              {isProcessing && batchJobId && (
                <div className="panel bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 p-6 mt-6">
                  <div className="panel-header mb-4">
                    <h2 className="panel-title text-lg font-semibold text-gray-800 dark:text-gray-200 flex items-center space-x-2">
                      <FiRefreshCw className="text-primary-500 dark:text-primary-400 animate-spin" />
                      <span>Batch Processing in Progress</span>
                    </h2>
                  </div>
                  
                  <button
                    className="btn bg-red-600 hover:bg-red-700 text-white dark:bg-red-600 dark:hover:bg-red-700 flex items-center justify-center space-x-2 px-4 py-2 rounded-md w-full"
                    onClick={() => {
                      try {
                        setIsProcessing(false);
                        
                        // Call API to cancel the batch job
                        if (batchJobId) {
                          cancelFormAutomation(batchJobId)
                            .then(result => {
                              if (result.success) {
                                ...
                                addToast('error', result.message || 'Failed to stop processing');
                              }
                            })
                            .catch(error => {
