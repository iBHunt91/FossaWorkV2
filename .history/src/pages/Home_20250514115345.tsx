  ...
  currentWeekOrders.forEach(order => {
    try {
      ...
      if (warnings && warnings.length > 0) {
        ...
        verificationWarnings.forEach(warning => {
          if (warning && !filterTotals.stores[storeName].warnings.includes(warning)) {
            filterTotals.stores[storeName].warnings.push(warning);
          }
        });
      }
    } catch (error) {
    ...
    <div className="pt-4">
      ...
      {Object.keys(filterTotals.types).length > 0 && (
        <div className="mb-4 p-4 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm">
          <h4 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">Filter Models Needed:</h4>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
            {Object.entries(filterTotals.types)
              .sort(([typeA], [typeB]) => typeA.localeCompare(typeB))
              .map(([type, count]) => {
                // Determine if this is a gas or diesel filter based on naming convention
                const isGasFilter = type.includes('MG') || type.includes('MB');
                const isDieselFilter = type.includes('HS');
                
                // Choose appropriate styling based on filter type
                const bgColor = isGasFilter ? 'bg-blue-50 dark:bg-blue-900/20 border-blue-200 dark:border-blue-800/50' : 
                                isDieselFilter ? 'bg-amber-50 dark:bg-amber-900/20 border-amber-200 dark:border-amber-800/50' : 
                                'bg-gray-50 dark:bg-gray-700 border-gray-200 dark:border-gray-600';
                                
                const textColor = isGasFilter ? 'text-blue-800 dark:text-blue-300' : 
                                  isDieselFilter ? 'text-amber-800 dark:text-amber-300' : 
                                  'text-gray-800 dark:text-gray-300';
                
                return (
                  <div 
                    key={type} 
                    className={`flex justify-between items-center px-3 py-2 ${bgColor} rounded-md border`}
                  >
                    <span className={`font-medium ${textColor}`}>{type}</span>
                    <span className={`font-bold ${textColor} ml-2 px-2 py-0.5 bg-white/50 dark:bg-gray-800/50 rounded-full text-sm`}>{count}</span>
                  </div>
                );
              })}
          </div>
        ...
        <div className="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-4 border border-blue-200 dark:border-blue-800/50">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm text-blue-700 dark:text-blue-300">Gas Filters</p>
        ...
        <div className="bg-amber-50 dark:bg-amber-900/20 rounded-lg p-4 border border-amber-200 dark:border-amber-800/50">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm text-amber-700 dark:text-amber-300">Diesel Filters</p>
                <div className="flex items-center">
      ...
      {/* Filter breakdown by store */}
      <h3 className="text-base font-medium text-gray-800 dark:text-white mb-3">Filter Requirements by Location</h3>
      ...
      {Object.keys(filterTotals.stores).length > 0 ? (
            ...
            .map(([store, data]) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Filter details */}
                  <div className="p-3">
                    {/* Filter models breakdown by type - primary focus */}
                          ...
                          <div className="grid grid-cols-2 gap-2">
                            {Object.entries(data.filterTypes)
                              ...
                              .map(([type, count]) => {
                                ...
                                // Choose appropriate styling based on filter type
                                const bgColor = isGasFilter ? 'bg-blue-50 dark:bg-blue-900/30 border-blue-200 dark:border-blue-800/50' : 
                                                isDieselFilter ? 'bg-amber-50 dark:bg-amber-900/30 border-amber-200 dark:border-amber-800/50' : 
                                                'bg-gray-50 dark:bg-gray-700 border-gray-200 dark:border-gray-600';
                                ...
                                return (
                                  <div 
                                    key={`${store}-${type}`} 
                                    className={`flex justify-between items-center px-3 py-1.5 ${bgColor} border rounded-md`}
                                  >
                                    <span className={`text-xs font-medium ${textColor}`}>{type}</span>
                                    <span className={`text-xs font-bold ${textColor} ml-1`}>{count}</span>
                                  </div>
                                );
                              })}
                          </div>
                    ...
                    {/* Gas/Diesel counts - secondary and more compact */}
                    <div className="flex items-center gap-3 mt-3">
                      {data.gas > 0 && (
                        <span className="inline-flex items-center px-2 py-1 rounded text-xs font-medium bg-blue-50 dark:bg-blue-900/30 text-blue-800 dark:text-blue-300">
                          <FiFilter className="mr-1 h-3 w-3" /> {data.gas} Gas
                        </span>
                      )}
                      {data.diesel > 0 && (
                        <span className="inline-flex items-center px-2 py-1 rounded text-xs font-medium bg-amber-50 dark:bg-amber-900/30 text-amber-800 dark:text-amber-300">
                          <FiFilter className="mr-1 h-3 w-3" /> {data.diesel} Diesel
                        </span>
                      )}
                    </div>
                  </div>
                  
                  {/* Warnings - modern styling */}
                  {hasWarnings && (
                    <div className="p-3 bg-amber-50 dark:bg-amber-900/20 border-t border-amber-200 dark:border-amber-700">
                      <div className="flex items-center mb-1.5">
                        <FiAlertTriangle className="h-4 w-4 text-amber-500 dark:text-amber-400 mr-1.5" />
                        <span className="text-xs font-medium text-amber-700 dark:text-amber-300">Verification Required</span>
                      </div>
                      <div className="space-y-1.5">
                        {data.warnings.map((warning, idx) => (
                          <div key={idx} className="flex items-start text-xs text-amber-600 dark:text-amber-400 pl-5">
                            <span className="w-1 h-1 bg-amber-500 dark:bg-amber-400 rounded-full mt-1.5 mr-2"></span>
                            <span>{warning}</span>
                          </div>
                        ))}
                      </div>
        ...
        <div className="text-center py-6 text-gray-500 dark:text-gray-400 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
          <FiFilter className="mx-auto h-8 w-8 mb-2 opacity-50" />
          <p>No filter requirements for current week</p>
        </div>
      )}
    </div>
  );
  ...
  return (
    <div className="container mx-auto px-4 pt-5 pb-8">
      ...
      <div className="space-y-5">
        ...
        {/* Filters Needed Panel */}
        {renderPanel('filters', 'Filters Needed', <FiFilter className="w-5 h-5" />, renderFiltersPanel())}
