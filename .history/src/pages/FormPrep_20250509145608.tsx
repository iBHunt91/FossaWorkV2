...
const FormPrep: React.FC<{}> = () => {
  ...
  const extractDispenserProgressLocal = (message: string, totalDispensers?: number): { current: number; total: number } => {
    ...
    // Pattern 1: "Dispenser #X of Y" or "Dispenser #X/Y" or "Dispenser X of Y" or "Dispenser X/Y"
    // This pattern should be quite specific for current and total.
    const specificPattern = /Dispenser(?:\s*#)?\s*(\d+)\s*(?:of|\/)\s*(\d+)/i;
    let match = message.match(specificPattern);

    if (match && match[1] && match[2]) {
      current = parseInt(match[1], 10);
      parsedTotalFromMessage = parseInt(match[2], 10);
    } else {
      // Pattern 2: If not the above, try to find just current dispenser like "Dispenser #X" or "Dispenser X"
      // or "Processing dispenser X" or "Working on dispenser X"
      const currentOnlyPattern = /Dispenser(?:\s*#)?\s*(\d+)|(?:Processing|Working on)\s+dispenser\s+(\d+)/i;
      match = message.match(currentOnlyPattern);
      if (match) {
    ...
    if (finalTotal > 0 && current === 0 && !message.match(/Dispenser(?:\s*#)?\s*0/i) ) {
      current = 1;
    }
