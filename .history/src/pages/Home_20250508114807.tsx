                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-5 min-h-[180px] divide-x divide-gray-200 dark:divide-gray-700">
                        {Array.from({ length: 5 }).map((_, dayIndex) => {
                          ...
                          // Generate a unique key for this day to track expanded state
                          const dayKey = `current-${currentDate.toISOString()}`;
                          const isExpanded = expandedDays[dayKey] || false;
                          
                          // Determine which jobs to show - show all if expanded, otherwise limit
                          const visibleJobs = isExpanded ? jobs : jobs.slice(0, 3);
                          const hasMoreToShow = jobs.length > 3 && !isExpanded;
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-5 min-h-[180px] divide-x divide-gray-200 dark:divide-gray-700">
                          {Array.from({ length: 5 }).map((_, dayIndex) => {
                            ...
                            // Generate a unique key for this day to track expanded state
                            const dayKey = `next-${currentDate.toISOString()}`;
                            const isExpanded = expandedDays[dayKey] || false;
