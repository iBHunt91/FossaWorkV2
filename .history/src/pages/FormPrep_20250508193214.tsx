...
const FormPrep: React.FC<{}> = () => {
          ...
          setBatchJobs(prev => {
            ...
            if (storedBatchJobs.length > 0) {
              ...
              for (const storedJob of storedBatchJobs) {
                if (!mergedJobs.some(job => job.jobId === storedJob.jobId)) {
            ...
            const jobIndex = mergedJobs.findIndex(job => job.jobId === currentBatchJobId);
            ...
            if (jobIndex !== -1) {
              ...
              if (status.message) {
                ...
                const dispenserProgress = extractDispenserProgressLocal(status.message);
                if (dispenserProgress.total > 0) {
                  formsCurrent = dispenserProgress.current;
                  formsTotal = dispenserProgress.total;
                }
                const fuelProgress = extractFuelProgressLocal(status.message);
  ...
  // Load batch jobs from localStorage during initialization
  useEffect(() => {
    // Load batch jobs from localStorage
    const storedBatchJobs = getFromStorage(userStorageKeys.BATCH_JOBS, []);
    if (storedBatchJobs.length > 0) {
      setBatchJobs(storedBatchJobs);
    }
