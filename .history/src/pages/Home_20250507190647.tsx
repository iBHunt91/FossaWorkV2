          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-5 border border-gray-200 dark:border-gray-700 hover:shadow-xl transition-all duration-300">
            ...
            <div className="flex flex-wrap gap-3 mt-3">
              {storeDistributionArray.map((store) => {
                ...
                // Get store styling
                const storeStyle = getStoreColor(store.type);
                
                return (
                  <div 
                    key={store.type} 
                    className={`flex items-center justify-between ${badgeSize} rounded-xl bg-${store.type === 'circle-k' ? 'amber' : 
                                                                         store.type === '7-eleven' ? 'red' :
                                                                         store.type === 'wawa' ? 'indigo' : 'blue'}-50 
                              dark:bg-${store.type === 'circle-k' ? 'amber' : 
                                      store.type === '7-eleven' ? 'red' :
                                      store.type === 'wawa' ? 'indigo' : 'blue'}-900/30
                              text-${store.type === 'circle-k' ? 'amber' : 
                                   store.type === '7-eleven' ? 'red' :
                                   store.type === 'wawa' ? 'indigo' : 'blue'}-700
                              dark:text-${store.type === 'circle-k' ? 'amber' : 
                                        store.type === '7-eleven' ? 'red' :
                                        store.type === 'wawa' ? 'indigo' : 'blue'}-300
                              border border-${store.type === 'circle-k' ? 'amber' : 
                                           store.type === '7-eleven' ? 'red' :
                                           store.type === 'wawa' ? 'indigo' : 'blue'}-200
                              dark:border-${store.type === 'circle-k' ? 'amber' : 
                                          store.type === '7-eleven' ? 'red' :
                                          store.type === 'wawa' ? 'indigo' : 'blue'}-800/50
                              flex-1 min-w-[120px]`}
                  >
                    <div className="flex items-center">
                      <span className="mr-2">{storeStyle.icon}</span>
                      <span>{store.name}</span>
                    </div>
                    <span className="font-bold">{store.count}</span>
                  </div>
                );
              })}
            </div>
    ...
    return (
      ...
      >
        ...
        <div className="p-4">
          <div className="flex items-center gap-4 mb-3">
            {dispenserQty > 0 && (
              <div className="flex items-center gap-2">
                <GiGasPump className={`h-5 w-5 ${storeStyle.text}`} />
          ...
          {instructions && (
            <div className={`rounded p-3 border ${storeStyle.cardBg} border-gray-200 dark:border-gray-700`}>
