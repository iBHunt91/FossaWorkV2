...
const FormPrep: React.FC<{}> = () => {
  ...
  const extractDispenserProgressLocal = (message: string, totalDispensers?: number): { current: number; total: number } => {
    let currentPhysicalDispenser = 0;
    const actualTotalDispensers = totalDispensers && totalDispensers > 0 ? totalDispensers : 0;

    if (actualTotalDispensers > 0) { // Only attempt to parse current if we know the total physical dispensers
      let match;

      // Pattern 1: "Dispenser #X/Y" or "Dispenser X/Y" or "... dispenser: X/Y ..."
      // These often refer to sub-units (e.g., sides of a physical dispenser)
      const subUnitPattern = /Dispenser(?:\s*#)?\s*(\d+)\s*(?:of|\/)\s*(\d+)|dispenser:?\s*(?:#)?\s*(\d+)\s*(?:of|\/)\s*(\d+)/i;
      match = message.match(subUnitPattern);

      if (match) {
        const valX = parseInt(match[1] || match[3], 10); // First captured group (either from "Dispenser X/Y" or "dispenser: X/Y")
        // Convert the 'X' from 'X/Y' (e.g., 1 from 1/2, 3 from 3/4) to physical dispenser number
        if (valX > 0) {
          currentPhysicalDispenser = Math.ceil(valX / 2);
        }
      } else {
        // Pattern 2: "Dispenser #X", "Dispenser X", "Processing dispenser X", "Working on dispenser X"
        // These usually refer to the physical dispenser number directly.
        const directPattern = /Dispenser(?:\s*#)?\s*(\d+)|(?:Processing|Working on)\s+(?:dispenser|disp)\s+(\d+)|dispenser:?\s*(?:#)?\s*(\d+)(?!\s*(?:of|\/))/i;
        match = message.match(directPattern);
        if (match) {
          if (match[1]) currentPhysicalDispenser = parseInt(match[1], 10);       // Dispenser #X or Dispenser X
          else if (match[2]) currentPhysicalDispenser = parseInt(match[2], 10); // Processing/Working on dispenser X
          else if (match[3]) currentPhysicalDispenser = parseInt(match[3], 10); // dispenser: X (not followed by / or of)
        }
      }

      // If after parsing, currentPhysicalDispenser is 0 but we have a message,
      // and totalDispensers is known, assume it's the first dispenser.
      // Avoid if message explicitly says "dispenser 0".
      if (currentPhysicalDispenser === 0 && message && !message.match(/(?:dispenser|disp)(?:\s*#)?\s*0/i)) {
        currentPhysicalDispenser = 1;
      }

      // Clamp currentPhysicalDispenser to be between 1 and actualTotalDispensers if it's positive
      if (currentPhysicalDispenser > actualTotalDispensers) {
        currentPhysicalDispenser = actualTotalDispensers;
      }
       if (currentPhysicalDispenser < 0) { // Should not happen with current regex, but defensive
        currentPhysicalDispenser = 0;
      }

    } // End if (actualTotalDispensers > 0)

    // If total is 0, current must also be 0.
    if (actualTotalDispensers === 0) {
        currentPhysicalDispenser = 0;
    }
    
    // The 'total' in the returned object is the total number of *physical dispensers* for the visit.
    // The 'current' is the current *physical dispenser* being processed.
    return { current: currentPhysicalDispenser, total: actualTotalDispensers };
  };
