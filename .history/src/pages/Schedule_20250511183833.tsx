...
const Schedule: React.FC = () => {
  ...
  const renderJobRow = (order: WorkOrder, dateGroupClass?: string) => {
    ...
    return (
      <div
        key={order.id}
        className={`job-card ${cardBgColor} shadow-xl hover:shadow-2xl rounded-md mb-6 border-l-4 ${styles.cardBorder} border border-gray-700 transition-all duration-300 hover:translate-y-[-2px] transform ${dateGroupClass || ''}`}
        data-store-type={storeType}
        data-work-order-id={order.workOrderId || 'N/A'}
        data-visit-id={visitNumber || 'N/A'}
      >
        {/* Header Section */}
        <div className={`${cardHeaderBgColor} p-3 flex justify-between items-center rounded-t-md relative`}>
          {/* No service count indicator as per request */}
          <div className="flex items-center">
            <h3 className={`text-md font-semibold ${textColor} ml-1`}>
              {order.customer.name}
              {order.customer.storeNumber && (
                <span className={`text-sm ${subTextColor} ml-1`}>
          ...
          <div className="flex items-center space-x-2">
            {order.workOrderId && (
              <span className={`px-2.5 py-0.5 text-xs font-medium rounded-full ${woBadgeBg} ${woBadgeText}`}>
                W-{order.workOrderId}
              </span>
            )}
            {visitNumber && visitNumber !== 'N/A' && (
               <span className={`px-2.5 py-0.5 text-xs font-medium rounded-full ${visitBadgeBg} ${visitBadgeText}`}>
                #{visitNumber}
              </span>
            )}
          </div>
        ...
        <div className="p-4 space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-x-6 gap-y-4 items-start">
            {/* Visit Date */}
            <div className="flex items-start col-span-1 md:col-span-1">
              <FiCalendar className={`h-4 w-4 mr-2 mt-1 ${iconColor} flex-shrink-0`} />
              <div className="flex flex-col">
                <p className={`text-xs ${subTextColor}`}>Visit Date</p>
                <p className={`text-sm font-medium ${textColor}`}>{formattedJobDate}</p>
              </div>
            </div>

            {/* Dispensers - Modified Display */}
            <div className="flex items-start col-span-1 md:col-span-1">
              <GiGasPump className={`h-4 w-4 mr-2 mt-1 ${iconColor} flex-shrink-0`} />
              <div className="flex flex-col">
                <p className={`text-xs ${subTextColor}`}>Dispensers</p>
                <div className="flex items-baseline">
                  <p className={`text-sm font-medium ${textColor} mr-1`}>{dispenserCount}</p>
                  <p className={`text-xs ${subTextColor}`}>{dispenserCount !== 1 ? 'Dispensers' : 'Dispenser'}</p>
                </div>
              </div>
            </div>
            
            {/* We've removed the Services section as requested */}
            
            {/* Custom Instructions */}
            <div className="flex items-start col-span-1 md:col-span-3">
                <FiFileText className={`h-4 w-4 mr-2 mt-1 ${iconColor} flex-shrink-0`} />
                <div className='flex-grow flex flex-col'>
                    <p className={`text-xs ${subTextColor}`}>Custom Instructions</p>
                    {order.instructions ? (
                      <p className={`text-sm font-medium ${textColor} mt-0.5 line-clamp-2 hover:line-clamp-none transition-all`}>
                        {filteredInstructionsOnCard}
                      </p>
                    ) : (
                      <p className={`text-sm font-medium ${textColor} mt-0.5`}>None</p>
                    )}
                </div>
            </div>
          </div>
        ...
        {/* Action Icons Footer */}
        <div className="flex items-center justify-end space-x-2 p-3 border-t border-gray-700/50">
          <button
          ...
          {/* Open Visit URL Button - Reverted to use order.visits.nextVisit.url */}
          <button
            onClick={(e) => {
              e.stopPropagation();
              const visitData = order.visits?.nextVisit; // Use .nextVisit as before
          ...
          {/* Button to open Filter Needs Modal */}
          <button
            onClick={(e) => {
              e.stopPropagation();
              // Check if we have context dispensers but the order doesn't have them directly
              const contextDispensers = dispenserData?.dispenserData?.[order.id]?.dispensers;
                  ...
                  {visibleGroups.map(([dateKey, ordersForDate]) => {
                    ...
                    return (
                      <div key={dateKey} className="rounded-md overflow-hidden">
                        ...
                        <div className="bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-200 font-medium px-4 py-2 flex items-center justify-between">
                          <div className="flex items-center">
                            <FiCalendar className="mr-2" />
                            <span>{dateDisplay}</span>
                          </div>
                          <span className="text-xs px-2 py-0.5 rounded-full bg-blue-200 dark:bg-blue-800 text-blue-800 dark:text-blue-200">
                            {ordersForDate.length} job{ordersForDate.length !== 1 ? 's' : ''}
                          </span>
                        </div>
                        
                        {/* Jobs for this date, with connector lines between them */}
                        <div className="relative pl-5 border-l-2 border-blue-300 dark:border-blue-700 ml-4 pt-2 pb-0">
                          {ordersForDate.map((order, index) => (
                            <div key={order.id} className="relative mb-4">
                              {/* Connector dot */}
                              <div className="absolute -left-[21px] top-5 w-4 h-4 rounded-full bg-blue-400 dark:bg-blue-600 border-2 border-white dark:border-gray-800"></div>
  ...
  const renderCompactJobItem = (order: WorkOrder, index: number, total: number) => {
    ...
    return (
      <div 
        key={order.id}
        className={`p-2 rounded-lg border ${styles.cardBorder} ${styles.cardBg} hover:shadow-sm transition-all duration-200 cursor-pointer ${!isFirst ? 'mt-1' : ''}`}
        onClick={(e) => {
          e.stopPropagation();
      ...
      >
        ...
        {/* Added visual indicators for multiple jobs on the same day */}
        {!isLast && (
          <div className="mt-1 border-t border-dashed border-gray-200 dark:border-gray-700 pt-1 flex justify-center">
            <span className="text-[8px] text-gray-400 dark:text-gray-500">Same day</span>
          </div>
        )}
      </div>
    );
