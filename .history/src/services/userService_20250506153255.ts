...
// Promisify fs functions for Electron compatibility
...
export async function loadUsers(): Promise<User[]> {
    try {
        ...
        if (existsSync(userManagerFile)) {
            try {
                const userManagerData = await readFileAsync(userManagerFile, 'utf8');
        ...
        const data = await readFileAsync(USER_DATA_FILE, 'utf8');
...
export async function saveUsers(users: User[]): Promise<void> {
    try {
        await writeFileAsync(USER_DATA_FILE, JSON.stringify(users, null, 2));
    } catch (error) {
