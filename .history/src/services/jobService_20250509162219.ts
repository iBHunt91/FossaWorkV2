...
const cleanAddress = (address: string): string => {
  ...
  // Remove duplicate address segments
  ...
  // Ensure there's only one instance of each directional indicator
  const directionals = ['North', 'South', 'East', 'West', 'NE', 'NW', 'SE', 'SW'];
  directionals.forEach(dir => {
    const regex = new RegExp(`\\b${dir}\\b.*\\b${dir}\\b`, 'i');
    if (regex.test(cleaned)) {
      const firstIndex = cleaned.toLowerCase().indexOf(dir.toLowerCase());
      cleaned = cleaned.substring(0, firstIndex + dir.length) + 
                cleaned.substring(firstIndex + dir.length).replace(new RegExp(`\\b${dir}\\b`, 'i'), '');
    }
  });
...
export const geocodeJob = async (job: Job): Promise<Job> => {
  try {
    ...
    // Clean and standardize address before geocoding
    const cleanedAddress = job.address;
    const cleanedCity = job.city;
    const fullAddress = cleanAddress(`${cleanedAddress}, ${cleanedCity}, ${job.state} ${job.zipCode}`);
