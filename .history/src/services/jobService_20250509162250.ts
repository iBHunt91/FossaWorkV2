...
const cleanAddress = (address: string): string => {
  ...
  // Remove duplicate address segments (more thorough method)
...
export const geocodeJob = async (job: Job): Promise<Job> => {
  try {
    ...
    const rawAddress = `${job.address}, ${job.city}, ${job.state} ${job.zipCode}`;
    const fullAddress = cleanAddress(rawAddress);
    
    console.log(`Geocoding cleaned address: ${fullAddress} (original: ${rawAddress})`);
