 ...
 */
export const getUnifiedAutomationStatus = async (jobId: string, isBatch: boolean): Promise<UnifiedAutomationStatus> => {  try {    // Call the appropriate API based on job type    if (isBatch) {      const batchStatus = await getBatchAutomationStatus(jobId);            // Extract fuel type information from message if not directly available      let enhancedFuelType = batchStatus.fuelType;      let enhancedFuelCurrent = batchStatus.fuelCurrent;      let enhancedFuelTotal = batchStatus.fuelTotal;            if (batchStatus.currentVisitStatus && (!enhancedFuelType || !enhancedFuelTotal)) {        const fuelRegex = /(?:fuel\s+(?:type|grade):\s+)([a-z\-\s]+)(?:[^(]*)\(?(\d+)\/(\d+)\)?/i;        const fuelMatch = batchStatus.currentVisitStatus.match(fuelRegex);                if (fuelMatch && fuelMatch[1] && fuelMatch[2] && fuelMatch[3]) {          enhancedFuelType = fuelMatch[1].trim();          enhancedFuelCurrent = parseInt(fuelMatch[2]);          enhancedFuelTotal = parseInt(fuelMatch[3]);        }      }            // Enhanced dispenser progress extraction      let enhancedDispenserCount = batchStatus.dispenserCount;      let enhancedDispenserCurrent = batchStatus.dispenserCurrent;            if (batchStatus.currentVisitStatus && (!enhancedDispenserCount || !enhancedDispenserCurrent)) {        // Try to extract from status message        const dispenserRegex = /(?:dispenser|form)[:\s]+#?(\d+)(?:\s*(?:of|\/)\s*(\d+))?/i;        const dispenserMatch = batchStatus.currentVisitStatus.match(dispenserRegex);                if (dispenserMatch && dispenserMatch[1]) {          enhancedDispenserCurrent = parseInt(dispenserMatch[1]);          if (dispenserMatch[2]) {            enhancedDispenserCount = parseInt(dispenserMatch[2]);          }        }      }            // Map batch status to unified format with enhanced values      return {        jobId: jobId,        status: batchStatus.status,        message: batchStatus.message || '',        timestamp: batchStatus.timestamp,        startTime: batchStatus.startTime,        endTime: batchStatus.endTime,                // Batch-specific fields        completedVisits: batchStatus.completedVisits,        totalVisits: batchStatus.totalVisits,        currentVisit: batchStatus.currentVisit || undefined,        currentVisitStatus: batchStatus.currentVisitStatus,        completedVisitIds: batchStatus.completedVisitIds,                // Progress tracking        currentItem: batchStatus.completedVisits,        totalItems: batchStatus.totalVisits,                // Enhanced dispenser progress        dispenserCount: enhancedDispenserCount,        dispenserCurrent: enhancedDispenserCurrent,                // Enhanced fuel progress        fuelType: enhancedFuelType,        fuelCurrent: enhancedFuelCurrent,        fuelTotal: enhancedFuelTotal      };
    } else {
