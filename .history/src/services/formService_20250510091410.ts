...
/** * Get automation status for either single or batch jobs using a unified interface * @param {string} jobId - The job ID to check * @param {boolean} isBatch - Whether this is a batch job or single visit * @returns {Promise<UnifiedAutomationStatus>} */export const getUnifiedAutomationStatus = async (jobId: string, isBatch: boolean): Promise<UnifiedAutomationStatus> => {  try {    // Call the appropriate API based on job type    if (isBatch) {      const batchStatus = await getBatchAutomationStatus(jobId);            // Map batch status to unified format      return {        jobId: jobId,        status: batchStatus.status,        message: batchStatus.message || '',        timestamp: batchStatus.timestamp,        startTime: batchStatus.startTime,        endTime: batchStatus.endTime,                // Batch-specific fields        completedVisits: batchStatus.completedVisits,        totalVisits: batchStatus.totalVisits,        currentVisit: batchStatus.currentVisit || undefined,        currentVisitStatus: batchStatus.currentVisitStatus,        completedVisitIds: batchStatus.completedVisitIds,                // Progress tracking        currentItem: batchStatus.completedVisits,        totalItems: batchStatus.totalVisits,                // Dispenser progress (if available)        dispenserCount: batchStatus.dispenserCount,        dispenserCurrent: batchStatus.dispenserCurrent,                // Fuel progress (if available)        fuelType: batchStatus.fuelType,        fuelCurrent: batchStatus.fuelCurrent,        fuelTotal: batchStatus.fuelTotal      };    } else {      const singleStatus = await getFormAutomationStatus();            // Map single visit status to unified format      return {        jobId: jobId, // Use the provided jobId since the API might not return it        status: singleStatus.status,        message: singleStatus.message,        timestamp: singleStatus.timestamp,                // Single visit-specific fields        visitId: singleStatus.visitId,        visitName: singleStatus.visitName,                // Dispenser progress        dispenserCount: singleStatus.dispenserCount,        dispenserCurrent: singleStatus.dispenserCurrent,                // Fuel progress        fuelType: singleStatus.fuelType,        fuelCurrent: singleStatus.fuelCurrent,        fuelTotal: singleStatus.fuelTotal,                // Map dispenser progress to generic progress fields        currentItem: singleStatus.dispenserCurrent,        totalItems: singleStatus.dispenserCount      };    }  } catch (error) {    console.error(`Error getting unified status for ${isBatch ? 'batch' : 'single'} job ${jobId}:`, error);    // Return an error status    return {      jobId: jobId,      status: 'error',      message: error instanceof Error ? error.message : 'Failed to get automation status'    };  }};/** * Cancel an active form automation process * @param {string} jobId - The job ID to cancel * @returns {Promise<{success: boolean, message: string}>} */export const cancelFormAutomation = async (jobId: string): Promise<{success: boolean, message: string}> => {
  try {
