...
export type LogType = 'workOrder' | 'dispenser' | 'server' | 'formPrep' | 'dev';
 ...
 */
// Function to get the in-memory dev logs
export const getDevLogs = (): LogEntry[] => {
  return [...devLogs]; // Return a copy to prevent direct modification
};

export const getScrapeLogs = async (type: 'workOrder' | 'dispenser' | 'server' | 'formPrep' | 'dev'): Promise<LogEntry[]> => {
  // Special case for dev logs - return from memory
  if (type === 'dev') {
    return getDevLogs();
  }
...
export const addFormPrepLog = async (message: string): Promise<void> => {
  ...
  // Add local console log to help debug
  console.log(`[FormPrep Log] ${message}`);
};
...
// In-memory storage for dev logs since they're not supported by the backend yet
const devLogs: LogEntry[] = [];

// Event system for dev logs to notify components when logs change
type DevLogListener = () => void;
const devLogListeners: DevLogListener[] = [];

// Function to notify listeners that dev logs have changed
const notifyDevLogListeners = () => {
  devLogListeners.forEach(listener => listener());
};

// Register a listener for dev log changes
export const addDevLogListener = (listener: DevLogListener) => {
  devLogListeners.push(listener);
  return () => {
    const index = devLogListeners.indexOf(listener);
    if (index !== -1) {
      devLogListeners.splice(index, 1);
    }
  };
};

/**
 * Add a log entry for dev console operations (stored locally only)
 */
export const addDevLog = async (
  ...
  // Store in memory instead of sending to server
  ...
  notifyDevLogListeners();
  
  // Log to console for debugging
  console.log(`[Dev Console] ${formattedMessage}`);
};

/**
 * Convenience functions for adding dev console logs with specific severity levels
 */
export const devLog = {
  debug: (message: string, data?: any) => addDevLog('DEBUG', message, data),
  info: (message: string, data?: any) => addDevLog('INFO', message, data),
  success: (message: string, data?: any) => addDevLog('SUCCESS', message, data),
  warn: (message: string, data?: any) => addDevLog('WARN', message, data),
  error: (message: string, data?: any) => addDevLog('ERROR', message, data),
  system: (message: string, data?: any) => addDevLog('SYSTEM', message, data),
  network: (message: string, data?: any) => addDevLog('NETWORK', message, data),
  progress: (message: string, data?: any) => addDevLog('PROGRESS', message, data),
  status: (message: string, data?: any) => addDevLog('STATUS', message, data),
};
