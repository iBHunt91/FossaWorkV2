...
import { shouldShowWelcomeModal } from './utils/tutorialUtils';
...
const AppContent: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const initializeApp = async () => {
      try {
        // Fetch active user and all users
        const activeUser = await fetchActiveUser();
        await fetchAllUsers();

        // Check if we should show the tutorial welcome modal
        if (activeUser && activeUser.id === 'tutorial') {
          const shouldShow = shouldShowWelcomeModal();
          setShowTutorialWelcome(shouldShow);
        }
      } catch (error) {
  ...
  const loadData = async () => {
    try {
      // Use the scrape context's update function instead of direct API calls
  ...
  return (
    <DispenserProvider>
      ...
      {showTutorialWelcome && (
        <TutorialWelcomeModal onClose={() => setShowTutorialWelcome(false)} />
      )}
    </div>
  );
};
