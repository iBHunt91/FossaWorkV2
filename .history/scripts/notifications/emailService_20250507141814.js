...
export async function sendEmail(options) {
  try {
    ...
    const transporter = nodemailer.createTransport({
      ...
      logger: true,
      requireTLS: true,
      // Add TLS options if they exist in the config (for VPN compatibility)
      ...(emailSettings.tlsOptions && { tls: emailSettings.tlsOptions })
    });
    ...
    console.log('[EMAIL_SERVICE_DEBUG] Attempting to send email via transporter.sendMail...');
    const info = await transporter.sendMail(mailOptions);
    console.log('[EMAIL_SERVICE_DEBUG] transporter.sendMail promise resolved.');
    console.log('Email sent successfully:', info.messageId);
    return { success: true, messageId: info.messageId };
  } catch (error) {
    console.error('[EMAIL_SERVICE_DEBUG] transporter.sendMail promise rejected. Error details:', error);
...
export async function sendScheduleChangeEmail(changes, user) {
    ...
    try {
        // Get email settings
        const emailSettings = await getUserEmailSettings(user.id);
        
        console.log(`Using email settings for user ${user.id}:`, {
            recipient: user.email || emailSettings.recipientEmail,
            hasSmtpServer: !!emailSettings.smtpServer
        });
