        ...
        const filteredChanges = {
            ...
            allChanges: changes.allChanges.filter(change => {
                ...
                if (change.type === 'added') {
                    console.log(`Found 'added' type - preference setting is ${preferences.showAdded ? 'enabled' : 'disabled'}`);
                    if (!preferences.showAdded) return false;
                }
                else if (change.type === 'removed') {
                    console.log(`Found 'removed' type - preference setting is ${preferences.showRemoved ? 'enabled' : 'disabled'}`);
                    if (!preferences.showRemoved) return false;
                }
                else if (change.type === 'date_changed') {
                    console.log(`Found 'date_changed' type - preference setting is ${preferences.showModified ? 'enabled' : 'disabled'}`);
                    if (!preferences.showModified) return false;
                }
                else if (change.type === 'swap') {
                    console.log(`Found 'swap' type - preference setting is ${preferences.showSwapped ? 'enabled' : 'disabled'}`);
                    if (!preferences.showSwapped) return false;
                }
                else if (change.type === 'replacement') {
                    console.log(`Found 'replacement' type - treating as modified`);
                    if (!preferences.showModified) return false;
                }
                else {
