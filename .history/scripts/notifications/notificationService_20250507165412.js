...
export async function sendScheduleChangeNotifications(changes, user) {
    ...
    function filterChangesForUser(changes, preferences) {
        ...
        console.log(`Filtering changes for user with preferences:`, preferences);
        console.log(`Before filtering: ${changes.allChanges.length} changes`);
        
        // Add debugging to see what types we're processing
        const typesBeforeFilter = changes.allChanges.map(change => change.type);
        console.log(`Change types before filtering: ${JSON.stringify(typesBeforeFilter)}`);
        
        // Get the actual preferences - first from notifications.preferences, then from the direct preferences object
        const notificationPrefs = preferences.notifications || {};
        
        // Log the actual preference values that will be used
        console.log(`Using actual preference values: showAdded=${notificationPrefs.showAdded}, showRemoved=${notificationPrefs.showRemoved}, showModified=${notificationPrefs.showModified}, showSwapped=${notificationPrefs.showSwapped}`);
        
        const filteredChanges = {
            ...changes,
            allChanges: changes.allChanges.filter(change => {
                // Debug the current item being processed
                console.log(`Checking change with type '${change.type}' against preferences`);
                
                // More explicit type checking with debug info
                if (change.type === 'added') {
                    const shouldShow = notificationPrefs.showAdded !== false; // Default to true if not explicitly set to false
                    console.log(`Found 'added' type - preference setting is ${shouldShow ? 'enabled' : 'disabled'}`);
                    if (!shouldShow) return false;
                }
                else if (change.type === 'removed') {
                    const shouldShow = notificationPrefs.showRemoved !== false;
                    console.log(`Found 'removed' type - preference setting is ${shouldShow ? 'enabled' : 'disabled'}`);
                    if (!shouldShow) return false;
                }
                else if (change.type === 'date_changed') {
                    const shouldShow = notificationPrefs.showModified !== false;
                    console.log(`Found 'date_changed' type - preference setting is ${shouldShow ? 'enabled' : 'disabled'}`);
                    if (!shouldShow) return false;
                }
                else if (change.type === 'swap') {
                    const shouldShow = notificationPrefs.showSwapped !== false;
                    console.log(`Found 'swap' type - preference setting is ${shouldShow ? 'enabled' : 'disabled'}`);
                    if (!shouldShow) return false;
                }
                else if (change.type === 'replacement') {
                    const shouldShow = notificationPrefs.showModified !== false;
                    console.log(`Found 'replacement' type - treating as modified - preference setting is ${shouldShow ? 'enabled' : 'disabled'}`);
                    if (!shouldShow) return false;
                }
                else {
                    console.log(`Unknown change type '${change.type}' - including by default`);
                }
                return true;
            })
        };
        ...
        filteredChanges.summary = {
            ...
            modified: filteredChanges.allChanges.filter(c => c.type === 'date_changed' || c.type === 'replacement').length,
            swapped: filteredChanges.allChanges.filter(c => c.type === 'swap').length,
            total: filteredChanges.allChanges.length
        };
        
        console.log(`After filtering: ${filteredChanges.allChanges.length} changes remain`);
        console.log(`Updated summary: ${JSON.stringify(filteredChanges.summary)}`);
    ...
    try {
        ...
        if (shouldSendEmail) {
            try {
                ...
                if (wasRecentlySent('email', filteredChanges, user.id)) {
                    ...
                    console.log(`Processing email notification based on frequency settings for user ${user.id}`);
                    
                    // Process notification based on frequency preference (immediate vs daily digest)
                    const { processNotificationByFrequency } = await import('./notificationScheduler.js');
                    const emailResult = await processNotificationByFrequency(filteredChanges, user);
                    
                    if (emailResult.success) {
                        if (emailResult.type === 'immediate') {
                            console.log(`Email notification sent immediately to user ${user.id}`);
                        } else if (emailResult.type === 'daily_digest_stored') {
                            console.log(`Changes stored for daily digest for user ${user.id}`);
                        }
                        // Record that we sent/processed this notification
                        recordNotificationSent('email', filteredChanges, user.id);
                    } else {
                        console.error(`Error processing email notification:`, emailResult.error);
                    }
        ...
        // Send Pushover notification if enabled (now will respect frequency setting like email)
        ...
        if (shouldSendPushover) {
                try {
                    ...
                    if (userPushoverSettings && userPushoverSettings.appToken && userPushoverSettings.userKey) {
                        ...
                        // Check frequency setting - only send immediate if not set to daily digest
                        const frequency = notificationSettings.email?.frequency || 'immediate';
                        
                        if (frequency === 'daily') {
                            console.log(`Pushover notification skipped - user ${user.id} is set to daily digest mode`);
                            results.pushover = { 
                                success: true, 
                                skipped: true, 
                                reason: 'daily_digest_mode'
                            };
                        } else {
                            // Send immediate Pushover notification
...
function filterChangesForUser(changes, userPreferences) {
    ...
    console.log(`Filtering changes for user with preferences:`, userPreferences);
    console.log(`Before filtering: ${changes.allChanges.length} changes`);
    ...
    if (notificationPrefs.scheduleChanges === false) {
        console.log("Schedule change notifications are disabled for this user");
        return { allChanges: [], summary: { added: 0, removed: 0, modified: 0, swapped: 0, total: 0 } };
    }
    ...
    // First apply store/location filters if configured
    const filters = notificationPrefs.filters || { stores: [], locations: [] };
    let filteredByLocation = changes.allChanges;
    
    // Apply store/location filtering if enabled
    if ((filters.stores && filters.stores.length > 0) || (filters.locations && filters.locations.length > 0)) {
        console.log(`Applying store/location filters: ${JSON.stringify(filters)}`);
        filteredByLocation = filterChangesByStoreAndLocation(changes.allChanges, filters);
        console.log(`After store/location filtering: ${filteredByLocation.length} changes remain`);
    }
    
    // Now filter by change type preferences
    const filteredChanges = {
        ...changes,
        allChanges: filteredByLocation.filter(change => {
            // Debug the current item being processed
            console.log(`Checking change with type '${change.type}' against preferences`);
