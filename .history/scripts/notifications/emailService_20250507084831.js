...
export async function getUserEmailSettings() {
  try {
    ...
    if (activeUser) {
      ...
      const userSpecificPath = path.join(__dirname, '..', '..', 'data', 'users', activeUser.id, 'email_settings.json');
      if (fs.existsSync(userSpecificPath)) {
        const userSettings = JSON.parse(fs.readFileSync(userSpecificPath, 'utf8'));
        if (userSettings && userSettings.recipientEmail) {
          console.log('Found user-specific email settings:', userSettings);
          return userSettings;
        }
      }
    }
