...
// Reference to the addFormPrepLog function
let addFormPrepLogCallback = null;

/**
 * Register the addFormPrepLog function
 * @param {Function} callback - Function that adds logs to formPrep log array
 */
const registerFormPrepLogger = (callback) => {
  addFormPrepLogCallback = callback;
};
 ...
 * @param {string|object} content - Content or component name for FORM_PREP logs
 */
function info(title, content) {
  // Check if this is a form prep log
  if (content === 'FORM_PREP' && addFormPrepLogCallback) {
    addFormPrepLogCallback(title);
  }
...
function warn(title, content) {
  // Check if this is a form prep log
  if (content === 'FORM_PREP' && addFormPrepLogCallback) {
    addFormPrepLogCallback(`⚠️ ${title}`);
  }
...
function error(title, content) {
  ...
  // If content is an error object, extract message and stack
  let errorInfo = [];
  if (content) {
    errorInfo.push(String(content.message || content));
