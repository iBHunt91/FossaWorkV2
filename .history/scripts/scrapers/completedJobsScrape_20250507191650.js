...
export async function scrapeCompletedJobs(options = {}) {
    ...
    try {
        ...
        if (activeUserId) {
            try {
                ...
                if (fs.existsSync(completedJobsFilePath)) {
                    ...
                    // Log current data before any changes
                    console.log(`Found completed jobs file with ${completedJobsData.completedJobs ? completedJobsData.completedJobs.length : 0} jobs`);
                    ...
                    if (completedJobsData.metadata && completedJobsData.metadata.scheduledForCleanup) {
                        ...
                        console.log(`No cleanup needed for completed jobs of user ${activeUserId}`);
                    }
                } else {
                    console.log(`No existing completed jobs file found for user ${activeUserId}, will create new one`);
                }
            } catch (cleanupError) {
        ...
        const completedJobsData = await page.evaluate(() => {
            ...
            const jobData = Array.from(jobElements).map(job => {
                ...
                if (workOrderLink) {
                    ...
                    console.log('Found job element without valid work order ID');
                }
            ...
            console.log(`After filtering, found ${jobData.length} valid completed jobs`);
            
            // Extra validation
            if (jobData.length > 0) {
                console.log(`Sample completed job: ID=${jobData[0].id}, Customer=${jobData[0].customerName}`);
            }
            
            // Extract just the IDs for our actual result
            const jobIds = jobData.map(item => item.id);
        ...
        if (activeUserId) {
            ...
            console.log(`New completed jobs to add: ${completedJobsData.jobIds.length}`);
            console.log('New job IDs:', completedJobsData.jobIds);
            
            // Merge with new data, avoiding duplicates
            const allJobIds = [...new Set([...existingData.completedJobs, ...completedJobsData.jobIds])];
            console.log(`Combined jobs count after merging: ${allJobIds.length}`);
            
            // Create the final data with jobs
            const updatedData = {
            ...
            // Explicitly check and ensure the jobs array is not empty if we have data
            if (completedJobsData.count > 0 && updatedData.completedJobs.length === 0) {
                console.error('ERROR: Jobs array is empty despite having completed jobs data!');
                // Force add the jobs if needed
                updatedData.completedJobs = completedJobsData.jobIds;
            }
            
            // Write the file
            fs.writeFileSync(completedJobsFilePath, JSON.stringify(updatedData, null, 2));
            
            // Verify the file was written correctly
            try {
                const verifyData = JSON.parse(fs.readFileSync(completedJobsFilePath, 'utf8'));
                console.log(`Verification: Saved ${verifyData.completedJobs.length} completed job IDs to ${completedJobsFilePath}`);
                console.log(`First few job IDs saved: ${verifyData.completedJobs.slice(0, 3).join(', ')}`);
            } catch (verifyError) {
                console.error(`Error verifying saved completed jobs: ${verifyError.message}`);
            }
        }
