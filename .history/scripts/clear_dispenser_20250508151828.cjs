 ...
 * Run with: node scripts/clear_dispenser.cjs <storeId> <userId>
 */
...
if (!storeId) {
  console.error('Please provide a store ID as the first argument');
  console.error('Usage: node scripts/clear_dispenser.cjs <storeId> <userId>');
  process.exit(1);
}
...
const dispenserStorePath = path.resolve(__dirname, '..', 'data', 'users', userId, 'dispenser_store.json');
console.log(`Targeting dispenser store for user ${userId} at: ${dispenserStorePath}`);
