...
export async function updateUserNotificationSettings(userId, settings) {
    try {
        ...
        // Deep merge for nested objects, but handle frequency/deliveryTime potentially coming from the root
        const mergedSettings = {
            ...currentSettings,
            enabled: settings.enabled !== undefined ? settings.enabled : currentSettings.enabled,
            email: {
                // Start with current email settings or an empty object
                ...(currentSettings.email || {}),
                // Merge settings if they exist in a nested email object (future-proofing)
                ...(settings.email || {}),
                // Explicitly override frequency and deliveryTime if they exist at the root level of incoming settings
                enabled: settings.email?.enabled !== undefined 
                    ? settings.email.enabled 
                    : (settings.enabled !== undefined ? settings.enabled : currentSettings.email?.enabled),
                frequency: settings.frequency || settings.email?.frequency || currentSettings.email?.frequency || DEFAULT_NOTIFICATION_SETTINGS.email.frequency,
                deliveryTime: settings.deliveryTime || settings.email?.deliveryTime || currentSettings.email?.deliveryTime || DEFAULT_NOTIFICATION_SETTINGS.email.deliveryTime
            },
            pushover: {
                ...(currentSettings.pushover || {}),
                ...(settings.pushover || {}),
                enabled: settings.pushover?.enabled !== undefined 
                    ? settings.pushover.enabled 
                    : (settings.enabled !== undefined ? settings.enabled : currentSettings.pushover?.enabled) // Link pushover enabled to main toggle if not specified
            }
        };
        
        // Ensure email enabled status is consistent with the main toggle if email settings exist
        if (mergedSettings.email) {
          mergedSettings.email.enabled = mergedSettings.enabled && (mergedSettings.email.enabled !== false);
        }
        // Ensure pushover enabled status is consistent with the main toggle if pushover settings exist
        if (mergedSettings.pushover) {
           mergedSettings.pushover.enabled = mergedSettings.enabled && (mergedSettings.pushover.enabled !== false);
        }
