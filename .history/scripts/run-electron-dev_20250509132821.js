...
async function startViteAndGetPort() {
  return new Promise((resolve, reject) => {
    ...
    viteProcess.stdout.on('data', (data) => {
      ...
      console.log(`[Vite STDOUT CHUNK] ${outputChunk.trim()}`); // Log the raw chunk
      
      // Check the accumulated buffer for the port
      ...
      // Optional: Limit buffer size to prevent memory issues if Vite is extremely verbose for a long time without matching
      if (stdoutBuffer.length > 1024 * 10) { // e.g., 10KB limit
          stdoutBuffer = stdoutBuffer.slice(-1024 * 5); // Keep last 5KB
      }
    });

    viteProcess.stderr.on('data', (data) => {
      // Log Vite stderr for debugging, but don't reject on warnings
    ...
    viteProcess.on('close', (code) => {
      ...
      // If resolved, it means port was found or another error occurred and was handled.
    });
    
    const timeoutDuration = 45000; // 45 seconds timeout for Vite to start and report port
    ...
    // Ensure timeout is cleared if promise settles
    const cleanupAndSettle = (settler, value) => {
        if (!resolved) {
            resolved = true;
    ...
    resolve = (value) => cleanupAndSettle(originalResolve, value);
    const originalReject = reject;
    reject = (reason) => cleanupAndSettle(originalReject, reason);
  });
}
