...
/**
 * Marks completed jobs for cleanup before a new scrape
 * @param {string} userId - User ID whose completed jobs to mark for cleanup
 */
function cleanupCompletedJobsBeforeScrape(userId) {
    if (!userId) {
        console.log('No user ID provided for cleanup marking');
        return;
    }
    ...
    try {
        ...
        // Read current data to include in log
        const completedJobsData = JSON.parse(fs.readFileSync(completedJobsFilePath, 'utf8'));
        const jobCount = completedJobsData.completedJobs ? completedJobsData.completedJobs.length : 0;
        
        console.log(`Found ${jobCount} completed jobs for user ${userId} - marking for cleanup after next completed jobs scrape`);
        
        if (jobCount === 0) {
            console.log('No completed jobs to mark for cleanup');
            return;
        }
        
        // We'll keep the existing jobs but mark them for cleanup after the next scrape
        const updatedData = {
            ...completedJobsData,
            metadata: {
        ...
        console.log(`Marked ${jobCount} completed jobs for cleanup after next completed jobs scrape for user ${userId}`);
    } catch (error) {
        console.error(`Error marking completed jobs for cleanup: ${error.message}`);
    }
}
...
async function runScrape(options = {}) {
    ...
    try {
        ...
        // Mark completed jobs for cleanup before starting new scrape
        if (activeUserId) {
            cleanupCompletedJobsBeforeScrape(activeUserId);
        }
        ...
        if (activeUserId) {
            ...
            try {
                console.log('Fetching completed jobs before analyzing schedule changes...');
                // Import and use the scrapeCompletedJobs function
                const { default: scrapeCompletedJobs } = await import('./scrapers/completedJobsScrape.js');
                ...
                await scrapeCompletedJobs({
                    ...
                    progressCallback: (progress, message) => {
                        console.log(`Completed jobs scrape progress: ${progress}% - ${message}`);
                        if (progressCallback) {
...
export { runScrape, compareCurrentAndPrevious, cleanupCompletedJobsBeforeScrape }; 