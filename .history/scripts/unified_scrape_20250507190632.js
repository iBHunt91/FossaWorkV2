...
/**
 * Cleans up completed jobs data before a new scrape
...
function cleanupCompletedJobsBeforeScrape(userId) {
    ...
    try {
        ...
        // We'll just keep the existing structure but note the cleanup
        // Don't actually clear the list until after the new scrape
        const updatedData = {
            ...completedJobsData,
            metadata: {
        ...
        console.log(`Marked ${jobCount} completed jobs for cleanup after next scrape for user ${userId}`);
    } catch (error) {
