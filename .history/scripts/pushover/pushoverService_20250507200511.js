...
function findUserSettingsPath(userId = null, settingsFileName = 'pushover_settings.json') {
  try {
    ...
    if (userId) {
      ...
      // If not found with specific user ID, return null immediately instead of looking elsewhere
      console.log(`Settings file ${settingsFileName} not found for user ${userId}`);
      return null;
    }
...
export function getUserPushoverSettings(userId = null) {
  try {
    ...
    // If a specific userId was provided but no settings found, don't fall back to env vars
    if (userId && !settingsPath) {
      console.log(`No pushover settings file found for specific user ${userId}, returning empty settings`);
      return {
        appToken: '',
        userKey: '',
        preferences: {
    ...
    // If no settings file was found (and no specific userId was provided), fall back to environment variables
    if (!settingsPath) {
...
function getAppToken(userId = null) {
  ...
  // If specific userId provided, only return the token from settings (no fallback)
  if (userId) {
    return settings.appToken || '';
  }
  
  // Otherwise fall back to environment variables (for global settings)
...
function getUserKey(userId = null) {
  ...
  // If specific userId provided, only return the key from settings (no fallback)
  if (userId) {
...
function formatScheduleChangeMessage(changes) {
  let message = '<b>📅 Schedule Changes</b>\n\n';
  ...
  if (changes.allChanges && changes.allChanges.length > 0) {
    ...
    if (dateChanges.length > 0) {
      for (const job of dateChanges) {
        message += `<b><font color="#FFCC00">━━━</font></b>\n`;
        message += `<b>📅 Date Changed</b>\n`;
        
        // Combine visit ID and store on one line
        const visitId = getVisitId(job.jobId);
        message += `<b>#${visitId}</b> | ${job.store}\n`;
        
        // Show date change
        message += `<b>Date:</b> ${job.oldDate} ➔ <b>${job.newDate}</b>\n`;
    ...
    if (swappedJobs.length > 0) {
      for (const swap of swappedJobs) {
        message += `<b><font color="#5AC8FA">━━━</font></b>\n`;
        message += `<b>🔄 Swapped Dates</b>\n`;
        
        // Job 1
        const visitId1 = getVisitId(swap.job1Id);
        message += `<b>Job 1:</b> #${visitId1} | ${swap.job1Store}\n`;
        message += `<b>Date:</b> ${swap.oldDate1} ➔ <b>${swap.newDate1}</b>\n`;
        message += `<b>Location:</b> ${swap.job1Location} | <b>Dispensers:</b> ${swap.job1Dispensers || 0}\n`;
        
        message += `<i>swapped with</i>\n`;
        
        // Job 2
        const visitId2 = getVisitId(swap.job2Id);
        message += `<b>Job 2:</b> #${visitId2} | ${swap.job2Store}\n`;
        message += `<b>Date:</b> ${swap.oldDate2} ➔ <b>${swap.newDate2}</b>\n`;
        message += `<b>Location:</b> ${swap.job2Location} | <b>Dispensers:</b> ${swap.job2Dispensers || 0}\n`;
      }
    }
    
    // Add replacement jobs
    const replacementJobs = changes.allChanges.filter(change => change.type === 'replacement');
    if (replacementJobs.length > 0) {
      for (const replacement of replacementJobs) {
        message += `<b><font color="#FF9500">━━━</font></b>\n`;
        message += `<b>♻️ Replacement</b>\n`;
        
        // Removed job
        const removedVisitId = getVisitId(replacement.removedJobId);
        message += `<b>Removed:</b> #${removedVisitId} | ${replacement.removedStore}\n`;
        message += `<b>Location:</b> ${replacement.removedLocation} | <b>Dispensers:</b> ${replacement.removedDispensers || 0}\n`;
        
        message += `<i>replaced with</i>\n`;
        
        // Added job
        const addedVisitId = getVisitId(replacement.addedJobId);
        message += `<b>Added:</b> #${addedVisitId} | ${replacement.addedStore}\n`;
        message += `<b>Location:</b> ${replacement.addedLocation} | <b>Dispensers:</b> ${replacement.addedDispensers || 0}\n`;
        message += `<b>Date:</b> ${replacement.date}\n`;
      }
    }
  }
  ...
  if (changes.summary.added > 0) {
    message += `➕ <b>${changes.summary.added}</b> added\n`;
  }
  if (changes.summary.removed > 0) {
    message += `🗑️ <b>${changes.summary.removed}</b> removed\n`;
  }
  if (changes.summary.modified > 0) {
    message += `📅 <b>${changes.summary.modified}</b> dates changed\n`;
  }
  if (changes.summary.swapped > 0) {
    message += `🔄 <b>${changes.summary.swapped}</b> swapped\n`;
  }
  if (changes.summary.replaced > 0) {
    message += `♻️ <b>${changes.summary.replaced}</b> replaced\n`;
  }
...
/**
 * Calculate the effective length of a message for Pushover (counting HTML tags)
 * @param {string} message - The HTML message
 * @returns {number} - The effective character count
 */
function calculatePushoverMessageLength(message) {
  // Pushover counts HTML tags in the character limit
...
export async function sendScheduleChangePushover(changes, users) {
  ...
  // Check if the message exceeds Pushover's limit (1024 characters)
  const PUSHOVER_CHAR_LIMIT = 1024;
  const messageLength = calculatePushoverMessageLength(message);
  ...
  for (const user of userArray) {
    ...
    try {
      ...
      if (messageLength > PUSHOVER_CHAR_LIMIT) {
        console.log(`Message exceeds Pushover character limit (${messageLength}/${PUSHOVER_CHAR_LIMIT}). Splitting into multiple messages.`);
        
        // Split changes by type for better organization
        const changesByType = {
          added: changes.allChanges.filter(c => c.type === 'added'),
          removed: changes.allChanges.filter(c => c.type === 'removed'),
          dateChanged: changes.allChanges.filter(c => c.type === 'date_changed'),
        ...
        let partNumber = 1;
        const totalParts = Object.values(changesByType).filter(group => group.length > 0).length + 1; // +1 for summary
        
        // Send summary first
        const summaryTitle = `${title} (1/${totalParts})`;
        const summaryMessage = `<b>📅 Schedule Change Summary</b>\n\n<b>Total Changes:</b> ${totalChanges}\n\n<b>Summary:</b>\n`;
        let detailedSummary = '';
        
        if (changes.summary.added > 0) {
          detailedSummary += `➕ <b>${changes.summary.added}</b> added\n`;
        }
        if (changes.summary.removed > 0) {
          detailedSummary += `🗑️ <b>${changes.summary.removed}</b> removed\n`;
        }
        if (changes.summary.modified > 0) {
          detailedSummary += `📅 <b>${changes.summary.modified}</b> dates changed\n`;
        }
        if (changes.summary.swapped > 0) {
          detailedSummary += `🔄 <b>${changes.summary.swapped}</b> swapped\n`;
        }
        if (changes.summary.replaced > 0) {
          detailedSummary += `♻️ <b>${changes.summary.replaced}</b> replaced\n`;
        }
        
        const summaryResult = await sendPushoverNotification({
          title: summaryTitle,
          message: summaryMessage + detailedSummary,
          userKey: userKey,
          userId: userId,
          priority: 0,
          sound: 'pushover',
        ...
        notificationResults.push(summaryResult);
        partNumber++;
        
        // Send each change type in a separate notification
        for (const [changeType, changeList] of Object.entries(changesByType)) {
          ...
          switch (changeType) {
            case 'added':
              typeTitle = 'Added Jobs';
              emoji = '➕';
              color = '#34C759';
              break;
            case 'removed':
              typeTitle = 'Removed Jobs';
              emoji = '🗑️';
              color = '#FF3B30';
              break;
            case 'dateChanged':
              typeTitle = 'Date Changes';
              emoji = '📅';
              color = '#FFCC00';
              break;
            case 'swapped':
              typeTitle = 'Swapped Jobs';
              emoji = '🔄';
              color = '#5AC8FA';
              break;
            case 'replacement':
              typeTitle = 'Replacement Jobs';
              emoji = '♻️';
              color = '#FF9500';
              break;
          }
          
          // For extremely large sections, split into multiple batches
          const batchSize = Math.min(5, Math.max(1, Math.ceil(changeList.length / 3))); // Adaptive batch sizing
          const batches = [];
          
          // Create batches of changes
          for (let i = 0; i < changeList.length; i += batchSize) {
            batches.push(changeList.slice(i, i + batchSize));
          }
          ...
          for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
            ...
            // Format a message just for this batch
            const partTitle = `${title} (${partNumber}/${totalParts}${isMultipleBatches ? ` Batch ${batchNumber}/${batches.length}` : ''}: ${typeTitle})`;
            let partMessage = `<b>${emoji} ${typeTitle} ${isMultipleBatches ? `(Batch ${batchNumber}/${batches.length})` : `(${changeList.length})`}</b>\n\n`;
            
            // Format each change in this batch
            for (const change of batch) {
              // Add a section header for this change
              partMessage += `<b><font color="${color}">━━━</font></b>\n`;
              
              // Format based on change type
              if (changeType === 'added') {
                const visitId = getVisitId(change.jobId);
                partMessage += `<b>${emoji} Added Visit</b>\n`;
                partMessage += `<b>#${visitId}</b> | ${change.store} | <b>Date:</b> ${change.date}\n`;
                
                // Create a compact location display
                let mapUrl;
                let displayAddress = change.location || 'Unknown';
                ...
                if (change.address) {
                  ...
                  // Create Google Maps URL if we have a full address
                  const fullAddress = [
                ...
                if (mapUrl) {
                  partMessage += `<b>Location:</b> <a href="${mapUrl}">${displayAddress}</a> | <b>Dispensers:</b> ${change.dispensers || 0}\n`;
                } else {
                  partMessage += `<b>Location:</b> ${displayAddress} | <b>Dispensers:</b> ${change.dispensers || 0}\n`;
                }
              } else if (changeType === 'removed') {
                const visitId = getVisitId(change.jobId);
                partMessage += `<b>${emoji} Removed Visit</b>\n`;
                partMessage += `<b>#${visitId}</b> | ${change.store} | <b>Date:</b> ${change.date}\n`;
                partMessage += `<b>Location:</b> ${change.location || 'Unknown'} | <b>Dispensers:</b> ${change.dispensers || 0}\n`;
              } else if (changeType === 'dateChanged') {
                const visitId = getVisitId(change.jobId);
                partMessage += `<b>${emoji} Date Changed</b>\n`;
                partMessage += `<b>#${visitId}</b> | ${change.store}\n`;
                partMessage += `<b>Date:</b> ${change.oldDate} ➔ <b>${change.newDate}</b>\n`;
                partMessage += `<b>Location:</b> ${change.location || 'Unknown'} | <b>Dispensers:</b> ${change.dispensers || 0}\n`;
              } else if (changeType === 'swapped') {
                partMessage += `<b>${emoji} Swapped Dates</b>\n`;
                // Job 1
                const visitId1 = getVisitId(change.job1Id);
                partMessage += `<b>Job 1:</b> #${visitId1} | ${change.job1Store}\n`;
                partMessage += `<b>Date:</b> ${change.oldDate1} ➔ <b>${change.newDate1}</b>\n`;
                partMessage += `<b>Location:</b> ${change.job1Location || 'Unknown'} | <b>Dispensers:</b> ${change.job1Dispensers || 0}\n`;
                partMessage += `<i>swapped with</i>\n`;
                // Job 2
                const visitId2 = getVisitId(change.job2Id);
                partMessage += `<b>Job 2:</b> #${visitId2} | ${change.job2Store}\n`;
                partMessage += `<b>Date:</b> ${change.oldDate2} ➔ <b>${change.newDate2}</b>\n`;
                partMessage += `<b>Location:</b> ${change.job2Location || 'Unknown'} | <b>Dispensers:</b> ${change.job2Dispensers || 0}\n`;
              } else if (changeType === 'replacement') {
                partMessage += `<b>${emoji} Replacement</b>\n`;
                // Removed job
                const removedVisitId = getVisitId(change.removedJobId);
                partMessage += `<b>Removed:</b> #${removedVisitId} | ${change.removedStore}\n`;
                partMessage += `<b>Location:</b> ${change.removedLocation || 'Unknown'} | <b>Dispensers:</b> ${change.removedDispensers || 0}\n`;
                partMessage += `<i>replaced with</i>\n`;
                // Added job
                const addedVisitId = getVisitId(change.addedJobId);
                partMessage += `<b>Added:</b> #${addedVisitId} | ${change.addedStore}\n`;
                partMessage += `<b>Location:</b> ${change.addedLocation || 'Unknown'} | <b>Dispensers:</b> ${change.addedDispensers || 0}\n`;
                partMessage += `<b>Date:</b> ${change.date}\n`;
              }
            }
            
            // Check message length and send
            const messageLength = calculatePushoverMessageLength(partMessage);
            if (messageLength > PUSHOVER_CHAR_LIMIT) {
              console.warn(`Batch message still exceeds Pushover limit (${messageLength}/${PUSHOVER_CHAR_LIMIT}). Sending with reduced content.`);
              // Truncate the message with an indicator that it was cut off
              partMessage = partMessage.substring(0, PUSHOVER_CHAR_LIMIT - 50) + '\n\n<i>... message truncated due to length limits</i>';
            }
            
            const typeResult = await sendPushoverNotification({
              title: partTitle,
              message: partMessage,
              userKey: userKey,
              userId: userId,
            ...
            notificationResults.push(typeResult);
          }
        ...
        const combinedResult = {
          ...
          data: notificationResults.map(r => r.data).filter(Boolean),
          error: notificationResults.find(r => r.error)?.error
        };
        
        allPromises.push(Promise.resolve(combinedResult));
      } else {
        // Normal case - just send a single notification
        const result = await sendPushoverNotification({
          title: title,
          message: message,
          userKey: userKey,
    ...
    for (const user of userArray) {
        ...
        // 2. Try from user settings if there's a user ID
            ...
            try {
                ...
                // If we have a user ID and no key found in settings, do NOT fall back to env vars
                else {
                    console.log(`No Pushover key found in settings for user ${user.id}`);
                    // Don't set userKey - it will remain null
                }
            } catch (error) {
                console.error(`Error getting Pushover settings for user ${user.id}:`, error);
            }
        }
        // 3. Try from environment variables ONLY if no specific user ID was provided
        else if (!user.id) {
            userKey = getUserKey();
            if (userKey) {
                console.log('Using Pushover key from environment variables (no user ID provided)');
            }
        }
...
/**
 * Remove user pushover settings
 * @param {string} userId - The user ID to remove settings for (optional)
 * @returns {boolean} - Whether the removal was successful
 */
export function removePushoverSettings(userId = null) {
  try {
    // Try to find the settings file
    const settingsPath = findUserSettingsPath(userId, 'pushover_settings.json');
    
    // If no settings file was found, consider it a success
    if (!settingsPath) {
      console.log('No pushover settings file found to remove');
      return true;
    }
    
    // Instead of deleting the file, just clear its contents by writing an empty object
    // with default preferences to maintain the file structure
    const emptySettings = {
      appToken: '',
      userKey: '',
      preferences: {
    ...
    fs.writeFileSync(settingsPath, JSON.stringify(emptySettings, null, 2), 'utf8');
    console.log(`Pushover settings cleared successfully at ${settingsPath}`);
    return true;
  } catch (error) {
    console.error('Error removing pushover settings:', error);
    return false;
  }
}