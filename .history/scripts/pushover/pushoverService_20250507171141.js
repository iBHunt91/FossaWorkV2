...
export async function sendScheduleChangePushover(changes, users) {
  ...
  for (const user of userArray) {
    ...
    try {
      ...
      if (message.length > PUSHOVER_CHAR_LIMIT) {
        console.log(`Message exceeds Pushover character limit (${message.length}/${PUSHOVER_CHAR_LIMIT}). Splitting into multiple messages.`);
        ...
        for (const [changeType, changeList] of Object.entries(changesByType)) {
          ...
          // Format a message just for this change type
          const partTitle = `${title} (${partNumber}/${totalParts}: ${typeTitle})`;
          let partMessage = `<b>${emoji} ${typeTitle} (${changeList.length})</b>\n\n`;
          ...
          for (const change of changeList) {
            ...
            if (changeType === 'added') {
              ...
              partMessage += `<b>Location:</b> ${change.location} | <b>Dispensers:</b> ${change.dispensers || 0}\n`;
              ...
              partMessage += `<b>Location:</b> ${change.job1Location} | <b>Dispensers:</b> ${change.job1Dispensers || 0}\n`;
              ...
              partMessage += `<b>Location:</b> ${change.job2Location} | <b>Dispensers:</b> ${change.job2Dispensers || 0}\n`;
              ...
              partMessage += `<b>Location:</b> ${change.removedLocation} | <b>Dispensers:</b> ${change.removedDispensers || 0}\n`;
              ...
              partMessage += `<b>Location:</b> ${change.addedLocation} | <b>Dispensers:</b> ${change.addedDispensers || 0}\n`;
              partMessage += `<b>Date:</b> ${change.date}\n`;
            }
            
            // For extremely large change sets, we might need to further split this message
            if (partMessage.length > PUSHOVER_CHAR_LIMIT * 0.8) { // 80% of limit as safety threshold
              ...
              // Reset the message for the next batch
              partMessage = `<b>${emoji} ${typeTitle} (continued)</b>\n\n`;
            }
          }
          
          // Send the message for this change type
