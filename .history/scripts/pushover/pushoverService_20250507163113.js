...
export async function sendAlertPushover(alerts, users) {
    ...
    if (!alerts || alerts.length === 0 || userArray.length === 0) {
        console.log('No alerts or users to process for alert notifications');
    ...
    const promises = [];
    for (const user of userArray) {
        // Get user key - either directly from user object or fall back to environment
        const userKey = user.pushoverKey || getUserKey(user.id);
