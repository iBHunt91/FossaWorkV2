...
async function startViteAndGetPort() {
  return new Promise((resolve, reject) => {
    ...
    const promiseRejector = (reason) => {
      if (!resolved) {
        resolved = true;
        clearTimeout(timeoutId);
        if (viteProcess && !viteProcess.killed) {
          // Attempt to kill Vite, but don't let kill errors prevent promise rejection
    ...
    timeoutId = setTimeout(() => {
      promiseRejector(new Error(`Timed out waiting for Vite to report its port after ${timeoutDuration / 1000}s.`));
    }, timeoutDuration);
    ...
    viteProcess.stdout.on('data', (data) => {
      ...
      if (portMatch && portMatch[1]) {
        ...
        console.log(`✅ Vite reported URL via buffer: http://localhost:${vitePort}`);
        if (viteProcess && !viteProcess.killed) { // Check viteProcess still exists
          promiseResolver(vitePort);
        } else if (!resolved) { // If viteProcess is gone but we found port, maybe it exited right after printing
          // This case is tricky; for now, we assume if killed, it's an issue.
          // Consider if resolving here is safe if process is already dead.
        }
      }
    ...
    viteProcess.on('error', (err) => {
      console.error('❌ Failed to start Vite process (on error event).', err);
      promiseRejector(err);
    });
    ...
    viteProcess.on('close', (code) => {
      ...
      if (!resolved && code !== 0) {
        promiseRejector(new Error(`Vite process exited with code ${code} before reporting port.`));
      }
      // If already resolved (port found), an exit code 0 or 1 (e.g. from SIGINT) is normal.
      // If it exits with error *after* reporting port, Electron might fail later, which is a separate issue.
    });
  });
}
