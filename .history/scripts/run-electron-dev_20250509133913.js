...
async function startViteAndGetPort() {
  return new Promise((resolve, reject) => {
    ...
    findAvailablePort(5173, 20).then(availablePort => {
      ...
      viteProcess = spawn(npmCmd, ['run', 'dev', '--', '--port', availablePort.toString()], { 
        env: { 
          ...process.env, 
          NODE_ENV: 'development',
      ...
      let stdoutBuffer = ''; 
      let timeoutId = null;
      const timeoutDuration = 90000; // Increased timeout to 90s
      ...
      viteProcess.stdout.on('data', (data) => {
        ...
        // Check for the "ready in XXX ms" message first as a quick success indicator
        if (stdoutBuffer.includes('ready in') && stdoutBuffer.includes('Local:')) {
