...
app.get('/api/debug/logs', (req, res) => {
  res.json({
    ...
    server: scrapeJobLogs.server.length,
    formPrep: scrapeJobLogs.formPrep.length,
    samples: {
      dispenser: scrapeJobLogs.dispenser.slice(0, 3),
      workOrder: scrapeJobLogs.workOrder.slice(0, 3),
      server: scrapeJobLogs.server.slice(0, 3),
      formPrep: scrapeJobLogs.formPrep.slice(0, 3)
    }
  });
});
...
// Register the addLogEntry function with the logger
logger.registerLogToUiCallback(addLogEntry);
