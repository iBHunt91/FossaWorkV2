...
import { router, addLogEntry, scrapeJobLogs, serverLog, workOrderLog, dispenserLog } from './routes/api.js';
...
import { runScrape, compareCurrentAndPrevious, cleanupCompletedJobsBeforeScrape } from '../scripts/unified_scrape.js';
...
const forceResetJobs = () => {
  ...
  serverLog.system('Force reset all job states to idle on server start');
  workOrderLog.system('Server restarted - forced reset of job state');
  dispenserLog.system('Server restarted - forced reset of job state');
};
...
serverLog.info('Server starting - System logging initialized');
dispenserLog.info('Server starting - Dispenser logging initialized');
workOrderLog.info('Server starting - Work order logging initialized');
...
app.use('/api/form-automation', formAutomationRouter);
  ...
  for (let i = 0; i < users.length; i++) {
    ...
    try {
      ...
      if (user.id) {
        ...
        try {
          info(`[User: ${user.label || user.email}] Fetching completed jobs before analyzing schedule changes...`, 'SCHEDULER');
          const { default: scrapeCompletedJobs } = await import('../scripts/scrapers/completedJobsScrape.js');
          ...
          await scrapeCompletedJobs({
            ...
            userId: user.id,
            progressCallback: (progress, message) => {
              info(`[User: ${user.label || user.email}] Completed jobs scrape progress: ${progress}% - ${message}`, 'SCHEDULER');
            }
          });
