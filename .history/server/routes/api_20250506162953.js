...
import { getScrapedFiles, getLatestScrapeFile, getPreviousScrapeFile, getScheduleChangesPath } from '../../scripts/utils/dataManager.js';
import { getActiveUser, resolveUserFilePath } from '../utils/userManager.js';
import { spawn } from 'child_process';
import { getLogLevel, setLogLevel, LOG_LEVELS, enableComponents, disableComponents, resetComponentFilters, debug, warn, error, info, success, getServerLogs } from '../utils/logger.js';
...
router.get('/scrape-logs/:type', (req, res) => {
  ...
  if (type !== 'workOrder' && type !== 'dispenser' && type !== 'server' && type !== 'formPrep') {
    return res.status(400).json({ error: 'Invalid log type' });
  }
  
  // If requesting server logs, return from the logger system
  if (type === 'server') {
    const serverLogs = getServerLogs().map(entry => ({
      timestamp: entry.timestamp,
      message: `[${entry.level.toUpperCase()}][${entry.component}] ${entry.message}`
    }));
  ...
  // If requesting formPrep logs, return logs with FORM_PREP component
  if (type === 'formPrep') {
    // Get logs that match FORM_PREP component and exclude API and HTTP components
    const formPrepLogs = getServerLogs('FORM_PREP', {
      excludeComponents: ['API', 'HTTP']
    }).map(entry => ({
      timestamp: entry.timestamp,
    ...
    return res.json({
      logs: formPrepLogs.length > 0 ? formPrepLogs : scrapeJobLogs[type] || []
    });
  }
    ...
    if (id.endsWith('.json')) {
      try {
        ...
        if (jsonData.summary) {
          ...
          summary.swapped = jsonData.summary.swapped || 0;
        }
        
        // Extract changes
        if (jsonData.changes) {
          // Handle both new format (allChanges) and old format (critical, high)
          if (Array.isArray(jsonData.changes.allChanges)) {
            // New format
            changesResult.allChanges = jsonData.changes.allChanges.map(change => {
              switch(change.type) {
                case 'removed':
                ...
                case 'swap':
                  return `- Jobs swapped: Visit #${change.job1Id} (Store #${change.job1Store}, ${change.job1Dispensers || 0} dispensers) and Visit #${change.job2Id} (Store #${change.job2Store}, ${change.job2Dispensers || 0} dispensers) exchanged dates between ${change.oldDate1} and ${change.newDate1}`;
                default:
                  return `- Unknown change: ${JSON.stringify(change)}`;
              }
            });
          } else {
            // Old format - combine critical and high into allChanges
            if (Array.isArray(jsonData.changes.critical)) {
              jsonData.changes.critical.forEach(change => {
                changesResult.allChanges.push(
                  formatChangeToString(change)
                );
              });
            }
            
            if (Array.isArray(jsonData.changes.high)) {
              jsonData.changes.high.forEach(change => {
                changesResult.allChanges.push(
                  formatChangeToString(change)
                );
              });
      ...
      if (summaryMatch) {
        ...
        // Try to find swapped jobs in summary
        const swappedMatch = content.match(/Jobs Swapped: (\d+)/);
        if (swappedMatch) {
          summary.swapped = parseInt(swappedMatch[1]);
        }
      }
      
      // Extract all changes section
      const allChangesSection = content.match(/All Changes:[\s\S]*?(?=$)/);
      if (allChangesSection) {
        const allLines = allChangesSection[0].split('\n').filter(line => line.trim().startsWith('- '));
        changesResult.allChanges = allLines.map(line => line.trim());
      } else {
        // For older format, try to extract critical and high severity sections
        // Extract critical changes section
        const criticalSection = content.match(/Critical Changes:[\s\S]*?(?=High Severity Changes:|$)/);
        if (criticalSection) {
          const criticalLines = criticalSection[0].split('\n').filter(line => line.trim().startsWith('- '));
          changesResult.allChanges.push(...criticalLines.map(line => line.trim()));
        }
        ...
        if (highSection) {
          ...
          changesResult.allChanges.push(...highLines.map(line => line.trim()));
        }
      }
    }
...
// Helper function to format a change object to string
function formatChangeToString(change) {
  switch(change.type) {
    case 'removed':
      return `- Visit #${change.jobId} (Store #${change.store}, ${change.dispensers || 0} dispensers) was removed from ${change.date}`;
    case 'added':
...
router.get('/dispensers', async (req, res) => {
  debug('Dispenser data API called', 'API');
  
  try {
    // Retrieve user-specific dispenser store file path
    const dispenserStorePath = resolveUserFilePath('dispenser_store.json');
    debug(`Loading dispenser data from: ${dispenserStorePath}`, 'API');
    
    let parsedData = { dispenserData: {}, metadata: {} };
    
    // Check if dispenser store file exists
    if (fs.existsSync(dispenserStorePath)) {
      debug('User-specific dispenser store file exists, reading data', 'API');
      const fileData = fs.readFileSync(dispenserStorePath, 'utf8');
      
      try {
        parsedData = JSON.parse(fileData);
        debug(`Successfully parsed dispenser data with ${Object.keys(parsedData.dispenserData || {}).length} stores`, 'API');
      } catch (parseError) {
        error(`Failed to parse dispenser data JSON: ${parseError.message}`, 'API', parseError);
        parsedData = { dispenserData: {}, metadata: {} };
      }
    } else {
      warn(`User-specific dispenser store file does not exist at ${dispenserStorePath}`, 'API');
      
      // Create the parent directory if it doesn't exist
      const parentDir = path.dirname(dispenserStorePath);
      if (!fs.existsSync(parentDir)) {
        fs.mkdirSync(parentDir, { recursive: true });
        info(`Created directory: ${parentDir}`, 'API');
      }
      ...
      info(`Created new dispenser store at ${dispenserStorePath}`, 'API');
      
      // Use the newly created store
      parsedData = initialStore;
    }
    
    // Initialize dispenserData if it doesn't exist
    if (!parsedData.dispenserData) {
      debug('Missing dispenserData field, initializing empty object', 'API');
      parsedData.dispenserData = {};
    }
    
    // Initialize metadata if it doesn't exist
    if (!parsedData.metadata) {
      debug('Missing metadata field, initializing', 'API');
      parsedData.metadata = {
        lastUpdated: new Date().toISOString()
      };
    }
    ...
    info(`Loaded dispenser data for ${numWorkOrders} work orders`, 'API');
    ...
    debug('Sending dispenser data response', 'API');
    return res.json(responseData);
  } catch (err) {
    error('Error loading dispenser data', 'API', err);
...
// Get schedule changes notification
router.get('/schedule-changes-notification', (req, res) => {
  try {
    ...
    // Get the path to the schedule changes file
    const changesPath = getScheduleChangesPath(userId);
    console.log(`Schedule changes path for user ${userId}: ${changesPath}`);
    
    if (!fs.existsSync(changesPath)) {
      console.log(`Schedule changes file does not exist: ${changesPath}`);
      return res.json({
        success: true,
        notification: {
    ...
    console.log(`Schedule changes file content (first 100 chars): ${data.substring(0, 100)}...`);
    
    let notification;
    
    try {
      notification = JSON.parse(data);
      console.log('Successfully parsed notification JSON:', notification);
    } catch (parseError) {
      console.error('Error parsing notification JSON:', parseError);
      // If it's not JSON, try to parse the plain text (older format)
      const lines = data.split('\n');
      notification = {
        text: lines[0] || 'Changes detected',
        detailedText: lines.slice(1).join('\n') || '',
        timestamp: new Date().toLocaleString()
      };
      console.log('Created notification from plain text:', notification);
    }
    ...
    console.error('Error getting schedule changes notification:', error);
    res.status(500).json({
      success: false,
      message: 'Failed to get schedule changes notification'
    });
  }
});
...
router.get('/logs/level', (req, res) => {
  ...
  res.json({
    currentLevel,
    availableLevels,
    description: `Current log level is set to ${currentLevel}`
  });
});
...
router.post('/logs/components/enable', (req, res) => {
  const { components } = req.body;
  
  if (!components || !Array.isArray(components) || components.length === 0) {
    return res.status(400).json({
      error: 'Missing or invalid required parameter: components (array)'
    });
  }
  ...
  res.json({
    success: true,
    message: `Enabled logging for components: ${components.join(', ')}`,
    enabledComponents: components
  });
});

router.post('/logs/components/disable', (req, res) => {
  const { components } = req.body;
  ...
  res.json({
    success: true,
    message: `Disabled logging for components: ${components.join(', ')}`,
    disabledComponents: components
  });
});

router.post('/logs/components/reset', (req, res) => {
  resetComponentFilters();
  
  res.json({
    success: true,
    message: 'Reset component filters - now logging all components'
  });
});
