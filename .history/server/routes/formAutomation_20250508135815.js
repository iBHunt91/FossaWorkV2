...
router.post('/preview-batch', async (req, res) => {
  try {
    ...
    try {
      ...
      const resolvedPath = filePath === 'data/scraped_content.json' || filePath === 'scraped_content.json' 
        ? resolveUserFilePath('scraped_content.json')
...
router.post('/batch', async (req, res) => {
  try {
    ...
    if (selectedVisits && Array.isArray(selectedVisits)) {
      ...
      try {
        // Use resolveUserFilePath for user-specific files
        const resolvedPath = filePath === 'data/scraped_content.json' || filePath === 'scraped_content.json'
          ? resolveUserFilePath('scraped_content.json')
          : path.isAbsolute(filePath) ? filePath : path.join(process.cwd(), filePath);
