...
router.delete('/email', async (req, res) => {
  try {
    ...
    fs.writeFileSync(emailSettingsPath, JSON.stringify(defaultSettings, null, 2), 'utf8');
    
    // Get the current user ID
    const userId = getCurrentUserId();
    
    // Update user notification settings if we have a user ID
    if (userId) {
      const { updateUserNotificationSettings } = await import('../../scripts/user/userService.js');
      
      // Disable email notifications in user settings
      await updateUserNotificationSettings(userId, {
        email: {
          enabled: false
        }
    ...
    // Update process.env for backward compatibility
    process.env.VITE_RECIPIENT_EMAIL = '';
    process.env.EMAIL_SHOW_JOB_ID = 'true';
    process.env.EMAIL_SHOW_STORE_NUMBER = 'true';
    process.env.EMAIL_SHOW_STORE_NAME = 'true';
    process.env.EMAIL_SHOW_LOCATION = 'true';
    process.env.EMAIL_SHOW_DATE = 'true';
    process.env.EMAIL_SHOW_DISPENSERS = 'true';
    
    res.json({ success: true, message: 'Email settings removed successfully' });
  } catch (error) {
    console.error('Error removing email settings:', error);
    res.status(500).json({ success: false, message: 'Failed to remove email settings' });
  }
});
