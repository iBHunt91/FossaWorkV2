...
const addLogEntry = (type, message) => {
  ...
  // Process the message to enhance it with additional visual indicators
  let enhancedMessage = message;
  
  // Check if the message contains a type indicator [TYPE]
  const typeMatch = message.match(/^\[(.*?)\]/);
  if (typeMatch && typeMatch[1]) {
    const messageType = typeMatch[1].trim().toUpperCase();
    
    // Add prefix emoji based on message type for better visual scanning
    if (messageType === 'ERROR') {
      ...
      enhancedMessage = `ℹ️ ${message}`;
    } else if (messageType === 'SYSTEM') {
      enhancedMessage = `🔧 ${message}`;
    } else if (messageType === 'NETWORK') {
      enhancedMessage = `🌐 ${message}`;
    } else if (messageType === 'PROGRESS' || messageType === 'STATUS') {
      enhancedMessage = `⏳ ${message}`;
    }
  } 
  // If no specific type indicator, analyze message content for patterns
  else {
    // Add common patterns based on the log type and content
    if (type === 'workOrder') {
      if (message.includes('scrape') && message.includes('start')) {
        enhancedMessage = `🔄 ${message}`;
      ...
      if (message.includes('dispenser') && (message.includes('of') || message.includes('/'))) {
        enhancedMessage = `🔄 ${message}`;
      } else if (message.includes('complete') || message.includes('success')) {
        enhancedMessage = `✅ ${message}`;
      } else if (message.includes('error') || message.includes('fail')) {
        enhancedMessage = `❌ ${message}`;
      } else if (message.includes('progress') || message.includes('processing')) {
        enhancedMessage = `⏳ ${message}`;
      ...
      if (message.includes('start')) {
        enhancedMessage = `🚀 ${message}`;
      } else if (message.includes('stop') || message.includes('shutdown')) {
        enhancedMessage = `🛑 ${message}`;
      } else if (message.includes('error') || message.includes('fail')) {
        enhancedMessage = `❌ ${message}`;
    ...
    // formPrep type is handled by the dedicated addFormPrepLog function
  }
 ...
 */
const serverLog = {
  debug: (message) => addLogEntry('server', `[DEBUG] ${message}`),
  info: (message) => addLogEntry('server', `[INFO] ${message}`),
  success: (message) => addLogEntry('server', `[SUCCESS] ${message}`),
  warn: (message) => addLogEntry('server', `[WARN] ${message}`),
  error: (message) => addLogEntry('server', `[ERROR] ${message}`),
  system: (message) => addLogEntry('server', `[SYSTEM] ${message}`),
  network: (message) => addLogEntry('server', `[NETWORK] ${message}`),
  progress: (message) => addLogEntry('server', `[PROGRESS] ${message}`),
  status: (message) => addLogEntry('server', `[STATUS] ${message}`)
};
 ...
 */
const workOrderLog = {
  debug: (message) => addLogEntry('workOrder', `[DEBUG] ${message}`),
  info: (message) => addLogEntry('workOrder', `[INFO] ${message}`),
  success: (message) => addLogEntry('workOrder', `[SUCCESS] ${message}`),
  warn: (message) => addLogEntry('workOrder', `[WARN] ${message}`),
  error: (message) => addLogEntry('workOrder', `[ERROR] ${message}`),
  system: (message) => addLogEntry('workOrder', `[SYSTEM] ${message}`),
  network: (message) => addLogEntry('workOrder', `[NETWORK] ${message}`),
  progress: (message) => addLogEntry('workOrder', `[PROGRESS] ${message}`),
  status: (message) => addLogEntry('workOrder', `[STATUS] ${message}`)
};
 ...
 */
const dispenserLog = {
  debug: (message) => addLogEntry('dispenser', `[DEBUG] ${message}`),
  info: (message) => addLogEntry('dispenser', `[INFO] ${message}`),
  success: (message) => addLogEntry('dispenser', `[SUCCESS] ${message}`),
  warn: (message) => addLogEntry('dispenser', `[WARN] ${message}`),
  error: (message) => addLogEntry('dispenser', `[ERROR] ${message}`),
  system: (message) => addLogEntry('dispenser', `[SYSTEM] ${message}`),
  network: (message) => addLogEntry('dispenser', `[NETWORK] ${message}`),
  progress: (message) => addLogEntry('dispenser', `[PROGRESS] ${message}`),
  status: (message) => addLogEntry('dispenser', `[STATUS] ${message}`)
};
...
const addFormPrepLog = (message) => {
  // We can simply delegate to the enhanced addLogEntry function
  // that now handles detailed log formatting for all types
  return addLogEntry('formPrep', message);
};

// Register the form prep logger with the logger utility
if (logger.registerFormPrepLogger) {
  logger.registerFormPrepLogger(addFormPrepLog);
  console.log('Form Prep logger registered successfully');
} else {
  console.warn('Form Prep logger registration failed: function not available');
}

// Add test log entry for each log type on server start to verify functionality
addLogEntry('workOrder', 'Work Order logging system initialized');
addLogEntry('dispenser', 'Equipment logging system initialized');
addLogEntry('server', 'Server logging system initialized');
addFormPrepLog('Form Prep logging system initialized');
...
/**
 * Add a new log entry for a specific scrape job type
 */
router.post('/scrape-logs/:type', (req, res) => {
  ...
  // Add the log entry using our helper function
...
router.post('/clear-dispenser-data', async (req, res) => {
  try {
    ...
    // Get active user ID to correctly target the user-specific dispenser_store.json
    const userId = (req.session?.userId) || getActiveUser();

    if (!userId) {
      console.error('Error: Could not determine active user ID for clearing dispenser data.');
      return res.status(400).json({ error: 'Could not determine active user ID. Unable to clear dispenser data.' });
    }

    // Load the current data from the user-specific file
    const dataPath = getLatestScrapeFile(); // This likely already uses user context
    ...
    console.log(`Running script to clear dispenser store: ${scriptPath} for user ${userId}`);
    
    const process = spawn('node', [scriptPath, storeId, userId], { // Pass userId to the script
...
// Add a logs-save endpoint for saving debug logs
router.post('/logs-save', async (req, res) => {
  try {
    ...
    console.log(`Debug logs saved to: ${filePath}`);
    ...
    return res.status(500).json({ 
      success: false, 
      error: error.message || 'Failed to save logs' 
    });
  }
});
