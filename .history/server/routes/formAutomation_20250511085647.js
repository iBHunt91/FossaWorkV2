  ...
  try {
    ...
    process.nextTick(async () => {
      try {
        ...
        formAutomation.processBatch(jobId, processPath, headless !== false, selectedVisits, resumeFromBatchId)
          .then(() => {
            logger.info(`Background batch processing completed for job ${jobId}`);
          })
          .catch(error => {
            logger.error(`Error in background batch processing for job ${jobId}: ${error.message}`);
            logger.error(`Stack trace for background batch processing error job ${jobId}: ${error.stack}`);
            formAutomation.updateBatchStatus('error', `Batch failed: ${error.message}`, { jobId });
          });
      } catch (error) {
...
router.get('/batch/status/:jobId?', async (req, res) => {
  try {
    ...
    const batchDataFromFileOrMemory = await formAutomation.getBatchStatus(jobId);
    // Log the data received from getBatchStatus
    logger.info(`[API /batch/status] Data from AutomateForm.getBatchStatus for job ${jobId}: ${JSON.stringify(batchDataFromFileOrMemory)}`);

    // The object batchDataFromFileOrMemory is what will be sent, so we log it before sending.
    // In this case, it's directly used as 'liveStatus'.
    logger.info(`[API /batch/status] liveStatus being sent to frontend for job ${jobId}: ${JSON.stringify(batchDataFromFileOrMemory)}`);
    res.json(batchDataFromFileOrMemory);
  } catch (error) {
