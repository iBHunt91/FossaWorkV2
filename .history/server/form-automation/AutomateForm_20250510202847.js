...
async function prepareForm(page, dispensers, formCount = null, isSpecificDispensers = false, formType = FORM_TYPES.ACCUMEASURE, batchUpdateCallback = null) {
  try {
    ...
    if (isCancelled) {
      ...
      return { success: false, formUrls: [] }; // Return structured object
    }
      ...
      for (let formIndex = 0; formIndex < remainingFormsToAdd; formIndex++) {
        ...
        if (batchUpdateCallback) {
          batchUpdateCallback(formAddingMessage, { 
            ...
            formsCurrent: currentCount, // About to add one more
            formsTotal: formsToCreate 
          });
        } else {
          // No direct equivalent for single status update here, previous updateStatus covered 'Preparing form'
        }
    ...
    const finalPrepMessage = `${formType} form preparation complete. Found/Created ${await getFormCount()} forms.`;
    logger.info(finalPrepMessage, 'FORM_PREP');
    if (batchUpdateCallback) {
      batchUpdateCallback(finalPrepMessage, { 
        currentVisitStatus: finalPrepMessage,
        formsCurrent: await getFormCount(), // Final count
        formsTotal: formsToCreate 
      });
    } else {
