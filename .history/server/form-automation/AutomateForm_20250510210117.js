...
function updateRealtimeStatus(status, message, details) {
  ...
  currentJobStatus.lastUpdated = new Date().toISOString();
  if (details) {
    currentJobStatus.progressDetails = { ...currentJobStatus.progressDetails, ...details };
  }

  // Emit status update for WebSocket clients (if any)
  // if (io) {
  //   io.emit('automationStatusUpdate', currentJobStatus);
  // }

  // NEW LOGIC: If a batch job is active, also update the batchStatus with detailed current visit progress
  if (currentBatchJobId && batchStatus && batchStatus.jobId === currentBatchJobId) {
    batchStatus.currentVisitStatusMessage = message; // Store the raw message from the current visit's progress

    // Assume extractDispenserProgress and extractFuelProgress functions exist in this file,
    // similar to FormPrep.tsx.
    // details.totalDispensersForCurrentVisit should be passed by automateVisit for the current visit.
    const totalDispensersForVisit = details && details.totalDispensersForCurrentVisit !== undefined 
      ? details.totalDispensersForCurrentVisit 
      : undefined;

    try {
      const dispenserProgress = extractDispenserProgress(message, totalDispensersForVisit);
      const fuelProgress = extractFuelProgress(message);

      // Update batchStatus with detailed progress for the CURRENT VISIT
      // These field names should align with what the frontend expects (status.dispenserCount, etc.)
      batchStatus.dispenserCount = dispenserProgress.total; // Total forms for the current visit
      batchStatus.dispenserCurrent = dispenserProgress.current; // Current form for the current visit
      batchStatus.fuelType = fuelProgress.type;
      batchStatus.fuelCurrent = fuelProgress.current;
      batchStatus.fuelTotal = fuelProgress.total;

      // Optional: Update overall batch message if desired, or keep it high-level
      // batchStatus.message = \`Processing visit ${batchStatus.completedVisits + 1}/${batchStatus.totalVisits}: ${message.substring(0, 50)}...\`;
      
      // Ensure batchStatus also reflects it's running
      if (status === 'running' && batchStatus.status !== 'running') {
        // This might happen if a visit starts while batch was briefly idle due to an error/restart
        // batchStatus.status = 'running'; 
        // Be cautious with this, usually processBatch manages batchStatus.status
      }

    } catch (e) {
      // console.error('Error extracting progress in updateRealtimeStatus for batch:', e);
      // Avoid crashing, but log that extraction failed for this update.
      // The frontend will see undefined for these fields.
    }
  }
}

// Ensure extractDispenserProgress and extractFuelProgress are defined or imported.
// For the purpose of this edit, we assume they are available.
// If not, they would need to be added, similar to:
/*
function extractDispenserProgress(message, totalDispensers) {
  // ... logic from FormPrep.tsx's extractDispenserProgressLocal ...
  // Simplified example:
  let current = 0;
  let total = totalDispensers || 0;
  const match = message.match(/Dispenser #?(\\d+)(?:\\s*of\\s*|\\/)(\\d+)/i);
  if (match) {
    current = parseInt(match[1], 10);
    if (!totalDispensers) { // If totalDispensers wasn't passed, use from message
        total = parseInt(match[2], 10);
    }
  } else {
    const singleMatch = message.match(/(?:Working on|Processing) dispenser (\\d+)/i);
    if (singleMatch) {
        current = parseInt(singleMatch[1], 10);
        // total remains as passed or 0
    }
  }
  if (total > 0 && current > total) current = total;
  if (current === 0 && total > 0 && message.toLowerCase().includes('dispenser')) current = 1;
  return { current, total };
}

function extractFuelProgress(message) {
  // ... logic from FormPrep.tsx's extractFuelProgressLocal ...
  // Simplified example:
  let current = 0;
  let total = 0;
  let type = undefined;
  const match = message.match(/fuel grade:\\s*([^\\(]+)\\((\\d+)\\/(\\d+)\\)/i);
  if (match) {
    type = match[1].trim();
