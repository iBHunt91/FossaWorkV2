 ...
 * @param {function} [batchUpdateCallback] - Optional callback for batch status updates
 * @returns {Promise<boolean>} - Success/failure of form preparation
 */
async function prepareForm(page, dispensers, formCount = null, isSpecificDispensers = false, formType = FORM_TYPES.ACCUMEASURE, batchUpdateCallback = null) {
  try {
    if (isCancelled) {
      logger.info('Job cancelled, skipping form preparation', 'FORM_PREP');
      if (batchUpdateCallback) batchUpdateCallback('Job cancelled', { currentVisitStatus: 'Cancelled' });
      else updateStatus('cancelled', 'Job cancelled');
      return { success: false, formUrls: [] };
    }

    const initialMessage = `Preparing ${formType} form...`;
    logger.info(initialMessage, 'FORM_PREP');
    const formsToCreate = formCount || dispensers.length || 1;
    if (batchUpdateCallback) {
      batchUpdateCallback(initialMessage, { currentVisitStatus: initialMessage, formsCurrent: 0, formsTotal: formsToCreate });
    } else {
      updateStatus('running', initialMessage);
    }

    const isVisitPage = await page.evaluate(() => window.location.href.includes('/visits/'));
    if (!isVisitPage) throw new Error('Not on a visit page');

    logger.info(`Available dispensers (${dispensers.length}): ${dispensers.map(d => d.title || 'Unknown').join(', ')}`, 'FORM_PREP');
    logger.info(`Need to create/ensure ${formsToCreate} ${formType} forms.`, 'FORM_PREP');

    const getFormCountOnPage = async () => {
      await page.waitForTimeout(250); // Short pause for dynamic content
      return page.evaluate((formTypeStr) => {
        const links = Array.from(document.querySelectorAll('a.none')).filter(a => a.textContent.trim().includes(formTypeStr));
        const spans = Array.from(document.querySelectorAll('span.text-sm')).filter(span => span.textContent.trim().includes(formTypeStr) && span.closest('a'));
        return Math.max(links.length, spans.length);
      }, formType);
    };
    
    let existingFormCount = await getFormCountOnPage();
    logger.info(`Found ${existingFormCount} existing ${formType} forms on page.`, 'FORM_PREP');
    let remainingFormsToAdd = Math.max(0, formsToCreate - existingFormCount);

    if (formsToCreate > dispensers.length && dispensers.length > 0 && isSpecificDispensers) {
        logger.warn(`Attempting to create ${formsToCreate} forms but only ${dispensers.length} specific dispensers provided. Capping forms to ${dispensers.length}.`, 'FORM_PREP');
        // This case should ideally be handled by `formCount` being accurate from the start for specific dispensers.
        // For safety, we would not proceed if formsToCreate > dispensers.length for specific dispensers.
        // However, the existing logic tries to correct `remainingFormsToAdd` later if it would exceed dispensers.length.
        // This warning is to highlight a potential mismatch if formCount wasn't set correctly for specific dispensers initially.
    }
    
    // This correction seems more about general cases, not specific dispensers matching one-to-one
    if (existingFormCount + remainingFormsToAdd > dispensers.length && dispensers.length > 0 && !isSpecificDispensers) {
      logger.warn(`CORRECTION: Planned forms (${existingFormCount + remainingFormsToAdd}) exceed available dispensers (${dispensers.length}). Adjusting.`, 'FORM_PREP');
      remainingFormsToAdd = Math.max(0, dispensers.length - existingFormCount);
      logger.info(`CORRECTED: Will add ${remainingFormsToAdd} forms for a total of ${existingFormCount + remainingFormsToAdd}.`, 'FORM_PREP');
    }


    if (remainingFormsToAdd === 0 && existingFormCount >= formsToCreate) {
      const msg = `All ${formsToCreate} required ${formType} forms already exist.`;
      logger.info(msg, 'FORM_PREP');
      if (batchUpdateCallback) batchUpdateCallback(msg, { currentVisitStatus: msg, formsCurrent: existingFormCount, formsTotal: formsToCreate });
      else updateStatus('running', msg);
    } else {
      logger.info(`Need to add ${remainingFormsToAdd} more ${formType} forms (current: ${existingFormCount}, target: ${formsToCreate}).`, 'FORM_PREP');
      for (let i = 0; i < remainingFormsToAdd; i++) {
        if (isCancelled) {
          logger.info('Job cancelled during form adding loop.', 'FORM_PREP');
          if (batchUpdateCallback) batchUpdateCallback('Cancelled', { currentVisitStatus: 'Cancelled' });
          return { success: false, formUrls: [] };
        }
        const currentCountBeforeAdd = await getFormCountOnPage();
        if (currentCountBeforeAdd >= formsToCreate) {
          logger.info(`Target of ${formsToCreate} forms met/exceeded (current: ${currentCountBeforeAdd}). Stopping add loop.`, 'FORM_PREP');
          break;
        }
        
        const addMsg = `Adding ${formType} form ${i + 1}/${remainingFormsToAdd} (page has ${currentCountBeforeAdd}, target ${formsToCreate}).`;
        logger.info(addMsg, 'FORM_PREP');
        if (batchUpdateCallback) batchUpdateCallback(addMsg, { currentVisitStatus: `Adding ${formType} ${currentCountBeforeAdd + 1}/${formsToCreate}`, formsCurrent: currentCountBeforeAdd, formsTotal: formsToCreate });
        
        const buttonLabel = (i === 0 && currentCountBeforeAdd === 0) ? 'Attach' : 'New';
        const selector = `li.plain-header:has-text("${formType}") a:has-text("${buttonLabel}")`;
        try {
          await page.waitForSelector(selector, { timeout: 7000 });
          await page.click(selector);
          logger.info(`Clicked '${buttonLabel}' for ${formType}. Waiting for form count to update.`, 'FORM_PREP');
          
          let attempts = 0;
          let formAddedSuccess = false;
          while (attempts < 15) { // Max ~7.5 seconds
            await page.waitForTimeout(500);
            const newCount = await getFormCountOnPage();
            if (newCount > currentCountBeforeAdd) {
              logger.info(`Successfully added form. New count: ${newCount}.`, 'FORM_PREP');
              if (batchUpdateCallback) batchUpdateCallback(`Added ${formType} ${newCount}/${formsToCreate}`, { currentVisitStatus: `Added ${formType} ${newCount}/${formsToCreate}`, formsCurrent: newCount, formsTotal: formsToCreate });
              formAddedSuccess = true;
              break;
            }
            attempts++;
            if (isCancelled) throw new Error('Cancelled during form add confirmation wait.');
          }
          if (!formAddedSuccess) throw new Error(`Failed to confirm ${formType} form addition after ${attempts} attempts (count ${await getFormCountOnPage()} did not increase from ${currentCountBeforeAdd}).`);
        } catch (e) {
          logger.error(`Error during form '${buttonLabel}' click or confirmation for ${formType}: ${e.message}`, 'FORM_PREP');
          if (batchUpdateCallback) batchUpdateCallback(`Error adding ${formType}: ${e.message.substring(0,70)}`, { currentVisitStatus: `Error: ${e.message.substring(0,70)}` });
          throw e; 
        }
      }
    }

    const finalCountOnPage = await getFormCountOnPage();
    let collectedUrls = [];
    logger.info(`Form addition phase complete. Final count on page: ${finalCountOnPage}. Target was ${formsToCreate}. Collecting URLs...`, 'FORM_PREP');

    if (finalCountOnPage > 0) {
        try {
            const formElements = await page.$$(`div.visit-form-list ul li a.none:has-text("${formType}")`);
            let tempUrls = [];
            for (const el of formElements) {
                const href = await el.getAttribute('href');
                if (href && href !== '#') {
                    const absoluteUrl = href.startsWith('http') ? href : new URL(href, page.url()).toString();
                    tempUrls.push(absoluteUrl);
                }
            }
            // Fallback if specific selector yielded nothing but forms are present
            if (tempUrls.length === 0 && finalCountOnPage > 0 && formElements.length === 0) {
                logger.info(`Primary URL selector found 0. Trying broader fallback for ${formType}.`, 'FORM_PREP');
                const altElements = await page.$$(`a.none:has-text("${formType}")`);
                for (const el of altElements) {
                    const href = await el.getAttribute('href');
                    if (href && href !== '#') {
                        const absUrl = href.startsWith('http') ? href : new URL(href, page.url()).toString();
                        if (!tempUrls.includes(absUrl)) tempUrls.push(absUrl);
                    }
                }
            }
            collectedUrls = tempUrls.slice(0, formsToCreate); // Ensure we don't return more URLs than intended forms
            logger.info(`Collected ${collectedUrls.length} ${formType} form URLs (target was ${formsToCreate}).`, 'FORM_PREP');
            if (collectedUrls.length < formsToCreate && finalCountOnPage >= formsToCreate) {
                logger.warn(`Collected only ${collectedUrls.length} URLs, but ${finalCountOnPage} forms of type ${formType} (target ${formsToCreate}) seem to be present. Check selectors or page structure.`, 'FORM_PREP');
            }
        } catch (urlError) {
            logger.error(`Error collecting ${formType} form URLs: ${urlError.message}`, 'FORM_PREP');
        }
    }

    const resultMessageBase = `${formType} form preparation complete. Page has ${finalCountOnPage} forms. Target was ${formsToCreate}.`;
    let successStatus = true;
    let finalMessage = `${resultMessageBase} Collected ${collectedUrls.length} URLs.`;

    if (formsToCreate > 0 && collectedUrls.length < formsToCreate) {
        finalMessage = `${resultMessageBase} CRITICAL: Failed to collect all required form URLs. Expected ${formsToCreate}, got ${collectedUrls.length}.`;
        logger.error(finalMessage, 'FORM_PREP');
        successStatus = false; 
    } else if (formsToCreate > 0 && collectedUrls.length >= formsToCreate) { // Successfully collected all needed URLs
        logger.info(finalMessage, 'FORM_PREP');
    } else if (formsToCreate === 0 && collectedUrls.length === 0) { // No forms were meant to be created, and none collected
        finalMessage = `${resultMessageBase} No forms were targeted for creation. Collected 0 URLs as expected.`;
        logger.info(finalMessage, 'FORM_PREP');
    } else if (formsToCreate === 0 && collectedUrls.length > 0) { // No forms targeted, but some found (e.g. pre-existing)
        finalMessage = `${resultMessageBase} No forms were targeted for creation, but ${collectedUrls.length} (likely pre-existing) URLs were found and collected (this is unexpected if formCount was 0).`;
        logger.warn(finalMessage, 'FORM_PREP');
        // This isn't necessarily a failure of prepareForm's objective if formsToCreate was 0,
        // but it's odd. For now, treat as success as it didn't fail to *create* and *collect* new forms.
    }


    if (batchUpdateCallback) {
      batchUpdateCallback(finalMessage, { 
          currentVisitStatus: finalMessage, 
          formsCurrent: finalCountOnPage, 
          formsTotal: formsToCreate,    
          error: !successStatus 
      });
    } else if (!successStatus) { 
       updateStatus('error', `Failed to collect form URLs (expected ${formsToCreate}, got ${collectedUrls.length})`);
    }
    // If it was success and no batchUpdateCallback, the status is likely already 'running' or a similar positive state from earlier in the function.


    return { success: successStatus, formUrls: collectedUrls };

  } catch (error) {
    const errMsg = `Error preparing ${formType} form: ${error.message}`;
    logger.error(errMsg, 'FORM_PREP');
    if (batchUpdateCallback) batchUpdateCallback(errMsg, { currentVisitStatus: `Error: ${error.message.substring(0,100)}...`, error: true });
    else updateStatus('error', errMsg);
    return { success: false, formUrls: [] };
  }
}
 ...
 */
async function fillFormDetails(page, formUrls, dispensers, isSpecificDispensers = false, formType = FORM_TYPES.ACCUMEASURE, batchUpdateCallback = null) {
  try {
    if (isCancelled) {
      logger.info('Job cancelled, skipping form filling');
      if (batchUpdateCallback) batchUpdateCallback('Cancelled', { currentVisitStatus: 'Cancelled' });
      else updateStatus('cancelled', 'Cancelled');
      return false;
    }

    const numForms = formUrls.length;
    logger.info(`Starting to fill ${numForms} ${formType} form(s)...`, 'FORM_PREP');
    if (batchUpdateCallback) {
      batchUpdateCallback(`Filling ${numForms} ${formType} forms...`, { 
        currentVisitStatus: `Starting to fill ${numForms} forms...`,
        formsCurrent: 0, // Reset for filling phase, or carry over? Let's say it's form index.
        formsTotal: numForms 
      });
    } else {
      updateStatus('running', `Filling ${numForms} ${formType} forms...`);
    }
    ...
    for (let i = 0; i < formUrls.length; i++) {
      ...
      const formFillingMessage = `Filling ${formType} form ${i + 1} of ${numForms} for dispenser: ${dispenser.title || 'Unknown'}`;
      logger.info(formFillingMessage, 'FORM_PREP');
      if (batchUpdateCallback) {
        batchUpdateCallback(formFillingMessage, { 
          currentVisitStatus: `Filling ${formType} ${i + 1}/${numForms} (${dispenser.title || 'Unknown'})`,
          formsCurrent: i + 1, 
          formsTotal: numForms,
          currentDispenserTitle: dispenser.title || 'Unknown' // Add dispenser title
        });
      } else {
        updateStatus('running', `Filling form ${i + 1}/${numForms}...`);
      }
      
      // Navigate to the form URL
      await page.goto(formUrl, { waitUntil: 'networkidle', timeout: 60000 });
      logger.info(`Navigated to form: ${formUrl}`, 'FORM_PREP');
 ...
 * @param {function} [batchUpdateCallback] - Optional callback for batch status updates
 * @param {number} [formIndex] - Optional form index for batch context
 * @param {number} [totalForms] - Optional total forms for batch context
 * @returns {Promise<boolean>}
 */
async function processAllFuelSections(page, dispenser, isSpecificDispensers = false, formType = FORM_TYPES.ACCUMEASURE, batchUpdateCallback = null, formIndex = 0, totalForms = 0) {
  const fuelTypes = dispenser.fuelTypes || [];
  logger.info(`Processing ${fuelTypes.length} fuel types for dispenser: ${dispenser.title || 'N/A'}`, 'FORM_PREP');

  if (batchUpdateCallback) {
    batchUpdateCallback(`Processing ${fuelTypes.length} fuel types for ${dispenser.title || 'N/A'}`, {
      currentVisitStatus: `Dispenser ${dispenser.title || 'N/A'}: ${fuelTypes.length} fuel types`,
      formsCurrent: formIndex, // Pass form context
      formsTotal: totalForms,
      currentDispenserTitle: dispenser.title || 'N/A',
      currentVisitFuelType: null, // Reset for new dispenser
      currentVisitFuelCurrent: 0,
      currentVisitFuelTotal: 0
    });
  }

  if (fuelTypes.length === 0) {
    logger.warn(`No fuel types found for dispenser: ${dispenser.title}. Skipping fuel section processing.`, 'FORM_PREP');
    if (batchUpdateCallback) {
      batchUpdateCallback(`No fuel types for ${dispenser.title || 'N/A'}`, {
        currentVisitStatus: `Dispenser ${dispenser.title || 'N/A'}: No fuel types`,
        formsCurrent: formIndex,
        formsTotal: totalForms
      });
    }
    return; // Nothing to do if no fuel types
  }

  // Load prover preferences once
  const proverPreferences = await loadProverPreferences();
  const allFuelTypes = JSON.parse(fs.readFileSync(resolveUserFilePath('fuel_types.json'), 'utf8'));

  for (const [fuelIndex, fuelType] of fuelTypes.entries()) {
    if (isCancelled) {
      logger.info('Job cancelled during fuel section processing.');
      if (batchUpdateCallback) batchUpdateCallback('Cancelled', { currentVisitStatus: 'Cancelled'});
      return; // Stop further processing
    }
    
    const fuelProcessingMessage = `Processing fuel type ${fuelIndex + 1}/${fuelTypes.length}: ${fuelType.name || 'Unknown Fuel'} for dispenser ${dispenser.title || 'N/A'}`;
    logger.info(fuelProcessingMessage, 'FORM_PREP');
    if (batchUpdateCallback) {
      batchUpdateCallback(fuelProcessingMessage, {
        currentVisitStatus: `Fuel ${fuelIndex + 1}/${fuelTypes.length}: ${fuelType.name || 'Unknown'} (Disp. ${dispenser.title || 'N/A'})`,
        formsCurrent: formIndex,
        formsTotal: totalForms,
        currentDispenserTitle: dispenser.title || 'N/A',
        currentVisitFuelType: fuelType.name || 'Unknown Fuel',
        currentVisitFuelCurrent: 0, // Reset for new fuel type
        currentVisitFuelTotal: (fuelType.hoses || fuelType.products || []).length // Estimate total items for this fuel type
      });
    }

    await fillFuelTypeForm(page, fuelType, allFuelTypes, proverPreferences, isSpecificDispensers, formType, 
      // Pass the callback down
      (fuelMessage, fuelProps) => {
        if (batchUpdateCallback) {
          batchUpdateCallback(fuelMessage, {
            formsCurrent: formIndex, // Maintain form context
            formsTotal: totalForms,
            currentDispenserTitle: dispenser.title || 'N/A',
            currentVisitFuelType: fuelType.name || 'Unknown Fuel',
            ...fuelProps // e.g., { currentVisitStatus, currentVisitFuelCurrent, currentVisitFuelTotal }
          });
        }
      }
    );
  }
  logger.info(`Finished processing all fuel sections for dispenser: ${dispenser.title || 'N/A'}`, 'FORM_PREP');
  if (batchUpdateCallback) {
    batchUpdateCallback(`All fuel sections done for ${dispenser.title || 'N/A'}`, {
      currentVisitStatus: `Dispenser ${dispenser.title || 'N/A'}: All fuel sections done`,
      formsCurrent: formIndex,
      formsTotal: totalForms
    });
  }
 ...
 * @returns {Promise<boolean>}
 */
async function fillFuelTypeForm(page, fuelType, allFuelTypes, proverPreferences, isSpecificDispensers = false, formType = FORM_TYPES.ACCUMEASURE, batchUpdateCallback = null) {
  try { // Add back the main try block for the function
    const fuelTypeName = fuelType.name || 'Unknown Fuel Type';
    logger.info(`Filling form for fuel type: ${fuelTypeName}`, 'FORM_PREP');
    if (batchUpdateCallback) {
      // Initial update for this fuel type
      batchUpdateCallback(`Starting ${fuelTypeName}`, { 
        currentVisitStatus: `Starting ${fuelTypeName}`,
        currentVisitFuelCurrent: 0,
        currentVisitFuelTotal: (fuelType.hoses || fuelType.products || []).length
      });
    }

    // Determine if it's an AccuMeasure or Open Neck Prover form type
    const isAccuMeasure = formType === FORM_TYPES.ACCUMEASURE;
    logger.info(`Fuel type "${fuelTypeName}" is ${isAccuMeasure ? 'AccuMeasure' : 'Open Neck Prover'}`, 'FORM_PREP');
    ...
    logger.info(`Meter determination for ${fuelTypeName}: ${hasMeters ? 'YES' : 'NO'}`);
    console.log(`Meter determination for ${fuelTypeName}: ${hasMeters ? 'YES' : 'NO'}`);
    ...
    try {
      ...
      if (radiosSelected.success) {
        ...
        logger.info(`Form type: ${formType}, Fuel type: ${fuelTypeName}`);
      ...
      logger.info(`Applying aggressive selection confirmation for ${fuelTypeName}`);
      ...
      try {
        ...
        try {
          ...
          for (const selector of radioSelectors) {
            ...
            if (exists) {
              logger.info(`Found ${fuelTypeName} Has Meter ${hasMeters ? 'Yes' : 'No'} radio with selector: ${selector}, clicking directly`);
        ...
        try {
          ...
          for (const selector of labelSelectors) {
            const exists = await page.$(selector);
            if (exists) {
              logger.info(`Found ${fuelTypeName} Has Meter ${labelText} label with selector: ${selector}, clicking directly`);
              await page.click(selector, { force: true });
              await page.waitForTimeout(200);
              break;
            }
        ...
        if (fullVerification.success) {
          logger.info(`SUCCESSFUL Radio verification for ${fuelTypeName}: ${fullVerification.targetRadioCount} ${hasMeters ? 'Yes' : 'No'} radios, ${fullVerification.radioCount} total radios`);
        } else {
          logger.warn(`FAILED Radio verification for ${fuelTypeName}: ${JSON.stringify(fullVerification)}`);
        }
        
      } catch (aggressiveError) {
        logger.error(`Ultra-aggressive approach failed for ${fuelTypeName}: ${aggressiveError.message}`);
      }
    } catch (error) {
    ...
    if (formType === FORM_TYPES.OPEN_NECK_PROVER && hasMeters) {
      try {
        ...
        logger.info(`Filling Blend Ratio fields for ${fuelTypeName}`);
        console.log(`Filling Blend Ratio fields for ${fuelTypeName}`);
    ...
    if (proverPreferences && Object.keys(proverPreferences).length > 0) {
      ...
      if (proverId) {
        logger.info(`Found preferred prover ${proverId} for fuel type ${fuelTypeName}`);
        logger.info(`Found preferred prover ${proverId} for fuel type ${fuelTypeName}`, 'FORM_PREP');
        console.log(`Found preferred prover ${proverId} for fuel type ${fuelTypeName}`);
        ...
        try {
          ...
          if (proverSelected.success) {
            logger.info(`Selected prover: ${proverSelected.selected} for fuel type: ${fuelTypeName}`);
            logger.info(`Selected prover: ${proverSelected.selected} for fuel type: ${fuelTypeName}`, 'FORM_PREP');
            console.log(`Selected prover: ${proverSelected.selected} for fuel type: ${fuelTypeName}`);
        ...
        logger.info(`No preferred prover found for ${fuelTypeName}, leaving prover unselected`);
        logger.info(`No preferred prover found for ${fuelTypeName}, leaving prover unselected`, 'FORM_PREP');
        console.log(`No preferred prover found for ${fuelTypeName}, leaving prover unselected`);
      }
    } else {
      logger.info(`No prover preferences available for ${fuelTypeName}, using default behavior`);
      logger.info(`No prover preferences available for ${fuelTypeName}, using default behavior`, 'FORM_PREP');
      console.log(`No prover preferences available for ${fuelTypeName}, using default behavior`);
    }
    
    logger.info(`=== COMPLETED FILLING FUEL TYPE FORM: ${fuelTypeName} ===`);
    logger.info(`=== COMPLETED FILLING FUEL TYPE FORM: ${fuelTypeName} ===`, 'FORM_PREP');
    console.log(`=== COMPLETED FILLING FUEL TYPE FORM: ${fuelTypeName} ===`);
    // After all items for the fuel type are processed
    const fuelTypeDoneMessage = `Finished processing fuel type: ${fuelTypeName}`;
    logger.info(fuelTypeDoneMessage, 'FORM_PREP');
    if (batchUpdateCallback) {
      batchUpdateCallback(fuelTypeDoneMessage, { 
        currentVisitStatus: `${fuelTypeName}: Done`,
        currentVisitFuelCurrent: totalItems, // Mark all as done
        currentVisitFuelTotal: totalItems
      });
    }
    return true; // Added return true for successful completion within try
  } catch (error) {
    const errorMessage = `Error filling fuel type form for ${(fuelType && fuelType.name) || 'Unknown Fuel Type'}: ${error.message}`;
    logger.error(errorMessage, 'FORM_PREP');
    if (batchUpdateCallback) {
      batchUpdateCallback(errorMessage, { 
        currentVisitStatus: `Error in ${ (fuelType && fuelType.name) || 'Unknown'}: ${error.message.substring(0,50)}...`,
        error: true 
      });
    }
    // console.log(`Error filling fuel type form for ${fuelType}: ${error.message}`); // Original log, ensure it's covered or re-add if necessary
...
async function processBatch(jobId, filePath, headless = true, selectedVisits = null, resumeFromBatchId = null) {
  logger.info(`Starting batch process. Job ID: ${jobId}, File: ${filePath}, Headless: ${headless}, Selected: ${selectedVisits ? selectedVisits.join(',') : 'All'}, Resume: ${resumeFromBatchId || 'N/A'}`, 'BATCH_MAIN');
  activeJobIds.add(jobId);
  isCancelled = false; // Reset cancellation flag for the new job

  let browser = null;
  let page = null;
  let completedVisitDetails = []; // Store details of completed/attempted visits
  
  // Initialize global batchStatus for this job with all expected fields
  batchStatus = {
    jobId: jobId,
    status: 'running',
    message: `Preparing batch processing from: ${filePath}`,
    totalItems: 0, // Will be updated once visits are loaded
    currentItem: 0,  // Current overall visit number being processed
    completedVisits: 0, // Count of successfully completed or skipped visits
    totalVisits: 0, // Overall total visits for this batch run (after filtering/resume)
    completedVisitIds: [],
    currentVisitId: null,
    currentVisitUrl: null,
    currentVisitStatus: null, // Detailed status for the current specific visit
    formsCurrent: 0,
    formsTotal: 0,
    currentVisitFuelType: null,
    currentVisitFuelCurrent: 0,
    currentVisitFuelTotal: 0,
    startTime: new Date().toISOString(),
    timestamp: jobId, // Use jobId as the primary timestamp for this batch instance
    filePath: filePath,
    headless: headless,
    error: null // For any critical batch-level error
  };
  updateBatchStatus('running', batchStatus.message, { ...batchStatus }); // Initial log of status

  try {
    const activeUser = getActiveUser();
    logger.info(`Active User for batch processing: ${activeUser || 'None'}`, 'BATCH_MAIN');
    let dataPath = path.isAbsolute(filePath) ? filePath : resolveUserFilePath(filePath);
    logger.info(`Batch data file path: ${dataPath}`, 'BATCH_MAIN');
    if (!fs.existsSync(dataPath)) throw new Error(`Data file not found: ${filePath}`);
    const rawData = JSON.parse(fs.readFileSync(dataPath, 'utf8'));

    let visitsToLoad = [];
    if (rawData.workOrders && Array.isArray(rawData.workOrders)) {
      visitsToLoad = rawData.workOrders.map(wo => ({
        id: wo.id,
        url: wo.visits?.nextVisit?.url,
        dispensers: wo.dispensers || [],
        formCount: wo.dispenserCount !== undefined ? wo.dispenserCount : (wo.dispensers || []).length,
        isSpecificDispensers: wo.serviceType?.code === '2862',
        formType: wo.serviceType?.code === '3146' ? FORM_TYPES.OPEN_NECK_PROVER : FORM_TYPES.ACCUMEASURE,
        serviceCode: wo.serviceType?.code,
        instructions: wo.instructions || ''
      }));
    } else if (Array.isArray(rawData)) { // Support for simple array of visit objects
      visitsToLoad = rawData.map(v => ({
        id: v.id || v.workOrderId || `visit_${Math.random().toString(16).slice(2)}`,
        url: v.url || v.visitUrl,
        dispensers: v.dispensers || [],
        formCount: v.dispenserCount !== undefined ? v.dispenserCount : (v.dispensers || []).length,
        isSpecificDispensers: v.serviceCode === '2862',
        formType: v.serviceCode === '3146' ? FORM_TYPES.OPEN_NECK_PROVER : FORM_TYPES.ACCUMEASURE,
        serviceCode: v.serviceCode,
        instructions: v.instructions || ''
      }));
    }
    visitsToLoad = visitsToLoad.filter(v => v.url); // Ensure visits have a URL

    // Apply specific dispensers logic if needed
    visitsToLoad.forEach(visit => {
        if (visit.isSpecificDispensers) {
    ...
    // Apply selectedVisits filter
    if (selectedVisits && selectedVisits.length > 0) {
      visitsToLoad = visitsToLoad.filter(v => selectedVisits.includes(v.id));
    }
    
    let initialCompletedCount = 0;
    let visitsToProcessInThisRun = [...visitsToLoad];

    if (resumeFromBatchId) {
      const prevBatch = await getBatchStatusById(resumeFromBatchId);
      if (prevBatch && prevBatch.completedVisitIds && prevBatch.completedVisitIds.length > 0) {
        logger.info(`Resuming from batch ${resumeFromBatchId}. Found ${prevBatch.completedVisitIds.length} previously completed/skipped visits.`, 'BATCH_MAIN');
        initialCompletedCount = prevBatch.completedVisitIds.length;
        completedVisitDetails = prevBatch.completedVisitIds.map(id => ({ id, status: 'completed_skipped' })); // Mark as skipped
        visitsToProcessInThisRun = visitsToLoad.filter(v => !prevBatch.completedVisitIds.includes(v.id));
        updateBatchStatus('running', `Resumed. Skipping ${initialCompletedCount} visits.`, { 
            completedVisits: initialCompletedCount, 
            completedVisitIds: prevBatch.completedVisitIds, 
            totalVisits: visitsToLoad.length // Total from source file before resume filter
        });
      }
    }
    
    const overallTotalVisitsInSource = visitsToLoad.length; // Total from file after selection, before resume
    batchStatus.totalVisits = overallTotalVisitsInSource; // Set totalVisits for the batch context
    batchStatus.completedVisits = initialCompletedCount;
    updateBatchStatus('running', `Found ${visitsToProcessInThisRun.length} visits to process in this run (Overall in source: ${overallTotalVisitsInSource}).`, 
        { totalVisits: overallTotalVisitsInSource, completedVisits: initialCompletedCount, totalItems: overallTotalVisitsInSource, currentItem: initialCompletedCount });

    if (visitsToProcessInThisRun.length === 0) {
      const endMsg = initialCompletedCount > 0 ? 'All selected visits were already processed in a previous run.' : 'No visits to process based on selection.';
      updateBatchStatus('completed', endMsg, { completedVisits: initialCompletedCount, totalVisits: overallTotalVisitsInSource, currentItem: overallTotalVisitsInSource });
      logger.info(endMsg, 'BATCH_MAIN');
      return;
    }
    
    const loginResult = await loginToFossa({ headless });
    if (!loginResult.success) throw new Error('Failed to login to Fossa');
    browser = loginResult.browser;
    page = loginResult.page;
    
    for (const [currentIndexInThisRun, visit] of visitsToProcessInThisRun.entries()) {
      const overallVisitNumberForDisplay = initialCompletedCount + currentIndexInThisRun + 1;
      const visitStartMessage = `Processing visit ${overallVisitNumberForDisplay}/${overallTotalVisitsInSource}: ${visit.id} (${visit.url})`;
      logger.info(visitStartMessage, 'BATCH_MAIN');
      
      // Update status for the start of this new visit
      updateBatchStatus('running', visitStartMessage, {
        jobId: jobId,
        currentItem: overallVisitNumberForDisplay, 
        totalItems: overallTotalVisitsInSource, 
        currentVisitId: visit.id,
        currentVisitUrl: visit.url,
        completedVisits: initialCompletedCount + currentIndexInThisRun, // Visits completed *before* this one starts
        currentVisitStatus: 'Navigating to visit...', 
        formsCurrent: 0, 
        formsTotal: visit.formCount || (visit.dispensers || []).length, // Initial estimate
        currentVisitFuelType: null, 
        currentVisitFuelCurrent: 0,
        currentVisitFuelTotal: 0,
        error: null // Clear any error from a previous visit in batchStatus context
      });

      if (isCancelled) {
        updateBatchStatus('cancelled', 'Batch cancelled by user.', { currentVisitId: visit.id });
        break;
      }

      try {
        await page.goto(visit.url, { waitUntil: 'networkidle', timeout: 60000 });
        updateBatchStatus('running', `Navigated to ${visit.id}`, { currentVisitStatus: 'Page loaded. Preparing forms...' });

        const prepResult = await prepareForm(page, visit.dispensers, visit.formCount, visit.isSpecificDispensers, visit.formType,
          (prepMsg, prepProps) => {
            updateBatchStatus('running', prepMsg, {
              jobId, currentVisitId: visit.id, currentVisitUrl: visit.url, 
              currentItem: overallVisitNumberForDisplay, totalItems: overallTotalVisitsInSource, 
              completedVisits: initialCompletedCount + currentIndexInThisRun, 
              ...prepProps
            });
          }
        );
        ...
        if (!prepResult.success) {
          throw new Error(`Form preparation failed for visit ${visit.id}. Reported reason: ${batchStatus.currentVisitStatus || 'Unknown failure in prepareForm'}`);
        }

        const formUrls = prepResult.formUrls;
        logger.info(`prepareForm for ${visit.id} (service code: ${visit.serviceCode || 'N/A'}) reported success. Collected ${formUrls.length} URLs.`, 'BATCH_MAIN');

        if (formUrls.length > 0) {
          const fillSuccess = await fillFormDetails(page, formUrls, visit.dispensers, visit.isSpecificDispensers, visit.formType,
            (fillMsg, fillProps) => {
              updateBatchStatus('running', fillMsg, {
                jobId, currentVisitId: visit.id, currentVisitUrl: visit.url, 
                currentItem: overallVisitNumberForDisplay, totalItems: overallTotalVisitsInSource, 
                completedVisits: initialCompletedCount + currentIndexInThisRun, 
                ...fillProps
          ...
          if (!fillSuccess) {
            throw new Error(`Form filling failed for visit ${visit.id}. Reported reason: ${batchStatus.currentVisitStatus || 'Unknown failure in fillFormDetails'}`);
          }
        } else {
          const expectedForms = (visit.formCount !== undefined && visit.formCount > 0) || (visit.dispensers && visit.dispensers.length > 0);
          const knownNoFormServiceCodes = []; // Populate if any service codes never have forms
          const requiresForms = expectedForms && !knownNoFormServiceCodes.includes(visit.serviceCode);
          
          if (requiresForms) {
            logger.warn(`Visit ${visit.id} (Service: ${visit.serviceCode}) expected forms based on count/dispensers, but 0 URLs were provided by prepareForm (which reported success). Skipping fill stage.`, 'BATCH_MAIN');
            updateBatchStatus('running', `Visit ${visit.id}: Expected forms, but none found/collected.`, { currentVisitStatus: 'Warning: No forms to fill despite expectation.'});
          } else {
            logger.info(`Visit ${visit.id} (Service: ${visit.serviceCode}) had no forms expected or 0 URLs were provided as expected. Skipping fill stage.`, 'BATCH_MAIN');
            updateBatchStatus('running', `Visit ${visit.id}: No forms to fill (as expected).`, { currentVisitStatus: 'No forms to fill (as expected).'});
          }
        }

        completedVisitDetails.push({ id: visit.id, url: visit.url, status: 'completed' });
        // Update completedVisits to reflect the one just finished
        const currentTotalCompletedForBatch = initialCompletedCount + currentIndexInThisRun + 1;
        updateBatchStatus('running', `Completed visit ${overallVisitNumberForDisplay}/${overallTotalVisitsInSource}: ${visit.id}`, {
          completedVisits: currentTotalCompletedForBatch,
          completedVisitIds: completedVisitDetails.map(v => v.id), // Update with all processed so far
          currentVisitStatus: 'Visit completed successfully.',
          error: null // Clear error from this successful visit
        });
        logger.info(`Successfully processed visit ${visit.id}`, 'BATCH_MAIN');

      } catch (visitError) {
        logger.error(`Error processing visit ${visit.id} (Overall ${overallVisitNumberForDisplay}/${overallTotalVisitsInSource}): ${visitError.message}\n${visitError.stack}`, 'BATCH_MAIN');
        updateBatchStatus('running', `Error on visit ${visit.id}: ${visitError.message.substring(0, 100)}`, { 
          currentVisitStatus: `Error: ${visitError.message.substring(0, 150)}`, 
          error: visitError.message // Log the error message specific to this visit for batchStatus
        });
        completedVisitDetails.push({ id: visit.id, url: visit.url, status: 'error', error: visitError.message });
      }
    } // End of for...of loop for visitsToProcessInThisRun

    const finalSuccessfulCount = completedVisitDetails.filter(v => v.status === 'completed').length;
    const finalStatusMessage = `Batch processing finished. ${finalSuccessfulCount}/${overallTotalVisitsInSource} visits completed successfully.`;
    const finalBatchState = (finalSuccessfulCount === overallTotalVisitsInSource && visitsToProcessInThisRun.length > 0) ? 'completed' : 'partial_completion';
    updateBatchStatus(finalBatchState, finalStatusMessage, {
      completedVisits: finalSuccessfulCount, // This is the count of *successful* ones
      totalVisits: overallTotalVisitsInSource,
      completedVisitIds: completedVisitDetails.map(v => v.id), // IDs of all attempted/skipped visits
      currentItem: overallTotalVisitsInSource, // Mark as having processed all items
      endTime: new Date().toISOString(),
      currentVisitId: null, // Clear current visit context
      currentVisitUrl: null,
      currentVisitStatus: 'Batch run ended.'
    });
    logger.info(finalStatusMessage, 'BATCH_MAIN');

  } catch (batchError) {
    logger.error(`Critical error in batch processing for job ${jobId}: ${batchError.message}\n${batchError.stack}`, 'BATCH_MAIN');
    updateBatchStatus('error', `Batch Error: ${batchError.message}`, { error: batchError.message, endTime: new Date().toISOString() });
  } finally {
    if (browser) {
      try { 
        updateBatchStatus(batchStatus.status, 'Closing browser...', { currentVisitStatus: 'Closing browser' });
      await browser.close();
      } catch (e) { 
        logger.error('Error closing browser: ' + e.message, 'BATCH_MAIN'); 
      }
      logger.info('Browser closed for batch job.', 'BATCH_MAIN');
    }
    activeJobIds.delete(jobId);
    // Update final status one last time after browser close, if it wasn't an error that set endTime
    if (!batchStatus.endTime) {
        batchStatus.endTime = new Date().toISOString();
    }
    if (batchStatus.status !== 'error' && batchStatus.status !== 'completed' && batchStatus.status !== 'cancelled') {
        // If it ended prematurely or in a weird state, mark as partial or error based on successful count
        const successfulCount = batchStatus.completedVisitIds.filter(id => completedVisitDetails.find(v => v.id === id && v.status === 'completed')).length;
        batchStatus.status = successfulCount === batchStatus.totalVisits ? 'completed' : 'partial_completion';
        batchStatus.message = `Batch run ended. ${successfulCount}/${batchStatus.totalVisits} completed.`;
    }
    logger.info(`Batch job ${jobId} finished. Final status: ${batchStatus.status}. Message: ${batchStatus.message}`, 'BATCH_MAIN');
    saveBatchStatusToHistory(batchStatus); 
  }
}
...
async function getBatchStatus(jobId) {
  // Prioritize live in-memory status if the requested jobId matches the active one, or if no jobId is given (implies current active)
  if (!jobId || (batchStatus && batchStatus.jobId === jobId)) {
    logger.info(`Returning current in-memory batch status (Live Job ID: ${batchStatus.jobId}, Polled for: ${jobId || 'current active'})`, 'BATCH_STATUS');
    // Ensure all fields are present, defaulting if necessary
    return {
      jobId: batchStatus.jobId || null,
      status: batchStatus.status || 'idle',
      message: batchStatus.message || 'No active batch job.',
      progress: batchStatus.progress || 0, // Overall percentage, might need review on how it's calculated
      currentItem: batchStatus.currentItem || 0, 
      totalItems: batchStatus.totalItems || 0, 
      completedVisits: batchStatus.completedVisits || 0,
      totalVisits: batchStatus.totalVisits || 0, // This should be the definitive total for the batch run
      completedVisitIds: batchStatus.completedVisitIds || [],
      currentVisitId: batchStatus.currentVisitId || null,
      currentVisitUrl: batchStatus.currentVisitUrl || null,
      currentVisitStatus: batchStatus.currentVisitStatus || null, 
      formsCurrent: batchStatus.formsCurrent || 0,
      formsTotal: batchStatus.formsTotal || 0,
      currentVisitFuelType: batchStatus.currentVisitFuelType || null,
      currentVisitFuelCurrent: batchStatus.currentVisitFuelCurrent || 0,
      currentVisitFuelTotal: batchStatus.currentVisitFuelTotal || 0,
      startTime: batchStatus.startTime || null,
      endTime: batchStatus.endTime || null,
      timestamp: batchStatus.timestamp || batchStatus.jobId || null, 
      lastUpdated: batchStatus.lastUpdated || new Date().toISOString(),
      filePath: batchStatus.filePath || null,
      headless: batchStatus.headless !== undefined ? batchStatus.headless : null,
      error: batchStatus.error || null
    };
  }

  // If a specific jobId is requested that is NOT the active one, retrieve from history
  logger.info(`Batch job ${jobId} is not the active one (Active: ${batchStatus.jobId}). Attempting to retrieve from history.`, 'BATCH_STATUS');
  try {
    const historicalStatus = await getBatchStatusById(jobId); // Assumes getBatchStatusById fetches from storage
    if (historicalStatus) {
      logger.info(`Returning historical batch status for job ${jobId}`, 'BATCH_STATUS');
      // Ensure all fields are present, defaulting from historical if possible
          return {
        jobId: historicalStatus.jobId,
        status: historicalStatus.status,
        message: historicalStatus.message,
        progress: historicalStatus.progress || 0,
        currentItem: historicalStatus.currentItem || 0,
        totalItems: historicalStatus.totalItems || 0,
        completedVisits: historicalStatus.completedVisits || (historicalStatus.completedVisitIds ? historicalStatus.completedVisitIds.length : 0),
        totalVisits: historicalStatus.totalVisits || historicalStatus.totalItems || 0,
        completedVisitIds: historicalStatus.completedVisitIds || [],
        currentVisitId: historicalStatus.currentVisitId || null,
        currentVisitUrl: historicalStatus.currentVisitUrl || null,
        currentVisitStatus: historicalStatus.currentVisitStatus || null,
        formsCurrent: historicalStatus.formsCurrent || 0,
        formsTotal: historicalStatus.formsTotal || 0,
        currentVisitFuelType: historicalStatus.currentVisitFuelType || null,
        currentVisitFuelCurrent: historicalStatus.currentVisitFuelCurrent || 0,
        currentVisitFuelTotal: historicalStatus.currentVisitFuelTotal || 0,
        startTime: historicalStatus.startTime || null,
        endTime: historicalStatus.endTime || null,
        timestamp: historicalStatus.timestamp || historicalStatus.jobId || null,
        lastUpdated: historicalStatus.lastUpdated || historicalStatus.savedAt || null,
        filePath: historicalStatus.filePath || null,
        headless: historicalStatus.headless !== undefined ? historicalStatus.headless : null,
        error: historicalStatus.error || null
      };
    } else {
      logger.warn(`Batch job ${jobId} not found in history.`, 'BATCH_STATUS');
      return { jobId: jobId, status: 'not_found', message: `Batch job ${jobId} not found.` };
    }
    } catch (error) {
    logger.error(`Error retrieving historical batch status for ${jobId}: ${error.message}`, 'BATCH_STATUS');
    return { jobId: jobId, status: 'error', message: `Error retrieving status for ${jobId}.` };
  }
}
