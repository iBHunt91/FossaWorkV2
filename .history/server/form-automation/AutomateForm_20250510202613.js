...
async function fillFuelTypeForm(page, fuelType, allFuelTypes, proverPreferences, isSpecificDispensers = false, formType = FORM_TYPES.ACCUMEASURE, batchUpdateCallback = null) {
  ...
  logger.error(`Error filling fuel type form for ${fuelTypeName}: ${error.message}`);
  logger.error(`Error filling fuel type form for ${fuelTypeName}: ${error.message}`, 'FORM_PREP');
  console.log(`Error filling fuel type form for ${fuelTypeName}: ${error.message}`);
  return false;
}
