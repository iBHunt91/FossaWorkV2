...
let currentJobId = null; // For single, non-batch jobs
let currentBatchJobId = null; // <<< ADDED: For the currently active BATCH JOB ID
...
let batchStatus = {
  ...
  // Add additional fields for detailed progress tracking
  dispenserCount: 0,      // Total dispensers in current visit
  dispenserCurrent: 0,    // Current dispenser being processed
  fuelType: null,         // Current fuel type being processed
  fuelCurrent: 0,         // Current fuel position being processed
  fuelTotal: 0,           // Total fuel positions in current visit
  currentVisitName: '',   // Name of the store currently being processed
  currentVisitStatus: ''  // Detailed status of current visit processing
};
...
const updateBatchStatus = (status, message, progressOrProps = null, currentItem = null, totalItems = null) => {
  ...
  // CRITICAL FIX: Ensure active batch jobs always have status "running"
  // The only valid statuses that should override this are 'completed', 'error', or 'cancelled'
  if (currentBatchJobId && status === 'idle' && 
     !(message.toLowerCase().includes('complet') || 
       message.toLowerCase().includes('error') || 
       message.toLowerCase().includes('cancel'))) {
    status = 'running';
    logger.info(`[updateBatchStatus] Overriding status from 'idle' to 'running' for active batch job ${currentBatchJobId}`);
  }
  
  // Check if all work is done for the batch job and force status to 'completed'
  // This ensures we don't stay in a "running" state when everything is actually done
  if (updatedProgress.totalVisits > 0 && 
      updatedProgress.completedVisits >= updatedProgress.totalVisits &&
      status !== 'error' && status !== 'cancelled') {
    if (status !== 'completed') {
      logger.info(`[updateBatchStatus] All visits completed (${updatedProgress.completedVisits}/${updatedProgress.totalVisits}). Setting status to 'completed'`);
      status = 'completed';
      message = message || 'All visits processed successfully.';
      // Set the end time if not already set
      if (!progressProps.endTime) {
        progressProps.endTime = new Date().toISOString();
      }
    }
  }
...
async function fillFormDetails(page, formUrls, dispensers, isSpecificDispensers = false, formType = FORM_TYPES.ACCUMEASURE) {
  try {
    ...
    for (let i = 0; i < formUrls.length; i++) {
      ...
      // Update batch status with current dispenser progress if in batch mode
      if (currentBatchJobId) {
        updateBatchStatus('running', `Processing dispenser ${i + 1} of ${formUrls.length}`, {
          dispenserCurrent: i + 1,
          dispenserCount: formUrls.length,
          currentVisitStatus: `Processing dispenser ${i + 1}/${formUrls.length}: ${dispenser.title || 'Unknown dispenser'}`
        });
      }
      ...
      while (moreIterationsExist) {
        ...
        try {
          ...
          if (fuelType) {
            ...
            // Update batch status with fuel progress if in batch mode
            if (currentBatchJobId) {
              updateBatchStatus('running', `Processing fuel type: ${fuelType} (${processedIterations}/${totalIterations})`, {
                fuelType: fuelType,
                fuelCurrent: processedIterations,
                fuelTotal: totalIterations,
                currentVisitStatus: `Processing ${fuelType} fuel (${processedIterations}/${totalIterations}) - Dispenser: ${dispenser.title || 'Unknown'}`
              });
            }
...
async function fillFuelTypeForm(page, fuelType, allFuelTypes, proverPreferences, isSpecificDispensers = false, formType = FORM_TYPES.ACCUMEASURE) {
  try {
    ...
    try {
      ...
      const radiosSelected = await page.evaluate(({ shouldHaveMeter, currentFormType, formTypesConstant }) => {
        try {
          ...
          const selectors = [
            ...
            `input[name="field[${currentFormType === formTypesConstant.OPEN_NECK_PROVER ? '5462' : '5438'}]"][value="${shouldHaveMeter ? '1' : '2'}"]`,
      ...
      }, { shouldHaveMeter: hasMeters, currentFormType: formType, formTypesConstant: FORM_TYPES });
 ...
 * @returns {Promise<void>}
 */
async function processBatch(jobIdParam, filePath, headless = true, selectedVisits = null, resumeFromBatchId = null) {
  const operationJobId = jobIdParam || Date.now().toString();
  currentBatchJobId = operationJobId; // <<< SET currentBatchJobId HERE

  logger.info(`Starting batch process. Job ID: ${currentBatchJobId}, File: ${filePath}, Headless: ${headless}`);
  activeJobIds.add(currentBatchJobId);
  ...
  const initialStatusProps = {
    ...
    completedVisitIds: [],
    startTime: new Date().toISOString(),
    jobId: currentBatchJobId, // Set the jobId for this batch
    timestamp: currentBatchJobId, // Use jobId as timestamp for lookup compatibility
  ...
  try {
    ...
    for (let i = 0; i < visitsToProcess.length; i++) {
      ...
      try {
        ...
        // Try to extract store info from visit data to display in frontend
        let storeName = 'Store';
        try {
          // Try to get store name from page title after navigation or from visit data if available
          await page.goto(visitUrl);
          storeName = await page.title();
          // Clean up the title to extract just the store name
          if (storeName.includes('|')) {
            storeName = storeName.split('|')[0].trim();
          }
          if (!storeName || storeName === 'Work Order') {
            storeName = `Work Order ${visit.id}`;
          }
        } catch (e) {
          logger.warn(`Could not extract store name from page: ${e.message}`);
          storeName = `Work Order ${visit.id}`;
        }
        
        updateBatchStatus('running', `Processing visit ${i + 1}/${visitsToProcess.length}: ${storeName}`, {
          currentVisit: visit.id,
          currentVisitName: `Visit ${i + 1}/${visitsToProcess.length}: ${storeName}`,
          currentVisitStatus: `Starting visit ${i + 1} for ${storeName}`,
          currentVisitNumber: i + 1,
          totalVisitCount: visitsToProcess.length
        });
        
        // Skip navigation since we already navigated in the previous step to get store name
        await page.waitForLoadState('networkidle');
        ...
        updateBatchStatus('running', `Completed visit ${i + 1}/${visitsToProcess.length}: ${storeName}`, {
          completedVisits: batchStatus.completedVisits + 1,
          currentVisitStatus: `Completed visit ${i + 1}`,
          currentVisitName: `Visit ${i + 1}/${visitsToProcess.length}: ${storeName} - Completed`,
          completedVisitIds: [...(batchStatus.completedVisitIds || []), visit.id],
          currentVisitNumber: i + 1,
          totalVisitCount: visitsToProcess.length,
          // Reset progress tracking fields for the next visit
          dispenserCount: 0,
          dispenserCurrent: 0,
          fuelType: null,
          fuelCurrent: 0,
    ...
    // Set status to completed as we've successfully finished processing all visits
    logger.info(`Batch processing finished. Processed ${batchStatus.completedVisits}/${batchStatus.totalVisits} visits.`);
    
    // Explicitly update the status to completed - this ensures the status is correctly shown in the UI
    updateBatchStatus('completed', `Batch processing completed. Processed ${batchStatus.completedVisits}/${batchStatus.totalVisits} visits.`, {
      ...
      currentVisitName: 'Batch Complete',
      currentVisitStatus: 'All visits processed successfully'
    });
    ...
    logger.error(`Error in background batch processing: ${error.message}`, 'FORM_PREP_BATCH');
    logger.error(`Stack trace: ${error.stack}`, 'FORM_PREP_BATCH_STACK');
    updateBatchStatus('error', `Batch processing error: ${error.message}`, { error: error.message, jobId: operationJobId }); // Pass operationJobId here
  } finally {
    if (page) await page.close().catch(e => logger.warn('Error closing page:', e.message));
    if (context) await context.close().catch(e => logger.warn('Error closing context:', e.message));
    if (browser) await browser.close().catch(e => logger.warn('Error closing browser:', e.message));
    
    activeJobIds.delete(operationJobId); // Use operationJobId
    
    // Set the final status to completed if it was running and has completed all visits
    // This is critical to ensure the UI shows the correct status
    if (batchStatus && batchStatus.jobId === operationJobId && 
        batchStatus.status === 'running' && 
        batchStatus.completedVisits >= batchStatus.totalVisits) {
      updateBatchStatus('completed', `Batch processing completed. Processed ${batchStatus.completedVisits}/${batchStatus.totalVisits} visits.`, {
        endTime: new Date().toISOString(),
        currentVisitName: 'Batch Complete',
        currentVisitStatus: 'All visits processed'
      });
      logger.info(`Batch complete: Processed ${batchStatus.completedVisits}/${batchStatus.totalVisits} visits.`);
    }
    
    // Log state before attempting to save history
    logger.info(`[processBatch finally] About to save history. operationJobId: ${operationJobId}, batchStatus: ${JSON.stringify(batchStatus || {})}`);

    // Save final status, ensuring it's for the correct job
    if (batchStatus && batchStatus.jobId === operationJobId) { 
        saveBatchStatusToHistory(batchStatus);
    } else if (batchStatus) {
        logger.warn(`Not saving history for ${operationJobId} because batchStatus.jobId is ${batchStatus.jobId}`);
    } else {
        logger.warn(`Not saving history for ${operationJobId} because batchStatus is null or undefined.`);
    }

    currentBatchJobId = null; // <<< RESET currentBatchJobId HERE
    logger.info(`Batch process for job ${operationJobId} finished. currentBatchJobId is now ${currentBatchJobId}.`);
  }
  return batchStatus;
}
 ...
 */
async function getBatchStatus(requestedJobId) {
  // If a specific jobId is provided to poll for
  if (requestedJobId) {
    // Check if the requestedJobId is the currently active batch job
    if (batchStatus && batchStatus.jobId === requestedJobId && currentBatchJobId === requestedJobId) {
      logger.info(`Requested job ${requestedJobId} is the current active batch. Returning live status.`);
      // Fall through to the end of the function to return the current in-memory batchStatus
    } else {
      // The requestedJobId is not the current active job. Try to find it in history.
      try {
        const specificStatusFromHistory = await getBatchStatusById(requestedJobId);
        if (specificStatusFromHistory) {
          // Found the non-active job in history
          logger.info(`Returning status for specific non-active batch job: ${requestedJobId} from history`);
          return {
            jobId: requestedJobId, // Ensure the response uses the ID that was asked for
            ...
            timestamp: specificStatusFromHistory.timestamp || requestedJobId, // History item's timestamp
            lastStatusUpdate: new Date().toISOString()
          };
        } else {
          // Requested job is not active and not in history.
          logger.warn(`Batch job ${requestedJobId} not found in history and is not the active job. Returning 'unknown' status.`);
          return {
            jobId: requestedJobId,
            status: 'unknown',
            message: `Status for job ${requestedJobId} not found. It may have completed, been cancelled, or the ID is invalid.`,
            totalVisits: 0,
            completedVisits: 0,
            currentItem: 0,
            totalItems: 0,
            completedVisitIds: [],
            currentVisitName: 'N/A',
            currentVisitStatus: 'N/A',
            startTime: null,
            endTime: null,
            filePath: null,
            headless: null,
            timestamp: requestedJobId, // Use requestedJobId as timestamp for consistency
            lastStatusUpdate: new Date().toISOString()
          };
        }
      } catch (error) {
        logger.error(`Error retrieving status for specific non-active job ${requestedJobId} from history: ${error.message}`);
        return {
          jobId: requestedJobId,
          status: 'error',
          message: `Error retrieving status for job ${requestedJobId}: ${error.message}`,
          timestamp: requestedJobId,
          lastStatusUpdate: new Date().toISOString()
        };
      }
    }
  ...
  // 1. No specific requestedJobId was provided (caller wants status of the absolute current job).
  // 2. A specific requestedJobId was provided, AND it IS the current active job.
  logger.info(`Returning current in-memory batch status (live job: ${batchStatus.jobId || 'N/A'}, polling for: ${requestedJobId || 'current'})`);

  let returnStatus = batchStatus.status;
  const messageForCheck = batchStatus.message || ""; // Handle null/undefined message safely

  // Double-check completion status based on visit counts
  if (batchStatus.totalVisits > 0 && 
      batchStatus.completedVisits >= batchStatus.totalVisits &&
      returnStatus !== 'error' && returnStatus !== 'cancelled') {
    if (returnStatus !== 'completed') {
      returnStatus = 'completed';
      logger.info(`[getBatchStatus] All visits completed (${batchStatus.completedVisits}/${batchStatus.totalVisits}). Ensuring status is 'completed'.`);
    }
  }
  // Additional check: If we have an "idle" status but all visits are complete, set to "completed"
  else if (returnStatus === 'idle' && 
           batchStatus.totalVisits > 0 && 
           batchStatus.completedVisits >= batchStatus.totalVisits &&
           returnStatus !== 'error' && returnStatus !== 'cancelled') {
    returnStatus = 'completed';
    logger.info(`[getBatchStatus] Converting idle status to completed for finished job (${batchStatus.completedVisits}/${batchStatus.totalVisits} visits).`);
  }
  // Otherwise override status to 'running' if it's the active job, currently 'idle', and message isn't terminal
  else if (currentBatchJobId &&
      batchStatus.jobId === currentBatchJobId &&
      returnStatus === 'idle' &&
      !(messageForCheck.toLowerCase().includes('complet') ||
        messageForCheck.toLowerCase().includes('error') ||
        messageForCheck.toLowerCase().includes('cancel'))) {
    returnStatus = 'running';
    logger.info(`[getBatchStatus] Overriding batch status from 'idle' to 'running' for active job ${currentBatchJobId}`);
  }

  // Ensure endTime is set for completed jobs
  let endTime = batchStatus.endTime;
  if (returnStatus === 'completed' && !endTime) {
    endTime = new Date().toISOString();
    logger.info(`[getBatchStatus] Setting missing endTime for completed job`);
  }
  
  return {
    jobId: batchStatus.jobId, // This is the ID of the current live batch
    status: returnStatus,
    message: returnStatus === 'completed' ? 'All visits processed successfully.' : batchStatus.message,
    ...
    // Always include detailed progress information
    dispenserCount: batchStatus.dispenserCount || 0,
    dispenserCurrent: batchStatus.dispenserCurrent || 0, 
    fuelType: batchStatus.fuelType || null,
    fuelCurrent: batchStatus.fuelCurrent || 0,
    fuelTotal: batchStatus.fuelTotal || 0,
    currentVisitName: returnStatus === 'completed' ? 'Batch Complete' : (
                      batchStatus.currentVisitName ||
                      ((returnStatus === 'running' || returnStatus === 'idle') && (batchStatus.totalVisits > 0) ?
                        (batchStatus.completedVisits < batchStatus.totalVisits ? 
                          `Visit ${batchStatus.completedVisits + 1}/${batchStatus.totalVisits}` : 
                          'Finalizing...') :
                        'Initializing...')
                      ),
    // Include visit number information
    currentVisitNumber: batchStatus.currentVisitNumber || 
                       (batchStatus.completedVisits < batchStatus.totalVisits ? 
                        batchStatus.completedVisits + 1 : batchStatus.totalVisits),
    totalVisitCount: batchStatus.totalVisitCount || batchStatus.totalVisits || 0,
    currentVisitStatus: returnStatus === 'completed' ? 'Completed' : (batchStatus.currentVisitStatus || batchStatus.message),
    startTime: batchStatus.startTime || new Date(0).toISOString(),
    endTime: endTime,
    filePath: batchStatus.filePath,
    headless: batchStatus.headless,
    timestamp: batchStatus.timestamp, // Timestamp of the current live batch
    lastStatusUpdate: new Date().toISOString()
  };
}
