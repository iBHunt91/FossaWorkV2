...
async function processBatch(jobIdParam, filePath, headless = true, selectedVisits = null, resumeFromBatchId = null) {
  logger.info(`[processBatch ENTRY POINT] Called with jobIdParam: ${jobIdParam}, filePath: ${filePath}. currentBatchJobId (before assignment): ${typeof currentBatchJobId !== 'undefined' ? currentBatchJobId : 'NOT DEFINED YET IN THIS SCOPE'}`);
  const operationJobId = jobIdParam || Date.now().toString();
  currentBatchJobId = operationJobId;
