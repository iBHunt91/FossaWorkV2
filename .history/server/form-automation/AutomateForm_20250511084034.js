...
const updateStatus = (status, message, progress = null) => {
  ...
  // If a batch job is active, also update the realtime status
  // Pass basic details for now; this can be expanded if more context is needed here.
  if (currentBatchJobId && batchStatus && batchStatus.jobId === currentBatchJobId) {
    // Construct a basic details object. It's important that visitId and visitName are populated
    // if they are available in a broader scope when updateStatus is called, ideally they'd be passed in.
    // For now, we'll rely on updateRealtimeStatus to use existing batchStatus values if these are undefined.
    const detailsForRealtime = {
      visitId: batchStatus.currentVisitId, // Attempt to use current from batchStatus
      visitName: batchStatus.currentVisitName // Attempt to use current from batchStatus
    };
...
const updateBatchStatus = (status, message, progressOrProps = null, currentItem = null, totalItems = null) => {
  ...
  // Also call updateRealtimeStatus to ensure consistency and detailed parsing if applicable
  // The message here might be general, but updateRealtimeStatus can handle it or extract from details
  // The progressProps often contains visitId, visitName, etc.
  if (currentBatchJobId && batchStatus.jobId === currentBatchJobId) {
    updateRealtimeStatus(status, message, progressProps);
  }
