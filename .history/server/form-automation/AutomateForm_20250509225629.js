...
async function processAllFuelSections(page, dispenser, isSpecificDispensers = false, formType = FORM_TYPES.ACCUMEASURE, totalDispensersInVisit = 0, currentDispenserIndex = 0, visitIdForContext = null) {
  try {
    ...
    if (fuelTypes.length === 0) {
      ...
      // Still update dispenser progress as "processed" even if no fuel types
    ...
    logger.info(`Processing ${fuelTypes.length} fuel types for dispenser: ${dispenser.title || 'Unknown'} (Index ${currentDispenserIndex}) in visit ${visitIdForContext}`, 'FORM_PREP');
    ...
    for (let i = 0; i < fuelTypes.length; i++) {
      ...
      // Update status before processing each fuel type
      // The message here will be picked up by updateStatus and updateBatchStatus's currentVisitStatus
      const fuelProcessingMessage = `Processing fuel type: ${fuelType} (${i + 1}/${fuelTypes.length}) - Dispenser #${currentDispenserIndex}`;
      updateStatus('running', fuelProcessingMessage, {
          dispenserCurrent: currentDispenserIndex,
          dispenserTotal: totalDispensersInVisit,
          fuelType: fuelType,
          fuelCurrent: 0, // Reset for this fuel type, fillFuelTypeForm will update actual current/total steps
          fuelTotal: 0  // Reset for this fuel type
      });

      await fillFuelTypeForm(page, fuelType, fuelTypes, proverPreferences, isSpecificDispensers, formType, totalDispensersInVisit, currentDispenserIndex, visitIdForContext);
    }
  } catch (error) {
    logger.error(`Error processing fuel sections for dispenser ${dispenser.title || 'Unknown'} in visit ${visitIdForContext}: ${error.message}`, 'FORM_PREP');
    updateStatus('error', `Error processing fuel for ${dispenser.title || 'Unknown'}: ${error.message}`,{
        dispenserCurrent: currentDispenserIndex,
        dispenserTotal: totalDispensersInVisit,
    });
 ...
 * @returns {Promise<boolean>}
 */
async function fillFuelTypeForm(page, fuelType, allFuelTypes, proverPreferences, isSpecificDispensers = false, formType = FORM_TYPES.ACCUMEASURE, totalDispensersInVisit = 0, currentDispenserIndex = 0, visitIdForContext = null) {
  try {
