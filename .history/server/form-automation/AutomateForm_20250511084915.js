...
let currentBatchJobId = null; // <<< ENSURE THIS IS DECLARED FOR THE CURRENT ACTIVE BATCH JOB ID
...
const updateStatus = (status, message, progress = null) => {
  ...
  // Diagnostic log
  logger.info(`[updateStatus] Check before calling updateRealtimeStatus: currentBatchJobId=${currentBatchJobId}, batchStatus.jobId=${batchStatus ? batchStatus.jobId : 'batchStatus_null_or_undefined'}, batchStatus defined=${!!batchStatus}`);
  if (typeof currentBatchJobId !== 'undefined' && currentBatchJobId && batchStatus && batchStatus.jobId === currentBatchJobId) {
    const detailsForRealtime = {
      visitId: batchStatus.currentVisitId,
      visitName: batchStatus.currentVisitName
    };
    updateRealtimeStatus(status, message, detailsForRealtime);
  }
};
...
const updateBatchStatus = (status, message, progressOrProps = null, currentItem = null, totalItems = null) => {
  ...
  const newStatus = status; // Use the passed status directly
  const newMessage = message; // Use the passed message directly
  
  batchStatus = {
    ...(batchStatus || {}), // preserve existing properties first
    status: newStatus,
    message: newMessage,
    currentItem: currentItem !== null ? currentItem : (batchStatus ? batchStatus.currentItem : 0),
    totalItems: totalItems !== null ? totalItems : (batchStatus ? batchStatus.totalItems : 0),
    jobId: currentBatchJobId || (batchStatus ? batchStatus.jobId : null), // Ensure jobId is present
    ...progressProps, // override with new properties from progressOrProps
    lastUpdated: new Date().toISOString()
  };
  
  logger.debug(`Batch Status Updated: ${newStatus} - ${newMessage} for job ${batchStatus.jobId}`);
  
  // Diagnostic log
  logger.info(`[updateBatchStatus] Check before calling updateRealtimeStatus: currentBatchJobId=${currentBatchJobId}, batchStatus.jobId=${batchStatus ? batchStatus.jobId : 'batchStatus_null_or_undefined'}, batchStatus defined=${!!batchStatus}`);
  if (typeof currentBatchJobId !== 'undefined' && currentBatchJobId && batchStatus.jobId === currentBatchJobId) {
    updateRealtimeStatus(newStatus, newMessage, progressProps); // Pass newStatus, newMessage
  }
  
  if (newStatus === 'completed' || newStatus === 'error') {
    // Ensure batchStatus has the correct jobId before saving
    if (batchStatus.jobId === currentBatchJobId) {
        saveBatchStatusToHistory(batchStatus);
    } else {
        logger.warn(`Skipping saveBatchStatusToHistory for job ${currentBatchJobId} because batchStatus.jobId (${batchStatus.jobId}) does not match.`);
    }
  }
...
async function fillFuelTypeForm(page, fuelType, allFuelTypes, proverPreferences, isSpecificDispensers = false, formType = FORM_TYPES.ACCUMEASURE) {
  try {
    ...
    try {
      ...
      const radiosSelected = await page.evaluate(({ shouldHaveMeter, formTypeOuter, FORM_TYPES_OUTER }) => {
        try {
          ...
          const selectors = [
            ...
            `input[name="field[${formTypeOuter === FORM_TYPES_OUTER.OPEN_NECK_PROVER ? '5462' : '5438'}]"][value="${shouldHaveMeter ? '1' : '2'}"]`,
      ...
      }, { shouldHaveMeter: hasMeters, formTypeOuter: formType, FORM_TYPES_OUTER: FORM_TYPES });
