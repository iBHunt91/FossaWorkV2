...
async function getBatchStatus(jobId) {
  ...
  // CRITICAL FIX: If there's an active batch job, ensure status is "running" and not "idle"
  // This guarantees progress bars will show up in the UI
  let returnStatus = batchStatus.status;
  if (currentBatchJobId && 
      batchStatus.jobId === currentBatchJobId && 
      returnStatus === 'idle' && 
      batchStatus.message && 
      !(batchStatus.message.toLowerCase().includes('error') || 
        batchStatus.message.toLowerCase().includes('complet') || 
        batchStatus.message.toLowerCase().includes('cancel'))) {
