...
let batchStatus = {
  ...
  jobId: null, // Explicitly add jobId here
  timestamp: null // Explicitly add timestamp here for lookup
};
 ...
 * @returns {Promise<void>}
 */
async function processBatch(filePath, headless = true, selectedVisits = null, resumeFromBatchId = null) {
  logger.info(`Starting batch process. Job ID: ${currentJobId}, File: ${filePath}, Headless: ${headless}`);
  activeJobIds.add(currentJobId);
  ...
  const initialStatusProps = {
    ...
    completedVisitIds: [],
    startTime: new Date().toISOString(),
    jobId: currentJobId, // Set the jobId for this batch
    timestamp: currentJobId, // Use jobId as timestamp for lookup compatibility
...
async function getBatchStatus(jobId) {
  ...
  if (jobId) {
    try {
      const specificStatus = await getBatchStatusById(jobId);
      if (specificStatus) {
        logger.info(`Returning status for specific batch job: ${jobId}`);
        return {
          ...specificStatus,
          totalVisits: specificStatus.totalItems || specificStatus.totalVisits,
          completedVisits: specificStatus.currentItem || specificStatus.completedVisits,
          currentVisit: specificStatus.currentVisitName || 
                      (specificStatus.currentItem > 0 ? `Visit ${specificStatus.currentItem}` : null),
          currentVisitStatus: specificStatus.message,
          startTime: specificStatus.startTime || new Date().toISOString(),
          lastStatusUpdate: new Date().toISOString()
        };
      }
      
      // If we can't find the specific batch or it's the current one, return the current status
      logger.warn(`Batch job ${jobId} not found in history, returning current status`);
    } catch (error) {
      logger.error(`Error getting specific batch status for ${jobId}: ${error.message}`);
      // Continue to return current status if there's an error
    }
  }
  
  // Return current batch status if no jobId is provided or if specific batch not found
  return {
    ...batchStatus,
