...
function updateRealtimeStatus(status, message, details) {
  ...
  currentJobStatus.details = details;
  ...
  // Log batchStatus BEFORE modification
  logger.info('[updateRealtimeStatus] batchStatus BEFORE update: ' + JSON.stringify(batchStatus));

  // Update batch status if a batch job is active
  if (batchStatus.jobId && batchStatus.status === 'running') {
    batchStatus.status = status; // Reflect the most current "overall" status (e.g., could be 'error' from script)
    batchStatus.message = message; // Update overall message
    batchStatus.lastStatusUpdate = new Date().toISOString();

    if (details) {
      // Extract and set current visit name if available
      if (details.currentVisitName) {
        batchStatus.currentVisitName = details.currentVisitName;
      }
      
      // Prefer specific currentVisitStatusMessage from details if provided by the script
      if (details.currentVisitStatusMessage) {
        batchStatus.currentVisitStatusMessage = details.currentVisitStatusMessage;
      } else {
        // Fallback: Construct a more descriptive current visit status message
        // This is where we can synthesize based on dispenser/fuel progress
        let visitProgressMessage = message; // Start with the general message

        if (details.dispenserProgress && details.dispenserProgress.total > 0) {
          visitProgressMessage = `Dispenser ${details.dispenserProgress.current}/${details.dispenserProgress.total}`;
          if (details.fuelProgress && details.fuelProgress.type && details.fuelProgress.total > 0) {
            visitProgressMessage += ` - ${details.fuelProgress.type} ${details.fuelProgress.current}/${details.fuelProgress.total}`;
          }
        } else if (details.fuelProgress && details.fuelProgress.type && details.fuelProgress.total > 0) {
          visitProgressMessage = `${details.fuelProgress.type} ${details.fuelProgress.current}/${details.fuelProgress.total}`;
        }
        batchStatus.currentVisitStatusMessage = visitProgressMessage;
      }
      
      // Update dispenser progress for the current visit
      if (details.dispenserProgress) {
        batchStatus.dispenserCount = details.dispenserProgress.total;
        batchStatus.dispenserCurrent = details.dispenserProgress.current;
      } else if (details.totalDispensersForCurrentVisit !== undefined) { // Legacy or alternative way to pass total
        batchStatus.dispenserCount = details.totalDispensersForCurrentVisit;
        // If only total is provided, current might be inferred or come from message parsing later
      }

      // Update fuel progress for the current visit
      if (details.fuelProgress) {
        batchStatus.fuelType = details.fuelProgress.type;
        batchStatus.fuelCurrent = details.fuelProgress.current;
        batchStatus.fuelTotal = details.fuelProgress.total;
      }

      // Update overall batch completion (completedVisits)
      if (typeof details.completedVisits === 'number') {
        batchStatus.completedVisits = details.completedVisits;
      }
      if (typeof details.totalVisits === 'number') {
        batchStatus.totalVisits = details.totalVisits;
      }
       // If the script signals an error for the current visit or overall
      if (status === 'error' && !batchStatus.errorDetails) {
         batchStatus.errorDetails = {
           visit: batchStatus.currentVisitName || 'Unknown Visit',
           message: message,
           details: details,
         };
      }
    }
    // If the overall status is 'completed' or 'error', ensure batch reflects this
    // And potentially finalize completedVisits if not explicitly set by 'details'
    if (status === 'completed' && batchStatus.status !== 'completed') {
        batchStatus.status = 'completed';
        batchStatus.endTime = new Date().toISOString();
        if (batchStatus.completedVisits < batchStatus.totalVisits) {
           // If script says 'completed' but completedVisits isn't updated, assume all remaining were done.
           // This might need refinement based on how 'processBatch' signals true completion.
            logger.warn('[updateRealtimeStatus] Batch marked completed by script, but completedVisits < totalVisits. Setting completedVisits = totalVisits.');
            batchStatus.completedVisits = batchStatus.totalVisits;
        }
    } else if (status === 'error' && batchStatus.status !== 'error') {
        batchStatus.status = 'error';
        batchStatus.endTime = new Date().toISOString();
    }


  } else if (batchStatus.jobId && (status === 'completed' || status === 'error')) {
    // If a batch was running and now we get a final status from the script for that job
    batchStatus.status = status;
    batchStatus.message = message;
    batchStatus.lastStatusUpdate = new Date().toISOString();
    batchStatus.endTime = new Date().toISOString();
    if (status === 'completed' && details && typeof details.completedVisits === 'number') {
        batchStatus.completedVisits = details.completedVisits;
    }
     if (status === 'completed' && batchStatus.completedVisits < batchStatus.totalVisits) {
        logger.warn('[updateRealtimeStatus] Batch marked completed (final status) but completedVisits < totalVisits. Setting completedVisits = totalVisits.');
        batchStatus.completedVisits = batchStatus.totalVisits;
    }
  }
  
  // Log batchStatus AFTER modification
  logger.info('[updateRealtimeStatus] batchStatus AFTER update: ' + JSON.stringify(batchStatus));

  // Emit the status update to any listeners (e.g., WebSocket connections)
  // This part would require a proper event emitter or WebSocket setup
  // For now, this function primarily updates the global state objects.
}
