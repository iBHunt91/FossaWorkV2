  ...
  try {
    ...
    if (isCancelled) {
      ...
      if (batchUpdateCallback) {
        batchUpdateCallback('Job cancelled, skipping form preparation', { currentVisitStatus: 'Cancelled' });
      } else {
        updateStatus('cancelled', 'Job cancelled, skipping form preparation');
      }
    ...
    // formsToCreate is defined here, and used throughout.
    ...
    if (remainingFormsToAdd + existingFormCount > dispensers.length && dispensers.length > 0) { // Added dispensers.length > 0 condition
    ...
    if (remainingFormsToAdd === 0) {
      ...
      const allFormsExistMessage = 'All required forms already exist, proceeding to fill forms';
      if (batchUpdateCallback) {
        batchUpdateCallback(allFormsExistMessage, { currentVisitStatus: allFormsExistMessage, formsCurrent: existingFormCount, formsTotal: formsToCreate });
      } else {
        updateStatus('running', allFormsExistMessage);
      }
    } else {
      ...
      for (let formIndex = 0; formIndex < remainingFormsToAdd; formIndex++) {
        if (isCancelled) {
          ...
          if (batchUpdateCallback) {
            batchUpdateCallback('Job cancelled, stopping form creation loop', { currentVisitStatus: 'Cancelled' });
          } else {
            updateStatus('cancelled', 'Job cancelled');
          }
          return { success: false, formUrls: [] };
        }
        ...
        const formAddingMessage = `Adding form ${formIndex + 1} of ${remainingFormsToAdd} (current count: ${currentCount}, target: ${formsToCreate})`;
        logger.info(formAddingMessage, 'FORM_PREP');
        if (batchUpdateCallback) {
          batchUpdateCallback(formAddingMessage, { 
            currentVisitStatus: `Adding ${formType} ${currentCount + 1}/${formsToCreate}`, 
            formsCurrent: currentCount,
            formsTotal: formsToCreate 
          });
        }
        const beforeFormCount = currentCount;
        const buttonLabel = (formIndex === 0 && existingFormCount === 0) ? 'Attach' : 'New';
        const addFormSelector = (formIndex === 0 && existingFormCount === 0) ? 
          `li.plain-header:has-text("${formType}") a:has-text("Attach")`:
          `li.plain-header:has-text("${formType}") a:has-text("New")`;
        try {
          await page.waitForSelector(addFormSelector, { timeout: 5000 });
          await page.click(addFormSelector);
          logger.info(`Clicked '${buttonLabel}' for ${formType}`, 'FORM_PREP');
          let attempts = 0;
          const maxAttempts = 20;
          let formAddedSuccessfully = false;
          while(attempts < maxAttempts) {
            if (isCancelled) {
              logger.info('Job cancelled during form add wait.');
            ...
            const newCount = await getFormCount();
            if (newCount > beforeFormCount) {
              logger.info(`Successfully added form. New count: ${newCount}`, 'FORM_PREP');
              totalFormsAdded++;
              if (batchUpdateCallback) {
                batchUpdateCallback(`Added ${formType} ${newCount}/${formsToCreate}`, { 
                  currentVisitStatus: `Added ${formType} ${newCount}/${formsToCreate}`,
                  formsCurrent: newCount, 
                  formsTotal: formsToCreate 
                });
              }
          ...
          if (!formAddedSuccessfully) {
            const errMsg = `Failed to confirm ${formType} form addition (count did not increase from ${beforeFormCount})`;
            logger.error(errMsg, 'FORM_PREP');
            if (batchUpdateCallback) batchUpdateCallback(errMsg, { currentVisitStatus: 'Error adding form' });
            throw new Error(errMsg);
          }
        } catch (e) {
          logger.error(`Error clicking '${buttonLabel}' for ${formType}: ${e.message}`, 'FORM_PREP');
          if (batchUpdateCallback) {
            batchUpdateCallback(`Error adding ${formType}: ${e.message.substring(0, 50)}...`, { currentVisitStatus: `Error: ${e.message.substring(0, 50)}...`});
          }
          throw e;
        }
      }
    ...
    // After all forms are added or confirmed to exist, collect URLs:
    const finalFormCountOnPage = await getFormCount();
    let collectedFormUrls = [];
    logger.info(`Attempting to collect URLs for up to ${formsToCreate} ${formType} forms. Current count on page: ${finalFormCountOnPage}.`, 'FORM_PREP');
    try {
      ...
      if (tempUrls.length < formsToCreate && formsToCreate > 0 && formElements.length === 0 && finalFormCountOnPage > 0) {
        logger.info(`Primary selector found 0 elements, but ${finalFormCountOnPage} forms counted. Trying fallback selector for ${formType} URLs.`, 'FORM_PREP');
        const altFormElements = await page.$$(`a.none:has-text("${formType}")`);
        for (const el of altFormElements) {
          const href = await el.getAttribute('href');
          if (href && href !== '#') {
            const absoluteUrl = href.startsWith('http') ? href : new URL(href, page.url()).toString();
            if (!tempUrls.includes(absoluteUrl)) {
              tempUrls.push(absoluteUrl);
            }
          }
        }
      }
      collectedFormUrls = tempUrls.slice(0, formsToCreate);
      logger.info(`Collected ${collectedFormUrls.length} ${formType} form URLs. (Target: ${formsToCreate})`, 'FORM_PREP');
      if (collectedFormUrls.length < formsToCreate && formsToCreate > 0 && finalFormCountOnPage >= formsToCreate) {
        logger.warn(`Collected fewer URLs (${collectedFormUrls.length}) than actual forms counted (${finalFormCountOnPage}) or expected (${formsToCreate}) for ${formType}.`, 'FORM_PREP');
      }
    } catch (urlError) {
      logger.error(`Error collecting ${formType} form URLs: ${urlError.message}`, 'FORM_PREP');
    }

    const finalPrepMessage = `${formType} form preparation complete. Found/Created ${finalFormCountOnPage} forms. Collected ${collectedFormUrls.length} URLs.`;
    logger.info(finalPrepMessage, 'FORM_PREP');
    if (batchUpdateCallback) {
      batchUpdateCallback(finalPrepMessage, { 
        currentVisitStatus: finalPrepMessage,
        formsCurrent: finalFormCountOnPage, 
        formsTotal: formsToCreate 
      });
    } else {
      updateStatus('running', `${formType} form preparation complete.`);
    }
    return { success: true, formUrls: collectedFormUrls };

  } catch (error) {
    const errorMessage = `Error preparing ${formType} form: ${error.message}`;
    logger.error(errorMessage, 'FORM_PREP');
    if (batchUpdateCallback) {
      batchUpdateCallback(errorMessage, { currentVisitStatus: `Error: ${error.message.substring(0,100)}...`, error: true });
    } else {
