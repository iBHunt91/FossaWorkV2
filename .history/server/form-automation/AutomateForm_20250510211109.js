...
function updateRealtimeStatus(status, message, details) {
  ...
  if (currentBatchJobId && batchStatus && batchStatus.jobId === currentBatchJobId && batchStatus.status === 'running') {
    logger.info('[updateRealtimeStatus] Active batch job detected. Updating batchStatus with detailed progress. Message: ' + message + ', Details: ' + JSON.stringify(details));
    
    // SIMPLIFIED DIRECT ASSIGNMENT - Bypassing complex extractors for now
    batchStatus.currentVisitStatusMessage = message; // The raw message from the current visit's progress
    batchStatus.currentVisitRawStatus = message; // Store the raw status message too

    if (details) {
      // These field names are hypothetical and depend on what `performSingleAutomationTask` actually puts in `details`
      batchStatus.dispenserCount = details.totalFormsForCurrentVisit !== undefined ? details.totalFormsForCurrentVisit : batchStatus.dispenserCount;
      batchStatus.dispenserCurrent = details.currentFormForCurrentVisit !== undefined ? details.currentFormForCurrentVisit : batchStatus.dispenserCurrent;
      batchStatus.fuelType = details.currentFuelType !== undefined ? details.currentFuelType : batchStatus.fuelType;
      batchStatus.fuelCurrent = details.currentFuelForm !== undefined ? details.currentFuelForm : batchStatus.fuelCurrent;
      batchStatus.fuelTotal = details.totalFuelForms !== undefined ? details.totalFuelForms : batchStatus.fuelTotal;

      logger.info('[updateRealtimeStatus] Applied direct details: dispCount=' + details.totalFormsForCurrentVisit + ', dispCurr=' + details.currentFormForCurrentVisit + ', fuelType=' + details.currentFuelType);
    } else {
      logger.info('[updateRealtimeStatus] No "details" object provided for direct assignment.');
    }

    // Clear out fuel/dispenser progress if not explicitly set by details (to avoid stale data from previous visits)
    // Only do this if details was NOT provided or if the specific fields were not in details.
    if (!details || details.totalFormsForCurrentVisit === undefined) {
        // batchStatus.dispenserCount = undefined; // Or 0 or null, depending on how frontend handles it
    }
    if (!details || details.currentFormForCurrentVisit === undefined) {
        // batchStatus.dispenserCurrent = undefined;
    }
    // ... and so on for fuelType, fuelCurrent, fuelTotal if necessary

    logger.info('[updateRealtimeStatus] batchStatus AFTER direct assignment: ' + JSON.stringify(batchStatus));
  } else {
    logger.info('[updateRealtimeStatus] No active batch job, or batchStatus does not match. Not updating detailed batch progress. currentBatchJobId=' + currentBatchJobId + ', batchStatus.jobId=' + (batchStatus ? batchStatus.jobId : 'N/A'));
  }
}
