...
async function prepareForm(page, dispensers, formCount = null, isSpecificDispensers = false, formType = FORM_TYPES.ACCUMEASURE, batchUpdateCallback = null) {
  try {
    ...
    const resultMessage = `${formType} form preparation complete. Page has ${finalCountOnPage} forms. Collected ${collectedUrls.length} URLs (target ${formsToCreate}).`;
    logger.info(resultMessage, 'FORM_PREP');
    if (batchUpdateCallback) {
      batchUpdateCallback(resultMessage, { currentVisitStatus: resultMessage, formsCurrent: finalCountOnPage, formsTotal: formsToCreate });
    }
    return { success: true, formUrls: collectedUrls };
