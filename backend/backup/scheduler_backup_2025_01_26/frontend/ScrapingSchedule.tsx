...
const ScrapingSchedule: React.FC = () => {
  ...
  const fetchSchedule = async () => {
    try {
      ...
      if (response.data && response.data.length > 0) {
        ...
        // Log scheduler availability
        if (schedule.scheduler_available !== undefined) {
          console.log('Scheduler availability:', schedule.scheduler_available);
          if (!schedule.scheduler_available) {
            setStatusMessage('⚠️ Scheduler service not available - schedules will be database-only');
  ...
  const updateSchedule = async (enabled?: boolean) => {
    ...
    try {
      ...
      console.log('Updated local schedule state immediately for instant UI feedback');
      ...
      // Include active hours in the updated schedule for immediate UI update
      const scheduleWithActiveHours = {
        ...updatedSchedule,
        interval_hours: intervalHours,
        active_hours: useActiveHours ? { start: activeHoursStart, end: activeHoursEnd } : null
      };
      console.log('Dispatching scraping-schedule-updated with:', scheduleWithActiveHours);
          ...
          <div className="flex items-center justify-between">
            ...
            {schedule && (
              <div className="flex items-center gap-2">
                ...
                {schedule.scheduler_available === false && (
                ...
                {schedule.scheduler_available === false && (
                  <div className="p-3 bg-orange-50 border border-orange-200 rounded-md">
                    <p className="text-sm text-orange-800">
                      <strong>Database-only mode:</strong> Schedule saved but automatic execution requires APScheduler installation. 
                      Use "Test Now" to manually trigger scraping.
                    </p>
                  </div>
                )}
              </div>
