...
class SchedulerService:
    ...
    async def add_work_order_scraping_schedule(
        ...
        if active_hours is not None and active_hours:
            ...
            # Calculate which hours to run based on interval
            hours_to_run = []
            current_hour = active_hours['start']
            while current_hour < active_hours['end']:
                hours_to_run.append(current_hour)
                current_hour += int(interval_hours)
            
            logger.info(f"Active hours scheduled for: {hours_to_run}")
            
            trigger = CronTrigger(
                hour=','.join(str(h) for h in hours_to_run),
                minute=30,  # Run at 30 minutes past each hour
                timezone='UTC'
            )
        else:
            # Use cron trigger for consistent timing (every hour at :30)
            logger.debug(f"Creating cron trigger for every {interval_hours} hours at :30 (no active hours restriction)")
            
            # When no active hours restriction, calculate next run based on current time
            from datetime import datetime, timedelta
            now = datetime.now(timezone.utc)
            
            # Find the next :30 mark
            if now.minute < 30:
            ...
            next_run = now.replace(minute=next_minute, second=0, microsecond=0) + timedelta(hours=hours_to_add)
            logger.info(f"Current time: {now}, calculated next run: {next_run}")
            ...
            if interval_hours == 1:
                ...
                # For other intervals, calculate hours starting from the next run hour
                hours = []
                start_hour = next_run.hour
                
                # Add all hours in 24-hour period that match the interval
                hours_added = set()
                for i in range(24 // int(interval_hours) + 1):
                    hour_to_check = (start_hour + int(interval_hours) * i) % 24
                    if hour_to_check not in hours_added:
                        hours_added.add(hour_to_check)
                        hours.append(str(hour_to_check))
                
                hours.sort(key=int)  # Sort numerically
                logger.info(f"Calculated hours for {interval_hours}h interval starting from hour {start_hour}: {hours}")
        ...
        # Log trigger details for debugging
        logger.debug(f"Trigger type: {type(trigger).__name__}")
        ...
        try:
            ...
            if existing:
                ...
                if not is_restore:
                    ...
                    # Store next_run without timezone info
                    existing.next_run = job.next_run_time.replace(tzinfo=None) if job.next_run_time else None
                    existing.updated_at = datetime.utcnow()
                    ...
                    if not existing.next_run and job.next_run_time:
                        existing.next_run = job.next_run_time.replace(tzinfo=None) if job.next_run_time else None
                        existing.updated_at = datetime.utcnow()
                        logger.info(f"Restore mode - updated missing next_run to: {existing.next_run}")
                    else:
        ...
        try:
            ...
            if interval_hours is not None or active_hours is not None:
                ...
                if active_hours is not None and active_hours and 'start' in active_hours and 'end' in active_hours:
                    ...
                    logger.info(f"Creating CronTrigger for consistent timing (no active hours restriction)")
                    # Get the current interval if not provided
                    if interval_hours is None:
                        # Get current interval from database
                        user_id = self.active_jobs[job_id]["user_id"]
                    ...
                    # When removing active hours restriction, calculate next run based on current time
                    # This ensures the next run is at the next :30 mark, not at some predetermined hour
                    from datetime import datetime, timedelta
                    now = datetime.now(timezone.utc)
            ...
            try:
                ...
                if schedule:
                    ...
                    if job and job.next_run_time:
                        ...
                        logger.info(f"Updated next_run in database to: {schedule.next_run} (from {job.next_run_time})")
